...
审计起点：`main@55f2652`（2026-08-28，OUT-001 已推送并确认与 `origin/main` 一致）；本文件继续记录其后的 OUT-002 完成证据。PHP 权威源为 `C:\cinagroup\cinashop-php`，Cloudflare 目标为本仓库 `workers-ts` 与五个 TypeScript 前端。生产数据库通过 Hyperdrive `9748c294e21c49a99579c9cef70102e0` 核验；OUT-002 的回放账本 DDL 已直接应用，业务场景只在随机 schema 执行，主 Worker 和正式前端没有因本次审计被发布。
...
| 仓库目标结构 | 外部 SQL 218 表；Worker 内嵌 218 表；表/列/主键漂移 0 | 完成 |
| 生产目标结构 | 218/218 表；缺失 0、额外 0 | 完成 |
| PHP HTTP 合同 | 精确注册 528/1,912；其中 15 条接入明确 501；2 条有证据退役 | 原始注册 27.6%，静态可执行上限 26.8%，退役后有效上限 26.9% |
| 真实数据复制 | `data_migration_run=0`，本机无 `SOURCE_MYSQL_URL` | 未开始 |
| Worker 单元测试 | 112 文件、651 项通过 | 本地业务回归通过 |
...
| `/outapi` | 41 | 35 | 35 | 0 | 6 | 0 | 6 | 85.4% / 85.4% |
| `/erpapi` | 8 | 0 | 0 | 0 | 8 | 0 | 8 | 0% / 0% |
| 合计 | 1,912 | 1,219 | 528 | 15 | 1,384 | 2 | 1,382 | 26.8% / 26.9% |
...
- PostgreSQL 16.14；应用 OUT 商品回放账本后，生产 `public` 当前 218 表、3,029 列、699 索引、205 主键。
- `order_print_job`、`order_print_job_action`、`order_waybill_job`、`order_waybill_job_action` 已创建且行数为 0。
- `out_product_write_replay` 已创建为 8 列、4 约束、3 索引且当前 0 行；仓库 218 表清单与生产集合差均为空。
...
- [x] **AUD-001 可重复路由审计器**：`workers-ts/scripts/route-parity-audit.ts` 与 `npm run audit:routes` 当前验收为 Kefu 63/48、Out 41/35、ERP 8/0；新增有证据退役清单并同时输出原始缺失、退役和可执行缺口，清单漂移、重复或与 TS 注册冲突时直接失败。
...
- [ ] **CORE-003 对外 API 资金与用户写入（部分完成）**：退款、分类和实物商品写入已经在逐路由 ACL、HMAC 脱敏审计和幂等状态机下恢复；优惠券和用户写入仍需逐项完成或证据化退役。每条接口都必须绑定独立权限 ID，不得因导入目录而自动开放。
...
`DB-001`、`DB-002`、`CORE-002`、`KEFU-004`、`OUT-001`、`OUT-002` 已完成；生产为 218/218。最新静态路由审计为 PHP 1,912、TS 1,219、精确匹配 528、可执行匹配 513、证据化退役 2、可执行缺口 1,382；Out 域已升至 35/41，剩余优惠券 3、用户 3 共 6 条。`DB-003` 涉及删除冲突配置，必须先由运营确认权威值；`DATA-001` 需要源 MySQL 连接；Kefu 剩余扫码/微信、游客和面单模板分别依赖新安全合同或第三方配置。等待外部条件期间下一批处理 **OUT-003 优惠券写入 3 条**；第三方配置和正式发布仍需要用户提供外部条件或明确批准。
