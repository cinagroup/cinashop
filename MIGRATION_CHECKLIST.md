...
- [ ] **API-008 门店/企业微信/内嵌 Admin（73 条起始缺口，已精确拆分）**：静态起点为 store 12、work 10、`/api/admin` 51；其中已有公开门店列表和核销写入不计缺口。外部写操作必须 Queue 化，不能同步调用第三方。
  ...
  - [ ] **WORK-C 企业微信回调 1 条（安全接收管道、生产 DDL 与隔离场景已收口，业务投影/真实回调/发布未完成）**：以 `ANY /api/work/serve` 精确注册，运行时仅允许 GET URL 验证和 POST 回调，其他方法 405；签名严格按 `sha1(sort(token,timestamp,nonce,ciphertext))`，AES-256-CBC 使用 43 字符 EncodingAESKey解码、32 字节 PKCS#7手工校验，并绑定解密尾部 receive-id 与明文 CorpID。POST 最多流式读取 64 KiB，只把22个协议字段白名单写入 PostgreSQL，不保存外层签名、nonce、密文或完整 XML；`WECHAT_WORK_CALLBACK_TOKEN/AES_KEY` 只允许 Worker Secret。`work_callback_event/outbox/watermark` 同事务接收，event/payload/subject均用 SHA-256身份，Queue消息严格只有 `action/outboxId/eventId/eventKey`；Queue投递失败保留 FAILED行，五分钟 Cron补偿，至少一次消费按事件唯一键幂等。每个主体以 advisory transaction lock和 `(event_time, sequence_rank)`水位处理乱序，同秒删除/解散优先于更新/创建，未知事件留痕为 IGNORED而不静默冒充成功。生产 PostgreSQL 16已应用 `0109_work_callback_pipeline.sql`（内嵌`migration_0115`）两遍，三表39列/10索引严格回读且均0行，六张既有业务表全行指纹不变；随机 schema 11/11覆盖明文回显、重复、伪造签名无写、乱序、同秒删除、未知事件、Queue失败和资源清理。父项仍不勾选：本批消费者只完成可信排序/投递，不把 `ORDERED`冒充成员、部门、客户、群或标签已刷新；还需把旧 `WorkListener`各事件的provider读取、本地投影及欢迎语/标签等远端写拆成独立可重试动作outbox，并取得CorpID、两枚回调Secret、真实租户回调、预发、影子流量和明确发布批准。
...
`DB-001`、`DB-002`、`DB-004`、`CORE-002`、`CORE-003`、`KEFU-004`、`OUT-001`～`OUT-004`、`API-001` 已完成；CORE-004 的扫码/OAuth 本地安全闭环、API-005 三端登录接入、KEFU-001 核心/前端、KEFU-002 安全游客协议、USER-CENTER-COMPAT 九条、DIY-HOME-WIDGETS 八条、PUBLIC-ARTICLE 七条、PRODUCT-REPLY-DETAIL 四条、API-008 STORE-A/STORE-B 各六条、WORK-A 两条、WORK-B七条及WORK-C可信接收管道已在代码侧收口，但相关父项继续等待数据、真实流程和发布。当前仓库/生产结构清单为227/227；新增回调三表与短视频、游客会话等26张表是Worker扩展，不冒充PHP安装SQL的201张共享源表。最新静态路由审计为PHP1,904、TS1,446、精确匹配746、可执行匹配728、明确不可用18、原始缺失1,158、证据化退役4、可执行缺口1,154，覆盖为39.2%/38.2%/38.3%；`/api`为PHP457、TS757、精确364、可执行361、不可用3、原始缺失93、退役1、可执行缺口92，覆盖为79.6%/79.0%/79.2%。`/kefuapi`仍为精确/可执行60/63、3条退役、`actionableMissing=0`，Out为41/41；`/api/v2`与`/api/pc`精确缺口均为0。生产WORK域成员、客户、跟进、群、群成员及CorpID/AgentID/旧回调配置仍为空；WORK-C三表已建但全为0行，生产主Worker仍未部署新入口。下一实现批继续WORK-C事件业务投影：把成员/部门/客户/群/标签的provider读取与本地状态机接入Queue消费，并把欢迎语、自动标签等所有远端写拆成独立事务outbox；完成后再进入内嵌Admin 51条。发布前还需完成上述数据/媒体/孤儿修复、防止PHP并行写再次漂移的切流/对账、文章visit热点策略、默认地址单运行时或修PHP后再评估唯一约束、收藏跨栈竞态，以及签到提醒定时投递。真实token、正向E2E、活动装饰/水印、CHECKOUT-DATA、真实支付/微信、Linux runtime、预发、影子流量和正式发布仍受现有门禁约束，正式发布必须另行获得明确批准。
