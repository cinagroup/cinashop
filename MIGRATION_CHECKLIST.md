...
审计起点：`main@55f2652`（2026-08-28，OUT-001 已推送并确认与 `origin/main` 一致）；本文件继续记录其后的 OUT-002、OUT-003、OUT-004、API-001、API-002、API-003 和 API-004 首批进展。PHP 权威源为 `C:\cinagroup\cinashop-php`，Cloudflare 目标为本仓库 `workers-ts` 与五个 TypeScript 前端。生产数据库通过 Hyperdrive `9748c294e21c49a99579c9cef70102e0` 核验；商品、优惠券、用户写入回放账本及订单商品赠券归属账本 DDL 已直接应用。除这些固定、幂等且有业务指纹守卫的 DDL 外，API-001～API-004 对 `public` 只读，合成业务场景只在随机 schema 执行；主 Worker 和正式前端没有因本次审计被发布。
...
| PHP HTTP 合同 | 精确匹配 595/1,904；其中 17 条接入明确 501；3 条有证据退役 | 精确注册 31.3%，静态可执行上限 30.4%，退役后有效上限 30.4% |
| 真实数据复制 | `data_migration_run=0`，本机无 `SOURCE_MYSQL_URL` | 未开始 |
| Worker 单元测试 | 120 文件、693 项通过 | 本地业务回归通过 |
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `/api` | 457 | 613 | 225 | 2 | 232 | 1 | 231 | 48.8% / 48.9% |
...
| 合计 | 1,904 | 1,287 | 595 | 17 | 1,309 | 3 | 1,306 | 30.4% / 30.4% |

API-004 首批后，`/api/user*` 的 PHP 精确缺失仍为 0，`/api/v2` 精确缺口从 54 降为 39；其中两条旧客服扫码登录合同明确不可用，不能计入可执行覆盖。下一层最大缺口组仍包括 `/api` 的 v2、内嵌 admin、PC、marketing、store 与 work；`/adminapi` 的 setting、marketing、product、user、app、supplier、order；`/supplierapi` 的 order、product、file、admin。精确逐路由清单以 `audit:routes` JSON 为准，不在本文复制 1,309 行。
...
- [ ] **API-004 `/api/v2` 54→39 条缺口**：已审计旧 UniApp、`admin-ts`、`pc-ts`、`supplier-ts`、`uniapp-ts` 与 `kefu-ts`；新五端只有 UniApp 抽奖仍直接调用 v2，旧 UniApp 则广泛依赖登录、发票、购物车、DIY、优惠券、分销和评价合同。首批新增 15 条精确路由：搜索列表/清理、客服记录、发票 6、订单发票 3、分销等级 2、评价 1。发票列表/详情/默认值恢复旧端 snake_case 与分页/筛选，新增/编辑补齐手机号、税号和银行字段；单条详情修复 PHP 未按 UID 限定的越权读取，默认项和查重用用户级短事务锁串行化，旧 GET 删除只作为兼容入口且仍按 UID 限定。生产只读确认发票 2、订单发票 0、搜索 0、客服消息 3、有效分销等级 5、任务 0；四类真实读取合同通过，随机 schema 的新增、snake_case、归属隔离、默认、重复拒绝和软删 6/6 通过，`public_state_unchanged=true`、临时 schema `0→0`。剩余清单：
  ...
  - [ ] **API-004-DIY 6 条公共配置/城市**：`get_diy`、绑定开关、门店开关、换色、商品详情 DIY、地址解析；复用已迁移目录时必须核对旧响应结构与参数。
  - [ ] **API-004-CART 4 条购物车/属性**：重置购物车、v2 列表、SKU 属性、改数量；复用现有库存与归属状态机，补旧客户端 E2E。
  - [ ] **API-004-COUPON 3 条优惠券**：新人券、今日券、可领取列表；当前通用列表忽略 v2 的 UID、商品/品牌和排序条件，不能直接别名。
  - [ ] **API-004-USER 5 条用户/分销**：微信资料更新、余额/佣金/提现分类流水、推广用户、推广收益与规则；需恢复 type 语义并保持资金只读。
  - [ ] **API-004-PROMO 3 条促销**：活动商品、赠品信息、凑单商品；旧 UniApp 的前两条只有 API 包装器、尚未发现页面调用，需继续做构建产物/运营证据后决定实现或证据化退役。
...
`DB-001`、`DB-002`、`CORE-002`、`CORE-003`、`KEFU-004`、`OUT-001`～`OUT-004`、`API-001` 已完成；`API-002` 与 `API-003` 的核心代码及生产隔离验证已收口，API-004 首批完成 15/54 条，但仍分别受安全回调/促销叠加/真实订单 E2E，付款码消费端/安全扫码挑战/微信真实凭据/五端 E2E，以及 v2 剩余 39 条合同门禁约束。仓库与生产结构均为 221/221。注释感知的最新静态路由审计为 PHP 1,904、TS 1,287、精确匹配 595、可执行匹配 578、明确不可用 17、证据化退役 3、可执行缺口 1,306；`/api` 为精确 225/457、可执行 223，Out 为 41/41。真实首页/推荐/详情内容数据、订单售后运营数据、Out 账号/逐路由权限、OUT-005/CORE-001 安全回调、真实客户验收和发布仍未完成。`DB-003` 涉及删除冲突配置，必须先由运营确认权威值；`DATA-001` 需要源 MySQL 连接；Kefu 与 ERP 仍依赖新安全合同或第三方材料。等待外部条件期间继续 **API-004-DIY/CART/COUPON/USER**，同时保留 API-002/API-003/API-006 与五端 E2E 门禁；第三方配置和正式发布仍需要用户提供外部条件或明确批准。
