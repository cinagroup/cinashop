...
`DB-001`、`DB-002`、`CORE-002`、`CORE-003`、`KEFU-004`、`OUT-001`～`OUT-004`、`API-001` 已完成；`API-002`、`API-003`、API-004-CART/DIY/COUPON/USER/PROMO/HOME、API-005-PC、API-006-ACTIVITY、API-006-MARKETING-NEWCOMER、API-006-MARKETING-SHORT-VIDEO、API-006-CHECKOUT 与 CHECKOUT-DATA 的核心代码及生产隔离验证已收口。当前仓库/生产结构清单为 223/223；短视频两表是 Worker 兼容扩展，不冒充 PHP 安装 SQL的 201 张共享源表。最新静态路由审计为 PHP 1,904、TS 1,356、精确匹配 665、可执行匹配 645、明确不可用 20、证据化退役 3、可执行缺口 1,236；`/api` 为精确 295/457、可执行 290，Out 为 41/41。CHECKOUT-DATA 当前只剩外部/运营门禁：取得或明确配置现存活动 SKU与限购、批准并取消 4 个未支付历史单、真实渠道/E2E/预发/发布。没有源数据或处置批准时，下一项可执行开发优先级转为 **API-004-AUTH/CORE-004**。短视频真实内容与 R2 媒体、真实用户 E2E、预发和正式发布仍未完成，正式发布仍需用户明确批准。
