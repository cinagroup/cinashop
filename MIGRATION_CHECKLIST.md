...
审计起点：`main@55f2652`（2026-08-28，OUT-001 已推送并确认与 `origin/main` 一致）；本文件继续记录其后的 OUT-002、OUT-003、OUT-004、API-001 和 API-002 进展。PHP 权威源为 `C:\cinagroup\cinashop-php`，Cloudflare 目标为本仓库 `workers-ts` 与五个 TypeScript 前端。生产数据库通过 Hyperdrive `9748c294e21c49a99579c9cef70102e0` 核验；商品、优惠券和用户写入回放账本 DDL 已直接应用，API-001/API-002 对 `public` 只执行只读事务，合成业务场景只在随机 schema 执行，主 Worker 和正式前端没有因本次审计被发布。
...
| 仓库目标结构 | 外部 SQL 220 表；Worker 内嵌 220 表；表/列/主键漂移 0 | 完成 |
| 生产目标结构 | 220/220 表；缺失 0、额外 0 | 完成 |
| PHP HTTP 合同 | 精确匹配 567/1,912；其中 15 条接入明确 501；3 条有证据退役 | 精确注册 29.7%，静态可执行上限 28.9%，退役后有效上限 28.9% |
| 真实数据复制 | `data_migration_run=0`，本机无 `SOURCE_MYSQL_URL` | 未开始 |
| Worker 单元测试 | 116 文件、674 项通过 | 本地业务回归通过 |
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `/api` | 460 | 585 | 197 | 0 | 263 | 1 | 262 | 42.8% / 42.9% |
...
| 合计 | 1,912 | 1,259 | 567 | 15 | 1,345 | 3 | 1,342 | 28.9% / 28.9% |

最大缺口组：`/api` 的 v2 54、admin 51、PC 22、user 13、marketing 13、store 12、work 10；订单组本批只剩必须归入 CORE-001 的 `ANY /api/order_call_back`，线下支付三条属于 OtherOrder 独立批次。`/adminapi` 的 setting 153、marketing 115、product 98、user 78、app 76、supplier 67、order 57；`/supplierapi` 的 order 30、product 16、file 11、admin 8。精确逐路由清单以 `audit:routes` JSON 为准，不在本文复制 1,345 行。

### 生产数据库事实

- PostgreSQL 16.14；应用 OUT 用户回放账本及三项数据库并发守卫后，生产 `public` 当前 220 表、3,046 列、708 索引、207 主键。
- `order_print_job`、`order_print_job_action`、`order_waybill_job`、`order_waybill_job_action` 已创建且行数为 0。
- `out_product_write_replay`、`out_coupon_write_replay` 均为 8 列，`out_user_write_replay` 为 9 列；三表都是 4 约束、3 索引且当前 0 行。有效手机号、Out 余额流水、Out 积分流水三个唯一索引均有效；仓库 220 表清单与生产集合差均为空。
...
- [x] **AUD-001 可重复路由审计器**：`workers-ts/scripts/route-parity-audit.ts` 与 `npm run audit:routes` 当前验收为 Kefu 63/48、Out 41/41、ERP 8/0；新增有证据退役清单并同时输出原始缺失、退役和可执行缺口，清单漂移、重复或与 TS 注册冲突时直接失败。
...
- [ ] **API-002 订单与售后 19 条缺口（代码/生产隔离验证部分完成）**：原 19 项中，`POST /order/comment` 已在 API-001 前完成；本批新增 16 条精确合同，覆盖配送能力、确认单、金额预览、订单统计/奖励/核销记录/评价商品/门店收银、退款原因/商品摘要/兼容申请/退货物流/再次申请/终态删除，以及五分钟不透明键的支付宝兼容入口。`GET /order/nopay` 的 PHP 控制器方法不存在且无一方调用，已带源行证据退役；`ANY /order_call_back` 仍缺失并转入 CORE-001，禁止复制旧 AES 解密后无签名、无重放账本的写状态逻辑。现有建单/支付/取消/收货/退款状态机补齐 PHP 参数别名、状态筛选、用户归属和退款行锁；部分退款改为依据服务端商品快照分配实付金额，整单也保存逐商品精确数量，不信任客户端 `refund_price`。生产 Hyperdrive 随机 schema 已验证下单并发/超卖、支付取消竞争、退款幂等/回滚/累计上限/补偿，以及高低价商品部分退款 `90.00` 与整单精确快照，三组均 `schema_removed=true/public_state_unchanged=true`。本项尚不能勾选：确认/计算接口的展示报价还没有完整复用运费、券、会员价、积分和首单定价核心；下单奖励尚未恢复旧商品券发放副作用；退款/核销/门店相关真实数据为空，旧客户端/五端浏览器 E2E、预发与发布均未完成。
...
`DB-001`、`DB-002`、`CORE-002`、`CORE-003`、`KEFU-004`、`OUT-001`～`OUT-004`、`API-001` 已完成，`API-002` 已完成 16 条精确注册、1 条既有评价确认和 1 条退役决策，但因展示定价/赠券副作用/安全回调/真实数据与 E2E 未闭合而保持未完成；仓库与生产结构均为 220/220。最新静态路由审计为 PHP 1,912、TS 1,259、精确匹配 567、可执行匹配 552、证据化退役 3、可执行缺口 1,342；`/api` 提升为 197/460，Out 保持 41/41。真实首页/推荐/详情内容数据、订单售后运营数据、Out 账号/逐路由权限、OUT-005/CORE-001 安全回调、真实客户验收和发布仍未完成。`DB-003` 涉及删除冲突配置，必须先由运营确认权威值；`DATA-001` 需要源 MySQL 连接；Kefu 与 ERP 仍依赖新安全合同或第三方材料。等待外部条件期间继续收口 **API-002 展示定价与订单奖励**，再进入 API-003；第三方配置和正式发布仍需要用户提供外部条件或明确批准。
