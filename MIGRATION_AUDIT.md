...
完整 Worker 单元 90502 已退出 0：371 文件／4,287 项通过，零失败／跳过／todo；1150 个选定路径在启动前和终态后哈希一致，报告哈希及 PG 零残留记录见 `workers-ts/audit/admin-config-batch-authority-20260910.json`。随后才解除源码冻结，补 shop 精确来源配置和生成类型；新测试先复现 4 失败，修改后联合旧登录边界共 21 项通过。这是独立增量，不把旧全量结果借给新候选。

shop 白名单仍为本地待发布配置，未更新生产 Worker。自身 CI、workerd、迁移／最小权限／正式登录与订单验收仍开放；清单维持 234／170／404。下方“90502 正在运行”“无新部署”等均为此前阶段记录；本次确有 PC 预览部署，尚无正式应用切换、提交或推送。

## 发布授权与正式域名基线（2026-09-10）

最终域名状态：08:57 UTC控制面三状态均active；HTTPS首页200，内置浏览器显示真实导航、推荐商品和分类。仍加载旧PC入口，不称新应用部署完成。以下pending为初始状态，完整记录已追加最终验证。

用户最新目标包括提交、推送、部署和线上测试，已确认Admin使用cinashop-admin.pages.dev，并要求不占用现有cinaseek.ai根域名／admin／api，改用shop.cinaseek.ai。Cloudflare控制面证实shop为空闲，已新增其到cinashop-pc的Pages绑定和CNAME，未改动其它DNS／旧部署。HTTPS首页200且加载旧index-ClV6Erfd.js，证书控制面尚pending；域名可达不等于新代码发布或登录验收。

Worker当前100%版本9f1fd655-e60f-41c1-8280-738bc85d73ef；PC正式部署5f3bfab4-64cc-443c-8d5c-a400508d519e，Admin正式部署00ac193e-be07-464e-8e77-b89295e9df7a，已记录回滚ID而未实施回滚。九个公开GET检出Admin同源API路径返回HTML壳，直接Worker则拒绝匿名访问（410000）；普通GET可能产生日志／浏览统计，不称数据库只读事务。详情见workers-ts/audit/release-domain-baseline-20260910.json。无新应用发布、提交或推送；REL发布批准缺口已消除，其余170项仍须逐项完成。

## 后台配置原子保存候选（2026-09-10）

已确认服务端`/api/admin/config/save`对超级管理员及config.manage可达，前端通用编辑器仍禁用。新增实际JWT／账户／角色中间件测试在轻量及完整ORM隔离PG中首轮36项12通过／24失败；现以完整请求校验、READ COMMITTED／配置表短期互斥保护、两条批量DML更新全局有效赢家并补缺失项、回读核验与提交后全量KV失效，修复部分提交／门店误写／重复输家更新及缓存过早清理。保留原业务值语义、门店与输家元数据，不删除历史数据或重开前端编辑器。

最终新增46项、联合6文件132项零失败／跳过，双类型通过；真实PG等待覆盖后台保存与下单提交／回滚、未提交政策和缺失键竞争，仍为合成账号／KV／取号，非正式Redis登录／Hyperdrive验收。证据`workers-ts/audit/admin-config-batch-authority-20260910.json`，合同`workers-ts/docs/admin-config-batch-authority.md`。专用PG核对四基线库及零残留后启动完整Worker单元90502；1150选定路径启动前哈希已记录，冻结源码等终态。不把旧4231项或本次132项叠加成完整通过。最低权限SHARE能力、其它入口与负载、workerd／CI／发布仍开放，清单234／170／404不变；无提交推送、生产访问或部署。下方“写端尚未修复”属于历史状态。

## 全量失败终态与局部修复复验（2026-09-10）

完整Worker单元首轮终态为370文件364通过／6失败，4231项4225通过／6失败／0跳过，另1个afterAll超时。终态后修复完整建单SQL政策种子、两处索引证据行号、活动快照合同断言和序列清理期限；没有改变原金额／库存／回滚断言。Out删券先占配置表锁再等优惠券行锁导致下单末端拒绝，现调整到券行锁之后NOWAIT取得配置锁，保留引用保护和事务。三时区矩阵进一步复现取消拼团预检／锁后隐式时区转换的3项错误，已统一UTC比较，未使用服务器UTC设置掩盖问题。

最终15文件275项零失败／跳过，Worker双类型通过；1148个选定路径的合并哈希在最终测试前后相同。专用PG核验仅四个基线库，零临时schema／角色／公共表／额外客户端；详情、阶段报告与哈希见`workers-ts/audit/pricing-full-regression-20260910.json`。这不是完整370文件重跑或正式发布通过。后台批量配置原子性／赢家选择、最小权限及其余门禁仍开放；通用配置前端本已禁用，但服务端路由仍在，不把直接处理器测试写成当前UI验收。清单仍234／170／404项，无生产写入、部署、提交推送。下方“全量仍运行”是本节之前的历史状态。

## 计价配置写端与权限反例（2026-09-10，尚未修复）

完整ORM隔离库的真实后台`/api/admin/config/save`处理链确认：批量请求第二个5001字符值写入失败时，首个包邮开关已经提交，报价运费由600分变为0；另一个重复键反例返回200，却更新了低优先级记录，权威SQL仍为0而非请求的1。该入口逐键自动提交且getOne未限定全局作用域／排序，需修复原子性与有效记录选择，并覆盖真实后台与下单的交错。PHP参考`save_basics`同样逐项写入，不能把历史行为当作允许部分计价配置提交的依据。未创建订单或调用生产接口。

独立非所有者LOGIN权限探针确认：SELECT-only以及SELECT加两表UPDATE(id)均在SHARE锁处报42501；角色无超级用户／建库／建角色／BYPASSRLS能力。两个探针的随机库与临时角色均已清理，五个源／帮助器哈希核对相符，见`workers-ts/audit/checkout-pricing-open-boundaries-20260910.json`。不得简单给结算只读角色增加配置表写权限来掩盖缺口。全量单元句柄41115仍运行，已见订单创建场景失败及序列执行器文件异常标记，最终原因／总数待终态报告；本轮未改动其源码或测试，不宣称全量绿色。清单仍234／170／404项，未生产写入、部署、提交或推送。

## 授权后的线上只读复验与计价候选状态（2026-09-10）

通过已连接的CUA内置浏览器复验正式PC首页→全部商品→价格升序→商品16详情：页面非空、导航及升序生效，采集的console error/warn为空，截图未见框架错误层。但列表12张商品图仍为未加载的`via.placeholder.com/300`；详情`/goods/16`的标题和画廊为空，价格99.90、库存100及购买按钮可见。没有点击购买／加购／收藏，没有登录或生产数据写入。此结果再次确认FE-002B/C的线上问题仍存在，不是新发现的源码缺口：当前PC代码已有字段归一化与图片失败态，未核验正式部署ID/SHA，不能把本地候选等同线上版本。未重跑窄屏、真实角色、支付或发布验收，FE-002与REL门禁不关闭。

结算计价候选已将11个配置键和两类会员权益改为单条SQL权威读取，末端READ COMMITTED／SHARE NOWAIT保护后重新核验；完整ORM新增20项已在进行中的全量Worker单元运行中通过，最终套件结果尚未产生，不宣称全量绿色。详见`workers-ts/docs/checkout-pricing-config-authority.md`。全量测试句柄41115与专用本机PG仍运行；真实角色锁权限、全局表锁竞争、其它配置组及运行时／发布门禁仍开放。清单保持234完成／170开放／404项，本轮未部署、提交或推送。

## Out库存条码候选边界（2026-09-10，工作区候选）

原发现查询在一个条码400重复、100条码各10重复时，真实返回400／1000行；业务虽已拒绝歧义，Worker仍先收集全部候选。改为先过滤合法平台实物／活跃普通SKU，再按条码窗口排名取前两行，发现结果最多200行；缺失／初始歧义在SKU取锁前整批拒绝，全部唯一才进入原最多100个SKU的NOWAIT及复核。第一版LATERAL限量虽通过7项，却经实际EXPLAIN复现100次SKU Index Scan，已改为单个过滤集合排名；不承诺数据库扫描／排序成本恒定，也未加业务索引或写生产DDL。

新增完整ORM13项涵盖返回量、执行计划、100条合法同步／重放、末项缺失零变更、七种非目标行过滤、中文／引号／大小写参数。最终12文件164项零失败／跳过、双类型通过，43个选定哈希在最终启动前与结束后相同。原0通过／2失败及第一版计划2通过／1失败均保留，阶段不累加。Workers／PostgreSQL技能驱动真实驱动观察及计划检查；合同与证据见workers-ts/docs/out-stock-candidate-boundary.md、workers-ts/audit/out-stock-candidate-boundary-20260910.json。

专用UTC PG16.15核验4基线库、零附加schema／public表／临时角色／其它客户端后07:08:47.475 UTC正常停止；原执行句柄退出0、PID和PID文件不存在，保留程序／数据目录／六份报告。仍234完成／170开放／404项；A3k11c、生产规模成本、其它并发组合、正式身份／Hyperdrive／workerd／完整CI／发布门禁未完成，未生产写入、部署、提交或推送。下文旧阶段哈希及待办是历史状态，条码返回量以本段为最新证据。

## Out商品管理并发与退役库存（2026-09-10，工作区候选）

实际Out上下架、商品编辑、跨商品库存同步与下单复现3个PG死锁，服务器06:35:12.355／06:35:17.517／06:35:22.569 UTC记录完整等待双方。商品／SKU取锁改为NOWAIT，保存／上下架同SQL锁定购物车候选再更新；仅显式锁语句55P03转刷新重试，原事务整笔回滚，保持Out原有所有购物车／关系联动政策和幂等锁序。扩大反例另外确认9件活跃库存误加99件退役库存得到108，汇总已排除退役行而保留历史。

新增完整ORM17项，涵盖实际死锁、忙后续行的先前写入／锁释放、四操作末端账本失败回滚、下单后库存／销量保留、同键独立连接创建及重放、非锁错误和退役总量。最终11文件151项全部通过、零失败／跳过，两套类型通过，42个选定哈希在测试期间及结束后相同。原红测0／3、扩大21／1及新测试DAO参数类型错误保留，不累计阶段结果。合同及报告哈希见workers-ts/docs/checkout-out-product-concurrency.md与workers-ts/audit/out-product-concurrency-20260910.json。

专用UTC PG16.15核验4基线库、零夹具／public表／临时角色／其它客户端后06:46:42.992 UTC正常停止。条码候选集合有界性、其它全局锁／引用／DDL与新购物车交错、正式身份／Hyperdrive／维护／workerd／完整CI／发布仍开放；A3k11c不关闭，234／170／404项，未写生产、部署或提交推送。Workers／PostgreSQL技能指导真实锁证据与整笔回滚对照。

## 移动规格更新请求体边界（2026-09-10，工作区候选）

三个真实控制器红测确认超大合法JSON可进入服务；现复用已有128 KiB流式读取器，校验实际字节数并在超限时取消读取，不改库存事务和500行上限。新增22项请求／流合同及1项完整ORM的500-SKU行级更新验收，覆盖超限零变更和恰好上限完整提交。最终9文件129项全部通过、零失败／跳过，两套类型通过；35个选定哈希在运行期间与结束后相同，不冒充不可变完整CI。

Windows workerd启动访问冲突，实际0项、退出1，仍未通过运行时验收。专用UTC PG16.15核对零夹具／public表／临时角色／其它客户端后06:26:12.738 UTC停止。合同与原始报告哈希见workers-ts/docs/mobile-product-request-body.md及workers-ts/audit/mobile-product-body-20260910.json。其它管理交错、正式身份／Hyperdrive／CI／发布等仍开放，A3k11c不关闭，234／170／404项；未写生产、部署或提交推送。Workers技能指导复用流式解析及跨运行时验证。

## 移动商品管理／批量操作三类死锁（2026-09-10，工作区候选）

实际上下架／规格库存／多商品批量运营与真实下单复现3个PG死锁，服务器分别05:55:50.961、05:55:57.540、05:56:02.397 UTC记录购物车／SKU／跨商品反向等待。管理服务商品和活跃SKU取锁改为NOWAIT；上下架同一SQL锁定购物车候选并更新，不跳过忙行、不将不限量ID拉入Worker。只将显式锁操作55P03转为刷新重试，整笔回滚；原价格库存、过滤、引用、时限、日志政策和下单守卫不变。

新增完整ORM19项覆盖3个死锁、购物车过滤和锁释放、明确刷新后的库存差额／销量／退役SKU排除、九类双商品批量操作、末端错误回滚及非锁错误传播。最终联合7文件101项全通过、零失败／跳过；双类型通过，28个选定源／测试哈希未变。初始3项真实红测和各阶段报告保留，不累计重叠通过数。合同及证据见workers-ts/docs/checkout-mobile-product-concurrency.md和workers-ts/audit/mobile-product-concurrency-20260910.json。

专用UTC PG16.15核验4基线库、零夹具／public表／临时角色／其它客户端后，06:05:56.838 UTC正常停止。仍需其它Out／SKU／引用管理交错、正式身份权限／Hyperdrive／维护／CI／发布等完整验收；移动规格更新控制器直接解析JSON的有界请求体合同待后续验证。本机专项不关闭A3k11c，清单234／170／404，未部署、写生产或提交推送。Workers／PostgreSQL技能指导真实锁证据、同SQL候选集合和完整回滚验证。

## SKU 退役／多商品三方死锁（2026-09-10，工作区候选）

完整 ORM＋实际下单／后台编辑／退役复现三方死锁，PG 05:34:27.605 UTC 明确记录退役持全局 SKU 身份锁等待买方商品 A，买方等待编辑商品 B，编辑等待退役全局锁。退役服务保留原建议锁顺序，商品和选定 SKU 行改为 NOWAIT；仅这些取锁读取的55P03提示刷新重试，整笔事务回滚，无自动重试，不改下单库存守卫、时限、引用与身份政策。

最终5文件73项全部通过、零失败／跳过，含新增17项完整ORM并发／回滚专项；后台／供应商退役恢复、忙商品／SKU、先取得锁释放、正常引用与身份／库存／日志、末端错误完整回滚均有证据。两套类型通过，21个选定源／测试哈希未变。首次扩大45通过／10失败全为新种子9字符超过原character(8)，仅修正种子，原失败保留；各阶段不相加。详细合同和原始报告哈希见workers-ts/docs/checkout-sku-lifecycle-concurrency.md及workers-ts/audit/sku-lifecycle-concurrency-20260910.json。

专用UTC PG16.15确认4基线库、零夹具／public表／临时角色／其它客户端后，05:50:09.927 UTC正常停止，报告和本机数据目录保留。其它Out／移动商品入口、多商品组合和正式鉴权／权限／Hyperdrive／CI／发布仍开放，A3k11c不勾选，234／170／404项。Workers／PostgreSQL技能指导精确锁屏障与完整回滚；未部署、写生产或提交推送。

## 商品／分类管理与下单真实死锁（2026-09-10，工作区候选）

完整ORM＋实际后台保存／下单＋独立PG连接复现商品／SKU反向死锁，服务器05:01:33.742 UTC确认买方等待编辑者商品行、编辑者等待买方SKU。共享SKU编辑函数现按ID使用FOR UPDATE NOWAIT，仅55P03提示刷新库存后重试，整笔编辑事务回滚；40P01、语句取消及连接失败不转换。不改下单库存守卫／公式，不自动重放旧库存输入。后台和供应商真实保存、新建、明确刷新重试、多SKU锁释放、末端失败回滚均有证据；Out分类树移动／显示／启停／删除与下单的先后竞争覆盖有效范围与引用保护。

新增完整ORM专项25项及SKU合同8项最终33通过、零失败／跳过，双类型通过。扩大58文件曾1449通过／3失败：旧阻塞锁字符串断言，以及两个新建对照的显式种子未推进序列；服务器主键冲突单独复现，修正仅在两个测试文件内。其余56文件1419项通过且依赖源码未变；两阶段不重叠覆盖58文件1452项，不冒称修正后单次全量58文件绿色或完整CI。86个选定源码／测试哈希、实际文件清单、原失败和类型诊断均记录于workers-ts/audit/catalog-management-20260910.json。

合同见workers-ts/docs/checkout-catalog-management.md。专用UTC PG16.15核验4基线库、零夹具／public表／临时角色／其它客户端后，05:26:53.554 UTC正常停止，原始报告和数据目录保留。其它商品／SKU入口、多商品组合、正式鉴权／权限／Hyperdrive／维护隔离、全局配置／主体／表单／PHP政策及不可变CI／发布仍开放；A3k11c不勾选，仍234／170／404项。Workers／PostgreSQL技能指导真实锁屏障和完整回滚对照，未部署、写生产或提交推送。

## 优惠券管理／下单真实处理链（2026-09-10，工作区候选）

最终同候选47文件1339项全部通过、零失败／跳过（管理专项52项包含在内），实际文件集合匹配，71个选定源码／测试／帮助器哈希保持不变，最终unit／runtime类型通过。首次扩大回归1333通过／6失败保留：5项完整数据库清理超时，现独立限时120秒；另1项分销时钟有效对照未记录底层异常，补充诊断后本次未复现，原始原因不宣称已确定。最终仅限制跨文件并行数为2，不改业务断言、SQL时限或用例内部真实PG竞争。专用UTC PG16.15核验4基线库及零夹具／角色／其它客户端残留后，04:55:41.200 UTC确认停止，原始报告及数据目录保留。详见coupon-management-20260910.json；这不是全项目CI或所有开放门禁完成。

新增实际HTTP处理链测试：后台保存、分类保存／删除、品牌显示信息保存，以及Out创建／启用／删除，经真实控制器→服务→DAO／SQL与实际订单创建交错。轻量列级结构和完整ORM生成DDL各执行相同26场景，扩展专项52项全部通过、零失败／跳过。完整模式使用专用随机数据库，不改写ORM约束；通过显式测试工厂复用原PC确认夹具，默认轻量模式不变，无生产URL回退。业务源码未改。

四独立后端和pg_blocking_pids证明：后台先持模板锁时下单拒绝，后台后到时等待真实下单提交／回滚；Out新建先后与库存锁竞争均完成，新模板／关系／幂等账本同事务生效；末端账本错误使三者全回滚，同key并发及重试只生成一次；Out启用先持模板、反向等库存时，由下单末端NOWAIT拒绝解除等待。Out删除保留已领券适用关系，reserved数量跟随实际下单结果；已有分类／品牌守卫与真实管理保存／删除交错按有效范围或显示字段区分。已生成Out模板可进入实际确认／下单，但已领券行仍为合成种子，不冒充领取或鉴权验收。

初始15项轻量通过；完整模式首轮26通过／4失败是两个调用点仍误用只允许轻量库的连接帮助器，保留原失败报告，改用专用数据库自身的独立连接接口后通过，没有扩大数据库身份白名单或放宽业务预期。最终双类型检查通过；涉及共享夹具的全部23个测试调用方纳入47文件最终回归，结果、实际清单与71个选定源／测试／帮助器哈希见workers-ts/audit/coupon-management-20260910.json，不将重叠阶段相加。

详细边界见workers-ts/docs/checkout-coupon-management.md。本轮仅补齐所列优惠券、分类和品牌显示管理入口的本机交错证据；商品编辑、Out分类树及其它组合，正式管理员／Out认证、全应用最小授权、生产Hyperdrive／维护隔离、配置／主体／表单／PHP政策、不可变CI与发布仍开放。A3k11c不勾选，清单234／170／404项。Workers／PostgreSQL技能促使保留真实锁屏障、两种schema及提交／回滚对照；最新类型获取EACCES后回退已安装5.20260828.1，无平台绑定变更。没有提交推送、部署、真实provider或生产数据写入。

## 优惠券商品／分类／品牌末端权威（2026-09-10，工作区候选）

最终关联33文件947项全部通过、零失败／跳过，新专项55项已包含在内；实际文件清单匹配，48个选定源码／测试／帮助器哈希未变，最终unit／runtime类型通过。PGlite原模板22通过／3个真实PG条件跳过单独记录。专用PG16.15本轮实际时区Australia/Darwin（日志ACST，不冒称UTC实例），核验4基线库、零临时库／schema／public表／测试角色／其它客户端后，04:17:52.312 UTC确认正常停止；程序、数据目录及原报告保留。报告及源哈希见coupon-item-authority-20260910.json，不与94／40／52及此前阶段叠加，也不冒充全项目CI。

首轮22项实际HTTP得到9通过／13错误放行：商品父归属、直接分类／品牌、分类pid／path、品牌pid／fid的晚到变化或删除，以及最终仍缺失的正向引用。现将原计算器的商品范围输入复制为内部创建依赖，在原库存写入与券模板校验后，按ID重读并SHARE NOWAIT锁定商品及实际使用的直接分类／品牌，比较既有quoteFacts的规范化有效集合。保留parent=0与自身ID、重复／排序编码及直接ID变化但完整集合等价；分类／品牌显示字段和未使用范围字段不新增拒绝条件。没有改变优惠公式或客户报价字段，也未改库存UPDATE语义。

保护要求所属READ COMMITTED事务；隐式关系锁同样用临时1ms lock_timeout有界等待，成功恢复原值，SQL失败不再执行恢复语句覆盖原异常。只读取原计算器使用的直接引用行的祖先编码，不新增递归祖先政策。最终缺失的正向引用无法锁定，创建失败关闭；晚到创建若已存在、被锁定且有效范围仍相同则允许。只用到对应券类型的分类／品牌表，非使用券路径不加入这些依赖。

原22项与原模板／关系两文件共94项通过；新增并发扩展40项通过；进一步事务约束、错误分类及完整ORM真实非所有者LOGIN专项52项通过。实际pg_blocking_pids证明未提交引用写者被拒绝、计算后独立提交被新读发现、库存等待后的新商品归属被校验，以及后来的商品／分类／品牌写者等待下单真实提交或下游错误回滚。真实低权限对照依次证明模板权限不足以锁商品，商品权限不足以锁引用；仅补相应单列UPDATE即可通过，不扩大至表级写授权。首轮类型检查因预留并发测试导入／帮助器未使用而失败，补齐测试后双类型通过；最终新增多商品及明确重确认用例的关联回归另记，不累加中间阶段。

证据见workers-ts/audit/coupon-item-authority-20260910.json，合同见workers-ts/docs/checkout-coupon-authority.md。本轮覆盖前轮商品／引用待验的本机子范围，不等于完整管理处理链、所有业务组合、正式Hyperdrive身份／权限、维护隔离或不可变CI／发布验收。A3k11c保持开放，清单234／170／404项；Workers／PostgreSQL技能指导了短事务、有序有界锁和实际权限验证，最新类型获取EACCES后回退已安装5.20260828.1，无平台绑定变更。没有生产写入、真实provider、部署、提交或推送。

## 优惠券关系下单读端与0157注册（2026-09-10，工作区候选）

最终关联回归32文件892项全部通过、零失败／跳过，实际文件集合与计划相符；新增关系读端47项、注册7项已包含在892项内，不另加。最终unit／runtime类型检查通过，14个源码／测试哈希及9份历史阶段报告哈希复核一致。专用PG16.15核验4基线库、零临时库／schema／public表／测试角色／其它客户端后，于03:57:32.140 UTC确认正常停止，保留二进制、数据目录和原始报告。详情见workers-ts/audit/coupon-relations-authority-20260910.json；不是全项目CI或正式环境验收。

首轮真实HTTP测试18项中14项错误放行：关系扩展／替换／末行删除／转移／空集合插入／TRUNCATE／超过20000行，以及七种协议缺失或漂移。现对实际使用商品券的下单，在已领取券占用后依次取得关系ACCESS SHARE NOWAIT、模板关系锁有界预取、核验保护目录、模板SHARE NOWAIT，再用独立新语句重读集合并规范化比较。保留重复／非正ID、无关券、编码回退等价和成功幂等；范围变化即使金额相同也要求重确认。无新增优惠公式、quoteFacts、持久化字段或事务外部I/O，不在请求内安装或授权。

目录核验覆盖READ COMMITTED／复制角色、同schema普通永久表、RLS／继承／列／父主键及函数源码／属性、触发器事件／转换表／启用状态／条件等，不缓存ready。只有55P03映射为重确认，其它死锁／超时／连接错误保留。行级NOWAIT不涵盖隐式关系锁，因此模板用零行FOR SHARE预取ROW SHARE关系模式，仅该语句临时lock_timeout=1ms，成功后恢复原设置，失败随整笔事务回滚恢复，避免持库存锁长时间反向等待。普通关系DML仍可并发；影响同一模板的写者由0151父锁协议等待本次下单结束。目录快照不能防止其它特权连接绕过触发器或改写函数，正式身份与维护隔离仍待验。

32文件第一轮回归889通过／1失败是实际非所有者权限反例：显式LOCK ROW SHARE需要表级写权限，单列UPDATE不够。改为上述零行FOR SHARE预取，没有扩大GRANT或放宽冲突拒绝；新增原lock_timeout为0／31ms时恢复的对照。失败报告与修复后的专项／最终回归分别保留，不将这一错误写成夹具偶发或已验收。

接入后18项通过；扩展与原模板测试60项通过，涵盖真实四独立后端的未提交写者拒绝、创建内独立提交后新语句读取、等价变化、关系写者等待实际提交／下游错误回滚、TRUNCATE等待、不兼容关系锁拒绝及隔离／复制角色对照。PGlite原模板22通过／3项真实PG条件跳过仅为内存兼容证据。最终新增属性和错误分类扩展及32文件回归另见workers-ts/audit/coupon-relations-authority-20260910.json，不累加中间阶段。

磁盘0151现注册到内嵌0157，新建库158步通过独立根事务执行器运行。旧0000–0156共157个迁移方法AST文本SHA-256前后相同（eba7ca7bf38eced9101b09cf9cc14d73ea49c8139b6ff450225e03d91640d4ad）。注册专项先确认缺失失败，接入后三文件41项通过；完整外部SQL、内嵌、ORM后独立升级均实际运行，前一步／本步／DDL后失败停止并回滚，不将already-exists冒充成功。ORM路径进一步以真实非所有者LOGIN执行读端与保持锁验证，最终结果在本轮报告记录。现有数据库禁止为安装它重放历史runAll，正式数据库没有被升级。

上述证据覆盖前轮“读端与注册待完成”的本机子范围。商品pid／cateId／brandId、分类／品牌引用及缺失引用新增、实际后台／Out处理链交错、正式Hyperdrive身份／权限、PHP政策、不可变CI与发布仍未完成；A3k11c不勾选，清单仍234／170／404项。Workers／PostgreSQL技能指导了新语句快照、非排他关系锁、失败关闭及短事务，最新类型获取EACCES后回退本机5.20260828.1，无绑定变更。未提交推送、部署、真实provider或生产数据写入。

## 优惠券商品关系写端保护（2026-09-10，工作区候选）

新增独立0151 SQL、等价Worker模块及显式维护执行器，以AFTER STATEMENT转换表计算券—商品组合的行数净变化，按模板ID排序获取NO KEY UPDATE并保持到事务结束。覆盖新增／首行、删除／末行、重复关系、商品变化、单行和批量转移、BEFORE触发器最终值、同事务新建模板与替换关系；完全相同的多重集合及零行语句不额外锁父行。不重写业务数据，不增加唯一性，不改变优惠公式。受影响模板缺失则DML回滚；安装前已有孤立关系、RLS／继承／列／父主键及已存在保护定义漂移均拒绝，安装中途失败不留下部分保护对象。

首轮41项通过；扩展关联7文件首轮149通过／1失败是测试在max:4连接池直接BEGIN触发UNSAFE_TRANSACTION，改为已有固定单连接帮助器后，最终7文件150项全部通过、零失败／跳过，其中新文件45项。保留两项缺口对照：无协议时模板SHARE挡不住关系INSERT；只有父锁时TRUNCATE仍能完成。额外关系ACCESS SHARE可阻止TRUNCATE，普通DML与该关系锁兼容。完整ORM＋独立非所有者LOGIN实际验证缺少模板UPDATE时42501整笔回滚、补单列UPDATE后真实锁等待／增删改成功及危险DDL拒绝，但不代表生产身份或全应用GRANT清单。

现有后台保存和Out创建的关系写入次序已静态核对，相关SQL自持父锁对照通过；并非这两个业务处理链的完整集成验收。累计消费迁移测试也执行了完整磁盘迁移目录，包括新0151；内嵌MigrationService仍157步，新关系协议尚未注册到下一步0157。更重要的是下单模板守卫仍使用原关系快照：协议目录核验、父锁后的新语句集合重读、商品／分类／品牌引用和真实下单并发仍待完成，不能宣称商品券读写合同已闭合。A3k11c保持开放，清单234／170／404。

源与报告哈希、失败修正和边界见workers-ts/audit/coupon-product-scope-fence-20260910.json，合同见workers-ts/docs/checkout-coupon-authority.md。最终回归源码／测试未变，不与首轮41或旧719项相加。专用PG16.15核验四基线库、零临时库／schema／public表／测试角色／其它客户端后，03:21:19.574 UTC确认正常停止，保留程序、数据及原始报告。Workers／PostgreSQL技能指导了短事务、转换表、有序锁和实际权限验证；最新Workers类型下载EACCES后回退已安装5.20260828.1，无绑定变更。本轮无提交推送、部署、真实支付或生产数据库写入。

## 优惠券模板的末端事务权威（2026-09-10，工作区候选）

真实HTTP红测发现9种模板晚到变化仍能建单：满减／折扣类型、范围类型、商品／分类／品牌的新旧编码字段及模板删除；5种无关／等价编辑对照成功。现从原优惠计算输入记录内部模板依赖，在原已领券占用后获取一行SHARE NOWAIT并规范化重读，保持到整笔事务结束。不改变客户报价投影、优惠公式、模板的领券／展示政策或成功订单幂等结果；无券和原本不使用券的路径不新增模板查询。

修复后14项通过，扩展25项最终全部通过：17实际HTTP、3四独立后端并发、1根客户端拒绝、4错误分类。独立写者未提交时下单拒绝，后来的模板编辑分别等待实际下单提交／下游错误回滚；用pg_blocking_pids核验真实阻塞。扩展回滚探针先后用了不存在的SQL字段和错误ORM属性，24通过／1失败均保留；按实际tempId／cityId纠正后25通过，没有弱化拒绝预期。unit/runtime类型通过；关联26文件回归的最终结果及哈希另记在workers-ts/audit/coupon-template-authority-20260910.json，不累加重叠阶段。

本批只保护模板行，不保护store_coupon_product集合新增／转移／末行删除、商品pid／cateId／brandId或分类／品牌引用及缺失行新增；完整设计和未完成项见workers-ts/docs/checkout-coupon-authority.md。下一步须验证数据库关系写入协议与父锁后新语句读取，不能用模板行锁冒充集合无幻读。FOR SHARE还需模板至少一列UPDATE权限，前轮已付订单权限预检不等于全应用授权。完整ORM、生产身份／Hyperdrive、其它政策及不可变CI／发布仍开放，A3k11c不勾选，仍234／170／404项。Workers／PostgreSQL技能指导了真实SQL锁、短事务及无新增外部I/O；未提交推送、部署、真实支付或写生产。

最终关联26文件719项全部通过、零失败／跳过，实际文件集合匹配；执行期间源码／测试未修改，最终源与报告哈希复核一致。专用PG核验4基线库、零临时库／schema／public表／测试角色／其它客户端后，03:00:47.739 UTC确认停止；保留二进制、数据目录及原始报告。不将本机定向结果外推成全项目或正式环境验收。

## 累计消费保护：真实运行身份与权限预检（2026-09-10，工作区候选）

新增显式只读命令 `npm run audit:paid-runtime-permissions`，要求专用连接变量，远端另需明确确认，不回退通用DATABASE_URL或自动连接Hyperdrive。REPEATABLE READ／READ ONLY事务检查必要对象、实际连接／会话／当前身份及可继承、可切换、可再授权角色，拒绝受检查的所有者控制、高权限属性、CREATE／TRIGGER／TRUNCATE、复制角色绕过及未审查的用户SECURITY DEFINER能力。输出仅固定检查名和布尔值，不输出身份、URL或异常原文。此命令不执行迁移／授权、不自动接入deploy、不替代下单的完整触发器／RLS目录检查。

帮助器现在创建真实独立非所有者LOGIN及合成随机凭据，验证后删除自身创建的角色和授权；不再把管理员SET ROLE当成低权限连接证据。实际余额付款、累计消费资格、全额退款／重放与独立登录间锁等待通过；渠道付款即使不扣余额，保护函数的用户行锁仍需要至少一列UPDATE，仅SELECT实际42501并整笔回滚，补UPDATE(now_money)后通过。八种实际破坏性DDL／复制绕过操作均被低权限角色拒绝。该列级例子不是全商城最小GRANT清单，也未改变付款、退款、保护SQL或SECURITY INVOKER策略。

审查保留两处预检自身的红绿证据：SET SESSION AUTHORIZATION可同时改变current_user与session_user，原预检错误通过，加入pg_stat_activity.usesysid后拒绝；NOT LIKE 'pg_%'的下划线通配符误排除普通pgx_review schema，原预检遗漏可执行SECURITY DEFINER函数，改为字面starts_with后拒绝。并发探针也从同订单行UPDATE改为零行UPDATE，仅取得订单关系锁，再验证用户保护锁的实际等待，不混淆两种阻塞。

最终权限专项22项全部通过、零失败／跳过，unit/runtime双类型通过。此前核心7文件185项通过，随后只修改独立预检及其测试；其中21项权限结果由最终22项取代，其余6文件164项依赖未变。共享帮助器另6文件121项零失败／跳过，涵盖全部其它直接消费者。各次报告分别记录，不相加为单次不可变全量结果，也不借旧204／1249项宣布候选CI通过。完整报告哈希、修复过程和边界见workers-ts/audit/brokerage-paid-runtime-permissions-20260910.json，使用合同见workers-ts/docs/paid-order-runtime-permissions.md。

专用PG16.15核验4基线库、零临时库／schema／public表／测试角色／其它客户端后，02:40:12.278 UTC确认停止，保留程序、数据目录和原始报告。PostgreSQL／Workers技能促使本批采用真实非所有者登录、短只读事务和秘密输出约束；最新Workers类型获取EACCES后仅回退已安装定义，未改平台绑定。生产Worker／Hyperdrive实际身份、完整授权及管理员维护隔离、其它账务角色、PHP政策、不可变CI和正式发布仍待验；本节仅覆盖此前“非所有者权限待验”的上述本机子范围。A3k11c仍开放，234勾选／170开放／404项，无提交推送、部署、真实provider或生产数据库写入。

## 累计消费保护与付款／退款业务事务（2026-09-10，工作区候选）

新增20项完整ORM建库＋独立0150安装的真实SQL业务测试，其中10项使用四个不同后端和精确pg_blocking_pids屏障。普通余额／合成渠道入账、重复支付、部分／全额退款、退货同意／拒绝、缺失／不匹配渠道确认、迟到outbox／退款账单异常回滚全部通过；四种真实拼团建单→付款→团长退款与待付款成员竞争，覆盖两种资金方式、两种先后顺序，验证继任团长、订单重关联、库存／配额、余额和幂等。没有为通过测试修改付款、退款、拼团业务或保护SQL。测试帮助器仅向已有独占测试库连接暴露DbClient/PID并验证不重连。

首轮16项通过，类型检查暴露新测试unknown查询行，改为有类型的Drizzle投影；扩展20项全通过，unit/runtime双类型通过。最终11文件204项全部通过、零失败／跳过，实际文件清单匹配、最终执行期间源码／测试未变，不与专项16／20或旧167／1249项相加。报告、哈希及覆盖边界见workers-ts/audit/brokerage-paid-business-transactions-20260910.json。隔离PG16.15核对四基线库、零测试库／schema／public表／其它客户端后，于01:56:30.892 UTC确认正常停止；保留二进制、数据目录及原始报告。

目录证据收窄了此前“完整外键”表述：完整ORM中存在外键，但user、store_order、store_order_refund自身没有外键；本批不能代表生产目录或其它历史／自定义触发器。finance_test为超级用户，非所有者最小权限、特权DDL／复制角色绕过仍未验收。部分退款仍按既有TS合同贡献原pay_price，不是新批准的净消费政策或PHP语义等价证明。其它活动、纯积分／免费订单、已结算奖励／佣金／供应商及角色重叠组合仍需扩展；真实provider签名／资金操作、生产身份、Hyperdrive和发布均未执行。Workers／PostgreSQL技能促使本批使用完整建库、真实事务及独立锁证据，避免以轻量夹具绿测外推完整验收。A3k11c仍开放，清单234勾选／170开放／404项，未提交推送、部署或写生产。

## 累计消费保护迁移注册（2026-09-10，工作区候选）

现已把磁盘0150接入MigrationService的内部0156，沿用独立执行器的根READ COMMITTED事务和超时上限，不进入通用外层事务。完整新建库按0000–0156执行157步；已有数据库仍只应显式运行独立升级器，不能为安装保护重放含历史补写的runAll。没有修改0000–0155的156个迁移方法、独立SQL或执行器，也未扩大调试迁移入口权限。ORM生成DDL本身不安装此触发器，ORM建库后须额外运行独立升级器。

新增7项验证先得到3通过/4失败：外部SQL、ORM后独立安装及前一步错误停止通过，内嵌注册/执行和末步错误探针因缺少0156失败。接入后4文件105项通过；测试查询返回值的unknown类型问题改为带类型的Drizzle投影后，最终5文件167项全部通过、零失败/跳过，unit/runtime类型检查均通过。覆盖完整外部SQL/内嵌/ORM建库、数据与关系物理标识保留、重复安装对象OID不变、真实触发器DML及缺失用户拒绝、前一步失败不运行0156、already-exists不误报成功、DDL后真实错误导致整步回滚，以及既有62项下单累计消费回归。最终执行期间未再修改源码/测试；这是本地定向结果，不是全项目不可变CI，也不与前轮1249项或中间105项相加。原始报告、源码哈希及隔离实例清理见workers-ts/audit/brokerage-paid-migration-registration-20260910.json。

本节覆盖下方旧阶段的“MigrationService注册待完成”。A3k11c仍开放：完整生产外键/角色权限、实际付款/退款跨语句锁序、特权替换函数/复制角色绕过门禁、PHP资格及绑定策略合同和正式环境验收尚未完成。清单保持234勾选/170开放/404项。未提交、推送、部署或写生产数据库。

## 累计消费资格下单读端（2026-09-10，工作区候选）

接续0150写端基础，现已把计算器实际执行的mode=3累计消费资格记录为内部临时依赖，并接入原下单末端分佣用户锁后的复核；不改变客户确认指纹、门槛严格大于的语义、佣金公式或收货政策。普通开启分佣且实际使用累计额时，每次检查保护函数源码/属性、三个触发器的事件/转换表/启用状态、关系和列/主键形状、READ COMMITTED/复制角色，以及本事务已有订单写锁，再分别重读至多两个用户的SUM。缺失或漂移拒绝，不在请求内安装迁移，也不缓存“就绪”。用户SHARE锁保持至提交，使后续合规贡献写入等待；资格未变的金额编辑仍可购买。

首轮36项17通过/19错误放行（8种晚到编辑×HTTP/独立PG，另3种协议未就绪）。修复后原36项与写端40项共76项通过；扩展53项51通过/2失败为更新后无ORDER BY的物理行序比较，按主键规范化全部订单字段后55项通过。最终专项62项全部通过、零失败/跳过：41实际HTTP处理链、19四后端并发探针、2事务调用约束检查。涵盖退款/删除/拆单/转移、门槛相等及跨越、资格不变、持锁到提交/回滚、写者占用立即拒绝、反向库存等待、自购/积分锁、缺失或漂移协议拒绝及未使用累计额时不要求协议。全部使用专用PG与合成身份/KV/取号夹具，不代表真实登录、支付provider或Hyperdrive验收。

最终49文件关联回归1249项全部通过、零失败/跳过，实际文件集合完全匹配，不与专项76/55/62及前轮1147项累加；unit/runtime双类型通过，最终源码与测试哈希匹配，证据见workers-ts/audit/brokerage-paid-read-authority-20260910.json。回归启动后修正了一处事务参数类型：DbClient将$client声明为必有，实际事务对象没有此字段；改用select与可选$client的精确结构类型，未弱化运行时拒绝。前后TypeScript产出的JavaScript SHA-256相同，但不能写成所有源码字节全程不变的不可变CI。技能检查促使本批使用锁后的独立READ COMMITTED语句与失败关闭目录核验，保持事务内无新增外部I/O；最新Workers类型下载遇EACCES，回退本地5.20260828.1，平台绑定没有修改。专用PG核验4基线库、零测试schema/public表/其它客户端后正常停止，01:12:06.423 UTC确认停止；保留二进制、数据与报告。

尚需MigrationService显式注册（内部下一编号0156，对应磁盘0150）、完整生产外键/权限及实际付款/退款跨语句锁序整合。尤其并发特权CREATE OR REPLACE FUNCTION或复制会话绕过须另设发布权限门禁，单次目录读取不宣称防住管理员改写。部署前必须先安装并验证写端协议，否则依赖累计额的mode=3下单将拒绝。全局SQL/KV、其它券/引用集合及不可变CI/workerd/正式发布仍开放，A3k11c未勾选；234/170/404项，无提交、推送、部署或生产写入。

PHP静态来源另有未裁定的语义差异，不能把本批“保护现有TS规则”写成已证明PHP全链路等价：UserServices.php:2145仅在传入sumPrice非零时重新求和，而StoreOrderTakeServices.php:265调用不传该参数；UserServices.php:1769的用户信息路径则传入累计额并可能永久设置is_promoter。内部重新求和没有传search=true，BaseDao.php:399默认使用直接where，其中pid条件为0；搜索器searchPidAttr的pid>=0不能自动套用于这次调用。getSpreadUid在UserServices.php:2480附近还包含绑定有效期策略。以上为实际源码静态观察，不是运行旧PHP站或授权复制其疑似缺陷；本批未调整TS现有计价/资格政策、未自动修改用户推广员标记，后续须分别验证并明确迁移合同。

## 已付订单集合写端保护与浏览器复核（2026-09-10，未提交）

新增独立0150 SQL、等价Worker模块及显式维护执行器，为mode=3累计消费资格建立写端锁协议基础。最终单文件40项通过、零失败/跳过（1项SQL镜像、39项专用PG夹具测试；其中1项专门证明未安装协议时用户SHARE锁不能冻结已付订单集合，不是应用合同已安全的证明）。首轮36项通过；初次类型检查因Node/Workers URL类型冲突失败，显式使用node:url的URL后unit/runtime双类型通过。最终报告与源码哈希见workers-ts/audit/brokerage-paid-write-fence-20260910.json，不与前轮47文件1147项累加。

AFTER STATEMENT转换表对INSERT/UPDATE/DELETE的有效消费贡献计算净变化，按uid排序锁定相关用户至事务结束；覆盖付款、退款、软删/恢复、拆单标识、转移用户、批量净额不变及UPSERT。安装重复执行保留匹配对象身份，定义漂移拒绝，维护锁等待有界，失败回滚；不回填业务数据。生产完整外键/权限、其它触发器组合及跨语句业务锁序尚未完整验收。下单读端的锁后资格重算、缺失/停用协议时失败关闭、MigrationService注册及真实付款/退款整合测试仍未接入，不能称mode=3已修复，A3k11c继续开放。专用PG16.15核验4基线库、零测试schema/public表/其它客户端后正常停止，00:45:46.682 UTC确认停止；保留二进制、数据和报告。清单仍234勾选/170开放/404项，未提交、推送、部署或写生产。

按浏览器授权只读访问配置中的https://cinashop-pc.pages.dev/，首页显示商品及分类，点击“全部商品”后到达/goods并显示分类/排序控件及12项商品。截图未见空白页或框架错误遮罩，但商品图片损坏：列表12张图均指向https://via.placeholder.com/300，DOM检查全部未成功加载，logo正常。当前捕获error/warn日志为空，不代表网络资源成功。只确认本次默认桌面视口与公开导航；未验证移动端、排序、登录、支付或后端生产数据库完整性，未修改前端或生产图片数据。浏览器仅检测到Codex内置连接，未检测到Chrome/Edge扩展连接。此结果不是新候选部署验收。

## 分佣用户/等级及事业部时效（2026-09-09，工作区候选）

接续商品/SKU分佣候选，新增创建时的推广关系、接收人资格字段、实际使用的代理等级权益及事业部比例/有效状态快照。它们仍是内部账务输入，不写入客户报价指纹或账务元数据；确认到创建之间可使用新值，创建读取之后的变化须整笔回滚。缺失的正向用户/等级引用没有可锁行，创建失败关闭，须显式修复引用后重试，不自动创建接收人或补等级。

事务末端最多新增两次有序SHARE NOWAIT SELECT（最多6个用户、2个等级），锁持有至提交，避免在已有库存写入后添加反向行锁等待；不承诺消除所有DDL/旧业务等待。原计算器记录实际调用的事业部比例角色，保留自购、员工、代理、事业部和角色重叠公式；等级查询保留原行以区别停用与缺失，计算仍只使用启用且未删除等级。事业部有效状态在其它可能阻塞的SQL之后，用最终数据库墙钟复验，续期且资格不变不拒绝。无新增事务内外部I/O、全局表锁、付款或收货政策变化。

首轮63项31通过/32错误放行：15种创建内变更×HTTP/独立PG，以及2种请求前引用缺失；修复后63项全通过。扩展3文件144项全通过（新文件92、上轮40、原分佣12）；员工/代理/自购/重叠等12项补充通过、92名称过滤；已取得分佣锁后再等待配送SQL至过期的4项通过、104名称过滤。不累加重叠报告。最终新增108项＝48实际Hono HTTP处理链＋56独立PG＋4合成错误分类；覆盖占用立即拒绝、持锁至提交、反向库存等待回滚、UTC/纽约自然截止与积分锁后重算。

全量相关回归及源/测试/报告哈希见workers-ts/audit/brokerage-authority-20260909.json。推广资格mode=3的已付订单SUM集合/幻读、全局SQL/KV和会员权益表仍未拥有事务权威，本批行锁不能代表它们已完成。券/引用集合、主体/表单、完整不可变CI/workerd及正式身份/存储/发布也仍开放；A3k11c不勾选，仍234勾选/170开放/404项。未提交、推送、部署或写生产。

最终专用Windows PG16.15回归47文件1147项全通过、零失败/跳过；新增108项全部进入该回归，不与144/12/4专项或前轮1039项累加。最终unit/runtime双类型通过，实际文件清单匹配，源/测试哈希未变。实例期间两次记录Windows共享内存预留error 487，未形成测试失败，不能宣称根因已修复。核对4基线库、零测试schema/public表/其它客户端后，13:55:54.806 UTC正常停止；保留二进制、数据与原始报告。

## 商品/SKU分佣快照边界（2026-09-09，工作区候选）

普通订单的isSub和指定一级/二级佣金属于创建时内部账务输入，不加入客户报价指纹。确认后、创建前的编辑使用新值建单；创建已读之后发生的有效变化，在原商品/SKU库存UPDATE中比较并整笔回滚。buildOrderBrokerageSnapshot仅增加本次计算使用哪些规则的临时元数据，不持久化或返回客户；积分锁后重新计算也保留这些依赖。无新增查询、锁、事务外部I/O或锁序变化，不修改佣金公式及收货入账政策。

首轮36项红测24通过/12错误建单：一级、二级、归零、开启/关闭指定分佣、事业部方式切换，分别在请求内及独立PG写锁等待后复现。修复后36项全部通过；扩展46项全部通过，256项仅名称过滤。新增40项独立文件（26实际HTTP、14独立PG）及6种营销订单正向对照，验证关闭分销/二级、无合格接收人、未使用指定金额不会因无关编辑被阻止；回滚、同key显式恢复、客户端伪造佣金无效、清缓存后幂等重放均有断言。成功仅创建未支付快照，佣金流水和账户佣金仍为零，不是支付/收货端到端。

推广关系、接收人资格、代理等级/比例/自然到期及全局SQL/KV权威尚未冻结；临时依赖元数据不能作为这些权威已经受保护的证明。A3k11c保持开放。完整相关回归及最终清理记录见workers-ts/audit/brokerage-item-quote-boundary-20260909.json；不累加重叠测试，不提交、推送、部署或写生产。

最终专用Windows PG16.15回归46文件1039项全通过、零失败/跳过；Worker unit/runtime双类型通过。实际文件清单匹配，源/测试哈希不变；相对前轮45文件993项新增46项，不叠加专项报告。仅4个基线库、零测试schema/public表/其它客户端，专用实例13:29:01.874 UTC正常停止；二进制、数据与报告保留。清单234勾选/170开放/404项，未完成整体迁移或不可变CI验收。

## 成本/供应商结算快照及新人SKU来源（2026-09-09，工作区候选）

本批接续赠送积分候选，区分用户报价和内部账务事实：cost/settlePrice不加入客户确认指纹，确认之后、创建请求之前的编辑可使用同一凭据成功建单，并以创建时的新值生成快照；创建已读值之后的编辑则在既有写入/锁边界拒绝并整笔回滚。普通订单取基础SKU，套餐/砍价/拼团/积分/秒杀取活动SKU；新人分支另发现并修复来源遗漏（下述）。数字零仍是有效SKU值，不回落到非零父商品。

已有秒杀SKU守卫保持原样；普通基础SKU、砍价/拼团活动SKU、积分活动SKU在原UPDATE比较cost/settlePrice，套餐比较原有锁后读出的packageSku。没有新增SQL往返或锁，不调整原锁顺序，不新增事务内外部I/O。第一轮65项红测41通过/24错误建单、186项名称过滤；补守卫后完整规则文件251项通过、零失败/跳过。这一中间候选还保留新人错误来源，不能当作最终迁移等价证据。

PHP StoreNewcomerServices.checkNewcomerStock返回type=7活动attrInfo，StoreCartServices将其用于costPrice，StoreOrderCartInfoServices读取相同attrInfo.settle_price。TS却声明局部const activitySku遮蔽外层，外层仍null，成本/结算和活动规格快照回落基础SKU。按PHP来源调整测试后12项全部失败：4项错误放行、4项错误快照、4项错误拒绝。已改为给外层activitySku赋值，在原新人SKU锁查询中补成本/结算比较与unique身份条件；没有把活动SKU库存改为扣减源。修复后新人相关27项通过、229项名称过滤；又补持久化活动SKU身份断言进入最终回归。

最终新增70项＝46实际HTTP＋24独立PG，覆盖七种下单路径的请求前刷新、请求内编辑、精确写锁等待、回滚/重确认/幂等重放，以及未使用基础值、零值和新人活动来源。供应商真实付款/结算/退款、佣金身份与比例配置权威未由本批验证，完整不可变CI/workerd和发布仍开放。

最终专用Windows PG16.15回归45文件993项全部通过、零失败/跳过；规则文件256项全部通过，其中111项独立PG。相对前轮41文件898项，新增本批70项及4文件25项纯函数/源码合同检查，不累加重叠报告。实际清单与请求一致、源码/测试哈希未变，最终Worker双类型通过。机器证据见workers-ts/audit/internal-ledger-quote-boundary-20260909.json，包含推翻中间绿测的PHP来源与两层红测。核对4个基线库、零夹具schema/public表/其它客户端后，专用实例13:06:25.997 UTC正常停止；保留二进制、数据及报告。清单234勾选/170开放/404项，不提交、推送、部署或写生产。

## 商品赠送积分的确认与提交保护（2026-09-09，工作区候选）

在af86b95及自然截止/金额/会员未提交候选上，补齐实际赠送积分来源与gainIntegral的确认投影。普通券单、套餐、拼团、积分和新人订单采用基础商品giveIntegral，在现有商品库存/销量UPDATE中原子比较；秒杀、砍价沿用已有活动赠送积分守卫，不冻结未使用的基础赠送值。无新增查询、锁或事务内外部I/O，未改价格与积分计算公式。

新增48项＝32实际HTTP＋16独立PG测试。红测18通过/30错误放行，另138项被名称过滤；30项来自五类订单、两种值变化和三个编辑时机，不是30个独立漏洞。修复后完整规则文件186项全部通过，零失败/跳过，Worker unit/runtime双类型通过。覆盖请求间编辑、创建核心内晚到编辑、精确pg_blocking_pids证明的写锁等待；明确拒绝后同key重确认成功，忽略客户端伪造gainIntegral，创建后改目录仍只重放原订单。0.10→0.20即使整数汇总仍为0，也须重新确认行级规则；活动覆盖基础值的六项正向对照通过。

这验证的是未支付订单及明细快照，不是支付/收货端到端。OrderRewardService仍只对普通type=0订单发放商品赠送积分，营销订单保存该字段不等于新增积分发放政策。内部成本、结算价、佣金的权威取值需要另行审计，不混作用户确认价格。

最终专用PG16.15扩大回归41文件898项全部通过、零失败/跳过，不与186项或前轮850项累加。实际文件清单与请求一致，源/测试SHA-256保持不变，证据见workers-ts/audit/gift-points-quote-boundary-20260909.json。PG日志曾记录一次Windows子进程共享内存预留error 487，未形成测试失败，也未证明其根因已修复。核验cinashop_finance_test/finance_test、UTC、4基线库和零测试schema/public表/其它客户端后，12:44:44 UTC正常停止，保留二进制/数据/报告。A3k11c、全局配置、券/引用集合及正式环境门禁仍开放，清单234勾选/170开放/404项；未提交、推送、部署或写生产。

## 会员资格与等级的提交末端保护（2026-09-09，工作区候选）

在af86b95及前述自然截止/金额保护的工作区候选上，新增会员与等级确认投影，以及事务末端用户/等级SHARE NOWAIT复核。会员自然到期按已锁定字段在最后一次数据库时钟SELECT检查；覆盖已有积分用户锁之后发生的等待。续费但资格不变、转终身且权益不变及等级名称编辑保持成功；同价切换等级ID仍要求重新确认。最多增加两次单行读取及一次可与活动合并的时钟读取，无全局表锁或新增事务内外部调用，原业务写入顺序不变。NOWAIT不增加反向行锁等待，但不宣称解决所有旧有等待/DDL锁问题。

首轮32项12通过/20失败，包含18项真实错误建单和2项占用行保护不变量；修复后32项、扩展45项分别全部通过，不累加。最终专用Windows PG16.15回归41文件850项零失败/跳过，Worker unit/runtime类型检查通过；新增会员文件50项中17实际HTTP、29独立PG、4合成错误分类。精确阻塞关系验证锁保留至提交、用户编辑者反向等待库存时确定回滚，以及UTC/纽约下自然过期。首轮TS6133未使用导入失败保留，最终未放宽类型选项。

行为限制明确：引用等级缺失时，预览仍可给出回退报价，但创建失败关闭，须修复引用或恢复等级后显式重确认；已补下单中新增缺失等级行的探针，没有自动修改生产数据。会员权益表/全局SQL/KV仍属A3k11d；赠送积分、其它结算事实、券/引用集合、完整CI与正式环境门禁仍未完成。证据为workers-ts/audit/membership-quote-boundary-20260909.json。临时PG身份、4基线库及零测试schema/public表/其它客户端核验后12:24:32 UTC停止。清单仍234勾选/170开放/404项；未提交、推送、部署或写生产，A3k11c保持开放。

## 商品/SKU计价事实的原子校验（2026-09-09，工作区候选）

基线为af86b95加前述未提交的活动自然截止候选。新增晚到HTTP编辑和独立PG写锁探针：基础SKU价格（普通券单、套餐、砍价、拼团、积分、新人）、砍价活动基准价、拼团/积分活动SKU价格及兑换积分，首轮16通过/20错误放行；另会员价与商品会员价开关矩阵2通过/6错误放行。名称过滤的92/128项不计执行通过。已有秒杀、套餐及新人活动价格守卫的正向回归保留。

现于原有SKU/商品扣减UPDATE中比较基础价、适用VIP价格与开关、规格身份/有效状态；补拼团活动SKU价格、积分SKU价格/消耗积分，将砍价活动price纳入既有规则投影。不新增锁序、SQL往返或事务内外部I/O，不改砍价公式。基础价为0时，现有计算器不使用会员价；首版补丁对此过严，新增两项免费商品对照均失败，已修正而非删除测试。未启用会员价及砍价公式未使用的活动SKU价格变化保持可购买。

最终专用Windows PG16.15回归35文件769项全部通过、0失败/跳过，Worker unit/runtime类型检查通过。规则文件138项，其中71项独立PG竞争；本轮新增46项（19实际HTTP、27独立PG），不与136项首轮绿测或此前657项累加。证据和源/测试摘要见workers-ts/audit/monetary-quote-boundary-20260909.json。业务快照回滚不保证SQL序列/订单取号回滚；SQL夹具、合成身份/KV/Sequence不代表完整生产鉴权或Hyperdrive验收。

仍待处理用户会员身份/自然到期、等级权益、商品赠送积分及其它结算事实、表单内容版本、券模板/范围/祖先关系与引用集合幻读；全局配置权威仍属A3k11d，A3i砍价价格政策未改变。A3k11c继续开放，清单234勾选/170开放/404项。实例核对4个基线库、零测试schema/public表/其它客户端后12:01:39 UTC正常停止。依照“完成checklist后提交推送”，本轮未提交、推送、部署或写生产；旧CI失败不能被本地子集结果覆盖。

## 活动在交易等待中自然过期（2026-09-09，基线af86b95，未提交）

按用户新目标“逐个完成checklist，然后提交推送”，本轮只保留工作区改动，不新增提交或推送。现有af86b95及更早本地提交保持不动，没有通过其它渠道绕过此前main推送审批。

专用PG16.15中，独立持锁者对夹具store_order取SHARE表锁，允许前面的订单读取，却阻塞已经完成活动预占后的订单INSERT。以4个不重连后端身份和精确pg_blocking_pids证明等待，按数据库墙钟等过截止后再释放，不改活动元组。套餐、秒杀、拼团在UTC和America/New_York会话时区均错误放行，6项真实失败；12项未过期/无截止对照通过。红测只运行新18项，因此报告另72项是名称过滤，不是已执行/零跳过。修复后完整90项全部通过。

候选保留锁定或原子校验后的活动时间边界，用同一边界的SQL表达式在所有其它业务SQL结束后再次读取clock_timestamp；不再由拼团NOW()或套餐等待前now决定最终提交资格。日期使用明确timestamptz，套餐整数秒截止转换为下一秒的开区间上界，保留其原有最后整秒语义。最多新增一次不读取业务表的时钟SELECT，无额外业务锁或外部I/O、不改变2秒锁上限。完整边界测试和扩大回归结果见确认合同文档；不将这一局部修复外推为完整结算/迁移验收。

最终专用PG16.15回归32文件657项全部通过、0失败/跳过，Worker unit/runtime双类型通过。其中确认规则92项，本文件44项为独立PG竞争（本轮新增20），另有砍价日期及参与竞争旧行为回归。套餐最后有效秒内释放仍成功的两个时区对照通过，不把历史622项、首轮90项与本轮相加。机器可读证据为workers-ts/audit/activity-deadline-boundary-20260909.json；临时PG身份/4个基线库和零夹具schema/public表/其它客户端核对后11:39:06 UTC停止。改动仅工作区、没有提交推送、生产写入或部署；A3k11c和整体清单保持未完成。
