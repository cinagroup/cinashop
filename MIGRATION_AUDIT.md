...
## 活动标量规则的最终原子校验（2026-09-09，基线1ca3d3d，本地候选）

专用PG16.15红测68项中52通过/16失败、0跳过：砍价num/stopTime、拼团people/effectiveTime、积分onceNum/num及秒杀deliveryType/isSupportRefund，分别在已验证凭据后立即编辑与独立SQL写锁等待时编辑，均仍按旧凭据创建成功。这16项是业务放行证据，不是连接或夹具错误。现将显式标量投影直接加入现有原子扣减WHERE，日期明确编码、null-safe比较；规则不符时抛出精确重确认异常并回滚。守卫不包含库存计数或展示字段，不新增锁/外部调用，不改变砍价公式。相同68项修复后全部通过；扩大回归另列于确认合同和机器可读证据，不重复累加。

最终扩大回归在专用PG16.15运行30文件622项全部通过、0失败/跳过，Worker unit/runtime双类型通过。含72项确认规则（本文件24项独立PG竞争）和3项SQL投影单元测试；不能把622项都称为并发SQL测试。新增库存正向竞争保留编辑并准确扣减，没有冻结库存计数。临时实例核验4个基线数据库、零测试schema/public表/其它客户端后11:22:19 UTC正常停止；证据为workers-ts/audit/activity-scalar-rule-boundary-20260909.json，保留原红测与报告摘要。

本轮仅覆盖既有确认投影中的活动标量字段。源码复核仍发现拼团准入使用事务开始时间NOW()、套餐复用等待前now；非秒杀活动SKU及普通基础SKU的部分价格/积分事实仍来自初读，不能把最终指纹重复计算当作这些字段已经重读。自然截止、其它计价事实、表单内容/券模板与适用关系、集合幻读和等价表达均继续保留在A3k11c范围，A3i价格政策及A3k11d全局配置权威不改变。详细字段/测试边界见确认合同文档。

用户本轮回复“授权”，但精确推送1ca3d3d至origin/main被安全审批拒绝，理由为缺乏精确默认分支批准；未重试或通过其它渠道绕过。origin/main仍为f58f617，96d7a71、1ca3d3d及新增工作均未推送，自身CI未运行。无生产数据库写入、部署或外部支付调用。

## 套餐和秒杀锁后版本缺口（2026-09-09，基线96d7a71，本地候选）

新增4项请求内变化红测，以及4项真实PG写锁等待红测，均证明套餐有效期/成员规则、秒杀日期/时段集合在原凭据检查后变化仍能同金额建单。修复复用原有锁定读取，比较完整显式套餐规则与秒杀排期投影，不新增跨请求事务、锁顺序或事务内外部调用。最终专用PG16.15扩大回归21文件518项零失败/跳过、Worker双类型通过，包含未选成员和父活动编辑及更严格回滚断言。首轮扩展514通过/4失败，是新增父活动未设置status=1、按真实默认0在初始确认阶段被拒绝，不能算业务竞争证据；修正夹具后全通过，不与首次11文件328项相加。

仍须完成套餐数据库墙钟截止和非协作SQL成员幻读保护，秒杀缺失引用时段被新增、其余配送/退款规则及等价表示，以及拼团/积分/砍价、券模板/关系的最后提交期守卫。新提交96d7a71尚未获得推送批准，未重试主分支推送；没有新候选自身CI。本轮未写生产、部署或支付。细节见确认合同文档，A3k11c不关闭。

## 授权推送、自身CI及占券原子校验（2026-09-09）

f58f617经用户明确授权推送origin/main。自身[Actions34339592124](https://github.com/cinagroup/cinashop/actions/runs/34339592124)已终态失败：分片1为1709通过/1失败，分片2为1553通过/1失败，各174文件；严格分片覆盖后置步骤因测试失败未执行，不冒充零跳过门禁已通过。失败分别是砍价正向首次建单没有提交quoteToken、UniApp旧模拟报价没有返回quoteToken；本地同两文件35通过/2失败复现后对齐，新增缺失/畸形凭据必须拒绝，不改接口生产规则。五端构建、workerd、密钥扫描和独立PG16目录任务成功。

继续A3k11c审计发现已领券的最后占用仅检查id/uid/status。手动隔离HTTP探针在核心已接受凭据、取号前后这一窗口写入SQL，券门槛改为1或999、isFail改为1仍按25元创建一单；3项真实缺口、其余11项原有守卫/对照通过，无外网或生产写入。新增7项请求内字段变化红测全部失败；独立PG写锁竞争有3项规则变化红测复现。首轮自然过期测试实际撞到既有2秒lock_timeout，不是旧版放行证据；将夹具有效期改为1.5秒后验证，不放宽服务端超时。

本地候选在原有UPDATE占券边界加入isFail、模板引用、金额、门槛、起止日期的null-safe相等守卫，保持原锁顺序；未匹配则回滚并返回精确报价重确认。日期受行锁保护，其他最后守卫完成后再以数据库UTC墙钟复验，覆盖等锁但元组未改变的自然过期。不在数据库事务内添加KV或外部调用，不改砍价公式。最终关联14文件266项零失败/跳过，Worker双类型通过，不与专项5文件98项累加；新增修复尚未推送，不能借用f58f617的CI结果。只是已领券行事实，不涵盖券模板类型/范围关系、分类品牌祖先及所有活动规则的并发编辑。A3k11c不关闭。专用PG16.15已核验4个基线库、零测试schema/public表/其它客户端，二进制和报告保留。

此前持续运行的PGlite全量诊断已退出1：346文件、3037通过、6失败、191跳过。除上述两个凭据夹具失败外，另有砍价日期模拟时钟1项和多语句触发器创建3项失败；需要单独处理PGlite测试兼容性，不视作本候选全量验收。运行期间源文件及测试清单改变，不能借其结果证明任何不可变提交；未重启或中止该运行。

## 确认报价语义规则扩展（2026-09-09）

提交前最新候选专用Windows PG16.15回归11文件206项全通过、零失败/跳过，含七类28项与独立连接2项；最终Worker双类型通过。实例身份、4个基线数据库、零夹具schema/public表/其它客户端已核验，随后正常停止。该结果不与下方各阶段数量相加，也不代替候选自身CI与正式发布验收。

追加实际HTTP红测确认优惠券门槛、套餐退款、砍价配送规则改变但金额不变时旧凭据仍可建单（3失败/3对照通过）。现绑定所选券类型、门槛、时间及规范化范围/匹配商品，套餐完整成员/退款等规则，砍价配送/时窗，以及秒杀排期/限购、拼团人数/有效时长、积分限购、新人配置等。七类各4项共28项实际confirm/computed/create矩阵通过；保留初期computed形状读取错误和扩展SKU char(8)夹具失败，均不当作业务漏洞证据。未修改价格公式，标题/排序/充足库存变化仍可正常建单。

本批只证明选定跨HTTP规则版本与恢复合同；初始规则对象未在全部create提交边界重取，须继续补活动/券规则锁后或原子写入复核、复杂适用范围及独立编辑竞争。全量诊断运行期间源文件/测试清单持续变化，不可借为不可变候选全量通过；最终以自身CI为准。A3k11c仍开放，详见 `workers-ts/docs/checkout-confirmation-contract.md`。没有生产写入或部署。

## 跨请求确认凭据候选续验（2026-09-09，基线c811c07）

本轮未提交候选为普通/代客创建接入服务端不可变报价凭据，PC与UniApp提交实际展示版本。配送报价26元后后台运费变更，实际本机Chrome页面首次提交得到精确报价拒绝，业务SQL快照不变；页面显示29元且不自动提交，同key换凭据后用户再次点击才创建一笔未支付订单。两端配送/自提/虚拟共6流程通过，身份/KV/序列等为隔离替身，无生产或付款请求。截图已实际检查，无空白、框架报错覆盖层或横向溢出；UniApp既有router弃用及提交后详情桩页非props警告另记，详情/付款未验收。

定向5文件107项零失败/跳过；在核验后的专用Windows PostgreSQL16.15上另跑5文件97项零失败/跳过，含两个新增独立连接用例：同key不同有效凭据并发串行化为一单、凭据等待准入锁期间过期则整笔回滚。连接助手核验四个不同且不重连的后端PID、实际pg_blocking_pids关系。纯凭据用例仍是单元测试，不能把97项全部称为并发测试。PC98会话/结算+7图片、Uni188工具链/类型/H5/微信/App构建及3产物通过。全量Worker尚在运行；c811c07自身CI已成功，仅属于修复前审计提交，不借给候选。

FE-003L-A3k11c仍开放：完整活动/套餐/优惠券规则版本绑定与跨请求变化矩阵、全量及自身CI、真实存储/登录/Hyperdrive/发布门禁待验。A3i价格政策与A3k11d全局配置一致性未改变。当前清单234勾选/170开放/404项，不把检查项计数当成业务完成率。原始8项缺口及候选细节见 `workers-ts/docs/checkout-confirmation-contract.md`。
...
三项修复提交`731e20341fcf1460681b5495107d1855d9c91e67`的[Actions 33966112676](https://github.com/cinagroup/cinashop/actions/runs/33966112676)完成：235文件/1,519项单元全部通过（4项真实PG无跳过），workerd3文件/24项、双TypeScript、五端构建、Kefu17项、前端依赖18项、tooltip10项、UniApp工具链8/产物3/Intlify9项均通过。Gitleaks扫描229提交，只命中历史a2366d2中原有两个公开Drizzle摘要；新审计基线未产生额外命中。仍为7/8，安全例外继续留在本地未提交，未绕过自动审核拒绝。

PG16新一轮实际执行外部136个文件、内嵌141步、ORM1,038条语句，三座临时数据库全部确认删除。三路径均**263表/3,700列/227序列**；外部与内嵌所比较的表、列、约束、序列字段均无差异；内嵌索引902→911，漏注册9项已消失，external-only61→52。ORMphantom表、11列及序列消失；外部↔内嵌4处列宽差异归零，ORM只剩4处默认表达式差异。最新摘要与完整同义索引候选保存在根目录`MIGRATION_SCHEMA_AUDIT.json`，状态明确为not_equivalent，并绑定代码SHA和CI URL。

不能把52个剩余索引自动判作冗余：严格结构匹配仅为其中5个找到candidate，47个仍未精确匹配；外部↔ORM75个reference-only中52个有同构candidate、23个未匹配。相同结构也不证明可删除，后续须检查pg_depend及FK conindid绑定；当前比较器未把索引依赖图纳入等价判定。ORM仍有56/15/9的约束缺失/额外/同名变化、75/97/57的索引缺失/额外/同名变化及1个客服序列差异；可能改名与NOT VALID/表达式形式必须单独分类，不通过删除安全围栏消差异。DB-009A/B/C/D1按已验证范围勾选，D2/E/F及DB-009父项继续开放。

路由仍为1,904/1,646/879/861/18/1,025/17/1,008，覆盖46.2%/45.2%/45.6%；本批没有新增路由，也没有解决全部功能迁移。生产DB-006/007、当前新DDL是否需要应用、真实账号/provider与发布门禁均未完成；本轮未接触生产DB/Redis/Cloudflare控制面、未部署或调用真实第三方服务。

## DB-008 公开校验和扫描例外授权与复验（2026-09-05）

用户针对“仅在`.gitleaks.toml`排除两条固定公开Drizzle校验和、保留其它规则、提交推送并重跑CI”的确认问题回复“授权”。首次提交尝试仍被自动审核认为缺少具体授权而拒绝，未发生暂存/提交/推送；通过只读任务记录核对原始确认问题后，在相同范围重新请求审核获准，未换用间接执行或更宽例外。提交`7ae3544f97dcc17e66d26033a2d307a8b6c384a0`只包含扫描配置与对应回归测试，已推送main。

例外严格限定`generic-api-key`、`workers-ts/scripts/patch-drizzle-pg-order.cjs`精确路径，以及该文件中`api.js`/`api.mjs`两条固定SHA-256完整行，路径与行表达式使用AND。根据[Gitleaks 8.29.0配置合同](https://github.com/gitleaks/gitleaks/blob/v8.29.0/README.md#configuration)，`targetRules`只作用于指定规则；未豁免整个文件、提交、目录或其它摘要。上游包版本与三入口字节校验仍由原有补丁测试约束。新增配置回归锁定完整例外块，验证改变摘要、键名、行前后内容、路径或规则不被豁免；该测试不是实际Gitleaks扫描的替代品。

本地专项4/4及单元/runtime两套TypeScript均通过；[Actions 33967084785](https://github.com/cinagroup/cinashop/actions/runs/33967084785)对精确提交`7ae3544`最终8/8任务全部成功。Gitleaks 8.29.0扫描231个历史提交后未发现泄露；Worker为235文件/1,520项全部通过、无跳过，workerd为3文件/24项。双TypeScript、五端构建、客服17项、公共依赖18项、ECharts10项、UniApp工具链8项/三端产物3项/Intlify9项及既有依赖审计门禁均通过。之前7/8的运行保留为历史失败记录，不改写旧结论。

实际PostgreSQL16.14再次执行外部136文件、内嵌141步及ORM1,038条语句，三座临时数据库确认清理。新摘要除提交/运行出处外，与上一轮根目录摘要的全部字段深度比对一致，包括输入/生成SQL摘要、三路径263表/3,700列/227序列、各类目录差异及完整同义索引候选；据此只刷新`MIGRATION_SCHEMA_AUDIT.json`的验证出处，保留`not_equivalent`。SQL文本审计仍为source201/target263/shared201、sourceGaps及窄范围表列漂移0，路由仍为PHP1904/TS1646/匹配879/可执行861/不可用18/缺失1025/退役17/待处理1008。CI全绿证明既有门禁成功，不证明DB-009差异已解决、全部依赖已修复或迁移完成。
...
实现提交`4fe0530ab696a5f7ea66245c3ce0c5a0c031efcc`已推送，[Actions 33968639077](https://github.com/cinagroup/cinashop/actions/runs/33968639077)最终8/8成功：Worker236文件/1,534项全部通过、无跳过，包含4项真实PostgreSQL16.14测试与本批14项索引回归；workerd3文件/24项、双TypeScript、五端构建、Kefu17项、公共依赖18项、ECharts10项、UniApp工具链8/产物3/Intlify9项及既有依赖审计门禁通过。Gitleaks扫描233个历史提交未发现泄露；此前授权的精确例外未增加或扩大。

三路径实际执行外部137文件、内嵌142步及ORM1,039条SQL，三座测试数据库确认删除。仍均263表/3,700列/227序列；外部索引963不变、内嵌911→958、ORM985→986。外部↔内嵌的表/列/约束/序列和同名索引均无变化，external-only52→5，剩余恰为上表五项且每项均有预期同构candidate；47项补齐全部获得真实PG16全链证据。外部↔ORM的reference-only75→74，其中无精确candidate23→22，充值唯一索引已消失；其余4处默认、56/15/9约束差异、97额外/57同名索引变化及1处序列差异均与上一轮深度比对不变。根目录`MIGRATION_SCHEMA_AUDIT.json`已保存新摘要和完整索引证据，仍明确标为not_equivalent。

SQL文本门禁仍为201→263/源列缺口0/窄范围漂移0；路由仍为1,904/1,646/879/861/18/1,025/17/1,008，未因索引修复改变。仅勾选DB-009D2a；D2b、E、F、DB-009父项及生产前置保持开放。最终回填只改根目录文档/JSON，不触发工作流；受测实现证据固定到4fe0530，不以文档提交代替新测试。

后续ORM索引复核已经确认一个不能按名称等价处理的例子：外部`0007_supplier_core.sql:33`为`system_supplier.admin_id`声明独立唯一索引`supplier_admin_id_uq`，而当前ORM `schema/supplier.ts:40`只有普通`supplier_admin_id`。它属于DB-009D2b的剩余22项之一，未夹带进本次47项基础索引提交，也未声称其唯一性已修复；其余包括客服查询、表单引用、供应商商品、点评关系及回调脱敏扫描索引，仍需完整逐项定义验证。

## DB-009D2b1 ORM缺失索引与精确目录门禁（2026-09-05）

从`041f1e3`继续，上一轮`4fe0530`/Actions33968639077的外部↔ORM缺失74项中，52项已有精确同构candidate，22项没有。本批只处理这22个真实缺失声明，来源清单固定在`workers-ts/audit/orm-query-index-reconciliation.json`，逐项包含原文件、行号、完整SQL及PG16目录行；回归独立固定22个完整key，验证真实源语句及不可改写的首次PG16基线，不允许只删manifest项来缩小检查范围。

10个ORM模型文件补齐21个普通索引、1个独立唯一索引；8项带部分谓词。覆盖客服订单/退款/最近访问/商品分类/话术、供应商商品和分类、表单引用、商品点评、同城配送核对及回调脱敏扫描。`supplier_admin_id_uq`恢复外部0007已要求的admin_id唯一性，保留原普通supplier_admin_id，不转换成UNIQUE约束、不删别名。现有SupplierApplicationService在同一事务内创建供应商、管理员并更新admin_id，本批没有改变其业务写入流程；新系统不承接旧PHP历史数据，本测试不请求旧MySQL或修复真实业务行。

首次真实生成验证发现：索引名称和键列虽对齐，Drizzle `.desc()`实际生成DESC NULLS LAST，而源SQL裸DESC在PostgreSQL默认为NULLS FIRST。按[PostgreSQL16 CREATE INDEX](https://www.postgresql.org/docs/16/sql-createindex.html)及[Drizzle索引API](https://orm.drizzle.team/docs/indexes-constraints)，仅为本批6个降序索引的8处列添加`.nullsFirst()`；没有全局改写既有57个同名差异，也没有放宽目录比较。新严格检查真实拦住这处偏差，修正后联合3文件11项通过。

实际CJS/ESM API测试从完整模型快照中仅移除这22项，再生成升级SQL，断言仅有22条CREATE INDEX、无DROP/ALTER；在禁止网络的全新PGlite内执行完整初始DDL后，插入两个admin_id相同的自建夹具，使真实增量以23505拒绝，显式事务回滚后全部旧索引和两行不变。只删除该内存测试库里已知的重复夹具，再实际应用22条SQL并比较所有目录字段、旧索引和原行，验证唯一性拒绝/不同ID接受、独立索引归属及重复生成no-op。PGlite证据不替代真实PG16；没有生产DML、DDL或部署。

审计运行器新增精确命名合同门禁：外部↔内嵌及外部↔ORM必须逐项完全相同，缺失、同名异构或重复目录身份即失败；报告单独输出verifiedIndexContracts。新增3项单元测试固定范围、来源及缺失/空集合/重复/定义/唯一性/归属/有效性反例。其他目录差异继续完整报告，发现命令成功仍不表示结构等价；不声称索引已有实测性能收益。全量本地和精确提交的Linux PG16/完整CI结果待回填，D2b1暂不勾选，D2b、E、F和生产前置保持开放。

本地双TypeScript通过。默认并发全量首轮237文件中236通过、2项旧列宽用例超5秒（1,531通过/4跳过/2失败）；第二轮旧列宽通过，但原Drizzle源码补丁验证耗时7.147秒超过5秒（1,532通过/4跳过/1失败）。三处都是时限失败，新增精确目录合同在两轮均通过；未放宽超时、删除断言或改CI配置，继续以本地`--maxWorkers=2`限制并发复跑全部用例。4项跳过仅为本地未提供专用PG16服务的既有测试，须由Linux隔离PG16服务补证。

限制本地并发后的全量结果为237文件全部通过，1,533通过/4项专用PG测试跳过，共1,537项，耗时158.03秒；所有原断言和超时保持不变。仅本机调用参数不同，Linux CI继续默认配置和真实PG16服务。候选没有外部/内嵌DDL变更、依赖升级或安全例外变更；接下来提交推送并以该SHA的完整CI复核。

### DB-009D2b1 Linux PG16最终证据

实现`404baf33d03bcd4968baf732bbfafc65a8b12e24`已推送，[Actions33970859482](https://github.com/cinagroup/cinashop/actions/runs/33970859482)最终8/8成功。默认配置下Worker237文件/1,537项全部通过、无跳过，含4项真实PG16测试；workerd3文件/24项、双TypeScript、五端构建、Kefu17项、公共依赖18项、ECharts10项、UniApp工具链8/产物3/Intlify9项以及既有依赖审计门禁均通过。Gitleaks扫描235个提交未发现泄露，没有新增例外；本地默认并发的时限失败没有在Linux复现。

PostgreSQL16.14实际执行外部137文件、内嵌142步、ORM1,061条SQL，三座随机测试库均确认清理。三路径仍为263表/3,700列/227序列；外部963和内嵌958索引不变，ORM986→1,008。22项精确命名合同在两条候选路径全部通过；ORM reference-only74→52，剩余52项及同构candidate与上一轮排除本批22项后的完整数组完全相同，无精确candidate项22→0。外部输入SQL摘要未变；外部↔内嵌的完整摘要及5项别名证据不变，其他类别差异计数不变；另独立将当前57项同名索引变化原始记录与不可改写基线深度比对，全部一致，未因新增降序声明意外改变旧索引。

根目录`MIGRATION_SCHEMA_AUDIT.json`已更新为本次受测SHA/运行出处及实际摘要，新增verifiedIndexContracts22项并继续明确not_equivalent。4处默认、约束缺56/多15/同名9变、52项别名候选、97额外/57同名索引变化及1处序列差异仍开放；当前57项可初步分为24项空值排序、30项缺少降序声明和3项遗漏部分谓词，已建D2b2待办，未直接修改或豁免。5个外部↔内嵌别名也未删除。
...
### DB-009D2b2 Linux PG16最终证据

实现`58ac3ce831dfeb803af3065a38b018819a8535c5`已推送，[Actions34002410416](https://github.com/cinagroup/cinashop/actions/runs/34002410416)最终8/8成功：默认并发下238文件/1,540项单元全部通过、无跳过，含4项真实PG16测试；workerd3文件/24项、两套TypeScript、五端构建、Kefu17项、公共依赖18项、ECharts10项、UniApp工具链8/产物3/Intlify9项及既有依赖门禁全部通过。Gitleaks扫描237个历史提交无泄露，没有新增或扩大例外。PG16升级探针属于额外审计步骤，不重复计入1,540项单元。

PostgreSQL16.14实际执行外部137文件、内嵌142步、全新ORM1,061条SQL；第四座随机库先执行旧快照1,061条，再实际执行114条索引替换SQL。日志明确确认57项精确替换、故障回滚、6行夹具/对象OID/外键依赖保留、79合同与no-op通过；报告upgradeVerification的rollbackConfirmed、rowsAndDependenciesPreserved、freshCatalogMatched均true。四座测试库确认清理，四路径均263表/3,700列/227序列；外部963、内嵌958、两条ORM路径1,008索引。

完整摘要与上一轮深度比对：外部输入SQL摘要、前三路径对象数量、全部缺失索引及同构candidate数组、其他各类差异计数均不变；唯一预期变化为外部↔ORM同名索引definition变化57→0。新建与升级后ORM的全部已比较目录一致；79个合同集合恰为原22项加本批57项，无遗漏或重复。根目录MIGRATION_SCHEMA_AUDIT.json已绑定本次受测SHA/CI并保存四路径及升级证明，仍明确not_equivalent，不改写首次463条历史基线。

剩余索引已从本次真实日志再分类：52个reference-only均有同构candidate；97个candidate-only中47个属于这些别名候选，其余50个均非唯一，已列D2b3待办。外部↔内嵌5个物理别名、4处默认、约束缺56/多15/同名9变及1处序列差异仍开放。SQL文本门禁继续201→263、源列缺口/窄范围漂移0；路由仍1,904/1,646/879/861/18/1,025/17/1,008，未因索引修复增加业务覆盖。仅勾选D2b2，D2b、E、F、总迁移和生产前置保持开放。

PostgreSQL技能促使本批以完整键序/空值位置/谓词及升级后真实目录为验收依据，并保留对性能收益的判断边界。本轮只修改本机代码、推送Git并运行隔离CI；没有连接生产、执行线上DDL/DML、调用provider或部署Cloudflare。最终审计回填只改根目录Markdown/JSON，测试证据固定到58ac3ce。

## DB-009D2b3a 28个PHP源查询索引补迁移（2026-09-06）

从干净59140d7及58ac3ce/Actions34002410416的真实目录继续，97个ORM额外索引扣除47个同构别名候选后，对剩余50个逐项读取PHP安装SQL。新增`audit/legacy-index-authority.json`只保留201个CREATE TABLE内288条非主键KEY/UNIQUE KEY的名称、完整键序、前缀长度、唯一性、原始SQL与行号，不包含INSERT或业务数据。原文件SHA-256为0096d86464b81935106311e4bb4092b647ab3acaf2bab0febe5695f8f66c593a，与既有legacy-schema-authority.sql绑定相同源；本机已对全部表和索引原始行及摘要逐一读回验证。PHP目录没有Git元数据，不虚构源提交；CI使用固定快照，不声称重新访问了不存在于CI checkout的PHP目录。

`audit/orm-extra-index-reconciliation.json`完整记录50个对象的实际PG16目录及源证据：28个存在完全相同键序的非唯一、无前缀PHP KEY，应补入部署迁移；22个没有该精确源KEY，保持review-orm-only，不据此判为冗余。express_company正确映射eb_express；user_message不在201表安装源中，显式记为不存在而非伪造空表。复合前缀覆盖、同列普通/唯一索引、低基数列都不自动构成删除许可，本批也不宣称经过EXPLAIN验证的性能收益或MySQL/PostgreSQL排序规则完全等价。

新增外部0136与逐字一致的内嵌0142，共28个独立普通B-tree。SQL只在当前明确schema下缺失时CREATE，要求永久普通表，按表获取SHARE锁并以2秒限制锁等待；已有对象须完整匹配表归属、键序、唯一性、有效/就绪/存活状态、立即检查、无INCLUDE/表达式/谓词、非副本标识及无约束所有权。任何同名异构、表缺失或schema缺失都报错，不做DROP、ALTER或业务DML。2秒不是索引构建耗时上限；生产实际目录、锁窗口和单独发布批准仍不可省略。本批不改ORM模型和既有47个别名。

新增4项来源/完整队列/精确镜像/107合同回归；首次测试指出两条源KEY后有多余空格，已按原SQL空白语法修正测试，保留原始行和键定义严格比较。共享升级探针在既有全模型隔离库中移除28个已知测试索引，再实际执行新SQL，验证恢复完整目录及所有非目标OID/外键conindid和pg_depend不变。两条相同type/link_id的自建user_money行须保持，证明没有误加唯一性；再次执行须连索引OID也不变。11类负向夹具覆盖唯一、倒序键、DESC、WHERE、INCLUDE、错误表、同名普通表、约束所有权、表达式、缺表、缺schema，事务回滚恢复全部目录/OID/行/依赖；额外验证非public schema和临时同名表隔离。用于故障验证的DROP及夹具UPDATE仅在新建隔离数据库中执行，不属于生产迁移SQL。

网络禁止的CJS/ESM真实升级探针已通过；PG16第四座测试库复用同一探针，三个候选路径的严格合同从79扩至107，50/28队列不得静默缩减。其余原始差异继续输出，根目录目录报告暂保留上一轮已验收证据，不提前写入预估的991/986/69数量。本地全量、两套TypeScript、本次提交与完整Linux CI结果待回填；D2b3a暂不勾选。PostgreSQL技能使本次以完整键序、归属、事务回滚与真实目录为准，没有以“额外索引”标签代替逐项来源审查。本轮无生产连接、线上DDL/DML、provider调用、Cloudflare变更或部署。

本地两套TypeScript通过。全量maxWorkers=2实际为239文件中的238通过/1失败，1,539项通过、1项既有列宽迁移测试触发5秒时限、4项专用PG测试因无本地服务跳过，共1,544项，耗时228.37秒；不记作全量成功。随后单并发复测该文件及本批/前两批索引合同，4文件15项全部通过，耗时6.79秒，没有修改任何超时配置或CI并发。CJS/ESM升级探针在全量中亦通过。接下来以精确实现提交的Linux默认并发、真实PG16与全部8项任务作最终验收；远端main已确认仍为59140d7，无待合并的他人新增提交。

### DB-009D2b3a Linux PG16最终证据

实现`2ef0d333ccf5485bb03e439685450d468872607c`已推送，[Actions34003919501](https://github.com/cinagroup/cinashop/actions/runs/34003919501)最终8/8成功。Linux默认并发下239文件/1,544单元全部通过、无跳过，含4项真实PG16测试；本地那项5秒时限失败没有在该次CI复现。workerd3文件/24项、两套TypeScript、五端构建、Kefu17项、公共依赖18项、ECharts10项、UniApp工具链8/产物3/Intlify9项及既有依赖门禁全部通过。Gitleaks实际扫描239个提交，无泄露、无新增或扩大例外；PG16审计中的额外探针不重复计入单元数量。

PostgreSQL16.14实际执行外部138文件、内嵌143步、全新ORM1,061条SQL；第四座库仍包含旧模型1,061条和生成的114条替换DDL，并额外执行本批恢复与11类拒绝探针。四路径均263表/3,700列/227序列；外部索引963→991、内嵌958→986、两条ORM路径仍1,008，外部/内嵌约束570、两条ORM529不变。日志和upgradeVerification明确确认28个恢复、2条重复流水夹具保留、故障回滚、非目标OID/FK依赖保留、幂等、非public/temp隔离，原57索引升级证明也继续通过；107项合同全部验证，新建与升级后ORM全部已覆盖目录一致，四座随机测试库确认清理。

自动深度比对上一轮真实摘要：唯一类别计数变化为externalVsOrm的candidate-only97→69；外部/内嵌各加28索引，ORM生成SQL摘要未变，52个缺失物理名称及精确candidate完整数组、外部↔内嵌5项别名证据、其他类别差异全部不变。107合同集合精确等于原79加本批28；28项均已从额外对象原始记录消失，22个review-orm-only对象完整保留，其余47个额外对象仍为别名候选。根目录MIGRATION_SCHEMA_AUDIT.json已绑定受测SHA/CI、实际计数及新增恢复证明，继续标记not_equivalent，不改写首次463条历史基线。

剩余4处默认、约束缺56/多15/同名9变、52个物理名称/47个别名候选、22个普通索引以及1处序列差异仍开放。窄SQL文本门禁仍201→263、源列缺口/列漂移0；路由仍1,904/1,646/879/861/18/1,025/17/1,008及46.2%/45.2%/45.6%，不把索引修复计作业务覆盖增长。仅勾选D2b3a，D2b3、E、F及总迁移不勾选；最终回填只修改根目录两份Markdown和JSON。全程没有生产连接、业务数据读取、线上DDL/DML、provider调用或Cloudflare部署。PostgreSQL技能对完整键序、唯一性和依赖的要求直接促使本批区分“漏迁移”与“待审查”，并用真实回滚和目录证据验收。

## DB-009D2b3b 剩余22个普通索引逐项处置（2026-09-06）

上一轮c247f0b回填的2ef0d33/Actions34003919501已完成28项补迁移，本轮从干净工作区继续，完整审查22个review-orm-only对象，而非只挑选有直接PHP声明的条目。新增`audit/orm-ordinary-index-reconciliation.json`绑定受测基线、22份旧目录、键序及DAO/控制器/服务中的精确查询片段和原始行号；历史50项来源清单保持不变。社区/评论时间范围和作者过滤、活动状态/参与者/团长查询、优惠券类型、物流模板、客服账号和会话排序、消息时间/受众/已读联表、佣金分类和关联订单均有代码消费者。user_message虽无PHP安装源表，其uid/message_id联表在UserMessageVisibility中真实存在，不按“无源KEY”删除。

20项按既有ORM完整定义补入外部0137和逐字一致内嵌0143，沿用永久普通表、显式schema、短锁等待、同名严格校验、缺失才创建、失败关闭的机制；不改业务行、不变更唯一性、不删除别名。源码中出现过滤/排序是声明来源证据，不等于本批20个索引都将在某一生产查询计划中使用，也不声称有实测性能收益。复合前缀重合的sm_user、sbu_uid等仍保持完整独立声明，未使用前缀规则草率删除。

另2项store_order.so_order_id与system_supplier.supplier_admin_id，当前部署迁移均已有对应so_order_id_uq/supplier_admin_id_uq独立唯一索引。隔离全模型目录确认两对除唯一性和物理名外定义相同；底层indkey、indclass、indcollation、indoption、键数/属性数、表达式/谓词、访问方法和表OID也完全相同。仅从两个ORM文件各移除一行普通索引声明，保留原唯一性、名称、对象类型及全部业务字段。依据[PostgreSQL唯一索引说明](https://www.postgresql.org/docs/16/indexes-unique.html)，完整同键唯一索引可提供相应索引访问，但这不是仅凭同列就发布删除的许可；[DROP INDEX的默认RESTRICT及锁行为](https://www.postgresql.org/docs/16/sql-dropindex.html)和[pg_depend依赖规则](https://www.postgresql.org/docs/16/catalog-pg-depend.html)仍必须逐项验证，生产目录与发布窗口仍未检查或授权。

CJS/ESM共享探针保留原57定义升级阶段和28项恢复证明，再对20项复用完整11类拒绝/回滚、重复消息状态行保留、幂等与非public/temp隔离；通用化只允许这两个固定队列，原28项测试合同保留。初始快照明确补回2个旧真实Drizzle声明，因此真实旧库仍包含二者；第二阶段生成SQL严格限制为2条精确DROP INDEX且无CASCADE。先确认各索引无入向pg_depend，再给每个索引创建真实regclass依赖视图，整个生成增量必须以2BP01拒绝且回滚恢复先前对象；另在第一条DROP后注入22012除零故障并检查回滚。成功分支保留4条订单/供应商夹具和全部非目标OID，两个自建FK仍通过conindid指向原唯一索引且拒绝不存在引用，重复订单号/管理员ID仍23505。强制关闭顺序/位图扫描后的等值查询计划可使用保留的唯一索引；这仅证明访问能力，非生产性能测量。所有视图/FK/业务夹具在隔离事务回滚清除，再在原测试库应用精确2条生成DDL并验证新快照no-op，不向生产输出或执行DROP操作。

新增5项单元回归，固定完整22项/20新增+2移除、查询来源、SQL镜像、旧快照与保留唯一合同、128项正向门禁及2个禁止重现的物理名。107个原正向合同完整保留，增加20项查询索引及原已部署但尚未纳入门禁的so_order_id_uq，supplier_admin_id_uq与旧队列的唯一明确重合被验证而非宽泛去重。所有4条目录路径均拒绝退役名重新出现（包括错误表上的同名索引），128个正向定义在三个候选路径精确比较，其他差异仍完整输出。两种加载方式的4项真实API验证及4文件15项合同测试已通过；全量、两套TypeScript与Linux PG16结果待回填，本项暂不勾选。PostgreSQL技能促使本批区分20个查询合同与2个完整重复对象，并要求真实依赖拒绝和唯一性保留证明。全程无生产连接、生产数据读取/改写、provider调用或Cloudflare部署。

本地最终检查：两套TypeScript通过；maxWorkers=2全量240文件全部通过，1,545项通过/4项专用PG测试因无本地服务跳过，共1,549项，耗时189.46秒。没有调高时限、减少测试或修改CI并发；前一轮本机列宽测试时限失败在本次全量未复现。差异检查通过，远端main只读确认仍为c247f0b。接下来将精确实现提交推送到原仓库，以Linux默认并发与四座随机PG16测试库验收，本地结果不替代真实PG16证明。

### DB-009D2b3b Linux PG16最终证据

实现`86cb73a83c9380b88406d464142b022632792bb1`已推送，[Actions34005185002](https://github.com/cinagroup/cinashop/actions/runs/34005185002)最终8/8成功。默认并发下240文件/1,549单元全部通过、无跳过，含4项真实PG16测试；workerd3文件/24项、两套TypeScript、五端构建、Kefu17项、公共依赖18项、ECharts10项、UniApp工具链8/产物3/Intlify9项及既有依赖门禁全部通过。Gitleaks扫描241个提交无泄露，未新增或扩大例外。额外目录升级探针不重复计入单元总数。

PostgreSQL16.14实际执行外部139文件、内嵌144步、全新ORM1,059条SQL；升级路径从仍带2项旧普通声明的1,061条真实旧DDL开始，先执行114条既有57定义替换，再执行2条独立验证的普通索引移除，共116条生成增量，另执行28/20项恢复及其拒绝探针。四路径仍263表/3,700列/227序列；外部索引991→1,011、内嵌986→1,006、两条ORM1,008→1,006，约束仍570/570/529/529。128正向合同和2退役物理名禁回归在全部要求路径通过，新建与升级后ORM的全部已覆盖目录一致，四座随机库确认清理。

日志及upgradeVerification同时证实原28项恢复证明完整不变、本批20项恢复和11类拒绝回滚，以及2个普通索引的实际生成移除。两个真实依赖视图均导致2BP01拒绝，第一条DROP后的故障回滚成功；4行订单/供应商夹具、非目标OID、原唯一索引和外键conindid/pg_depend保持，唯一/FK拒绝行为及等值索引计划、最终no-op均通过。这些是在隔离PG16中实际执行的证据，不用本机PGlite代替生产数据库现状，也不是线上删除或发布授权。

本次完整原始差异记录与上一轮2ef0d33日志逐条深度比较，恰好移除22项candidate-only记录，其余每条原始差异完全一致，不仅是数量相同。摘要candidate-only69→47；52个缺失物理名称及精确candidate数组、外部↔内嵌5项重复对象证据均不变。128合同集合恰为原107加20个查询索引及so_order_id_uq，2退役集合精确匹配已审对象。根目录MIGRATION_SCHEMA_AUDIT.json绑定本次受测SHA/CI和实际升级元数据，继续标记not_equivalent，历史来源/首次目录快照不改写。

剩余52个物理名称按实际目录拆成44个普通名称、3个约束附属名称和5个外部重复对象（含1唯一索引），已新增D2b3c/d/e；47个candidate-only全部属于前两组的别名候选，不再有未处置的50项非别名额外对象。4处默认、约束缺56/多15/同名9变及1处序列仍开放。窄SQL文本门禁仍201→263、源列缺口/列漂移0；路由仍1,904/1,646/879/861/18/1,025/17/1,008及46.2%/45.2%/45.6%，本批不增加业务覆盖。只勾选D2b3b，父项、E、F、总迁移及生产前置不勾选。最终证据回填只改根目录两份Markdown和JSON，全程没有生产连接、生产业务数据读写、provider调用或Cloudflare部署；PostgreSQL技能要求的完整定义与依赖证据决定了20补齐/2去重的不同处置。

## DB-009D2b3c 44个普通索引名称一致化（2026-09-06）

从0c8a6b1记录的86cb73a/Actions34005185002目录继续，完整提取44对定义相同而名称不同的普通索引，排除3个约束附属名称及5个外部重复对象。新清单orm-index-name-reconciliation.json保留44对完整PG16行、旧Drizzle快照、25表/10模型文件的前后单行声明，以及45处原始CREATE INDEX语句和行号；store_service_record的同一目标名在0006和0021各声明一次，两份来源均保留。测试从不可变首次目录的47个别名候选重新推导普通44项，不以本轮减少后的差异报告充当预期。应用代码没有发现索引提示或按这44个物理名称执行的查询消费者；模型只替换名称字面量，不改变字段、顺序、唯一性或业务DML。

外部0138与逐字镜像内嵌0144对每项要求旧名/目标名恰有其一，并完整校验永久普通表、普通B-tree、键序、无表达式/INCLUDE/谓词、有效状态、非副本标识及无约束所有权；两名并存、两名皆无和任何异构都中止。实际修改只执行带schema的ALTER INDEX RENAME，验证原OID和完整新定义。依据[PostgreSQL16 ALTER INDEX](https://www.postgresql.org/docs/16/sql-alterindex.html)，重命名不改存储数据，且约束附属索引会连带改约束名，故3项约束名称不混入本批。表与索引的SHARE UPDATE EXCLUSIVE锁以2秒限制等待，不代表总耗时或生产窗口已获批准。

真实Drizzle生成器对这44项产生44 DROP+44 CREATE共88条建议；本批明确不把这88条作为成功升级路径。共享CJS/ESM/PG16探针对44个索引各建立regclass依赖视图，逐条DROP及整份生成提案共45次都须按[默认RESTRICT规则](https://www.postgresql.org/docs/16/sql-dropindex.html)以2BP01拒绝，绝不加CASCADE。随后执行受守卫的重命名，逐项比较全目录、pg_class OID、relfilenode/实际文件路径、全部索引pg_depend、视图重写规则及6行用户/购物车/订单夹具；视图仍引用原OID，重复执行不得有任何目录或文件变化。

12类负向夹具覆盖缺两名、并存、唯一性、DESC、WHERE、INCLUDE、表达式、错表、约束所有权、目标名异构、缺表和缺schema；末项故障必须回滚先前43次重命名。另在首条ALTER后注入除零故障验证原子回滚，验证22旧/22新混合状态及非public schema/临时同名表隔离。全部故障、业务行、视图夹具仅在隔离库事务内创建和回滚。新建ORM及已有库升级须在全部已覆盖目录一致，正向合同128→172，所有四路径新增44个旧物理名禁回归，原2个退役名门禁和57/28/20项先前证明保留。

首次本机探针发现真实API快照带undefined可选where/opclass而持久JSON省略这些字段，已按Drizzle JSON序列化语义比较；没有放宽已定义字段。修正后两种加载方式4项通过，96.90秒；新增5项来源/镜像/严格合同测试，与前两批共3文件14项通过。两套TypeScript、全量及当前实现提交的Linux PG16结果待回填，本项暂不勾选。PostgreSQL技能要求的完整定义与依赖核查决定了本批采用保留对象身份的重命名，并将生成器的破坏性建议作为拒绝测试。无生产连接、业务数据读取、线上DDL/DML、provider调用或部署。

本地最终两套TypeScript均通过；maxWorkers=2全量241文件通过，1,550项通过/4项专用PG测试因无本地服务跳过，共1,554项，耗时210.20秒。没有更改测试超时或CI并发，新增CJS/ESM完整升级探针在全量亦通过。差异检查通过，GitHub main只读核对仍为0c8a6b1，随后提交当前实现供Linux默认并发、真实PG16及全部8项任务验收；本机结果不代替PG16证明。

### DB-009D2b3c Linux PG16最终证据

实现`d416f318863f2ccf127cfde477f137b9b100e282`已推送，[Actions34007245605](https://github.com/cinagroup/cinashop/actions/runs/34007245605)最终8/8成功。Linux默认并发241文件/1,554单元全部通过、无跳过，含4项真实PG16测试；workerd3文件/24项、两套TypeScript及五端构建全部通过。Kefu17项、公共依赖18项、ECharts10项、UniApp工具链8/产物3/Intlify9项及既有依赖门禁均通过。Gitleaks扫描243个提交无泄露，没有新增或扩大例外。独立目录探针不重复计入单元数量。

PostgreSQL16.14实际路径如下；四路径均263表、3,700列和227序列，索引/约束数量与86cb73a完全不变，名称收口不虚增或删除对象：

| 路径 | 实际步骤 | 约束 | 索引 |
|---|---:|---:|---:|
| 外部SQL | 140个文件 | 570 | 1,011 |
| Worker内嵌 | 145步 | 570 | 1,006 |
| 全新ORM | 1,059条生成SQL | 529 | 1,006 |
| 旧ORM升级 | 1,061条初始SQL + 117条升级语句 | 529 | 1,006 |

117条升级语句严格等于既有57定义的114条生成替换、本批1条含44次ALTER的守卫DO、既有2条重复普通索引移除；未将只做检查/拒绝测试的88条生成建议算入成功升级。外部/内嵌此时均已有规范名，0138/0144完整验证后为无变化；旧ORM路径才执行44次改名。实际日志与upgradeVerification逐项确认44个依赖视图仍指向原OID，所有索引OID/文件、外键及索引pg_depend、6条自建业务行保留；45次DROP/整份提案拒绝、12类漂移回滚、首条改名后故障回滚、混合状态、幂等及schema/temp隔离全部通过。原57定义/28恢复/20恢复/2移除证明字段与上一轮逐项相同，新建与升级后ORM全部已覆盖目录一致，四座随机测试数据库确认清理。

完整日志包含120条ORM_DDL_AUDIT记录（1摘要、119差异）。与上一轮原始记录深度比较，恰好删除44条reference-only、44条candidate-only及44条possibleRenames，共132条；其余每条记录完全一致。摘要索引缺52→8、多47→3、可能改名47→3，同名变化仍0；8个缺失名称的精确candidate列表为原列表减去已收口44项，外部↔内嵌5项重复对象证据保持不变。172正向合同精确等于原128加44个规范名，2退役合同完整保留，44旧名禁回归集合与清单精确相同。输入和生成SQL摘要已更新；根目录MIGRATION_SCHEMA_AUDIT.json绑定本次受测SHA/CI和实际输出，仍明确not_equivalent，首次463条目录基线不改写。

剩余3个约束附属索引名与5个外部重复物理对象继续分别由D2b3d/e处理。4处默认、约束缺56/多15/同名9变（15组可能改名）及1处序列差异完全未变。窄SQL文本审计仍201→263、源列缺口/列漂移0；路由仍1,904/1,646/879/861/18/1,025/17/1,008与46.2%/45.2%/45.6%，不把索引改名计成业务覆盖增长。本次只新勾选D2b3c并更新清单顶部现状，父项、E、F、真实角色/provider及生产前置全部保留开放。最终回填仅修改根目录两份Markdown与JSON，受测实现不再改动。全程无生产连接、生产数据读写、provider调用或Cloudflare部署；PostgreSQL技能的完整定义和依赖要求直接决定了保留对象身份的重命名及破坏性建议拒绝测试。

## DB-009D2b3d 3个约束附属索引名称一致化（2026-09-06）

从干净1fe1e9b及d416f31/Actions34007245605的实际PG16目录继续，剩余3个同构candidate均由约束所有：data_migration_checkpoint的(run_id,table_name)主键，以及kefu_visitor_session的token_hash/visitor_uid唯一约束。新orm-constraint-name-reconciliation.json完整保存3对索引、3对约束的原始目录、真实旧Drizzle快照、0020/0104原始SQL行及两个模型的前后声明。源SQL使用默认名称，目录中的_pkey/_key才是部署名称；模型现显式指定这些名称，不改变键、类型、非空、唯一性或游客序列。原始47个别名队列中的3个constraintOwned条目作为单元测试权威，不从本轮将减少的报告重建预期。

真实生成器对仅这3项的增量提出3条DROP CONSTRAINT和3条ADD CONSTRAINT。本批不将重建约束当作成功升级路径：外部0139/逐字内嵌0145要求约束与附属索引的旧名或目标名恰有一整对，校验主键/唯一类型、conindid、完整键序与定义、validated/deferrable/deferred/继承状态、立即唯一性、NULLS语义、索引有效性和普通永久表等；未知结构、名称并存或缺失均中止。实际只执行带schema的ALTER TABLE RENAME CONSTRAINT，并确认约束OID和索引OID同时保留。依[PostgreSQL16 ALTER TABLE](https://www.postgresql.org/docs/16/sql-altertable.html)，此形式需要ACCESS EXCLUSIVE锁；本批将等待限制2秒，并不表示生产锁窗口、目录或执行权限已经批准。保留[约束及附属索引联动命名](https://www.postgresql.org/docs/16/sql-alterindex.html)，不退化成独立唯一索引或放弃外键。

升级探针在既有57定义/28恢复/20恢复后、44普通改名/2普通移除前推进这3个约束；初始快照明确恢复真实旧名，其他阶段继续使用各自已审查的旧/新状态。为3个附属索引各建立真实regclass视图，为3个键各建立ON UPDATE/DELETE CASCADE外键，验证原conindid、约束/索引OID、物理文件、pg_depend和触发器记录不变。逐条DROP INDEX、逐条生成DROP CONSTRAINT及整份提案共7次须以2BP01拒绝。成功重命名后重复键仍23505且错误返回新约束名，错误外键仍23503，两个父实体的删除级联及三个键的更新级联实际执行后回滚。5条自建父数据和1条子数据保留；这些只在随机隔离库事务中创建，与旧MySQL历史数据无关。

12类负向夹具覆盖缺配对、无约束独立索引、目标索引冲突、两名约束并存、错键、可延迟、NULLS NOT DISTINCT、INCLUDE、错误约束类型、目标名错键、缺表及缺schema。末项失败须撤销之前已执行改名，首条ALTER后另注入除零验证原子回滚；同时测试部分已改名状态、非public schema及临时同名对象隔离、重复执行。首次测试中新夹具的a/b哈希与原57定义测试的夹具冲突，唯一约束正确拒绝，随后仅把新增夹具改用d/e/f独立值，不改现有夹具或放宽唯一性。两种加载方式4项真实升级测试通过，93.25秒；两套TypeScript和2文件10项来源/镜像/合同测试通过。全量与精确实现提交的Linux PG16结果待回填，本项暂不勾选。

正向索引合同172→175，新增3个完整约束合同及旧约束/索引物理名禁回归，所有四路径必须验证；原2退役名、44普通旧名、172个已审定义均保持。应用源码未发现这3个旧名的ON CONSTRAINT、索引提示或错误名分支消费者；UserCenterService按名字判断的23505仅针对签到us_uid_shanghai_day_uq，与本批无关。其他约束/default/序列差异和5个外部重复对象继续完整输出，未修改首次目录和先前来源清单。PostgreSQL技能促使本批同时审查拥有关系、外键、触发器和短事务锁，保持约束语义而非只使名称计数相等。全程无生产连接、生产DDL/DML、provider调用或部署。

本地最终全量maxWorkers=2为242文件全部通过，1,555项通过/4项专用PG测试因无本地服务跳过，共1,559项，耗时182.88秒；新约束名的23505错误字段断言也在全量通过。没有改变60秒子进程时限、测试时限或CI并发。两套TypeScript已通过，差异检查通过，远端main确认仍为1fe1e9b。接下来提交精确实现供Linux默认并发、四座随机PG16库和完整8项CI验收，不用本地模拟引擎结果代替PG16证据。

### DB-009D2b3d 首次CI失败与审计夹具修正

首次实现a9a58745669fb22ffba0e5b303b8070feb89f41f的Actions34008418403为7/8；不是成功验收。242文件中241通过，1,558项通过/1项失败，无跳过；CJS完整升级子进程在60,083ms触发明确ETIMEDOUT，ESM同样全部断言在50,295ms通过，PG16独立目录步骤尚未执行。因升级探针已累计57定义、28/20恢复、3约束改名、44普通改名及2移除六个阶段，将仅该子进程上限60→90秒、外层对应测试90→120秒；保留全部数据/依赖/回滚断言、默认CI并发与数据库超时，并让超时断言附带已采集输出。没有把失败记作通过或通过重跑同一失败提交掩盖问题。

另外检查已安装postgres.js的connection.js确认协议字段n映射为constraint_name，PGlite则为constraint；首次CI尚未到此PG16断言，不能称为已复现的数据库失败。共享错误校验现同时支持这两个确定字段，仍强制SQLSTATE和每个已提供名称精确相等，缺失/错误/二者冲突都失败；新增1项回归验证两种真实拼法及拒绝分支。此修正不改变迁移SQL、应用模型或数据库行为。后续以新精确提交重新验收完整CI。

修正后2文件10项本机专项通过，包含CJS/ESM全部六阶段及两类驱动错误字段正反例，耗时88.55秒。未删除任何探针或变更迁移定义；两套TypeScript复核与新提交的完整CI继续执行，旧a9a5874失败记录不覆盖。

### DB-009D2b3d Linux PG16最终证据

修正提交`b0c15fca2c299a195c0f197d7bb3def11301e919`的[Actions34008849823](https://github.com/cinagroup/cinashop/actions/runs/34008849823)最终8/8成功；此前a9a5874/34008418403的7/8失败历史保留。Linux默认并发242文件/1,560单元全部通过、无跳过，含4项专用PG16测试；CJS完整探针实际65,208ms、ESM50,374ms，证明保留全部检查的CJS运行确实超过旧60秒上限，现有90秒子进程边界内完成。workerd3文件/24项、两套TypeScript和五端构建全部通过；Kefu17项、公共依赖18项、ECharts10项、UniApp工具链8/产物3/Intlify9项及既有依赖门禁全部通过。Gitleaks扫描246个提交无泄露，没有新增或扩大例外。独立目录审计的额外探针不重复计入单元数。

PostgreSQL16.14实际执行外部141文件、内嵌146步、全新ORM1,059条SQL；升级路径为包含已审旧声明的1,061条初始DDL，加118条升级语句（57定义的114条生成替换、3约束的1条守卫DO、44普通名的1条守卫DO、2条普通索引移除），共1,179步。本批生成器6条DROP/ADD仅用于检查和依赖拒绝，不算成功升级。四路径均263表/3,700列/227序列；索引仍1,011/1,006/1,006/1,006，约束仍570/570/529/529，与d416f31完全相同，改名没有增删物理索引或约束。

PG16实际确认3个约束与3个附属索引OID、relfilenode/path文件标识（不是磁盘内容摘要）、全部所检查依赖和触发器记录不变。3条外键conindid及3个regclass视图仍引用原索引；7次DROP/生成提案拒绝、12类漂移及首条ALTER故障回滚、部分已改名、schema/temp隔离和幂等通过。3项唯一性错误均保留23505并返回统一后的名称，postgres.js实际constraint_name字段与PGlite.constraint均由严格校验覆盖；外键23503、三个键的更新级联和父实体删除级联实际通过。5条自建父数据及1条子数据保留，所有夹具事务回滚。175个完整索引、3个完整约束、2退役名/44普通旧名/3约束旧名门禁通过，新建和升级后的全部已覆盖ORM目录一致，四座随机数据库确认清理。原28/20恢复、44普通改名及2移除的完整验证元数据与上一轮逐字段相同，原57升级/回滚证明继续通过。

完整日志102条ORM_DDL_AUDIT记录（1摘要、101差异）已与d416f31/34007245605的120条逐条深度比较：只移除3个索引和3个约束各自的reference-only/candidate-only/possibleRenames，共18条，其余每条完全一致。索引缺8→5、多3→0、别名候选3→0、同名变化仍0；约束缺56→53、多15→12、别名候选15→12、同名变化仍9。175索引集合精确等于原172加本批3项，3约束/旧名集合精确绑定清单；剩余5索引的精确candidate列表及外部↔内嵌证据不变。根目录MIGRATION_SCHEMA_AUDIT.json绑定新受测SHA/CI、实际摘要和验证字段，保持not_equivalent，不改写首次463条历史目录。

本轮只新勾选D2b3d；5个外部重复物理索引继续D2b3e，已只读定位0003/0004/0005的初始对象及0108/0082/0085中的后续保留候选，尚未作依赖安全或删除判定。4处默认、其余约束和1处序列仍开放。窄SQL文本门禁仍201→263、源列缺口/列漂移0；路由仍1,904/1,646/879/861/18/1,025/17/1,008及46.2%/45.2%/45.6%，不把结构改名计作业务覆盖。最终回填仅修改根目录两份Markdown与JSON，整体迁移、父项、真实角色/provider和生产前置均不勾选。无生产连接、生产DDL/DML或部署。PostgreSQL技能对拥有关系、短事务和外键的要求促使本批保留真实约束身份，并用完整依赖与级联验证作为完成依据。

## DB-009D2b3e 5个外部重复索引的受守卫退役（2026-09-06）

从干净8b38afc和b0c15fc/Actions34008849823实际目录继续。仅处理外部独有的sor_store_order_id_idx、ur_uid_idx、wu_openid_uq_idx、wu_uid_idx、wu_unionid_idx；分别保留sor_store_order_id、ur_uid、wu_openid_uq、wu_uid、wu_unionid。完整来源、定义和查询消费者固定在audit/external-duplicate-index-reconciliation.json：0003/0004/0005原始CREATE与0108/0082/0085后续不同名CREATE共同造成重复，内嵌和ORM只声明后者。保留行预期来自上一轮exactCandidates的全字段同构证据（仅剔除key/name），并非伪称原始差异日志包含不存在的candidate-only行；隔离探针执行前会独立读出并严格断言两条实际目录行。旧迁移、模型和历次不可变清单不改写。

外部0140与逐字内嵌0146只接受当前schema的普通永久表，先加ACCESS EXCLUSIVE锁并限制等待2秒；即使旧对象缺失也完整验证保留对象。严格检查唯一性、有效/就绪/存活/立即标志、单列完整键序、操作符类、排序/NULLS/校对规则、无表达式/部分谓词/INCLUDE/特殊存储选项/表空间/集群或复制标识用途、无拥有约束。旧对象另须无任何pg_constraint.conindid、入向pg_depend、未知出向所有权/扩展/分区或共享依赖；只执行schema限定DROP INDEX RESTRICT，确认保留OID/relfilenode不变，不重建、不改行或重绑外键。[PostgreSQL16 DROP INDEX](https://www.postgresql.org/docs/16/sql-dropindex.html)说明普通删除的锁及CONCURRENTLY限制；依[pg_depend规则](https://www.postgresql.org/docs/16/catalog-pg-depend.html)，RESTRICT仍可能自动带走AUTO/INTERNAL依赖，因此显式依赖拒绝不能省略。这些守卫不构成生产窗口或删除授权。

新增独立探针，不加重已受90秒边界约束的原CJS/ESM六阶段子进程。PGlite本机路径生成完整当前ORM后执行五条原始历史CREATE；它不是从头执行外部历史，也不冒充PostgreSQL16。CI将在专用loopback身份、随机数据库的完整外部历史抵达0140前运行同一探针，完成夹具回滚后才提交该迁移；内嵌、全新ORM、旧ORM升级三路径继续独立执行。五个regclass视图和明确绑定旧唯一索引的FK均须拒绝；绑定保留唯一索引的FK、视图、触发器、OID和文件标识须不变。另覆盖键列/排序/部分/表达式/INCLUDE/存储选项/同名表/错表/CLUSTER、唯一性丢失/拥有约束/复制标识、缺保留索引/表/schema共34次拒绝。真实DO首条DROP后除零须回滚恢复所有原对象；两条已退役混合状态、临时同名对象和重复执行同样覆盖。六条显式ID夹具验证非唯一重复值保留、openid仍23505且准确返回保留名、外键23503和更新/删除级联。显式ID避免nextval不可回滚带来的序列副作用，业务夹具全部在隔离事务回滚。

正向合同175→180，新增五个退役名禁回归；原2退役、44普通旧名、3拥有约束/索引旧名及完整约束门禁不变。额外把全部索引类别改为四路径精确门禁，不只比较180白名单或数量；新增、缺失、别名、定义变化和重复目录键均失败。其余约束/default/序列差异仍逐条输出，MIGRATION_SCHEMA_AUDIT.json在新提交CI实际通过之前继续绑定上一轮证据。当前本机专项1文件/5项全部通过（39.01秒），两套TypeScript通过；完整单元、Linux默认并发及真实PG16结果待回填，因此清单e和父项暂未勾选。PostgreSQL技能的依赖与短事务要求直接影响本批守卫和拒绝测试。无生产连接、生产DDL/DML、provider调用或部署。

本机最终全量maxWorkers=2为243文件全部通过，1,561通过/4项专用PG测试跳过，共1,565项，耗时195.00秒；新专项不增加原六阶段CJS/ESM子进程内容，原90秒边界与CI默认并发不变。两套TypeScript与差异检查通过，远端main确认仍为8b38afc。准备提交精确实现，待Linux完整8项CI及四座PG16隔离库证据后回填结论。

### DB-009D2b3e Linux PG16最终证据与索引子项收口

实现`cc2e5db9ac1a13975c30c713bed90f4e2c4e79d2`已推送，[Actions34010180395](https://github.com/cinagroup/cinashop/actions/runs/34010180395)最终8/8成功。Linux默认并发243文件/1,565单元全部通过、无跳过，含4项专用PG16测试；原CJS六阶段实际60,708ms、ESM61,505ms，均在原90秒子进程边界内，本批未放宽时限或减少检查。workerd3文件/24项、两套TypeScript、五端构建、Kefu17项、公共依赖18项、ECharts10项、UniApp工具链8/产物3/Intlify9项全部通过。Gitleaks扫描248个历史提交无泄露，没有新增或扩大例外；独立PG16探针不重复计入单元数量。

PostgreSQL16.14实际路径如下；四路径均263表、3,700列、227序列：

| 路径 | 实际步骤 | 约束 | 索引 |
|---|---:|---:|---:|
| 外部SQL | 142文件 | 570 | 1,006 |
| Worker内嵌 | 147步 | 570 | 1,006 |
| 全新ORM | 1,059条生成SQL | 529 | 1,006 |
| 旧ORM升级 | 1,061条初始SQL + 118条升级语句 | 529 | 1,006 |

外部完整141文件历史实际创建两套五索引后，共享探针逐项验证其完整目录并执行34次依赖/漂移拒绝。五个旧索引的regclass视图及绑定旧唯一索引的FK正确阻止删除；绑定保留唯一索引的FK、视图、约束/触发器/依赖、OID和relfilenode（文件标识，不是磁盘内容摘要）均保留。首条真实DROP后故障回滚、六行自建业务夹具、重复值等值查询、保留唯一名23505、FK 23503、更新/删除级联、两项已退役混合状态、同名temp隔离及重复执行全部通过；夹具回滚后最终提交0140仅移除五个旧物理索引，非目标目录/OID/文件标识/约束/触发器/依赖及原行不变。内嵌本来没有这五个旧名，0146在完整核验保留对象后为无变化；ORM模型和全部生成SQL摘要与上一轮完全相同。原57定义/28恢复/20恢复/2移除/44普通改名/3约束改名的全部upgradeVerification字段与b0c15fc逐字段一致，仍为175个历史阶段合同；最终四路径全局合同额外加入五个保留名为180，另对全部1,006索引实行严格比较。新建与升级后ORM全部已覆盖目录一致，四座随机数据库确认清理。

完整日志92条ORM_DDL_AUDIT记录（1摘要、91差异）与上一轮102条逐条深度比较：恰好移除externalVsEmbedded、externalVsOrm各五条reference-only索引记录，共10条；其余每条记录完全一致。两组missingIndexEvidence都为空。外部↔内嵌表/列/约束/索引/序列的reference-only/candidate-only/changed/possibleRenames全部0；外部↔ORM索引这四类也全部0。180正向名称精确等于原175加本批五个保留名，五个禁回归名称精确绑定新清单，原2退役/44旧普通名/3旧拥有约束名与约束合同完全保留。根目录MIGRATION_SCHEMA_AUDIT.json绑定本次受测SHA/CI和实际摘要，仍为not_equivalent；首次463条目录基线及历次不可变来源清单未改写。

据此勾选D2b3e及已完成全部子项的D2b3/D2b/D2，范围明确为代码/隔离PG16验收，未应用生产。DB-009本身、E与F整体继续开放，F仅索引类别完成严格门禁。剩余外部↔ORM列默认4处、约束缺53/多12/同名9变/可能改名12、序列1处变化完全未减：其中12对均为外键名称，41项非别名缺口为39 CHECK与2 FK（spr_order_cart_info_fk、wco_event_id_fk），8个缺失约束处于NOT VALID（7 CHECK/1 FK）；另8个同名CHECK的验证状态和1个事件集合顺序仍待审。清单新增E1默认/E2缺约束/E3外键名/E4同名语义/E5序列五个待办，不把NOT VALID自动改成已验证，也不为消差异删除校验。窄SQL审计仍201→263、源列缺口/列漂移0；路由仍1,904/1,646/879/861/18/1,025/17/1,008及46.2%/45.2%/45.6%，索引对齐不计作业务覆盖增长。
...
### DB-009E1 Linux PG16最终证据与列类别门禁

实现`9af7fb665d65919a0b0c217db3005838347dc92a`已推送，[Actions34011673361](https://github.com/cinagroup/cinashop/actions/runs/34011673361)最终8/8成功。Linux默认并发244文件/1,572单元全部通过、无跳过，含4项专用PG16测试。新增默认值CJS/ESM探针实际36,519/34,097ms，既有六阶段CJS/ESM实际67,155/66,034ms，均在原90秒子进程边界内，未放宽时限或减少检查。两套TypeScript、workerd3文件/24项和五端构建通过；Kefu17项、公共依赖18项、ECharts10项、UniApp工具链8/产物3/Intlify9项及依赖门禁均通过。Gitleaks扫描250个历史提交无泄露，没有新增或扩大例外。本机对提交后的两套TypeScript也再次通过。

PostgreSQL16.14实际执行五条路径，全部为263表、3,700列、1,006索引、227序列：

| 路径 | 实际步骤 | 约束 |
|---|---:|---:|
| 外部SQL | 143文件 | 570 |
| Worker内嵌 | 148步 | 570 |
| 全新ORM | 1,059条生成SQL | 529 |
| 旧索引ORM升级 | 1,061条初始SQL + 原118条升级语句 | 529 |
| 旧默认ORM升级 | 1,059条初始SQL + 1条守卫DO | 529 |

最后一路的守卫DO实际仅修改四处默认；真实生成器4条SET DEFAULT在回滚夹具中执行并证明同义，不混计为最终已提交步骤。PG16独立证实旧ORM的两个省略JSON插入23502，SET DEFAULT NULL确实归为无pg_attrdef状态而非未知表达式；14类真实拒绝全部通过。8行原值、4视图、4外键及其触发器/依赖、表和索引OID/relfilenode（物理文件标识，不是磁盘内容摘要）及非目标默认保持；目标默认OID允许仅在实际变更时改变，重复执行全体默认OID不变。首ALTER故障回滚、混合状态、非public与临时同名表隔离、未知nextval默认未执行（is_called=false）、新建与默认升级后全部已覆盖目录一致均通过。原索引升级的全部upgradeVerification和5个外部重复退役证明与cc2e5db逐字段相同，没有把旧默认修复冒称为原索引升级阶段已覆盖。

每条路径均另在回滚事务中执行四组省略插入、四组显式DEFAULT、四组显式值、四组UPDATE DEFAULT和四组带精确列名的SQL NULL拒绝；两个JSON null写入保留原数据库语义，金额12.345→12.35、上界9999999999.99成功/10000000000越界拒绝、200字符text原样保留全部通过。五组columnWriteVerification内容逐字段相同；此为数据库写入合同，不是支付、死信回放或生产业务E2E。所有夹具均回滚，五座准确随机名称的测试库全部确认清理，没有连接生产或读取生产行。

完整88条ORM_DDL_AUDIT记录（1摘要、87差异）已与cc2e5db的92条逐条深度比较：只移除四条externalVsOrm.columns.changed，其他每条原始差异完全相同。外部↔内嵌五类目录继续全0；外部↔ORM列和索引的缺失/额外/同名变化/别名候选均0。全部3,700列与1,006索引在五路径精确比较，未对默认表达式作文本归一化；四个来源默认合同、原180索引/3拥有约束合同及所有退役名门禁完整保留。输入和生成SQL摘要实际改变，数量不变。根目录MIGRATION_SCHEMA_AUDIT.json绑定本次受测SHA/CI、五路径写入及完整升级证明，仍明确not_equivalent，首次463条目录及历史清单不改写。
...
### DB-009E2A Linux PostgreSQL16最终证据

实现`1641e653c455e53d6b07d1d097dbbbae114992ba`已推送，[Actions34013882048](https://github.com/cinagroup/cinashop/actions/runs/34013882048)最终8/8成功。Linux默认并发245文件/1,577单元全部通过、无跳过，含4项既有真实PG测试；双TypeScript、workerd3文件/24项、五端构建、Kefu17、通用依赖18、ECharts10、UniApp工具链8/产物3/Intlify9及既有依赖门禁全部通过。Gitleaks252提交全历史无泄露，例外配置未改。既有六阶段CJS/ESM实际47,371/51,063ms，仍使用原90秒子进程边界，没有缩减检查。

新独立步骤在PostgreSQL16.14上分别执行真实CJS与ESM生成器，两座随机库各覆盖12条初始SQL、5条NOT VALID新增；三个CHECK/两个FK跨public/nv_tenant，初始与增量目录一致。每路4行原坏数据、原表/索引/已有约束身份保留，5次VALIDATE实际拒绝、8种非法元数据拒绝、精确INSERT/UPDATE错误名、SQL NULL、跨schema组合键租户隔离、更新/删除级联、变更CHECK与FK的原始DROP/ADD提案回滚、移除标记后验证失败回滚、持久化无变化和只读反向读取拒绝均通过。常量诊断被确认来自convalidated前置读取；测试仅拦截上游进度显示器的exit(1)，应用代码不拦截退出或放行反向生成。两座库最终按本次创建的准确名称删除并复查不存在，不使用FORCE；原五座目录库亦全部确认清理。

完整88条ORM_DDL_AUDIT记录（1摘要、87差异）与上一轮9af7fb6/34011673361逐条深度相等，包括全部目录差异、索引/默认升级证明、五路径写入合同和清理结果。外部输入摘要和完整业务模型生成SQL摘要也完全相同。五路径仍263表、3,700列、1,006索引、227序列；外部/内嵌约束570、三条ORM路径529。列与索引整类零差异；约束仍53缺失/12额外/9同名变化/12对别名，序列仍1处变化。根MIGRATION_SCHEMA_AUDIT.json绑定受测实现及CI，保留not_equivalent，额外记录独立notValidGeneratorVerification，不将夹具约束算作业务修复，也不把两座夹具库混为新增完整业务路径。审计设计文件pg-not-valid-generator.md保留受测提交时的候选快照，最终验收状态以本节及根清单为准。

本批只勾选E2A；41项业务约束E2B、E2/E4/E5、DB-009整体及生产/业务发布仍开放。窄SQL结构201→263零源列缺口/列漂移与路由1,904/1,646/879/861/18/1,025/17/1,008均不变，生成器前置不计业务覆盖增长。受测代码不再修改，最终只回填根两份Markdown及JSON。无生产连接、业务DDL/DML、provider调用或部署；PostgreSQL审查要求的验证状态与外键引用语义由上述真实引擎证据确认。

## DB-009E2B：41项缺失约束的模型及前向升级（候选，2026-09-06）

本批从固定目录基线排除12组外键别名，逐项接入39 CHECK与2 FK，其中7 CHECK和评价外键保留NOT VALID。实际新建模型生成1068条SQL，PGlite目录为263表、3700列、570约束、1006索引、227序列；41项目标定义/验证状态与原始外部参考逐条相等。新增0142及内嵌0148的完全相同守卫，只添加缺失对象；先校验17表/96列和全部目标，未知定义、验证状态、父键或触发器状态直接拒绝。源码来源、类型化字段和快照固定于orm-missing-constraint-reconciliation.json，不改变既往不可变来源清单。

测试中修正了三处升级守卫的实质问题：CHECK引用列在pg_constraint.conkey中的顺序不是表字段顺序，应在表达式仍精确匹配前提下比较字段集合；PostgreSQL不能依赖AND短路避免访问尚未赋值的record，外键记录改为独立分支；反解析CHECK文字重新执行可能改变Boolean节点分组，新增DDL改用原始SQL而非pg_get_constraintdef输出。没有为此规范化SQL或放宽验证状态。

独立完整旧模型CJS/ESM探针验证33项校验型新增拒绝旧坏行、8未验证约束保留旧行并限制新引用、99类定义/状态/字段/引用漂移拒绝、41组新INSERT/UPDATE及16边界、两视图/用户触发器/依赖外键夹具、原对象/文件/依赖/数据保留、首ADD故障回滚、生成提案同义、幂等/混合状态/temp和非public隔离。数据库可存在同名用户函数，守卫优先解析系统内置函数并恢复调用方search_path。夹具LIKE INCLUDING DEFAULTS产生的跨schema序列依赖被单独纳入夹具基线，并在清理后恢复完整公共目录，不将其误报为迁移效果。

另加8条跨事务已提交坏旧行验证：先提交旧行，再提交NOT VALID新增，然后检验新坏INSERT/变更外键值UPDATE/显式VALIDATE失败及新合法行成功。已提交孤立行在外键值不变时可以更新其他字段，是PostgreSQL的真实语义；不得声称所有旧孤立行更新都被禁止。相关脚本只在自建隔离测试库中创建准确命名schema，创建成功才允许finally清理。现有六阶段索引子进程保持90秒边界，新完整约束子进程独立180秒，不挪用旧测试时限。

本机完整回归247文件、1580通过/4项专用PG跳过、1584总数，423.20秒；随后增加上述已提交行边界并专项重跑。双TypeScript已通过（本机需显式4GiB堆，默认2GiB曾不足，未计作通过）。Linux审计扩为六座完整路径库并保留两座NOT VALID生成器夹具库。精确提交的真实PG16结果待回填，E2B/E2与DB-009仍开放，根JSON仍保留上一已验收基线。未连接生产、调用provider或发布。PostgreSQL技能关于验证语义、引用索引与短锁等待直接影响了本批实现；生产应用仍需维护窗口及单独授权。评价外键仅有活跃记录局部索引，父端检查性能另待审，不擅自新增索引改变对齐口径。

### DB-009E2B Linux PostgreSQL16最终证据

实现`70103a0291f270412e0bfa1fb51af19e06b89dec`已推送，[Actions34017020788](https://github.com/cinagroup/cinashop/actions/runs/34017020788)在attempt 2最终8/8成功。本节取代前述候选段及受测实现中的设计文件对验收状态的暂定描述；保留它们作为实施时点记录，不回写受测代码。Linux最终247文件/1,584单元全部通过、无跳过，包含4项真实PG16测试；双TypeScript、workerd3文件/24项、五端构建与Kefu3文件/17项通过。Gitleaks扫描254个历史提交无泄露，没有扩大例外。最终本机已提交旧行探针及声明来源绑定专项为2文件/7项全部通过，202.91秒；完整最终代码的全量证据以上述Linux结果为准。

首轮并非全绿：246文件通过/1失败，1,583单元通过/1失败；既有ESM索引API子进程在17,504ms输出`corrupted size vs. prev_size`，退出status为null，并非原90秒超时，也无SQL断言失败诊断。新约束5项测试首轮已全部通过，CJS/ESM分别149,225/106,820ms；首轮尚未执行后续PG16完整目录审计。仅对同一SHA重跑一次失败的完整Worker任务，其他7个成功任务保留，代码、并发、时限和断言均未改；第二轮既有六阶段CJS/ESM为59,767/61,034ms，原90秒边界不变，新独立约束子进程仍为180秒。没有core或原生回溯，根因未定位，作为TEST-005继续开放。[Node #64500](https://github.com/nodejs/node/issues/64500)与[PGlite #1053](https://github.com/electric-sql/pglite/issues/1053)的相近并发原生崩溃仅为排查线索，不能据此断定同源或声称重跑修复了根因。

PostgreSQL16.14六条完整建库/升级路径均为263表、3,700列、570约束、1,006索引、227序列：
...
| Worker内嵌 | 149步 |
| 全新ORM | 1,068条生成SQL |
| 旧索引ORM升级 | 1,070条初始SQL + 原118条升级语句 = 1,188 |
| 旧默认ORM升级 | 1,068条初始SQL + 1条守卫DO = 1,069 |
| 旧缺失约束ORM升级 | 1,059条初始SQL + 1条守卫DO = 1,060 |

新增完整旧约束路径实际恢复41项：39 CHECK/2 FK，保留7 CHECK/1 FK的NOT VALID状态；生成器41条ADD提案在回滚夹具中执行并证明目录同义，不混计为最终已提交步骤。PG16通过33项旧坏行导致校验型ADD拒绝、8项未验证旧行保留、99类完整定义/验证状态/字段/父键/触发器漂移拒绝、41组非法INSERT/UPDATE及16边界。原行、表/索引/非目标约束与触发器OID、relfilenode和依赖保留；首ADD故障原子回滚、包含新约束/触发器OID的no-op、混合状态、temp与非public隔离、内置函数优先绑定且恢复调用方search_path、新建和守卫升级后全部已覆盖目录相同均通过。评价FK的NULL允许与父键NO ACTION、回调FK的ON DELETE CASCADE均有实际写入证据。

跨事务测试另确认8条坏旧行先提交，再提交NOT VALID约束；原行继续存在，新坏INSERT、变更为坏值的UPDATE与显式VALIDATE各8次拒绝，8条合法新行成功。已提交历史孤立行保持FK值不变时，更新其他字段允许，这是引擎语义而非豁免。拒绝/漂移/已提交旧行三阶段累计约16,098/33,715/36,688ms，完整PG16约束探针约37.44秒。所有这些都是隔离数据库夹具，不是新生产数据或真实角色业务E2E。

每路径全部3,700列、1,006索引及41项完整目标约束均精确比较，包含定义、验证、延迟和继承状态；原180索引来源合同、3拥有约束合同及全部旧名/退役名门禁保留。既有索引升级证明除初始SQL从1,061增至1,070外，与1641e65逐字段相同；既有默认升级证明除初始SQL从1,059增至1,068外也逐字段相同。原五路径默认写入证明全部不变，第六路径与全新ORM相同；外部5重复索引退役证明完全不变。独立E2A生成器两座PG16库的全部证明也与上一受测提交逐字段相同。六座完整目录库及两座独立生成器库全部确认清理，不使用FORCE。

完整ORM_DDL_AUDIT为47条（1摘要/46差异），与1641e65/Actions34013882048的88条深度比较，恰好移除本批41条referenceOnly约束；剩余46条原始记录逐条完全相同，未规范化SQL、名称或验证状态。外部↔内嵌五类目录全部零差异；外部↔ORM表/列/索引全部零差异，约束仍12缺失/12额外/9同名变化/12组可能改名，序列仍1处变化。剩余为12组外键别名、8个其他CHECK的NOT VALID状态、1处outbox事件集合顺序及客服游客序列类型/拥有关系；因此根JSON仍明确`not_equivalent`。外部输入SHA256为`7b08db5144e584bc49ede7b53540c29c7d41258ac7ab7dd94c1efd3f0cf15213`，生成SQL SHA256为`9710b2b520169555fc19138c708d93154b6337a439d0584472b29a8b9ab7c2bc`，均直接取自受测日志。

窄结构仍201→263、共享201、源列缺口/外部独有/内嵌独有/列漂移全部0；路由仍1,904 PHP/1,646 TS/879匹配/861可执行/18不可用/1,025缺失/17退役/1,008可执行缺口，覆盖46.2%/45.2%/45.6%。本批仅将E2B/E2勾为代码及PG16 CI完成，E3～E5、F整体、DB-009、TEST-005及所有业务/生产发布门禁继续开放；不把约束数量相等当作目录整体等价，不计作HTTP覆盖增长。

最终回填只修改根审计、清单和JSON，绑定受测代码SHA与attempt 2证据；本轮无生产连接、生产DDL/DML、provider调用或部署。依PostgreSQL技能与[PG16 ALTER TABLE语义](https://www.postgresql.org/docs/16/sql-altertable.html)，本批限制2秒锁等待并验证约束/FK/回滚，但ACCESS EXCLUSIVE及校验型新增仍可能扫描旧行，不能宣称在线零锁；CI外层30秒语句上限是测试边界，不替代生产维护计划。生产应用仍需单独明确授权；评价FK父端检查的索引性能问题与真实角色验证不在本批完成范围。

## DB-009E3 十二组外键名称与保持身份升级（候选，2026-09-06）

从干净3bc64ee及70103a0/Actions34017020788最终目录继续。原始12对外键在已比较定义/验证/延迟/继承字段内仅名称不同；六模型将inline references改为显式foreignKey命名，保留11个ON DELETE RESTRICT、1个迁移账本CASCADE、12个NO ACTION更新及2个可空引用。全部原始SQL、旧新快照、字段/父引用和完整目录绑定orm-foreign-key-name-reconciliation.json，未改既往来源清单；应用显式错误名消费者检索未发现依赖这12个旧名的处理分支，仍逐外键测试新错误名。

外部0143/逐字内嵌0149以2秒锁等待、17父子表排序锁和19字段形状检查，预检全部12项后仅RENAME CONSTRAINT。检查精确父PK/索引、conindid引用关系、引用列/内置相等运算符及4个不同角色的内置RI触发器；缺失、双名、未知等价别名、定义/验证/延迟或触发器状态漂移直接拒绝。conindid指向父索引而非子表拥有索引，不对它错误改名。实际改名后整个约束记录只允许conname变化，内部触发器记录/OID必须原样保留；schema显式限定并恢复search_path。此为有界等待的ACCESS EXCLUSIVE操作，不宣称无锁在线迁移。

独立完整旧ORM探针实际生成1,068条初始SQL，建立2视图和用户触发器等依赖夹具，并比较全部公共目录、对象/OID/relfilenode、pg_index/约束/触发器元数据、依赖及原行。141类异常拒绝包含每外键11类及9类字段/表/schema/父键情况；另逐外键隔离验证12非法INSERT/UPDATE、12 NO ACTION父更新、11 RESTRICT删除、1 CASCADE删除及2 SQL NULL成功，避免由其他子表错误遮蔽被测外键。PG16与PGlite18的RESTRICT删除错误码按引擎版本各自严格断言，不放宽成多个可接受错误。故障注入首RENAME回滚、混合状态、幂等、临时同名表和非public完整父子关系也被核验。

首次探针错误地预期9个长名称DROP会失败；实测确认PostgreSQL对SQL标识符和已有对象使用相同63字节截断，已修正错误测试预期并在沟通中澄清。真正实测风险是12对原始DROP/ADD均替换约束及其4个内部触发器OID；提案仅在回滚夹具中验证，不作为实际升级。保持身份的守卫不依赖删除/重建或自动VALIDATE。独立新子进程使用180秒边界，原索引90秒及其余时限/并发/扫描例外不变。

双TypeScript通过；本机新CJS/ESM及既有来源绑定专项4文件23项通过，194.38秒，全量本机及精确提交Linux/PG16待回填。CI新增第七座完整路径orm_fk_names，七路径须继续满足全部列/索引、既有41约束和新增12 FK完整合同，并拒绝旧物理名及重复别名；新建与各类完整旧模型升级目录须一致。根JSON仍为上一已验证基线，E3/DB-009整体及生产发布门禁不提前勾选。PostgreSQL与Workers技能影响了锁等待、引用/触发器校验、事务内等待和无导入副作用的SQL镜像；未改变绑定、平台API或Secret，未连接生产、调用provider或部署。父端检查的子表索引性能仍是独立审查面，不以改名测试推导性能结论。
...
### DB-009E3 Linux PostgreSQL16最终证据

受测实现为`eebe827f7eeab8630df7faf4acfc0cce07d1ddd5`，[Actions34020575416](https://github.com/cinagroup/cinashop/actions/runs/34020575416) attempt 1最终8/8成功；它包含9fba575的外键迁移及显式类型检查堆上限修正，不是对失败旧SHA重跑。本节取代前述候选段和受测设计文件中的待验收状态，保留原失败事实。Linux双TypeScript、248文件/1,589单元全部通过，无跳过，含4项专用真实PG16测试；全量单元835.38秒。workerd为3文件/24项，Kefu为3文件/17项，五端构建与既有安全门禁通过；Gitleaks实际扫描257个历史提交，无泄露且未扩大例外。本机最终248文件/1,585通过及4项PG跳过的结果不替代上述Linux全量证据。

七条完整建库/升级路径均为PostgreSQL16.14，263表、3,700列、570约束、1,006索引、227序列：
...
| 旧默认ORM升级 | 1,068条初始SQL + 1条守卫DO = 1,069 |
| 旧缺失约束ORM升级 | 1,059条初始SQL + 1条守卫DO = 1,060 |
| 旧外键名称ORM升级 | 1,068条初始SQL + 1条守卫DO = 1,069 |

新增PG16完整旧FK路径通过141类漂移拒绝（约44,996ms完成拒绝及原始提案探针）、12组精确非法INSERT/UPDATE与NO ACTION父更新、11次RESTRICT父删除、1次实际CASCADE删除和2个可空引用。12个约束原OID、全部内部RI触发器完整记录及OID均保留，其他行/索引/文件/依赖不变；首RENAME故障回滚、重复执行包含新名称和内部/用户触发器身份的no-op、混合状态、临时同名表、非public父子关系及调用方search_path恢复均通过。九个长名称正确按63字节解析；十二组原始DROP/ADD均能执行，但实测替换各约束与其四个RI触发器身份，因此只在回滚夹具中验证，不用作实际升级。没有ADD/DROP/VALIDATE或业务行修正被引入该改名守卫。

七路径全部3,700列和1,006索引精确比较，180索引来源合同、3拥有约束、41项完整CHECK/FK合同及12项完整新FK名称/定义/状态合同通过，旧物理名和重复等价别名均被拒绝。全新ORM与索引/默认/缺失约束/FK名称四条升级路径的全部已比较目录一致。既有索引升级、四默认升级、41约束新增及外部重复索引退役证明，与70103a0的对应JSON逐字段完全相同；原六路径默认写入结果不变，第七路径与新建ORM相同。独立E2A生成器两库的完整CJS/ESM、旧坏行/新写入/NULL/级联/反向只读拒绝证明也逐字段不变。七座目录库和两座生成器夹具库均确认清理，不使用FORCE。

无截断的ORM_DDL_AUDIT实际11条（1摘要/10差异）。与70103a0/Actions34017020788 attempt 2的47条逐条深度比较，恰移除12条referenceOnly、12条candidateOnly及12条possibleRenames；其他10条原始记录全部完全相同，没有规范化名称、SQL、验证标志或数组顺序。外部↔内嵌五类全部零差异；外部↔ORM表/列/索引全部零差异，约束缺失/额外/别名归零，但仍9条同名变化及1条序列变化。剩余是8个CHECK的NOT VALID状态、1个outbox允许事件顺序及客服游客序列类型/拥有关系，根JSON继续明确`not_equivalent`。外部输入SHA256为`5b026028c730c5d1834f4999425f86e9b5bc4043bdf0b90591dc21a8847839c0`，模型生成SQL SHA256为`c9e02af32dd77130576e29d7bf66a6270986451edbc1ebda4313bbf27b5256b6`，均直接取自受测日志。

本批只将E3勾为代码/PG16完成；E4/E5/F/G、DB-009整体及业务/生产门禁继续开放。下一批E4已只读核对0013/0014/0017/0018/0019及0133原始CHECK和五模型，尚未实施业务升级。[PG16 ALTER TABLE](https://www.postgresql.org/docs/16/sql-altertable.html)不支持把现有CHECK直接ALTER为NOT VALID，不能承诺像本批FK改名一样保留目标约束OID，也不允许直接写系统目录来伪造验证状态。另一本机PGlite18内存小夹具实际拒绝此操作（0A000）和CHECK延迟属性修改（42809）；受事务保护的同义替换会换OID、保留行并继续拒绝新坏行，回滚恢复原OID。初始小探针误猜语法错误42601，已改为采集实际错误；这不是PG16业务升级完成证据，也未修改任何模型。

新增TEST-006工程容量风险：本轮Worker任务从07:58:53至08:17:47 UTC，用18分54秒，原20分钟上限仅余66秒；全量单元835.38秒，七路径目录步骤161秒。本次全部通过且未超时，但后续加完整路径前应审查独立任务拆分、重复初始化成本和资源配置，不能无依据放宽断言或子进程边界。TEST-005的既往原生allocator根因仍未定位；本轮不复现不等于修复。类型检查4,096MiB old-space只作用于两次编译器进程，不改变Worker运行时内存；原索引90秒、新独立约束/FK子进程180秒、默认测试并发、依赖和工作流均未改动。

窄结构审计仍201→263、共享201、源列缺口/外部独有/内嵌独有/列漂移全0；路由仍1,904 PHP/1,646 TS/879匹配/861可执行/18不可用/1,025缺失/17退役/1,008可执行缺口，46.2%/45.2%/45.6%不变。最后只回填根审计、清单和JSON，绑定受测代码及首轮CI；不回写历史来源清单或把文档提交当作受测SHA。PostgreSQL/Workers技能影响了锁等待、限定schema、RI协议及事务等待/无导入副作用的设计；本轮无生产连接、DDL/DML、provider调用或部署。生产应用仍须实际目录预检、维护计划与单独明确授权，ACCESS EXCLUSIVE不等于零锁上线。

## DB-009E4 九处CHECK状态与精确守卫替换（2026-09-06）

### 实施范围与首次失败

本批从92a4f5c文档提交及eebe827/Actions34020575416已验收代码继续，冻结清单orm-check-state-reconciliation.json绑定原始9条changed约束记录、五模型旧新声明、16个字段、原始外部/内嵌SQL及Drizzle快照。8个数值CHECK保留写入谓词并对齐NOT VALID状态，store_order_outbox.soob_event_type_ck仍validated=true，只调整同一9个允许事件的原始表达式顺序；不改变事件处理或允许集合。

外部0144与内嵌0150逐字一致、无导入I/O，仍是显式手工迁移注册。六张普通永久表按序ACCESS EXCLUSIVE锁定，2秒锁等待，检查列类型/default/NOT NULL/校对/identity/generated和继承状态。全部九项先预检再替换，允许精确旧已验证状态或精确目标状态，未知定义、错误验证状态、缺失、重复等价别名及额外依赖/安全标签拒绝。每列只允许自身表字段的automatic与normal两种依赖；检查注释捕获并恢复。仅同名DROP RESTRICT/原始ADD，不使用CASCADE、目录写入、业务DML或自动VALIDATE。

这批替换会改变九个目标CHECK的OID及其出向依赖的objid，不能套用E3外键RENAME保持目标身份的结论；其他约束字段、依赖、注释和非目标对象仍严格核对。八个NOT VALID ADD不扫描旧行，但outbox的已验证ADD会扫描并持锁至事务结束。2秒锁等待不是2秒总执行承诺，也不表示无锁在线升级；生产仍须实际目录/数据量预检、维护窗口、调用方语句超时及单独明确授权。本轮未连接生产或执行任何生产DDL/DML、provider调用和部署。

首提交8494360的[Actions34023806931](https://github.com/cinagroup/cinashop/actions/runs/34023806931) attempt 1为8成功/2失败：250文件、1,596单元全部通过（952.19秒、无跳过），但PG16目录步骤在09:11:54 UTC以“0144 replacement postcondition failed: division_apply.da_status_ck”退出1；原索引/默认/41约束/FK探针已运行，却没有最终ORM目录摘要，独立NOT VALID生成器步骤未执行。不得把该次视为完整目录或清理验收。新汇总检查在unit成功、catalog失败时实际失败，验证了失败不会被其他绿项掩盖。

根因是初版对8项验证状态替换错误要求内部conbin逐字一致。[PG16 StoreRelCheck](https://raw.githubusercontent.com/postgres/postgres/REL_16_STABLE/src/backend/catalog/heap.c)调用nodeToString，而[PG16序列化代码](https://raw.githubusercontent.com/postgres/postgres/REL_16_STABLE/src/backend/nodes/outfuncs.c)保留字符位置；[PG18普通序列化路径](https://raw.githubusercontent.com/postgres/postgres/REL_18_STABLE/src/backend/nodes/outfuncs.c)写为-1。CREATE TABLE与ALTER TABLE的字符偏移不同，本机PGlite18未暴露该差异。194be1c只排除内部树中有明确边界的整数:location字段，保留运算符/函数OID、列、常量类型/字节/NULL、cast及Boolean树结构；pg_get_constraintdef原始SQL、名称、验证状态和数组顺序仍逐字比较。真实SQL比较器另以7种语义变动验证不能被该例外隐藏。修正后新PG16结果恰好记录8个数值CHECK仅location改变，确认诊断和窄修正；不是重跑失败旧SHA碰绿。

### 受测提交、完整目录与行为证据

受测实现为194be1cb517efb4fc7db0a73d25fe460101c76a2，[Actions34024902767](https://github.com/cinagroup/cinashop/actions/runs/34024902767) attempt 1。最终10/10任务全部成功：双TypeScript、250文件/1,596单元全部通过且无跳过（646.96秒，含4项真实PG测试），workerd3文件/24项、五端构建及Kefu3文件/17项通过；Gitleaks扫描260个历史提交无泄露，例外未扩大。 本节取代受测设计文件的待验收描述，保留首次失败和局部预研历史；根JSON绑定受测代码而非随后文档提交。

PostgreSQL16.14八条完整建库/升级路径均为263表、3,700列、570约束、1,006索引、227序列：
...
| Worker内嵌 | 151步 |
| 全新ORM | 1,076条SQL |
| 旧索引ORM升级 | 1,078初始SQL + 原118升级语句 = 1,196 |
| 旧默认ORM升级 | 1,076初始SQL + 1守卫DO = 1,077 |
| 旧缺失约束ORM升级 | 1,067初始SQL + 1守卫DO = 1,068 |
| 旧外键名称ORM升级 | 1,076初始SQL + 1守卫DO = 1,077 |
| 旧CHECK状态ORM升级 | 1,068初始SQL + 1守卫DO = 1,069 |

新完整旧CHECK路径约38,235ms，通过55项结构/验证/定义/重复/依赖漂移拒绝；原始生成器9 DROP+9 ADD全部在回滚夹具中实际执行，证明确会换OID且丢注释，不能直接替代守卫。旧与目标状态共同验证58组非法INSERT/UPDATE、70组合法边界INSERT/UPDATE及32组逐列NOT NULL INSERT/UPDATE，覆盖全部16字段和9个事件值。八条已提交的非法旧行只构造在合法的NOT VALID状态中；守卫保留原行，新坏INSERT、坏值UPDATE与显式VALIDATE各8次拒绝，准确清理自建键后恢复空测试表。没有假设已验证约束能正常包含坏历史行。

实际对象捕获确认仅九个目标CHECK OID及其依赖objid改变，NULL/带引号/Unicode注释保留，非目标表/文件/索引/约束/触发器/函数/依赖和合成业务行不变。最后一项缺失先于第一个DROP被发现；首DROP及首ADD后注入故障均原子回滚，重复执行保留对齐后OID/注释，混合状态、临时同名表、非public schema及调用方search_path恢复均通过。PG16双连接实测锁争用以55P03在2,005ms拒绝，另一连接合法写入在迁移事务未结束时也被锁住，回滚后全状态恢复；不存在被并发写入利用的DROP/ADD空档。小夹具不证明生产吞吐或完整生产权限正确。

八路径全部3,700列、1,006索引和570约束类别现在精确零差异；180索引来源、3个拥有约束、41新增约束、12 FK名称及9 CHECK状态合同保留，所有旧名/退役名/重复别名禁回归，新建与各完整旧模型升级的已覆盖目录一致。相对eebe827，原索引/默认/缺失约束/FK四份升级证明仅initialStatements分别1070→1078、1068→1076、1059→1067、1068→1076（新增8条独立NOT VALID生成SQL）；其他字段逐条完全相同。原七路径默认写入证明不变，第八路径与全新ORM相同；外部重复索引退役和独立NOT VALID生成器两库的全部证明逐字段不变。八座完整目录库及两座生成器库均确认准确删除且不存在，不使用FORCE。

完整、无截断ORM_DDL_AUDIT为2条（1摘要/1差异），与eebe827/34020575416的11条深度比对，恰好移除本批9条changed CHECK，余下客服序列记录逐字段完全不变。外部↔内嵌五类零差异；外部↔ORM的表、列、约束、索引全部零差异，仅kefu_visitor_uid_seq的type/ownedBy不同。因此MIGRATION_SCHEMA_AUDIT.json仍为not_equivalent，而不是数据库迁移全部完成。外部输入SHA256为cb216b3cfe71066b50fa4683da397e30cc6e70225a92d74640f646c54ca5badb，完整生成SQL SHA256为00ea146c2254522ca7347a7983d424c6edaaca948d03191fac7aeb375b143e8a，直接取自受测日志。

本机首轮定向15项中两个引擎探针通过，仅来源窗口断言失败：冻结CHECK为ALTER TABLE行内精确子串，不含行尾分号，修为冻结行范围内精确包含而非整行相等。完整本机250文件、1,592通过/4专用PG跳过（1,596总数）496.14秒，最后初版定向15/15为125.29秒；字符位置修正后定向3文件15/15为128.78秒、双TypeScript再次通过，包含完整CJS/ESM网络拒绝子进程。它们不替代最终Linux全量及PG16证据。

### TEST-006容量拆分与剩余边界

原eebe827合并Worker任务18分54秒，20分钟上限仅余66秒，目录阶段161秒。本批把目录及独立生成器步骤放入独立PG16任务，单元/类型/结构/路由仍为另一任务，各自原20分钟上限不变。原12个执行步骤的内容保持不变，只重复4个固定setup步骤与专用PG服务；依赖版本、默认单元并发、旧索引90秒/其他180秒子进程边界、断言和测试项均不放宽。原检查名保留为always汇总，只有两个依赖均success才通过；runtime、五前端、安全扫描和触发条件不变。新增2项工作流合同测试，工作流从8扩为10项。

194be1c目录任务4分5秒，其中完整目录201秒、独立生成器3秒，距20分钟上限955秒；单元/类型任务12分30秒，余450秒，汇总2秒。新增PG服务和setup/清理成本已经包含在245秒目录总任务内。首提交8494360的同一拆分工作流中单元任务17分51秒（952.19秒单元），余129秒；最终单元646.96秒不应作为固定性能保证。采用两次观测的保守129秒余量，比原66秒增加63秒，虽新增了完整CHECK引擎测试仍保留全部执行项。本批将TEST-006标为容量拆分/汇总合同已验收，而非保证后续任意增长都能满足20分钟；E5条目继续要求加完整路径前复核容量。 TEST-005既往原生allocator错误仍未定位，本轮不复现不等于修复，不与此前已明确定位的TypeScript V8堆耗尽混为一谈。双类型检查继续显式4,096MiB old-space，不改变Worker运行时内存。后续增加完整引擎路径仍需复核单元阶段实际余量，不能依赖无证据加并发或持续加超时。

下一子批E5已只读核对安装版PgSequenceOptions、admin.ts:353及外部0104/内嵌0111：数值边界/步长/缓存/不循环一致，安装生成器模型未表达AS类型或OWNED BY。OWNED BY是删列/删表时自动删除序列的依赖，不等于owner角色；nextval/setval的取号状态不能套用普通行的事务回滚假设，见[PG16 ALTER SEQUENCE](https://www.postgresql.org/docs/16/sql-altersequence.html)及[序列函数](https://www.postgresql.org/docs/16/functions-sequence.html)。仅本机PGlite18.3内存小夹具观察到：AS integer保留OID、注释、last_value/is_called，却更换目标relfilenode；DDL回滚恢复原文件/类型/依赖，单独nextval的取号则不因回滚撤销；OWNED BY后删列自动带走序列且可回滚，上限耗尽报2200H。这不是PG16业务路径验收，也未修改E5模型、生成器或迁移。下一批需独立验证类型化生成、真实计数器/依赖/权限、边界/耗尽、并发/回滚，不重置序列、不在生产“试取号”。

本批勾选E4代码/PG16完成，DB-009E5/F/G、DB-009整体及业务/生产门禁继续开放。权限、完整函数/触发器/RLS/视图仍不属于完整目录比较器，夹具保留证明不能替代它们的全量审计。窄结构及HTTP合同未改：201→263且源列缺口为0；路由1,904 PHP/1,646 TS/879匹配/861可执行/18不可用/1,025缺失/17退役/1,008可执行缺口，46.2%/45.2%/45.6%不变。PostgreSQL与Workers技能影响了原始SQL/验证语义、限定schema、短锁等待、依赖与事务回滚、无导入副作用镜像；未发布当前主Worker或任何正式前端。 最终根审计/清单/JSON回填后，文档相关3文件/21项回归通过（2.79秒），差异检查通过；清单269个带编号项只关闭E4与TEST-006，其他完成状态不变。

## FE-002A：生产PC匿名浏览器审计（2026-09-06）

用户确认浏览器安装并授权后，本轮使用前端测试技能要求的页面身份、非空、错误层、console、截图和真实交互证据链。当前实际连接的是CUA的Codex应用内浏览器；没有声称Chrome扩展已连接，没有安装依赖或使用独立Playwright进程。入口为`https://cinashop-pc.pages.dev/`，页面标题`CinaShop - PC 商城`。本轮代码基线为`1b7339f`，工作区原有11项序列生成器/测试相关改动保持原样，未把它们当作通过验证的发布候选。仅补充本节及checklist，不执行生产数据库命令、数据清理或部署，也没有提交登录、短信、加入购物车、收藏、订单或支付；普通页面访问仍可能产生服务自身的日志/缓存等副作用，未宣称生产数据库字节级不变。

| 检查 | 实际结果 | 边界 |
|---|---|---|
| 页面身份/非空/框架错误层 | 首页、搜索、详情及登录页均有目标页面内容，未见框架错误层 | 不等于字段、数据或完整业务正确 |
| 搜索交互 | 输入“测试商品A”后回车和搜索按钮均到达`/search?keyword=…`并显示24项结果；公开列表响应200 | 首次AX setValue未形成有效搜索，换用真实输入事件后成功，不记为应用搜索故障 |
| 详情导航 | 搜索首项进入`/goods/70`；接口200、价格99.90/销量250/库存100可见，但标题为空 | 详情合同失败，见B |
| 未登录访问 | 点击购物车后到达`/login?redirect=/cart`，显示账号/手机号登录入口 | 未输入凭据、未验证真实账户和服务端授权 |
| Console/网络 | 采集的console error/warn为空；详情页观测的公开API响应均200 | 200和空console不能发现字段缺失；不等于全站无错误 |
| 截图/窄屏 | 首页损坏图片、详情空标题/画廊、390×844搜索导航裁切均已截屏展示 | 窄屏实际`innerWidth=390/clientWidth=375/scrollWidth=608`；检查后重置窗口覆盖，恢复时为933×792，未留修改后的窗口尺寸 |

**FE-002B，确认的字段合同缺口。** 网络捕获的`GET /api/product/detail/70`返回顶层`data.storeName='测试商品A'`、`data.storeInfo`和`data.sliderImage=[]`，没有snake_case对应字段。线上详情标题节点为空，价格仍显示。仓库`workers-ts/src/services/product/StoreProductService.ts`的`getProductDetail`构建同样的camelCase字段；`view/pc-ts/src/api/product.ts`直接将解包响应作为`GoodsDetail`，`types/product.ts`与`pages/goods/GoodsDetail.vue`却要求`store_name/store_info/slider_image`。这给出API→解包→模板的完整错配证据，不应只等候发布当前代码。后续修复需同时保留明确的价格、购买限制和两类响应形状，不能以泛型断言替代运行时映射/验证。样本的真实轮播数组本身为空，即使修正字段也不会产生运营图片。

**FE-002C，素材与失败态。** 首页8张商品图的`currentSrc`都指向`https://via.placeholder.com/300`，`complete=true/naturalWidth=0`，logo正常。`Home.vue`和`GoodsSearch.vue`只在URL为空时取本地占位图，非空但不可加载的URL没有失败态处理。还观察到首页两种名称重复出现，但搜索接口实际返回24项、24个不同ID、全部名为“测试商品A”；因此只将其归为新系统运营数据需要认领/整理的证据，不指控Vue重复渲染，也不授权任何删除或合并。真实素材、SKU和商品配置继续由DATA-006验收。

**FE-002D，PC窄屏缺口。** 390×844截图中导航文本挤成纵列，搜索区和部分入口超出可见宽度；DOM宽度实测608大于375。保留桌面站是否需要完整窄屏支持的产品边界，修复或明确引导方案后再复验；没有测试H5/小程序/App，不能将此结论扩展至其他端。

线上静态制品观测为`assets/index-ClV6Erfd.js`、`assets/GoodsDetail-MVHNGXbu.js`，只作为本次所见制品标识，不从文件名推定部署Git SHA。本轮未重新核验Cloudflare部署版本或数据库目录；之前`194be1c`/Actions`34024902767`的数据库/CI证据、剩余序列差异及`not_equivalent`判定保持原义。新增checklist只关闭FE-002A的匿名发现范围，B/C/D和FE-002、DATA-006及全部真实业务/发布门禁仍未完成。

## DB-009E5A：类型化序列生成前置（候选，2026-09-06）

本批补充显式序列类型和拥有列的本地装饰器，限定drizzle-kit0.31.10 CLI/CJS/ESM三个校验和固定入口，保留之前排序和NOT VALID补丁的完整合同。生成器持久化`smallint/integer/bigint`及`OWNED BY/NONE`，使用严格JSON传输而不依赖分号分割；数值字符串按BigInt验证，五个数值选项共20种不安全Number输入在转换为字符串前拒绝。22种非法快照/破坏性序列变更拒绝、实际CLI新建/增量/导出/无变化、两个API四序列/两schema的目录/编号/原OID/注释/旧行/回滚/依赖删除/不循环/倒序默认/耗尽及只读introspection边界均有回归。完整设计与限制见`workers-ts/audit/pg-sequence-generator.md`。

验证过程没有削弱旧门禁：修正了新ownership语句插入点对旧排序块完整性的破坏，保留旧NOT VALID代码窗口；源码恢复使用校验过的完整变体缓存，每次输入仍做完整hash和逐字节比较。新增源码回归集中到单项时在5,361ms超过默认5秒，改为CLI/CJS/ESM独立测试保留全部断言和同一默认上限，未改旧测试时限、CI默认并发、扫描例外或依赖。最终本机定向4文件18项通过（98.48秒），全量251文件/1,599通过/4项PG跳过（共1,603；522.26秒），双TypeScript通过。新审计入口以禁网预加载实际拒绝非测试地址，exit=1且networkAttempts=0；临时报告已删除。

本批仍是前置候选，Linux CI待验证，不能勾选E5A。CI新增独立序列夹具步骤，要求精确专用loopback PG16身份、自建两库及确认清理；原八条完整业务路径和两座NOT VALID夹具继续运行。完整业务模型、外部/内嵌迁移均未改变；`MIGRATION_SCHEMA_AUDIT.json`继续保留194be1c/34024902767的已验证结果及剩余序列差异，不提前更新证据。E5B仍须完整旧ORM升级、当前计数器/所有依赖/ACL/锁与并发取号的真实PG16守卫验证。本轮无生产连接/写入/部署，也没有完成新增PC页面缺口。

## DB-009E5A：最终Linux/PG16验收（2026-09-06）

受测实现为`ce5805aae8adfc03e3b3daf05ab789b4791f6b56`，[Actions34028634339](https://github.com/cinagroup/cinashop/actions/runs/34028634339) attempt 1最终10/10成功，没有重跑失败任务。本节取代前述E5A候选段及受测设计`workers-ts/audit/pg-sequence-generator.md`中的待验证状态，保留最初设计与本机失败修正过程；根JSON绑定此受测实现，不绑定随后文档提交。

Linux全量251文件/1,603单元全部通过、无跳过，耗时954.90秒；三份真实PG测试文件均通过，包含4项由专用测试URL启用的PostgreSQL16.14并发断言。新增序列文件7项通过，双TypeScript检查成功，workerd为3文件/24项（15.08秒），Kefu为3文件/17项（784ms），五端构建通过。Gitleaks扫描263个历史提交，无泄露，扫描范围与例外未扩大。

独立目录任务完整输出2条ORM_DDL_AUDIT记录（1摘要、1序列差异）、1份NOT_VALID_GENERATOR_AUDIT及1份SEQUENCE_GENERATOR_AUDIT。与194be1c/34024902767的两条原始记录整份深度比较完全相等，包括全部目录摘要/旧证明；旧NOT VALID生成器报告也整份完全相等。原始SQL、字段、状态、名称及数组顺序没有新增规范化。八路径仍均263表/3,700列/570约束/1,006索引/227序列，外部146步、内嵌151步及六条ORM路径步骤数全部不变；外部输入SHA-256仍为`cb216b3cfe71066b50fa4683da397e30cc6e70225a92d74640f646c54ca5badb`，生成SQL仍为`00ea146c2254522ca7347a7983d424c6edaaca948d03191fac7aeb375b143e8a`。本批没有消除原始业务差异，之前E4消除9 CHECK的证据明确保留为历史。

新增PG16.14序列夹具对实际CJS/ESM各执行13条新建、9条升级，覆盖public与seq_tenant的4条序列、含引号/分号标识符和精确大整数。每格式22类非法状态与20种不安全Number输入均拒绝；快照往返、新建/升级目录相等、当前编号/原OID/注释/旧行保持、DDL及拥有依赖删除回滚、NONE/NO CYCLE、倒序类型默认值、nextval不随事务回滚、耗尽2200H、定制introspection只读拒绝及普通SERIAL/IDENTITY/无主bigint兼容均为true。原八座业务库、两座NOT VALID夹具库与新两座序列夹具库共12库确认清理。夹具结果独立记录在根JSON的sequenceGeneratorVerification，不能计作第九条完整业务升级路径，也不额外计入单元测试数量。

时长按GitHub任务开始/结束时间计算：单元/类型任务17分53秒（1,073秒）距20分钟上限余127秒；独立目录任务3分58秒（238秒）余962秒，其中完整目录190秒、旧生成器3秒、新序列生成器4秒，全部setup/清理成本已包含；汇总3秒。相对已有1,071/750秒单元任务观测，最新保守余量从129降为127秒。TEST-006的拆分/失败汇总合同保持通过，但E5B加完整路径前必须再做容量审查；此次较慢运行不足以单独归因为新7项测试，更不据此提高并发或延长上限。TEST-005原生allocator根因仍未确定，未复现不等于修复。

本批只勾选DB-009E5A。客服游客业务模型仍为bigint/无拥有关系，而迁移目标为integer/OWNED BY public.kefu_visitor_session.visitor_uid；E5B尚未实现精确旧状态守卫及完整PG16业务升级验证。DB-009E5/E/F/G、DB-009整体、真实角色/provider/运营数据和生产发布门禁全部保持开放，`status=not_equivalent`不变；PC详情字段、损坏图片与窄屏缺口也未因本批而完成。本轮无生产连接、业务写入、数据库升级或部署。

最终根审计/清单/JSON回填后，文档相关3文件/21项回归通过（2.50秒），差异检查通过。276个带编号清单项的集合与顺序保持不变，只有E5A从待完成改为完成；根JSON既有33个非CI/来源字段逐字段保留，新增生成器证明不覆盖E4历史差异或首次失败记录。

## TEST-006B：完整序列升级前的容量修正（候选，2026-09-06）

上一轮ce5805a已推送并验收，属于实质进展。本轮复核主分支93c0604干净；E5B仍未实现，先处理新完整路径的127秒时长余量。采用[Vitest原生分片](https://vitest.dev/guide/improving-performance.html#sharding)与[GitHub矩阵](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/run-job-variations)，两个独立Linux机器分别执行1/2、2/2，each保留PG16服务和原测试隔离/进程默认并发/时限。非单元检查仅第一片执行，目录任务保持完整；原required-check名依赖整个矩阵和目录任务，fail-fast=false保留另一个分片诊断。不增加每进程并发、不调长超时、不跳过用例、不改变库版本或扫描例外。

实测固定Vitest4.1.10的`list --filesOnly --shard=1/2`仍返回251文件，源码显示该分支在进入执行池/原生sequencer之前返回。故新增验证器使用真实createVitest配置、globTestSpecifications与同一个BaseSequencer.shard，确认126/125完整且不重叠；并在每片实际测试后读取JSON报告，逐文件精确匹配所选集合，逐断言拒绝skipped/pending/todo/failed且核对总数。合成报告只用于14项拒绝测试，不能作为实际运行证据。3项容量合同本机通过（658ms），验证器输出完整清单与分片摘要，不记录测试内容或环境变量。Linux全量、两片真实覆盖和最长任务余量尚待同一SHA验证，本项不提前勾选，也不更新已验收的根JSON或关闭E5B。

初次类型检查发现测试夹具的Record<string,string>不满足仓库扩展后的ProcessEnv，已改用既有测试的复制后严格白名单清除方式，仍不传递业务环境变量或凭据；修正后双TypeScript通过、最后容量专项3/3（669ms），相关文档/容量4文件24项通过（2.54秒）。未以类型断言或放宽全局环境类型绕过检查。

## TEST-006B：最终分片验收与E5B本机候选（2026-09-06）

受测实现`94816da2c02c9e4d54b60528c10714ad6c3eb525`的[Actions34030550652](https://github.com/cinagroup/cinashop/actions/runs/34030550652) attempt 1最终11/11全部成功，没有重跑。两片实际JSON覆盖证明分别为126文件/810项、125文件/794项，合计251文件/1,604项、0跳过，真实4项PG16测试未丢失；共同清单SHA-256为b2cd4c8a3426426ba18838bfb345295605ac71d867b7cfccd3e4e3aad04ce72d，两个执行集合摘要不同、并集完整且互斥。新增的1项单元为原生分片与14类报告拒绝合同；两片测试时长549.96/413.88秒，不能把963.84秒相加当并行整体耗时。双TypeScript、workerd3文件24项（15.28秒）、五端构建、Kefu3文件17项（845ms）通过；Gitleaks扫描265个历史提交无泄露。

按任务起止时间，两片总任务671/447秒，最长11分11秒，在原20分钟上限下余529秒；较ce5805a的127秒增加402秒。完整目录任务255秒、余945秒，原required-check汇总3秒。新配置仅有这一轮完整观测，未承诺以后恒定余量；每任务默认并发、隔离、版本、断言和原子进程边界保留。此前真实目录失败导致汇总失败的证据仍有效；本轮没有故意制造GitHub矩阵失败，不把合成拒绝测试称为远端失败注入。

完整2条ORM目录记录、NOT VALID报告及序列生成器报告与ce5805a逐字段深度相等；八路径步骤数、所有旧证明、SQL输入/生成摘要及12库清理均不变。根JSON改绑94816da/本轮CI，新增实际两片报告及容量前后对比；仍保留E4与E5A阶段的原始历史证明。仅关闭TEST-006B，E5B、DB-009及业务/生产门禁不变。

同时已在本机编写未提交的0145序列升级SQL和完整旧ORM CJS/ESM探针，尚未改业务模型、MigrationService登记或正式PG16九路径审计。精确旧bigint/无拥有关系或目标integer/指定拥有列之外均拒绝；先锁会话表，再以同schema操作取得序列ACCESS EXCLUSIVE锁，拒绝已启用DDL事件触发器、非READ COMMITTED、RLS/继承、未知依赖/角色/默认/唯一性和不安全现有编号。技能中的短事务与锁顺序要求落实为2秒锁等待、限定schema、先检查后变更及完整回滚；真正的PG16双连接锁/并发执行仍待完成，不以PGlite18替代。

本机最终两格式各执行1,076条旧模型建库SQL+1守卫，保留2条已提交旧行、原OID/ACL/角色/注释/编号及所有捕获的非目标对象；30类拒绝、AS/OWNED BY后故障回滚、重复执行不改存储、临时影子/带引号非public schema、未调用首值、默认新写入、上界/耗尽、删拥有列及回滚通过。固定禁网预加载下CJS39,529ms、ESM38,936ms，TCP/UDP/监听尝试0，临时报告清理；明确输出modelAligned=false、lockVerification=null，未纳入上述94816da的CI通过范围。

预研首轮断言错误把AS重写等同于清零log_cnt；[PG16 init_params](https://raw.githubusercontent.com/postgres/postgres/REL_16_STABLE/src/backend/commands/sequence.c)表明此处显式边界不变、只改AS/OWNED BY时保留该值。本机实际31也保持31，因此收紧迁移后置条件为log_cnt原值不变，而非放宽断言。另修正测试预期的seqtypid JSON字符串/数值表现，仍精确要求int4 OID 23且其余字段不变。生成器原始两条建议包含冗余选项，不直接作为带数据升级执行。三份候选文件继续留待模型/内嵌/来源绑定、真实PG16双连接和完整路径接入验收；本轮无生产连接、写入或部署。

根报告回填后，4文件24项文档/容量回归通过（2.53秒），差异检查通过；既有35个非来源/CI字段逐字段保留，清单编号集合与顺序不变，只有TEST-006B关闭。此次文档提交不包含上述三份未提交的E5B候选文件，也不触发生产部署。
...
所有九路径保留全部旧索引、列、约束及默认写入合同，并增加全部227个序列的精确名称/类型/选项/拥有关系比较。遗漏、额外对象、重复身份或拥有关系别名、部分对齐均不豁免；新的升级路径还与新建ORM的五类原始目录全量比较。当前本机4文件23项定向测试通过：完整序列升级/目录2文件13项92.35秒，原生分片/序列生成器2文件10项19.01秒；双TypeScript通过。随后新增纯SQL镜像后的3项来源/守卫合同通过，2项重型用例仅在这次筛选运行中未执行（此前完整运行已通过）；可观测性合同仍有6个既有发布阻断。本批没有修改版本、默认并发、既有断言或CI/子进程时限。

本节为待CI候选，不更新根JSON中94816da的已验收证据，不提前把剩余1条差异改成0；E5B/E5/DB-009继续开放。下一步为精确实现SHA的全量Linux两分片、第九路径/双连接证据及旧证明增量比较，之后补完内嵌登记验收。没有生产DDL/DML、提供商调用或部署。

## DB-009E5B：九路径/真实锁候选验证结果（2026-09-06）

实现提交`f3cf196ba9826bf3ef7207ebc190574f5c3ddc2f`已推送；[Actions34032529050](https://github.com/cinagroup/cinashop/actions/runs/34032529050) attempt 1最终11/11通过，没有重跑。两分片真实报告各126文件、810/799项，合计252文件1,609项、0跳过；共同完整清单SHA-256为4ee612b63a212f93e3673d4f0f6423e683eff930b8620a8734f7e673c622305d，第二片包含本批5项新测试。两片测试550.96/514.22秒，任务总时长668/545秒，余532/655秒；目录任务260秒余940秒，汇总2秒。与上一轮671/447秒观测合并后，容量仍以保守529秒衡量，不把一次较快运行当性能保证。双TypeScript、workerd3文件24项（14.56秒）、五端构建、Kefu3文件17项（834ms）及267个历史提交无泄露均通过。状态读取曾有一次EOF，随后只读跟踪恢复，不涉及测试重跑。

真实PostgreSQL 16.14客服序列探针26,611ms：完整旧模型1,076条建库SQL加1守卫，2条已提交旧行、原OID/角色/ACL/注释/当前号码及log_cnt保留；30类拒绝、AS/OWNED BY后故障回滚、无动作重跑不改存储、临时/带引号schema隔离、未调用首值、新默认取号、上界/耗尽和拥有列删除回滚均通过。双连接在2,007ms后按2秒lock_timeout拒绝升级；升级持锁时另一连接的nextval、setval与会话INSERT全部被拒绝。原连接取号即使回滚也不回收，DDL回滚保留原状态；提交升级后并发16次取号互不重复且正确前进。此行为与[PG16序列函数说明](https://www.postgresql.org/docs/16/functions-sequence.html)一致，未对生产编号执行重置。

九路径步骤依次为external147、embedded151、orm1077、orm_upgrade1197、orm_default_upgrade1078、orm_constraints1069、orm_fk_names1078、orm_checks1070、orm_sequences1077；每条路径都为263表/3,700列/570约束/1,006索引/227序列，外部与内嵌/ORM的五类原始差异全为0。原两条输出记录（summary+1序列差异）变为1条summary；没有归一化名称、类型、校验状态或拥有关系来豁免差异。旧五组升级证明只有初始建库SQL数各+1，其余字段严格相等；新增归属语句导致生成摘要变化，外部0145导致输入摘要和外部步数+1。原八条默认写入证明不变，第九条相同；NOT VALID及独立序列生成器两份报告与94816da逐字段相等。9座完整业务测试库加2座NOT VALID库和2座序列夹具库共13库全部清理。
...
## DB-009E5B：独立执行入口真实PG16验收（2026-09-06）

提交`c9de50997c3a898376ce392de5ba52ecf55590f6`的[Actions34034710427](https://github.com/cinagroup/cinashop/actions/runs/34034710427) attempt 1最终11/11通过，无CI重跑。两片127/126文件、821/799项，合计253文件1,620项、0跳过；共同清单摘要为f8b5bdf4c431d9a050bab0f9c45cae848882859465f519e6ecdcd331b1cf6a05，实际逐文件/逐断言覆盖检查通过，第二片文件集合与f3cf196完全相同。测试586.27/501.95秒、任务710/539秒，20分钟上限余490/661秒；目录262秒余938秒，汇总2秒，五端构建/workerd/密钥扫描均通过。分片后三次观测中的保守余量由529秒更新为490秒，未提高上限、并发或减少断言；TEST-005原生分配器根因仍未解决。

第一片新增`kefu-sequence-runner.test.ts`的11项全部实际运行，31,309ms完成。独立入口使用正式Drizzle postgres-js适配器完成1,076条旧ORM建库SQL后的提交升级及no-op；原30类拒绝、2行旧数据、OID/角色/ACL/注释/计数器及非目标对象保护、故障回滚/边界/耗尽/新默认写入均通过，原双连接锁探针2,008ms拒绝且并发16个号码无重复。专用500ms语句时限测试527ms完成，等待另一个连接的表锁时实际收到57014，回滚后结构/号码/会话设置复原；三组时限、隔离级别、嵌套事务拒绝及设置/ALTER后错误均通过。额外随机专用库已删除并查询确认不存在；这不操作共享CI库public，更未连接生产。

九路径目录报告与f3cf196逐字段比较只有两处变化：kefuSequenceUpgradeVerification新增`committedUpgradeExecution=direct-sql`说明旧目录路径仍直接执行守卫，以及锁等待观测2007→2006ms。九路径、步骤数、全部原始结构/默认写入/旧升级合同、输入及生成SQL摘要均未变；NOT VALID与独立序列生成器两份报告整份深度相等。独立客服报告耗时26,611→26,568ms也是观测值，不作语义变化。目录/生成器13库和新执行器1库全部清理。保留旧目录路径与新增真实Drizzle路径的区别，不把direct-sql标签改成已接入内嵌登记。

根JSON仅追加sequenceExecutionBoundaryVerification，绑定本批SHA、11项入口证明、全部CI与容量证据；原f3cf196目录及所有历史字段逐值保留。294个清单编号/顺序/勾选状态不变；内嵌0151登记、整合验收、E5B/E5/DB-009、业务和生产门禁继续开放，未连接或修改生产、未发布Worker/Pages、未调用提供商。最终本地复验2文件15项通过/1项PG16跳过（161.84秒），这项已由本批真实CI执行通过；不把本机跳过写成通过。下一步只将已验证的独立执行器登记到新建库流程并验证执行边界，不为现存数据库重放runAll。

## DB-009E5B：内嵌0151登记候选（2026-09-06）

完整复核MigrationService后，保留0000～0150所有历史SQL、排序、0118→0119取代规则及旧错误处理；只在末尾登记0151，复用与外部0145逐字一致的纯SQL常量。新分支直接把根Drizzle连接交给已验收runKefuSequenceAlignment，在通用事务之外等待独立提交，避免事务内再开事务成为savepoint；失败沿用0115起的fail-closed分支，既不记成功，也不重试或按already exists跳过。SQL核验方法只返回字符串、不执行升级。现有受operations权限与调试门禁保护的路由未改动，不新增自动执行或生产入口；runAll包含历史数据补写，不作为现存数据库的单项升级工具。

新增9项编排测试覆盖152个精确有序登记、SQL镜像、根连接/无外层事务、最终异常与旧现代步骤失败、历史兼容跳过和0118取代。首轮测试钩子误将mockReset返回的mock函数当作清理函数，修成显式void后，新9项及既有18项企业微信测试全部通过（3.11秒），两套TypeScript检查通过；这不是运行时守卫失败。未mock的真实新建库runAll继续由既有PG16目录任务执行，预期内嵌步骤151→152；须等待同一实现提交全量CI及九路径报告增量比较后验收。根JSON和清单完成状态暂不更新，无生产连接、DDL/DML、提供商操作或部署。

追加本地3文件回归24通过/1项PG16专用跳过（86.05秒），跳过项待真实Linux PG16运行，未当作通过。按Git工作区换行差异统一CRLF/LF后，AST提取的151个历史migration方法正文与55c055b全部逐字相同，既有方法只有runAll改变；冻结SQL、模型和来源清单未变。差异检查通过。

### DB-009F后续稳定门禁审计

本轮只读复核发现，orm-ddl-audit对全部路径的列/索引/约束/序列已逐类别assert，对新建ORM与各升级ORM则比较五类并抛错；但externalVsEmbedded和externalVsOrm的tables变化只进入摘要与差异输出，没有相同的拒绝条件。若外部表的持久性或RLS开关与所有ORM路径一致偏离，而其余四类相同，当前任务可以输出差异后成功退出。合成比较器诊断仅翻转一行table.rowSecurity，实际只得到tables.changed，其他四类全部为空；这是代码/比较器证据，不冒充真实引擎或生产复现。本轮不在已推送受测SHA上追加代码，F继续未勾选并记录明确下一步：全部路径完整表目录硬门禁，以及逐字段/增删/重复身份负例。RLS开关检查也不等同于完整策略、权限、函数、视图、触发器等价。

## DB-009E5B：内嵌登记最终验收（2026-09-06）

实现提交`7d14b887378a850cd6e9d38b3b37c2916dfbfb80`的[Actions34036234197](https://github.com/cinagroup/cinashop/actions/runs/34036234197) attempt 1最终11/11通过，无CI重跑。两片各127文件，824/805项，总计254文件1,629项、0跳过；逐文件/逐断言核对使用共同清单摘要7469c147ffbdcfd7254aae6a50b93113b0d125d239d0de4a2048b7eec2b2789d，两片文件摘要分别bdb5d363dff0abaf8a72c6f92419c85a7beb7b1d28df419852b968a1d882fbf8、c0693adae2f28d44672d5376ead2681db13419c35e6b6c09ba7158953001c37d。本轮新增文件使真实原生分区重新分配，不能沿用旧分片的文件集或测试数。

0151编排9项82ms通过，独立执行器11项31,163ms通过，其中PG16现有500ms语句时限取消被阻塞DO的测试527ms，实际断言57014、回滚及会话状态恢复。执行器完整旧模型报告与c9de509仅锁等待观测2008→2007ms；1,076条旧ORM建库SQL后的真实Drizzle提交/no-op、30类漂移拒绝、2行旧数据/原编号、OID/角色/ACL/注释和非目标对象保留、故障回滚、边界/耗尽与16个并发唯一号码全部保持通过；独立随机库删除并查询不存在。

PG16目录任务实际通过未mock的MigrationService.runAll：external147、embedded152、orm1077、orm_upgrade1197、orm_default_upgrade1078、orm_constraints1069、orm_fk_names1078、orm_checks1070、orm_sequences1077步。每条路径均263表/3,700列/570约束/1,006索引/227序列；五类原始差异全部0，无别名/类型/拥有关系归一化豁免。相对c9de509整份目录摘要只变三字段：embedded.steps从151到152、旧CHECK锁拒绝2005→2006ms、客服锁等待2006→2007ms；其他路径/原保护证明/原始输入和生成SQL摘要严格相同。独立客服报告另有耗时26,568→29,962ms；NOT VALID和序列生成器报告整份深度相等。9座目录库、2座NOT VALID库、2座序列库及额外1座执行器库共14库均确认清理，未操作共享CI库public或生产。

两片测试580.95/507.24秒，任务697/539秒（20分钟上限余503/661秒）；目录任务285秒余915秒，汇总3秒，五端构建、workerd和密钥扫描均通过。分片后四轮仍采用历史最小490秒余量，原并发/隔离/断言/版本/时限不变；本次较快不构成任意增长保证，TEST-005原生allocator根因仍开放。窄结构复审仍201→263、共享201、源列缺口/外部独有/内嵌独有/列漂移全部0；没有增加HTTP行为覆盖或进行浏览器业务验收。

根JSON仅新增sequenceEmbeddedRegistrationVerification，以本批SHA/CI明确取代两个历史字段中的“待登记”状态；原根f3cf196目录、c9de509独立入口和全部历史字段原样保留，不重绑历史结果或覆盖463记录冻结基线。E5B/E5/E仅标为代码与隔离PG16 CI完成；F仍保留已发现表级失败门禁缺口，G、DB-009整体及所有业务/生产发布门禁不关闭。清单顶部的八路径/旧序列差异/旧CI数字同步纠正，生产版本只标已记录且本轮未重查，避免把历史观测写成当前线上事实。本轮未连接生产、未执行生产DDL/DML、未调用提供商或发布Worker/前端。

最终回填核验：去掉唯一新增的sequenceEmbeddedRegistrationVerification后，根JSON与提交前逐字段深度相等；新增字段与读取的完整CI证据对象精确一致。294个清单编号及顺序保持，只将E、E5、E5B由未完成改为代码/PG16 CI完成，DB-009/F/G/TEST-005仍未勾选。4文件23项来源范围、目录、容量和登记回归通过（4.00秒），差异检查通过。文档回填仅涉及三份根审计文件，不触发生产部署。

## DB-009F：表目录失败门禁候选（2026-09-06）

按[PG16表目录字段定义](https://www.postgresql.org/docs/16/catalog-pg-class.html)，新增完整表门禁：全部九条路径都须与外部路径的263个唯一表身份及8个原始字段精确一致，包含name/key、kind、persistence、rowSecurity、forceRowSecurity、partition和partitionKey。额外/缺失/重复身份、两侧共同收缩、读者两侧同时漏字段、额外字段及隐式类型转换都拒绝；不规范化分区表达式或豁免改名，不修改既有catalog SQL、迁移SQL、模型或历史基线。失败沿现有CLI非零退出路径传播，原四类完整门禁、所有来源合同及升级保护验证继续执行。报告mode从发现模式改为五类精确门禁，仍明确不覆盖完整权限/策略/函数/触发器/视图。

使用[ALTER TABLE](https://www.postgresql.org/docs/16/sql-altertable.html)在自建空测试库创建263张轻量表（与完整项目九路径分开计数），实际验证启用RLS、强制RLS、UNLOGGED、普通/分区表类型、分区键、分区成员、改名、缺失和额外表9类拒绝，正反比较方向都须抛错；其中前6类必须证明其余四类目录完全不变，防止被其他门禁偶然拦截而掩盖缺口。每例在短事务内操作并回滚，随后全五类目录必须复原；同名临时表不能遮蔽public表。测试库仍只能由专用loopback/finance_test/PG16协调连接创建，精确随机名验证、只清理成功创建者、关闭自身连接并查询确认删除；额外夹具不冒充第十条完整项目路径。PostgreSQL技能影响了RLS开关与完整策略的区别、真实DDL/回滚验证及短事务边界，没有把技能示例用来启用生产RLS。

新增13项本地测试（8字段逐项、身份集合、读者字段形状、原始表达式/顺序、完整引擎夹具及全路径接入），与既有目录测试合计2文件21项通过（4.78秒）。本地PGlite只能作为辅助，须由同一实现SHA的真实PG16完整CI复验后才勾选F；现有JSON的7d14b88登记验收和所有历史字段暂不更新。本轮没有生产连接、DDL/DML、提供商操作或部署。
...
## FE-002B：浏览器授权后的详情合同候选修复（2026-09-06）

本批按用户最新浏览器授权，优先推进已记录的PC页面缺口；DB-009G的外键索引测量没有执行，不将其标为完成。CUA应用内浏览器可用，正式`https://cinashop-pc.pages.dev/goods/70`仍复现空标题和正常99.90价格；公开GET读回确认camelCase标题/简介/轮播图/原价/会员价字段，且没有cart_button。没有提交登录、短信、购物车、订单或支付，没有数据库命令及生产部署。

新增PC专用详情适配器，API先解包unknown再做显式字段映射，模板继续消费固定snake_case视图模型；不修改Worker返回合同及其他端。兼容旧字段、保留空值/0/false的明确状态和十进制金额字符串，不以泛型断言掩盖错配；无效价格/ID/库存拒绝，缺失或不合法购买状态保守禁用。现有服务缺少cart_button时，仅对明确普通、非预售、无表单且在售有库存的商品推导允许；特殊/预售/表单商品、明确禁用、无库存和下架/删除始终禁用。没有把适配器当作服务端授权或完整交易校验。

新增10项合同回归，其中直接调用实际StoreProductService.getProductDetail，DAO/cache/保障查询隔离，验证非缓存、缓存及多规格价格输出；另覆盖旧视图形状、空/零/false、三类限制、坏数据、图片数组与API接入。新旧两文件16项通过（5.19秒），Worker单元TypeScript通过，PC类型检查与生产构建通过（1829模块，最后构建7.80秒）；VueUse既有两处PURE注释构建告警仍存在。没有变更依赖、工作流、数据库模型/迁移或冻结目录证据，本地结果不冒充新SHA的Linux CI结果。

前端测试技能要求实际渲染与交互，因此启动仅loopback的5218只读模拟API（不导入数据库、不转发请求、非GET拒绝），5219本地PC仅代理到该夹具。CUA检查URL/title、非空DOM、无框架错误层、console error/warn为空；默认窗口显示标题、简介、99.90/199.00/79.90金额，数量从1变2；另一个预售样本的加入购物车/立即购买均禁用，刷新后保持。截图已在任务中展示。390×844检查失败：clientWidth375、scrollWidth791，固定400px画廊将商品信息推到屏幕外，继续归FE-002D；临时窗口覆盖随后恢复。轮播夹具为空，没有以假素材冒充运营数据。

额外只读源代码检查发现：GoodsDetail.addToCart仍按商品ID拼接sku标识，不选择接口attr_value中的真实unique；buyNow仍显示接入中提示并转为加入购物车，而非真正直接结算。本批没有触发生产写流程，也没有修复或验收这两个交易缺口；FE-002整体及完整订单生命周期继续开放。B仍等待精确提交的Linux与发布后复验，C/D和所有真实角色/provider/发布门禁保持未完成。

## FE-002B：精确提交Linux复核（2026-09-07）

`9308448d89087853332e4f427f502e892802d281`的[Actions34039794059](https://github.com/cinagroup/cinashop/actions/runs/34039794059) attempt 1最终11/11成功，未重跑。两片各128文件，837/815项合计256文件1652项、0跳过；共同清单摘要e9525f289cf9ba134b2737021e1528bcc866c18ca6f4b3ce9e00cb4ed8821ef5，执行文件摘要bcf530f2a7fe62a3eb33db2923607f1b866cf2a2e6b070c56a33bd28c8579d5e、3efc67bd2be836b6e9ce1cd62173ab9a7c2d08e95415da890d08ce37a396e858。新增详情10项在第二片13ms通过；双类型、五端构建、workerd、目录任务及密钥扫描通过。两片任务557/549秒；本节只绑定本次实际读取的结果，不覆盖旧数据库目录JSON或声称发布已完成。

## FE-002D：窄屏导航和详情本地候选（2026-09-07）

修复前再次在本地390×844复现clientWidth375/scrollWidth791，搜索不可见。现有DefaultLayout在768px以下同时隐藏导航和搜索；GoodsDetail保持横向400px画廊。本批将导航、搜索和账号区显式排为响应式网格，1100px以下导航独占一行，600px以下搜索也独占一行；导航允许换行但不删除任何八个既有目的地。搜索回车与按钮统一通过一个有可访问名称的原生表单提交，Logo改为键盘可访问链接。详情900px以下纵向排列，画廊等比例且不超过400px，价格/按钮/评价允许换行，搜索长词与商品网格可收缩；搭配购弹窗宽度收敛到视口，但弹窗业务未通过本轮实际登录验收。

使用前端测试技能执行真实渲染闭环，而非把CSS结构检查当像素证明。仅启动自建loopback5218 GET模拟API和5219本地PC，Vite明确只代理到该夹具，夹具无数据库/外部转发且拒绝非GET及未列明路径。公开商品70保持空图，72只加入标明LOCAL TEST的两张本地SVG几何夹具；这不是运营图片补齐，也不是FE-002C验收。没有访问生产商城、提交认证/短信/购物车/订单/支付、数据库命令或生产部署。

CUA应用内浏览器实际验证详情在320/390/768/1024/1280×844下clientWidth与scrollWidth分别精确同为305/375/753/1009/1265，头部可见链接/输入/按钮左右边界均在视口内。画廊及轮播高度分别约242.67/312.67/400/400/400px，有图夹具均完成加载；点击第二个实际轮播控件后活动图确认为LOCAL TEST 2，数量1→2。390px搜索回车、按钮、无结果→非空结果恢复、搜索→详情、分类→列表及320px登录入口通过；320px连续192字符无空格关键词仍无横向溢出，1280px按钮搜索回归通过。URL/title、非空内容及无错误层检查通过，最终采集console error/warn为空；不把仅模拟页面导航当真实后台搜索正确性或真实认证证明。
...
## FE-002E：真实规格与独立立即购买候选（2026-09-07）

从main@672f9e2继续；上一轮窄屏提交、Linux证据和审计回填属于实际进展。本批读完整StoreCartService、OrderController和OrderDaos后确认：add接收isNew却无条件findExisting，DAO未限定isNew，故普通加购和立即购买会相互合并；v1列表也混合读取两种记录。本批立即购买总是新建且保留本次数量，普通合并仅匹配isNew=0/status=1。新增显式scope=cart与scope=buy&ids=读取，后者最多100个唯一正整数ID，按当前uid、未支付、未删除和isNew=1取行，并对缺失、外用户、普通车、状态/库存/规格失效整组拒绝，保持请求顺序。普通PC列表/角标显式使用cart范围；未传scope的旧v1合同暂时保留，其他端的读取迁移没有因此完成。

PC详情适配实际attr_value，保留真实unique、组合名称、各规格金额字符串和库存，不再拼接SKU。商品详情提供规格选择，价格/原价/会员价及数量上限随选择更新，无SKU/缺货/限制商品不可购买；提交前再次校验选择与数量，进行中锁定规格、数量和购买按钮。未登录先保存所选SKU/数量到同站返回路径，登录成功后恢复，但不自动提交购买。立即购买传new=1，验证返回ID后进入独立确认页；确认页解析非法/空/重复/超量ID时失败关闭，通过专用读取加载目标行，不回落到普通已选商品。普通车刷新保留有效勾选；订单重试使用当前确认页稳定key并加重复提交守卫，但本批没有执行建单/支付来证明完整幂等业务。

同一确认页的拼团调用补new=1以符合新读取边界；其原先硬编码sku00001仍未解决，不能据此声称拼团完成。套餐/拼团实际规格与交易、确认页服务端完整报价（运费/优惠/会员）、会话过期时保留完整查询参数、真实账号、跨请求并发与生产发布仍待继续。特别是普通add的读-合并-写并未在本批改为串行事务；浏览器按钮锁不等于服务端并发安全。

按Workers技能读取最新官方最佳实践和5.20260906.1类型（独立临时目录），并核对项目Wrangler schema；未升级项目依赖/兼容日期或改变绑定。按PostgreSQL技能检查owner/ID过滤，现有模型含sc_uid_new(uid,isNew)，没有新增索引/DDL，也没有性能计划测量结论。按前端测试技能启动仅127.0.0.1的5218隔离PGlite购物车服务与5219本地PC，登录为公开local-qa合成账号，真实购物车控制器/Service/DAO只操作内存表；订单创建、支付、短信和provider路径拒绝，未连接生产。夹具初始化曾因图片超256字符和SKU超char(8)失败，修正为本地短图片路径和8字符合成SKU后才开始验收，未改模型约束。

新增9项DAO/Service/HTTP隔离回归与5项PC选择/入口合同回归；原详情回归补实际服务SKU映射断言。12文件82项在本地通过（5.79秒），Worker双类型检查通过，PC类型/生产构建通过（1830模块；既有两处VueUse PURE告警保留）。临时恢复旧的无条件合并入口时，核心测试明确失败：期望3个独立ID、实际仅1个；随后恢复修复并跑全批通过，不把该单用例红测中的8个过滤跳过计入通过数。隔离辅助表生成器验证真实SQL/行级行为，但不提供完整生产约束目录或并发PG16证据；仍待本次提交Linux核验。

CUA浏览器验证：红色普通加购2件保留在普通车，蓝色立即购买2件仅在确认页显示蓝色/29.9/2/59.80；普通车勾选后确认红色/2/39.80未丢失。非法参数及把普通车ID冒充立即购买均显示错误、清单为空且提交禁用。模拟库存错误后仍停留原规格，按钮从锁定恢复，再试仅生成一次成功记录。总计5次模拟加购请求（含1次明确失败）得到4条内存记录：普通红2、直接蓝2、直接重试1、另一次直接蓝2；没有创建订单。390px详情/确认页375=375，1280px确认页1265=1265；辅助技术库存上限在切换规格后8→2，输入99在失焦后限为2，缺货规格禁用。URL/title、非空DOM、无框架错误层和最终console error/warn为空；截图保留在任务中。恢复视口、关闭自建标签并停止两服务，5218/5219复查无监听。

FE-002E保持未勾选，其真实交易、完整报价、活动规格、并发及发布验证不能用本批局部闭环替代。未改变任何其他清单完成状态、根MIGRATION_SCHEMA_AUDIT.json或冻结ORM目录基线；DB-009G继续开放。

### FE-002E：精确提交Linux复核（2026-09-07）

`eb1a39da91ef7171672f16836e2f7e7662220d96`的[Actions34074786807](https://github.com/cinagroup/cinashop/actions/runs/34074786807) attempt 1最终11/11成功，未重跑。两片实际130/129文件、853/819项，共259文件1672项、0跳过；共同清单摘要8276434cebb913e35847305d11ed5b4dbd468a1b1df2c5c8f6126fa17c53d507，执行文件摘要分别fbe984e63524557aff6b9cbae43f0be3fbf696bf46b1eaea16235a2515bdf50a、6afe91bade28683229a8bc03093af44785a7ef913d3f3b241bb4c0908c749a1c。两片实际JSON逐文件/逐断言校验完整、不重叠且无跳过；新增购物车9项在第二片423ms通过，该CI环境使用专用loopback PostgreSQL 16，辅助器另校验数据库名、角色和主版本，不能回落到生产。新增PC选择5项在同片14ms通过，详情合同10项在第一片14ms通过。

两片任务724/556秒，距原20分钟上限余476/644秒；把当前容量审查的保守余量从历史490秒更新为476秒，不把正常波动解释为本批代码的性能因果。隔离目录任务263秒、PC构建36秒、汇总4秒；双类型、五端构建、workerd及密钥扫描通过。本次只核验目录任务成功，没有重新下载逐字段比较完整目录工件，也没有DB-009G索引测量。TEST-005原生allocator根因仍未解决；没有改变时限、并发、隔离、依赖或工作流。E标为普通商品候选通过Linux，因上述剩余业务门禁仍不勾选；全部347个清单勾选状态保持不变。根数据库审计JSON、冻结ORM目录基线、模型和迁移均不变，生产未连接或发布。

### FE-002E：完整报价与会话返回的下一步证据（仅审计，未实施）

等待eb1a39d的Linux检查期间，读完整LegacyOrderCompatibilityService、PC订单API与请求层，再核对Checkout及订单控制器。现有POST `/order/confirm`已调用checkoutPreview→StoreOrderCreateService.quoteOrder，返回cartInfo、orderKey和priceGroup，后者包含服务端应付、运费、运费减免、会员/等级、优惠券、积分与首单优惠；确认key按uid隔离，KV有效期30分钟。POST `/order/computed/:key`可用同一key和更新后的地址/配送/优惠选项重算。下一批应先接入这些既有合同，不另造前端报价公式；当前没有调用这两个接口或写入确认KV。

当前PC Checkout仍将购物车小计求和后仅减首单优惠；请求首单优惠失败时静默清空优惠，地址/配送选择变化也不触发完整报价。loadCheckout在地址、门店、表单与报价请求完成前已解除checkoutLoading，后续异步写入没有沿用前段的generation校验，故还需验证慢响应、切换路由和失败重试不会展示旧报价/表单或提前允许提交。PC建单参数目前发送拼接地址而不发送addressId/cityId，下一批须与报价采用同一地址及配送选项，并验证确认金额和建单结果一致；不能以本轮合成购物车总额当作完整应付验收。以上是代码缺口，未执行真实建单来推断实际收费差异。

PC请求层在410000/410001/410002时只将window.location.pathname写入登录返回地址，会丢失SKU、数量及立即购买确认ID等查询参数；这与本批已验收的主动登录返回不是同一条路径。还需保留完整同站路径并验证会话过期后的返回、非法返回地址拒绝及并发失效响应，不能把公开local-qa夹具登录成功外推为真实会话过期恢复已完成。本节不改变任何清单勾选状态。

## FE-002E：完整服务端报价与失败关闭候选（2026-09-07）

从main@1aa6d14继续。本批不再由PC购物车金额减首单优惠推算应付，改用既有orderConfirm/orderComputed→checkoutPreview→quoteOrder。confirm补useIntegral，computed兼容保留全部旧平铺金额并附同次cartInfo/addressInfo/orderKey。前端显式验证金额十进制字符串、积分整数、完整唯一商品集合、数量/SKU/type/isNew、地址与key；无效数据不回退为0。商品原价、优惠单价和费用明细均来自同次预览，避免旧购物车价格与新总额混显。

地址、配送和积分变动立即清空旧报价，generation与选项指纹拒绝晚到成功/失败，报价或配送错误时显示原因并禁止提交。地址/门店/表单加载及路由切换沿用整页generation，初次Promise.all完成前保持加载；保存地址期间报价失效。首次建单冻结接受报价的key、addressId、配送和表单/备注，后续重试保持相同内容，不传客户端总额或payType；成功后的购物车刷新失败不再把已成功建单误报为失败。优惠券选择当前仍固定0；提交结果不确定后保持输入锁定，不能靠本批浏览器锁宣称服务端幂等/并发完成。

新增2文件13项：PC适配/异步/接线8项，真实controller/service/SQL报价5项；后者经financePostgres身份守卫使用本地PGlite或CI专用loopback PG16，不连接生产。5项验证会员/首单/运费、积分与地址重算、自提、非本人/失效确认key以及价格变化/删除地址失败关闭，并比较业务快照无订单/流水/库存/积分变动；只允许合成KV确认缓存写入，TTL1800。测试身份为注入的合成UID，不等同真实JWT；夹具14表不替代完整约束目录。最终相关15文件109项全部通过（9.93秒），包括次卡product/validity；较早命令中不存在的second-card-migration过滤名不计为实际测试文件。Worker单元/运行时双类型检查通过，PC类型与构建1831模块通过（8.62秒），保留既有VueUse PURE告警。跨项目类型导入改为相对systemForm路径以避免Worker别名错误，无依赖、模型、DDL、绑定或工作流改动。

按前端测试技能用CUA现有应用内浏览器验收127.0.0.1:5219，代理仅指向5218的PGlite夹具。默认甲地址19.20→使用积分18.70→乙地址21.70→自提15.70；每次待报价时金额隐藏且提交禁用。模拟乙地址computed失败后只显示错误，手动重取恢复；甲→乙→甲在约1.05秒内完成，旧乙响应延迟1.6秒，最终仍显示甲与18.70。390/1280×844的clientWidth=scrollWidth分别375/1265；手机最初表格商品名逐字竖排，按实际截图改为窄屏卡片后名称/SKU/金额可读。页面身份、非空内容、无框架错误层、最终console error/warn为空均通过，截图在任务内展示。

延迟2.5秒的系统表单完成后显示并可填写，捕获式create两次请求的key/body完全相同，携带addressId=11、cartIds=[2]及填写的表单/备注，失败后输入锁定并显示重试提示。捕获端点始终返回模拟失败，未挂真实create处理器；订单/流水仍空、两商品及SKU库存仍8、积分仍100。没有观察到慢表单加载瞬间的浏览器状态，因此仅记录已渲染与源码守卫，不声称该时序浏览器验收完成。实际建单/库存占用/优惠券/支付/provider完全未调用，不能把捕获请求视为实际计费一致性证据。
...
### FE-002E：报价合同修正精确 Linux 验收（2026-09-07）

修正提交`478ba799d2870c086cf7ec59f5a3f16cf6b55225`的[Actions34078783081](https://github.com/cinagroup/cinashop/actions/runs/34078783081) attempt1最终11/11成功，无重跑。单元两片合计261文件1,685项、零跳过：第一片131文件860项（测试467.85秒、任务583秒），第二片130文件825项（测试500.50秒、任务536秒）。相对20分钟任务上限分别余617/664秒；不能根据本次时长推断原生allocator根因已解决，容量保守历史余量476秒仍保留。

两片原生执行清单与JSON覆盖核验均通过且完整、互斥；共同inventorySha256为`d5c17334965734a7338dd80fe5a4b2f046418bbe59bb888c8307a6f1bdd53658`，第一片executedFileSha256为`2f7cbca31d759603edc333399b8f63c3a3f1409e0aaf4651f2359f1873ce6bfb`，第二片为`a4e1351993894752a76fa86dc488ab04c79ad39ea1a7c5ca475531f1692ebebd`。新增报价8项27ms、真实专用PG16报价5项583ms、修正的newcomer6项9ms均在第二片通过。目录任务283秒、PC40秒，其他四端构建、workerd、密钥扫描及最终汇总均成功。上方f982683首轮失败记录不删改；本节只关闭该报价修正候选的精确Linux待验状态，不关闭FE-002E、TEST-005或发布门禁。

### FE-002E：表单预校验、上传守卫与明确拒绝纠错（2026-09-07，本地候选）

本批将既有后端纯表单规范化器移动到`view/common/order-system-form.ts`，PC预校验与Worker共用10类组件、必填标记、文本子类型、选项、范围和容量限制。服务端继续加载权威模板并检查附件归属，客户端标题/选项/必填标记不取得授权作用。原Worker导出保留ValidateException兼容层；只有权威表单验证在建单事务提交前拒绝、且createOrder尚未返回时，控制器才返回`data.errorCode=ORDER_FORM_REJECTED`及本次orderKey。SQL/网络失败、普通业务异常和建单之后的付款错误都不能冒充明确拒绝。

PC请求层保留机器可读错误数据；仅首次确定拒绝且key匹配时允许保留表单就地编辑并重新报价。先前有任何未知结果时，即便后续收到表单拒绝也保持同key/同payload重试锁；不自动发起付款。SystemFormFields按组件记录上传所有权，上传时禁止提交，同字段不能并发上传；卸载/换表单使旧ticket失效，迟到结果不能写入新表单或清除新上传状态，追加图片采用最新值。Element控件本身也显式禁用，不只依赖原生fieldset。

新增36项纯逻辑/接线回归和5项真实SQL/控制器回归；最终相关14文件117项本地通过（9.28秒），Worker unit/runtime两套类型检查通过，PC构建1834模块9.57秒通过。SQL使用隔离PGlite、可在CI的专用PG16执行，只有Sequence DO编号替换为本地生成；坏邮箱、停用模板、无归属附件均经真实createWithRuntime事务拒绝，首单资格、库存、购物车、积分、订单/表单/快照未残留。修正后同key真实创建一笔未付测试单：18.70元、库存8→6、积分100→50、表单与商品快照各1条；再次重试返回相同订单，无重复扣减。付款后错误分类的独立用例使用建单/付款stub，不冒称真实支付证据。没有连接生产数据库。

CUA应用内浏览器在本地`127.0.0.1:5219/checkout?mode=buy&cartIds=2`验证：慢表单加载期间禁交、必填邮箱/选项及非法邮箱禁交；首次捕获式明确拒绝可编辑，改邮箱后模拟未知结果锁定，第三次明确拒绝仍保持锁定。服务端捕获3次相同key，第二/三次payload完全相同，第一/二次邮箱确实不同；浏览器夹具订单/账单为0、库存8/8、积分100不变。390×844与1280×844下clientWidth/scrollWidth分别375/375和1265/1265，无框架错误层；刷新恢复必填拦截，截图在任务中展示。

另通过文件选择器将仓库公开logo.png（65161字节）发给本地临时接收端，两次均实际触发上传等待及禁交；第一次成功出现`/api/assets/901`、提交恢复并可移除，第二次模拟失败后等待消失、提交恢复且图片数0。夹具只捕获元数据，不存文件、不写R2/附件/订单。失败场景有1条预期console error，来源已核实为Element Plus的use-handlers.mjs:34在已处理上传错误时调用console.error；不是无故白屏或未处理Promise。此前纠错流程console error/warn为0。文件选择工具单次约256/301秒，未再执行第三次迟到上传浏览器场景；跨字段/跨生命周期迟到响应仅有自动化状态机回归，不能说已做浏览器实测。

本机`npm run test:runtime`未执行任何测试：Windows workerd启动发生`0xc0000005`访问冲突、3个启动错误，并伴随沙箱Wrangler日志路径EPERM；退出1。不能用类型检查或上一提交Linux绿色替代本批运行时验收，本批仍待精确SHA完整Linux。没有改依赖、DDL、冻结JSON、工作流或生产配置，也未发布。FE-002E保持开放：真实R2/角色/支付、活动及并发、硬刷新/离页后未知订单意图恢复、会话完整返回、优惠券合同/选择及发布后验证仍未完成。

### FE-002E：表单候选精确 Linux 验收（2026-09-07）

表单提交`f1d3ec48d6314b07816710f78a921334c76544d8`的[Actions34081360728](https://github.com/cinagroup/cinashop/actions/runs/34081360728) attempt1最终11/11成功，无重跑。两片合计263文件1,726项单元、零跳过；第一片132文件896项（测试594.98秒、任务716秒、余484秒），第二片131文件830项（测试530.02秒、任务568秒、余632秒）。本次容量仍未低于此前保守476秒余量；不由速度波动推断TEST-005原生根因已修复。

两片JSON覆盖证明完整且互斥、实际执行清单与原生划分一致。共同inventorySha256=`701c1427172a95b73373804eec64a0ed76694bc9672e64d488ed213726cd632a`；第一片executedFileSha256=`11dd2a627fb91d80aaccb238393d55ba94a4159f1c4e76875f612dbf2531708e`，第二片=`f93bbdca8eac4ca647b5bd9ba0c5c8f040b514f2763112df301998ee37019029`。新增表单36项在第一片25ms通过，5项真实事务回归在第二片专用PG16中660ms通过。workerd任务43秒通过，补足本提交Windows启动失败未执行的运行时验收；本机失败记录仍保留。目录279秒、PC34秒，其余四端、密钥扫描和汇总均成功。本节只验收表单候选的完整Linux，不关闭真实交易/R2/活动/并发、FE-002E或发布门禁。

### FE-002E：登录失效返回与客户端会话隔离（2026-09-07，本地候选）

审计发现：PC请求层失效时只编码pathname，丢失立即购买/活动query和hash；没有核对响应所属token，旧请求可清除新登录态；clearAuth未同步Pinia/购物车，旧成功响应可回填旧用户数据；logout等待服务端时也可能在新登录之后清除新身份。路由守卫还漏了退款与物流入口，且startsWith会误保护同前缀非子路径。商品详情购买生成登录返回地址时保留SKU/数量但漏hash。

本批引入每标签页、每次身份变化递增的会话版本，token与展示UID原子写入sessionStorage并继续清除旧localStorage。Axios用WeakMap绑定真实请求配置与当时的版本/token；迟到的旧成功/失败响应一律不交付给新会话，只有仍匹配的有token失效响应才能清理状态。HTTP200业务信封和HTTP错误中的失效信封均处理，纯网络错误不推断失效。应用启动在请求/认证store循环之外绑定Pinia同步；清理或换身份同时清空购物车，旧列表/计数及finally不能覆盖新会话状态。迟到logout返回标识不会触发新会话清理或首页跳转；本机退出与服务端撤销未确认仍明确区分。

统一同源返回路径规范化：保留pathname/search/hash，拒绝外部URL、协议相对路径、反斜杠/控制字符及其编码、login循环；登录页返回目标随当前query变化，OAuth的一次性存储返回也继续经过同一检查。退款/物流加入分段匹配的登录守卫，购买入口补route.hash。未引入token持久化、客户端角色授权或新的后端认证方式。

验证分层：Worker侧相关16文件134项通过（9.86秒），新增导航3项覆盖完整参数和负向URL，另1项核对导航栏及个人中心两个退出入口均正确处理当前会话/撤销状态；PC新增Node测试用现有Vite加载实际Axios、auth工具和Pinia stores，10项覆盖清理同步、旧token失效、相同token新版本、旧成功购物车/角标、迟到logout、撤销失败、HTTP/网络差异及匿名请求竞态，零跳过。该10项已接入PC build的test:auth前置步骤，不把它们混入Worker分片计数。没有新增依赖或改工作流，PC最终类型/构建1836模块12.05秒通过；Worker双类型通过。最初跨目标类型检查发现纯导航函数引用DOM Location，已改为窄结构类型；最后新增文件断言的Worker/Node URL类型冲突改为显式NodeURL导入，随后双类型和134项重新通过；测试加载器的无关依赖扫描关闭时日志已通过禁止自动发现消除，不屏蔽断言或错误输出。

CUA本地应用内浏览器验证：`/checkout?mode=buy&cartIds=1&type=0#details`匿名守卫→expired-qa模拟失效→fresh-qa重登完整返回，显示同一规格/2件/19.20元并可提交。随后fresh会话在登录页的配置请求延迟20秒返回410002，期间new-qa完成登录；服务端记录旧失效已返回、新会话quote已完成，浏览器仍位于同一结算URL、保持登录可提交。退出后`/refund/local-order?source=qa#details`及`/express?order_id=local-order#tracking`均转登录且保留完整返回路径。商品详情`/goods/70?campaign=qa#buy`将数量改2、匿名点立即购买→登录后回到`/goods/70?campaign=qa&sku=qared001&qty=2#buy`，保持已选SKU/数量，不自动加购/建单。390×844与1280×844下分别375/375、1265/1265无横向溢出，页面非空、无框架遮罩、console error/warn为0，截图在任务中展示；本轮只用合成身份和隔离夹具，没有真实账号/订单/支付，订单及账单0、库存8、积分100不变。

最终调用点复查发现个人中心也调用logout，已同步修正其旧布尔返回值用法，避免对象恒真误报撤销成功或迟到退出跳走新会话。增加offline-qa合成场景，在个人中心实际点击“退出登录”，模拟HTTP503后DOM明确显示“本机已退出，但服务器会话撤销未确认”，UID变0、导航切换登录并返回首页，console error/warn为0；不将夹具身份撤销失败算作真实服务端验证。页面/夹具验收后已关闭本轮标签页并停止两个本地服务，不影响MySQL/3306隧道。

本批会话候选仍待精确SHA完整Linux与正式环境复验；真实扫码/短信/OAuth回调、硬刷新/离页后的未知建单意图恢复、优惠券合同/选择、活动和服务端并发仍开放。不把sessionStorage的页面刷新保持等同服务端会话撤销，不把完整URL恢复等同未知订单key/payload已持久恢复。主Worker、Pages和生产数据库本轮均未变更。

### FE-002E：会话候选精确 Linux 验收（2026-09-07）

会话提交`a4974b7900130b19101a176d97006c091da7a791`的[Actions34083722664](https://github.com/cinagroup/cinashop/actions/runs/34083722664) attempt1最终11/11成功，无重跑。两片共264文件1,730项Worker单元、零跳过；第一片132文件893项（测试417.03秒、任务521秒、余679秒），第二片132文件837项（测试495.69秒、任务526秒、余674秒）。容量审查继续保留历史保守476秒余量，不能由速度波动推断TEST-005原生allocator根因已解决。

两片nativePartitionCompleteAndDisjoint与executedFilesMatchNativePartition均为true；共同inventorySha256=`fa1217a61ddd4e90c57b77ed1e40565da0eedb07e8e1420d69562ca98fd82a72`，第一片executedFileSha256=`d66c44e9b2f5ba16854ae5e415ef1132f2dfc9d1716e0981f895f56424ed5c75`，第二片=`3f7ac465d89a335e23a2610392e9f5c8a6c6bcf8b375e38f5972dac191d2edd6`。新增导航合同4项在第一片4ms通过；另读取PC实际日志确认Axios/Pinia的10项test:auth全通过、0失败/0跳过，不混入Worker分片数量。PC任务39秒、workerd36秒、目录280秒，其他四端构建、密钥扫描及汇总均成功。本节取代会话候选的待精确Linux状态，不关闭完整交易、真实身份/provider或发布门禁。

### FE-002E：优惠券钱包与结算选择（2026-09-07，本地候选）

本批确认PC此前将couponId固定0；钱包按路径0/1/2切换，但控制器只读取query status，页面读取snake_case而接口输出camelCase原始模型，且缺少订单占用状态3。优惠含义不能按字段同名硬套：TS发行模板couponType对应PHP适用范围type（0通用/1品类/2商品/3品牌），TS模板type对应PHP优惠coupon_type（1现金/2折扣）。折扣85表示支付85%即8.5折；显示按现有服务端计算器截取整数百分比，85.99亦显示8.5折，不把它渲染为85.99元。

新增只读UserCouponWalletService，始终绑定当前uid，保留camelCase消费者并添加PC明确字段、状态、有效期及范围类别。未使用列表排除过期/失效模板，未来生效券显示但禁选；使用/占用状态不因到期变回可用。0/1/2/3分别为未使用、已使用、过期/失效、未付订单占用；显式query status继续覆盖路径，以保留现有UniApp调用。结果维持旧data数组，新增X-Coupon-Next-Cursor响应头；PC按倒序ID游标翻页，limit默认20/最大100，显式旧page最大1000且拒绝混用游标。响应private/no-store，未新增缓存/DDL。按PostgreSQL技能核对现有uid/status与uid/issue索引，只收紧归属、查询范围和分页，不声称已测生产执行计划或优化所有连接成本。

PC钱包和结算共用严格适配器/卡片，现金/折扣、门槛、有效期、状态分开展示；切换状态立即清空旧页，迟到成功/失败不得污染新页，分页失败保留既有行及重试游标。普通结算展示“未使用候选券”，明确不是商品预筛出的可用券；选券/取消/刷新会作废原报价，再将couponId交既有confirm/computed完整服务端报价。范围、门槛、所有权及优惠金额仍由StoreOrderCreateService判断，客户端不计算总额；首单/活动不叠加时显示未抵扣说明。接受报价后couponId随既有key/options冻结，未知结果后禁改券并用同一payload重试，不增加自动付款。

新增16项适配/状态机/接线和5项真实SQL回归，共2文件21项；SQL夹具仅使用受守卫的本地PGlite或CI专用loopback PG16，验证本人/他人、四状态、游标、别名、现金/折扣/取消、门槛/范围/失效拒绝与首单互斥。真实订单控制器用本地Sequence编号替身创建1笔未付16.00元测试单、将券41占用；同key重试没有重复订单或资源扣减。报价失败/成功快照均无业务写入。该自动化建单与下述浏览器只捕获请求是两个独立夹具，不混淆订单数。

相关20文件168项曾在本地全通过（13.58秒），Worker unit/runtime双类型检查通过，PC test:auth实际10项及vue-tsc/生产构建通过（1840模块、11.11秒；两处既有VueUse PURE注释告警保留）。最初测试对象的可选查询字段导致类型报错，已将参数类型修正为允许undefined并重跑双类型通过。最终源码收尾复跑20文件时，旧pc-direct-purchase-cart的beforeAll隔离数据库初始化超过默认10秒，19文件159项通过、9项跳过，退出1；不算完整通过，也没有业务断言失败或原生崩溃证据。结束已完成的浏览器夹具后，单独复核该文件9项通过（6.79秒）；未改超时、并发、依赖或测试，资源快照不足以归因该次波动。随后仅作一次完整20文件复核，168项全部通过、0跳过（12.23秒），未改代码、断言、超时或并发配置；不将重跑通过冒称初始化波动根因已修复。本机workerd仍在启动阶段0xc0000005访问冲突，伴随Wrangler沙箱日志EPERM、3个启动错误，0项测试、退出1；保留失败，不以父提交Linux绿色代替本批精确SHA验收。

按前端测试技能使用CUA应用内浏览器、本地5219→5218隔离代理验收：`/checkout?mode=buy&cartIds=1&type=0`基价21.00；其他商品专用券和未达20元适用金额门槛均显示服务端错误并禁交；现金券5元→16.00，8.5折→18.30，取消→21.00。会员商品金额18.00参与门槛，不按原价20.00误判。未知提交后选券/取消禁用；两次捕获请求key/body完全相同且couponId均为41，浏览器夹具订单/流水0、库存8、积分100未变，没有真实建单或付款。钱包验证使用券单独展示、过期列表首次模拟失败显示错误/重试而非空态、重试后出现过期/失效券、占用券单列。390×844和1280×844下clientWidth=scrollWidth分别375和1265，页面身份/非空DOM/无框架错误层通过，最终console error/warn为空；手机冻结提交、钱包占用和桌面钱包截图已展示在任务中。验收后恢复视口、关闭自建标签，仅按完整进程命令校验停止本轮两个服务，5218/5219无监听，未操作MySQL/3306。

仍有独立迁移缺口：PHP `GET /coupons/order/:price`（旧route/api.php:275、PC order_confirm.vue的cartId/new调用）按当前购物车预筛券的接口尚未迁移；不能用上述钱包候选列表替代。旧type/issue_type筛选、具体商品/分类/品牌范围和层级标签、UniApp折扣展示及分页/结算完整合同尚未收口。未知建单意图硬刷新/离页恢复、服务端并发、活动规格与真实角色/provider仍开放，新增FE-002E-C未完成子项显式跟踪，不勾选父项。
...
### FE-002E-C：订单筛券后端精确 Linux 验收（2026-09-07）

`d33bccd3d93fbb7182bf72500a66a02641a059c3`的[Actions34088007196](https://github.com/cinagroup/cinashop/actions/runs/34088007196) attempt1最终11/11成功。两片267文件1,774项、零跳过；第一片134文件932项（测试346.05秒、任务440秒、余760秒），第二片133文件842项（测试402.32秒、任务448秒、余752秒）。共同inventorySha256=`d0ed1e8b5f57454b1db9ccde18379533fdfb70d6c8fc7148fbbf5a8ebb5ff24b`；第一片executedFileSha256=`5098ca206b22b1dcef92e729a4cfe1c9cda88ac7bb375ebec7759ee8a90645f7`，第二片=`9fcd877f63675d93e24e0cb69104e9a1bfda23be9475047f1b890d13fe56aa10`。两片完整互斥且实际文件符合原生划分；新增筛券23项在第一片专用PG16中1,009ms通过。

workerd40秒、目录272秒、PC36秒及其他四端/密钥扫描/汇总通过。本节取代上一后端增量的待Linux状态，不抹去Windows启动失败，也不将较快CI视为TEST-005根因已修复；容量审查仍保留历史保守476秒。没有运行生产发布。

### FE-002E-C：PC订单筛券与范围隔离（2026-09-07，本地候选）

PC确认页现调用`/coupons/order/0`，只发送当前已验证cartId/new、配送、门店及limit20/before，不发送客户端商品总额或用“我的优惠券”列表代替预筛。保留独立钱包页面和原钱包严格游标合同；新`orderCoupons.ts`适配最后扫描候选游标，允许结果为空但仍可继续，拒绝重复/逆序/不前进、坏金额及非可用状态。商品数量、SKU、购买方式、目标行或配送/门店改变会作废旧券页和已选券；迟到成功/失败不能恢复旧范围。分页失败保留既有行与同一游标，提供明确重试；未确认提交暂停在途结果并冻结当前选择，不把尚未扫描完的空列表显示为最终无券。

券卡新增服务端适用商品金额/预计抵扣；选券与取消继续走完整quote，不用估计值计算总应付。刷新时即便已选ID原本为0也刷新完整报价，避免资格变化后只有券列表更新。提交仍冻结同一报价key/完整payload，禁止未知结果后换券；没有自动付款、生产测试账号或新增后端写接口。

新增26项纯合同/状态机/接线与5项实际控制器/SQL回归，合计31项，最终专项2文件31项通过；相关18文件184项最终通过、零跳过（12.29秒），Worker两套类型通过，PC实际Axios/Pinia10项与vue-tsc/生产构建通过（1841模块、8.70秒，保留两处既有VueUse PURE注释告警）。早期测试类型检查发现实际购物车服务声明unknown[]、测试直接quote错误传addressId，以及it.each把非法范围数组展开成参数。最后一项会让粗糙toThrow只验证到TypeError；已包装具名items对象并断言具体业务错误，逐项verbose复核9种真正非法输入，未将初轮129项结果当最终证明。unknown边界由已有CartItem合同接入真实运行时范围校验，去除直接quote不接受的addressId；不更改生产模型/参数类型以迁就夹具。

按前端测试技能，用现有CUA应用内浏览器验收本地5229→5228隔离代理、合成登录、真实筛券及报价服务；旧Browser技能未列出，未安装/切换独立Playwright。初始化一次ACL故障后重连同一标签恢复；普通购物车Playwright setChecked没有改变状态，改用同一CUA可见复选框点击并观察选中/金额/按钮变化，不通过脚本改应用状态。页面URL/标题、非空DOM、无框架错误层和console error/warn为空通过。1280×844与390×844下clientWidth/scrollWidth分别1265/1265、375/375；桌面折扣18.30与手机现金16.00截图已连续展示。

实际交互为：24个门槛不足样本使首20候选成为空适用页，继续查找首次模拟失败显示错误，重试仍before104并返回现金/折扣两券；会员适用商品18.00，现金5→16.00、8.5折→18.30、取消→21.00。未选券时开启隔离首单配置并刷新，券集完整为空且总价同步19.20。旧配送筛券响应延迟20秒期间改自提并加载当前券页，旧响应返回后两张当前券仍保留；纯状态机另覆盖迟到失败。直接购买行3的一件商品重新筛得适用金额9.00、预计1.35且不再有满10现金券；普通购物车行2经可见勾选→结算实发new=0，立即购买行1为new=1。手机端未知提交后选券/取消/刷新均禁用，两次捕获key及body完全相同、couponId均41；fixture订单/账单0、库存8、积分100未变，未调用真实建单或支付。验收后视口重置、标签7关闭，核对命令后停止本轮夹具/前端进程，5228/5229无监听，未操作MySQL/3306。

### 新增开放审计：UniApp不能仅替换筛券接口

`view/uniapp-ts/src/pages/order/confirm.vue:267/274/294`仍基于展示商品价在客户端扣局部优惠产生totalPay，没有完整服务端费用报价；折扣85经/10后封顶1会算成0优惠。`:370`直接取钱包未分页列表按本地状态/时间/门槛筛，失败吞为空；`:460`每次submit重建随机key，结果未知重试不能保证同一意图；`:495`仍请求`/coupons/user/0`，`:519`的buy模式仍从cartStore查找目标。钱包`pages/user/coupon.vue:24/40`固定显示¥金额，`:100`无分页且失败保留旧列表/仅console报错。现新增FE-003L明确跟踪完整报价、精确购买范围、正确券合同和冻结重试，而非把PC功能照搬后宣称跨端完成。清单变为204勾选/145开放/349项；FE-002E-C、FE-003及发布门禁均不关闭。
...
### FE-003L：结算页CI回归修复与UniApp钱包增量（2026-09-07，本地候选）

`acac7c36ee83c4e470e8dd189648490a1e1225bc`的[Actions34095671406](https://github.com/cinagroup/cinashop/actions/runs/34095671406)最终9成功/2失败。Worker第二片135文件中134通过/1失败，869项中868通过/1失败；失败为diy-home-frontend-migration要求38个目的页保留DiySuspendedNavigation，confirm重写时遗漏该组件。汇总门禁正确失败，不能当作暂时性基础设施故障或借用0a20ec6的通过。本轮恢复confirm模板中的全局组件，不删除/跳过该测试；相关DIY与共用钱包/真实控制器SQL共3文件32项本地通过。五端、workerd、PG16目录、密钥与第一分片在旧SHA通过，不代表修复候选已通过整套Linux。

钱包从旧固定金额、未分页和吞错列表改为apiCouponWallet/useCouponWallet：保留20条有界游标和响应头，复用view/common的严格券金额/类型、Asia/Shanghai有效期和服务端可用状态。未使用（含未开始）、已使用、过期/失效、订单占用分四页；金额与折扣分别显示¥5.00/8.5折，null日期显示不限。加载失败显式重试；追加失败保留前页与原游标，刷新清前页及详情，不显示跨状态旧券。详情按当前列表ID查找，切页、隐藏、卸载和登录会话变更立即清理；代际与身份双重校验丢弃迟到的成功/失败。非available券不显示浏览入口，可用券也仅浏览普通商品列表，明确由结算报价决定实际适用性，不自动用券。

提取原结算测试的真实Vue/Pinia运行器，保持11项结算测试；新增10项钱包测试覆盖首/续页、失败同游标重试、格式/无截止时间、旧成功及旧失败、隐藏/卸载、换账号、无效参数/游标及非可用券禁止导航。替换的只有平台生命周期与I/O，不使用假Vue响应式。工具链29项零跳过、UniApp类型、H5/MP-WEIXIN/APP构建与3项产物检查通过。当前没有独立新增UniApp钱包SQL桥接文件；浏览器则真实贯通UniApp请求层、UserActivityController/UserCouponWalletService和一次性PGlite。

前端测试技能要求的实际QA：使用已授权CUA受控浏览器（旧browser技能未列出，不另装依赖），127.0.0.1:5176仅代理5229的隔离夹具，未继承生产数据库URL，登录用户为合成。首屏20张，续页before104人为失败后同游标重试得到29张；8.5折/满10减5与未来开始样本正确，未来券无浏览按钮；订单占用详情只有关闭，已使用/过期状态独立。延迟未使用响应12秒并切已使用，迟到响应后仍仅显示已使用样本。页面身份/非空/无框架错误浮层、状态交互和截图通过；1280×900、390×844已观察页面/详情无截断或横向溢出，移动端document.scrollWidth=innerWidth=390。控制台仅DCloud自带vue-router导入弃用warn，无业务error。隔离夹具记录无建单捕获，订单/账单0、库存8、积分100不变；并未测试生产账号、微信设备、原生App或provider。

进一步核对PHP旧wallet：它还支持各状态数量、全部/快过期/通用/分类/商品/品牌筛选、具体券规则，以及按category_id/product_id/brand_id导航。当前服务端钱包和新页面尚未提供这些合同；商品页也未完整消费此类范围参数。这些显式保留为FE-003L/FE-002E-C缺口，不能把本批的基础状态/分页修复勾为整个钱包迁移完成。另保留活动整链、上传真机、跨标签订单竞争和人工恢复、数据初始化、真实角色/provider与生产发布门禁。

三端业务图i18n审计另外9项通过（21.37秒，实际重建H5/MP-WEIXIN/APP），无新增Intlify消费者；仍不覆盖真机Vue、APK/IPA和预打包代码。当前仅前端和测试/文档变更，未重复跑Worker双类型，不借用旧结果描述为本轮执行。
...
### d058f6f钱包精确Linux验收（2026-09-07）

精确SHA `d058f6f1ebb93390a4bfed852bd80a162d34962d` 的 [Actions34107810221](https://github.com/cinagroup/cinashop/actions/runs/34107810221)最终11/11成功。分片101696770891/101696770700各137文件、963/882项，总274文件1,845项、零跳过；共同inventorySha256=`e75a30e562c09f6b0a7db469c0a137228e7d94eab8d49e41e69f27757e1720ef`，executedFileSha256分别`b1a50d019f75407cd5c2f1348084091e33c9e04f082104ec6f4edfa11076553f`与`b698e31be90fdcd6f1c1c16f80ff9c435c92739f20b4b66e5e10b070ef843aad`，两片完整互斥与实际执行文件匹配均true。PC钱包17项在第一片30ms通过，PC实际19项/构建、另外四端、workerd、PG16目录、密钥及汇总通过。S4按代码/Linux勾选；不将它推广到尚未提交的范围总览或生产。

### FE-003L-S5：两端配置范围名称、层级和独立分页（2026-09-07，本地候选）

源UniApp `view/uniapp/pages/users/user_coupon/index.vue` SHA256=`A991DE0FC405E79F4655BA2609E47595CC8C41D7D3D37B3501B14AE3A433CDF1`，按category_id/category_type/category_name、product_id集合、brand_id导航；目标既有owner绑定商品页只展示四类摘要，没有具体配置集合。本批继续完善已列S2完整展示合同，不恢复可任意提交商品集合的旧授权边界，也不把“配置总数”称为可买商品数量。

现有GET /coupons/user/:id/products增加view=scope只读视图；缺view保持原目录合同，其他view拒绝。Controller共用请求内Service的owned校验：当前UID、持有券归属、可用状态、真实模板及当前用户，每次续页重查。范围定义复用结算同一JSON/CSV/关系表调和方法，冲突及不存在模板仍拒绝；通用范围和空指定范围明确区分。配置ID独立降序limit/before分页，不受本页目录扫描命中影响，无固定首100项截断；响应含总数、精确配置游标和类型+完整有序ID集合的SHA256摘要，客户端总数或摘要变化拒绝拼接，即使配置数相同也不混旧集合。

Cloudflare技能指导沿用请求内container/Hyperdrive，不新增连接、全局私有状态、配置或依赖；查阅官方当前最佳实践及5.20260907.1平台类型。PostgreSQL技能指导批量主键查询，按父级分层批查，最多100个配置根和32条父边/根；典型20个三级品类页含认证共不超过5次select，有真实SQL spy断言。公共平台品类/品牌及目录/VIP可见商品才输出名称，隐藏、已删、私有或缺失节点name=null但保留配置名额；环、断链、陈旧path/fid和超深层级返回hierarchy_complete=false，不输出虚构完整层级。展示父链绝不参与结算判定，也不据名称生成购买/价格或导航权限。

两端新增“配置范围总览”，按需读取、20项续页、原游标失败重试、明确范围与商品数区别、名称纯文本及缺名/异常层级提示。共享严格适配器校验身份、scope_only、类型、集合摘要、非负安全总数、递减唯一ID、32级无环祖先、名称类型/长度及游标；独立请求代次隔离刷新、身份续签、离页和迟到成功/失败。刷新商品同步清理范围说明，范围读取失败时不允许旧商品动作；范围读取不自动请求商品、领取、锁券、下单或支付。

新增4项真实Controller/SQL测试，包括23项配置分页、配置等量替换摘要变化、原始脚本文字、隐藏/私有/删除/VIP名称隔离、品牌祖先、环/断链/陈旧路径/35层深链及owner/不可用/冲突/无写入；原9项真实商品合同保留。PC实际Axios/Vue/Pinia新增4项共23项，UniApp新增4项范围运行时共12项/全工具链48项。最终相关Worker6文件70项7.59秒、双Worker类型、PC类型/23项/1,853模块构建8.88秒、UniApp类型/48项/H5、MP-WEIXIN、App重建及产物3项通过。没有调整并发/时限或删旧断言。

本机workerd首次在沙箱出现日志EPERM和收集前0xc0000005；以授权执行环境复核一次，EPERM消失但同样3个原生启动错误、0项执行。明确仍未通过，不推定为代码断言失败或VC运行库版本原因，不安装/升级系统库或循环重试；必须由本候选自己的Linux验证。前一d058f6f的Linux workerd通过仅是前一SHA证据。

前端测试技能要求真实渲染闭环，使用已授权CUA内置浏览器（本会话未列独立Browser skill，未安装替代依赖）。临时Temp夹具显式删除TEST_FINANCE_POSTGRES_URL，API仅127.0.0.1:5229一次性PGlite及合成登录，真实Controller/Service而非伪造范围JSON。PC 127.0.0.1:5177/user/coupon/60/products与H5 127.0.0.1:5178/#/pages/user/couponProducts?couponId=60分别验证20/24→模拟失败保留→before203重试24/24，商品扫描游标独立；H5施加5秒范围延迟后刷新商品，迟到范围没有回填。最终PC933×792与H5 390×844截图及DOM确认名称换行、独立滚动、纯文本script无节点、没有横向溢出/框架遮罩、正确URL和内容。测试中浏览器视口覆盖与原生缩放叠加使中间截图比例异常，清除临时CDP覆盖并重设浏览器视口后重载、重拍正常最终图，未修改页面样式掩盖测试工具缩放。PC error/warn为空，H5仅既有DCloud vue-router导入弃用warn，无应用error。

最终复核PublicCatalogService的品牌可见性：store_id非零不代表私有品牌，已移除候选中额外的storeId=0过滤并在真实SQL品牌祖先测试加入storeId=88样本，避免无依据隐藏公开品牌；公共品类维持平台类型/归属约束。最终相关6文件70项再通过7.75秒。UniApp额外runtime-i18n九项21.511秒通过，H5 80chunks/282loaded、MP101chunks/325loaded、App250entries；既有native/APK/IPA排除边界不变。
...
### b98d137范围搜索后端精确Linux验收（2026-09-07）

精确SHA `b98d137f01a31e0de35fad343ad2c45a3a71cec0` 的 [Actions34115191204](https://github.com/cinagroup/cinashop/actions/runs/34115191204)最终11/11成功。分片101720240123/101720240190分别138/137文件、970/896项，总275文件1,866项，0跳过，耗时373.10/514.85秒。共同inventorySha256=`153c30694a63ae8d796db5539497c3a0c526927b8fa6b0d3fbb3af97f0267686`，executedFileSha256分别`1d4171c2890d182b498a60512295b872177898885523e911111fc6ba9897e772`、`149a55e136c178684e209fb4dbcca337c4e701405577abea82e2e9c79a69a441`；两片完整互斥和实际执行文件匹配均true。五端、workerd、PG16目录、密钥及汇总成功。S6后端按代码/Linux勾选，前端新候选不能借此SHA验收。

### FE-003L-S7：PC/UniApp券范围搜索与排序交互（2026-09-07，本地候选）

两端范围页改用已验收的view=search后端，保留旧数字游标API/adapter以兼容既有直接调用及其测试，配置范围view=scope仍独立分页。新增共享couponProductSearch模块：七值sort/100字符keyword校验、显式scope_only/持有券/关键词和排序回显、每页最多20行、0～500扫描计数及布尔上限、512字符有界不透明字符串游标。商品安全图片/金额投影提取成原有和新适配器共用方法，旧ID降序适配合同保持；新搜索尊重服务端全局顺序，不按ID重排结果。

共享Session保留请求代次，切词/排序/刷新移除旧商品、游标、累计扫描数及配置名称；迟到成功失败均不回填。已提交filters独立于输入框草稿，续页/失败重试继续使用提交时条件和精确游标，不因尚未提交的文本变化而拼接异条件结果。身份续签/换号清空搜索条件，隐藏/离页移除数据并禁用导航；配置名称页也随搜索刷新失效。重复商品或返回已见游标时拒绝追加、保留先前页和重试上下文且禁用商品入口，提示目录变化后刷新；不宣称动态目录是稳定快照，也不无限自动扫描。

PC增加可提交名称表单、清空按钮和带label的排序选择框；UniApp使用搜索输入/确认及原生picker，保留悬浮导航。展示当前已提交关键词、累计检查候选数、已找到范围商品数和单次500上限；空扫描页但仍有游标时提示继续查询，真正结束的无匹配另提示换词或清空，结束提示明确为本次搜索。展示销量说明保留sales+ficti口径，与结算和库存非承诺分开。原生picker在本机英文环境下其系统按钮为Cancel/Done；本批不更改全局语言配置，中文业务排序项与实际事件已验证。

PC既有23项保留并更新为search响应/字符串游标，新增4项实际Axios/Vue/Pinia测试：七排序与中文/LIKE字面量参数、服务端非ID序保留、搜索切换迟到成功/失败、已提交与草稿隔离、重复商品/循环游标和坏回显/计数拒绝、身份条件重置；27/27通过。UniApp既有48项保留，新增同类4项，完整工具链52/52通过4.461秒；PC类型/1,854模块构建10.76秒、UniApp类型、H5/MP-Weixin/App及产物3/3通过。runtime-i18n9/9通过22.623秒，H5 80chunks/283loaded、MP101/326、App251entries，既有native/APK/IPA排除边界不变。相关Worker7文件87项11.65秒、双Worker类型通过，未改依赖/构建配置/工作流/路由/DDL。

前端测试技能指导真实渲染验收；独立Browser skill未列出，使用用户已授权的CUA内置浏览器及其Playwright/AX接口，无新浏览器依赖。PC `http://127.0.0.1:5177/user/coupon/60/products`、1280×900；H5 `http://127.0.0.1:5178/#/pages/user/couponProducts?couponId=60`、390×844，API只代理loopback5229一次性PGlite的真实Controller/Service，登录和30正常/605稀疏商品为本地合成。流程：搜索样本→PC价格升序/H5价格降序→20项→模拟分页失败保留20且入口禁用→同游标/关键词/排序重试30项，服务端事件日志两次IdenticalCursorRetry均true。稀疏查询500候选零匹配仍可继续，605候选后仅返回确属范围的稀疏样本0。H5旧查询延迟5秒时切词，旧响应完成后仍为新关键词/零匹配状态；真正无匹配提示及清空恢复通过。H5初次点击滚轮文字未改变选择，改按实际滚轮拖动再确认后核对降序和商品列表，未把无效尝试计为通过。

URL/标题/非空/无框架遮罩、无横向溢出及控制台检查通过，PC和H5当前error/warn均空。截图已在任务中展示；H5 fullPage捕获出现比例异常，DOM实际390px无溢出，改用fullPage:false获得正常移动视口截图，不以截图工具差异改生产CSS。所有业务读取仅在一次性库，最终orders=0/bills=0/captured=[]/visits=0/productLogs=0、库存8和积分100不变。未访问生产/做DDL/正式发布，也不新增旧历史复制门禁。S6勾选后新增S7，209勾选/147开放/356项；S7待自己的Linux，真机、真实角色/provider、促销/活动与发布仍未完成。

验收后已关闭两个临时标签并重置视口覆盖，停止本批API/PC/H5监听，5177/5178/5229监听均为空。追加路由审计器/新系统口径两文件5项247ms通过，git diff检查通过；未删除用户文件或改动生产资源。

### 698553d两端范围搜索精确Linux验收（2026-09-07）

精确SHA `698553d0a0606aa64e49aba6c93f0e8f238764d8` 的 [Actions34117384660](https://github.com/cinagroup/cinashop/actions/runs/34117384660)最终11/11成功。分片101727267005/101727267116分别138/137文件、970/896项，合计275文件1,866项，0跳过，耗时606.68/512.94秒。inventorySha256=`153c30694a63ae8d796db5539497c3a0c526927b8fa6b0d3fbb3af97f0267686`，两片executedFileSha256分别`1d4171c2890d182b498a60512295b872177898885523e911111fc6ba9897e772`、`149a55e136c178684e209fb4dbcca337c4e701405577abea82e2e9c79a69a441`；完整互斥和执行文件匹配均true。五端、workerd、专用PG16目录、密钥扫描及汇总均通过。S7按代码/Linux勾选，S2真机和父项保持开放，新活动候选不能借用本次SHA验收。

### FE-003L-A1～A3：活动入口复审与秒杀SKU前置合同（2026-09-07，本地候选）

本轮未把活动页面的存在当作迁移完成。PC `pages/activity/Seckill.vue`仍把index的`{seckillTime,seckillTimeIndex}`对象声明/使用成数组，并假定不存在的is_active字段；列表误传start_time文本而不是时段ID，标题读取store_name而实际合同为title，点击跳普通goods路由丢失活动。PC拼团详情join仍硬编码`sku00001`。UniApp `ActivityPurchase.vue`读取基础商品SKUs而非活动SKUs，可能展示不参加活动的选项；详情脚本原始any、登录返回和页面隐藏/会话代次仍需逐项完善。

PHP `app/services/activity/seckill/StoreSeckillServices.php:627`明确读取type=1的productAttr/productValue；约566～605行还根据启用时段和时钟设置活动状态/last_time。目标ActivityService旧详情只返回活动行、父活动和进度，没有选规合同。StoreCartService与StoreOrderCreateService目前复核子活动日期、显隐、限购及库存；检查所示路径未见统一读取store_seckill_time/父活动排期。不能用本批date_window替代完整资格校验，A2明确作为接入购买UI之前的前置任务，A3保留三类入口及浏览器逐链路验收。

本批在既有GET `/api/seckill/detail/:id/:time?`上增加显式`view=skus`，由独立SeckillSkuCatalogService读取；无view继续原始详情，未知view报错，不新增路由或改变注册计数。新视图private/no-store，ID严格正十进制int32，非法ID/内部UID在SQL前拒绝。身份只取已验证请求UID并重新查当前未删/启用用户，VIP沿用现有目录isMoneyLevel口径；不信任query UID/VIP。活动启用/可见/未删且基础商品上架/未删/审核通过才投影。没有直接连接、日志输出或持久缓存用户信息。

读取按活动商品ID/type=1及基础商品ID/type=0各一次、is_retired=0、ID排序、500+1上限；已有spav_product_active索引覆盖过滤/排序前缀，未增DDL。匿名固定三次SQL（活动/商品联查和两份SKU），登录另有一次用户读取；不逐规格调用解析器。超过500、重复unique/suk、缺有效基础映射、带边缘空白名称或多规格混入空名称均明确失败，不返回静默截断/错误默认项；真正单个无名称SKU仍支持。返回最小字段，剔除成本、佣金、虚拟内容等，只保留活动unique、基础unique、suk、精确numeric目录价、划线价、安全图片及五种库存的最小值。max_quantity再取单笔/累计配置上限和32767，但不是扣除个人已购后的剩余额度。selection_only=true；日期窗口future/active/ended不是时段/父排期资格或库存预留。多次查询不是并发快照，后续必须重新报价/验证，不把目录价当最终支付额。

复核还发现共享ActivityOrderSkuService在活动unique与基础unique同值但suk不同，或显式unique与suk互相矛盾时，会选用另一身份继续解析。现新增三个小守卫：双命名空间同标识异规格拒绝、已找到基础SKU与指定suk冲突拒绝、最终活动SKU与指定suk冲突拒绝。覆盖type=1/2/3，保留两种正常unique别名和一致的同标识同规格。新目录同步拒绝跨命名空间冲突，真实cart控制器拒绝新建冲突行、已有购物车发生后续冲突时立即购买读取失败。原建单路径已有baseSku.id一致性检查，本批不夸大成已证明发生错误扣款，也不修改退款/库存账本。

前端测试技能推动先核对真实契约并将未接UI明确留在A3；本批没有页面修改、浏览器点击或新截图，不复用旧优惠券截图证明秒杀可用。Workers技能用于请求内状态、类型与有界输出，PostgreSQL技能用于两份有界批读取及已有索引；官方Workers最佳实践已检索，官方npm最新类型5.20260907.1与本机临时参考相同（首个镜像查询被网络权限拒绝，改为授权的官方只读元数据查询，无安装）。无生产连接、DDL、部署、真实订单/支付或提供商请求，历史数据复制继续N/A。

本批新增`seckill-sku-catalog-postgres.test.ts`共31项一次性SQL场景：三查询最小投影/只读、参数在SQL前拒绝、旧详情兼容/no-store、商品和活动可见性、VIP与禁用/撤销、类型/商品/退役隔离、四种重复映射、缺基础映射/空名称、跨命名空间冲突、三类unique/suk矛盾、售罄及库存/限购上限、最大numeric与零价/负价拒绝、日期边界、安全图片、两种501超限拒绝和恰好500完整返回、真实加购与活动价读取、读后库存变化拒绝及后续标识冲突。读取快照确认商品/活动/SKU/用户/订单不变；仅显式cart链路测试在一次性库创建购物车，无订单或支付路由。首次测试暴露findForAuth不含status过滤，已增加服务端显式启用检查；随后三项矛盾输入测试证明只检查最终活动suk还不足，补上已识别基础suk的前置冲突拒绝后通过。

扩展15文件回归在与类型检查并行时有两项旧测试超过5秒，maxWorkers=2仍有一项超时；记录保留，不改超时配置或跳过测试。待类型检查结束，maxWorkers=1同组119项全部通过；再加入真实单个空名称SKU边界，最终相同15文件120项全部通过33.00秒。另有路由审计器/新系统口径两文件5项465ms通过。未重试已知Windows workerd原生失败，本批Linux仍待自身提交；前端未改，不将旧浏览器或构建记录充作本批活动E2E。S7勾选并新增A1/A2/A3后，按任务行统计为210勾选/149开放/359项；开放数增加来自审计拆分，不是把旧历史数据对账重新列成阻塞。

最终源码冻结后双Worker类型检查（unit/runtime）再次通过，git diff检查通过。提交范围仅两份迁移文档、旧控制器的显式视图分支、共享规格解析器三个拒绝守卫、新只读规格服务与新SQL测试；未修改前端、工作流、依赖、数据库结构或部署配置。

### 38f3439秒杀规格合同精确Linux验收（2026-09-07）

精确SHA `38f34396b7821c7d3ad97821e336ea3e386cc48e` 的 [Actions34119944273](https://github.com/cinagroup/cinashop/actions/runs/34119944273)最终11/11成功。分片101735459624/101735459352各138文件、998/899项，合计276文件1,897项、0跳过，耗时602.65/507.35秒。共同inventorySha256=`16b8b68b953522bbf82a6310ecafdf743dbb28cfe16664cdc009434b70bd0838`；executedFileSha256分别`ba86600cab47660d4dc9835de5c174f1579a46b30bd53a7835e8c9ef3f5f5897`、`1da9f3bae41ffb465b9131bf7fbde69a7fe20156702966f27931a9288017af43`，完整互斥及实际执行文件匹配均true。五端、workerd、专用PG16目录、密钥扫描及汇总成功。A1按代码/Linux勾选，不将其验收借给本批新排期代码。

勘误：上轮文字中的“五种库存”应为六类约束，即子商品stock/quota、活动SKU stock/quota、基础SKU stock、基础商品stock。代码一直取六项最小值，本轮只纠正文档和测试名称，不改变此计算。

### FE-003L-A2a：购买排期统一守卫（2026-09-07，本地候选，A2未完成）

源端证据来自PHP `StoreSeckillServices.php:getSeckillCount`：约115～116行按start_time与stop_time+一天筛日期，约138行时段开始含/结束不含；save约225～226行把section_time日期字符串转时间戳，约320行展示停止日期23:59:59。`StoreSeckillTimeServices.php`保存HHmm而编辑/展示也有HH:mm；父`StoreActivityServices.php:saveActivitySeckill`复制父time_id和section_data到子商品。目标迁移清单仅将子时间戳转换为Date，没有补全天结束，原购物车和建单也未读取启用时段/父排期。父start_time/end_time不是秒杀epoch日期，不混用这两个字段。

新增SeckillScheduleService：上海日历日、日内时段开始含/结束不含，HHmm/HH:mm严格解析，24:00仅允许作结束；相等或跨午夜时段拒绝并提示拆成两段。日期仅对上海午夜值按旧日期包含当天处理，非午夜值保留原毫秒精确包含语义。子商品status/isShow/isDel为首道门禁；关联父存在且type=1/status=1/isDel=0，父startDay/endDay须为有效午夜日期。父子日期和time_id允许集合取交集，不允许陈旧子配置扩大父范围；activityId=0的独立旧式商品仍须自己的有效启用时段。CSV最多64项/1,024字符、正int32，时段批量有界读取；无有效启用时段、缺父、畸形数据明确拒绝。策略返回active/future/ended/waiting/unavailable/invalid及当前允许的精确窗口，不代表价格、剩余限购或库存已预留。

真实加购及购物车读取接入同一策略；购物车失效项保留，严格buy读取拒绝。StoreOrderCreateService在预检报价路径复用，confirm/computed与直接create均受约束；既有type=2/3日期不变。真实建单事务在幂等已建订单返回之后，按父行SHARE→子行UPDATE→时段ID升序SHARE读取；发现到加锁间换父会失败，而不是再锁第二个父。扣秒杀库存前重新评估时钟，原子UPDATE同时要求数据库clock_timestamp处于精确窗口；不用固定在事务开始的NOW。时段或父配置在获得锁后无法在当前事务内被另一事务更新，过期、配置停用或后续库存失败均走现有整单回滚。此处定义是库存准入时刻，不承诺事务一定在截止前提交。

新增纯策略19项覆盖日期前后、开始/结束毫秒边界、UTC等价、父子交集、缺失/停用/错误父、隐藏/删除子、禁用/缺失时段、坏格式、跨午夜拒绝、整天时段、午夜与精确截止，以及CSV上限。新增一次性SQL26项覆盖最小字段/真实事务读取；14类不可购买状态在add/list/quote/create一致拒绝且不生成订单号；截止当天仍可加购/报价；预检后4类父/时段/换父/截止变化在事务内拒绝；最后策略判断后数据库时钟跨界；成功创建及停用后的幂等重放；活动SKU与基础SKU两种晚期库存失败整单回滚；真实HTTP confirm之后停父/时段，computed和新confirm不复用旧价且不刷新缓存。行快照覆盖基础商品、SKU、子活动、购物车、用户、订单、明细、账单、状态和打印意图。仅隔离成功样本创建1条未支付订单，无真实支付/提供商请求。

验证过程中失败记录保留：新SQL样本最初未启用自提门店、显式购物车ID未同步本地序列，补全夹具后通过；SQL无ORDER BY的结果不得用数组位置断言SKU，已改为ID定位；子截止与父窗口不相交返回配置不一致，已核对错误合同。PGlite与JS共享被模拟的Date，最初不能证明独立数据库时钟；现对真实策略函数仅作透传观察，在第三次（最后）评估后恢复真实时钟，再让未替换的UPDATE执行，验证SQL二次守卫，明确不冒充真实多连接锁等待。HTTP新测试最初误读confirm平铺金额，实际合同为priceGroup，computed才平铺，修正测试而未改接口。类型检查最初暴露事务没有$client，排期读取函数已收窄为Pick<DbClient,"select">，未添加不安全强制转换。旧规格目录夹具补显式父和全天时段；旧PG集成脚手架只补独立时段表及样本，本轮没有连接生产执行该脚手架。

相关17文件最终137项全部通过、0跳过，36.54秒，串行maxWorkers=1；其中两新文件45项。扩展前失败为1文件中的2个HTTP断言，其余16文件通过，没有跳过或调整超时。Workers/PostgreSQL技能用于请求内有界查询、事务能力类型、统一锁序以及数据库真实时钟；官方Workers最佳实践和PostgreSQL时间函数文档已检索，使用当天5.20260907.1参考类型，未更改绑定/配置/依赖/工作流/DDL。未重复Windows workerd原生失败，Linux待本批自己的提交。

剩余边界：本批没有更改目录。ActivityService.seckillTimes仍字符串比较且结束含等号，列表DAO仍旧子日期谓词，view=skus仍旧date_window。A2b须处理父/时段/日期一致及有界正确分页，不能分页后过滤制造假耗尽，也不能把将来场次的浏览入口一并禁止。A2c需独立PG16多连接验证父/时段更新等待、换父竞争、锁等待跨截止、建单与取消/退庫及后台保存完整锁顺序；当前取消先基础SKU后活动库存，存在相反顺序，不能从本批有序排期锁推导全系统无死锁。事务内限购和活动SKU价格/退役状态仍需要按当前记录复核，数量修改也不应冒充完整校验。A3活动前端和真机仍开放，本批没有页面或浏览器E2E。

A1勾选、A2拆分a/b/c后任务行统计为211勾选/151开放/362项。开放项增加来自细化证据要求；旧站历史数据复制与行级对账仍N/A。整个批次不连接或修改生产，不部署Worker/Pages，不启动浏览器或额外服务。
...
### FE-003L-A2c1：报价到写入之间的秒杀规则漂移（2026-09-07，本地候选）

审查真实StoreOrderCreateService发现：子活动onceNum/num、活动SKU价格和成本在事务外读取，事务内累计订单数虽然在advisory lock后重新计算，仍用旧限额比较；活动SKU UPDATE缺isRetired和价格身份条件，基础SKU UPDATE仅检查ID与库存，购物车认领也未匹配预读数量/规格。六个实际SQL复现（单笔限购3→1、累计限购10→1、活动价6.25→9.99、活动SKU退役、基础SKU退役、购物车2→3）在修复前全部错误返回新订单，测试以promise resolved instead of rejecting明确失败；26项旧测试在该定向诊断命令中未执行，不计为验证通过。

新增SeckillPurchaseSnapshot仅生成参数化SQL谓词：原有认领/扣减语句同时用IS NOT DISTINCT FROM比较预读计价字段，不再另做一个无锁检查。子活动比较父/商品ID、onceNum/num、运费/模板、赠积分、表单、标题/图；两类SKU比较身份、规格、价格、成本、结算、积分和图，基础SKU另比较重量/体积及核销/卡密履约字段，均要求isRetired=0。购物车认领匹配数量、商品、SKU、类型、活动和new模式，原owner、staff/tourist、有效/未付条件保留；基础商品扣减要求可见、未删、审核通过，并比较归属、商品类型、VIP标识、退款能力与结算价。未审核商品也在预检阶段拒绝，避免无谓分配订单号；旧隔离集成场景的正常商品明确设置isVerify=1，没有执行该生产Hyperdrive场景。

任一谓词不匹配都抛出刷新重试错误，由既有withTx回滚此前认领、活动库存、订单与快照；库存/配额/销量不作为静态相等条件，继续使用当前原子余量判断，因此其他正常库存变化不会导致固定快照冲突。没有增加读锁或重排现有锁；没有把客户端金额当权威，也没有在失败后悄悄修改当前事务的收费。Sequence编号仍在事务外分配，失败可能消耗一个编号，此处只承诺数据库业务事务回滚，不宣称外部编号回收。

新增31项真实SQL测试：28类报价后字段变更全部拒绝并核对商品/SKU/购物车/活动/订单/明细/账单/打印状态无额外变化；新价再次计算为15.00后可成功建单，后续限购变动仍返回同一幂等订单；未审核商品报价/创建不分配编号；四层当前库存足够时正常扣减。秒杀文件26→57项；最终相关14文件219项27.53秒通过。先前56项、12文件213项是中间结果，不能代替最终新增未审核预检。CI工作流的两个unit分片均显式绑定独立PG16.14服务，本机这些测试则显式清空TEST_FINANCE_POSTGRES_URL，使用一次性PGlite；新候选仍需自身CI，不借用旧SHA。

边界仍开放：当前写谓词只绑定已选行，不冻结新增歧义SKU、共享系统/运费配置、用户身份或会员资格；setNum仍在检查后以普通DAO更新，未按isPay/活动排期和限购做原子复核。取消路径仍先基础SKU→商品→活动SKU→子活动，而建单为子活动→购物车→活动SKU→基础SKU→商品，后台/退款路径尚未整体统一；没有独立连接等待/死锁测试，不能标为并发完成。A2c1待自身Linux，A2c与A3保持开放；此时目录4b170a2的CI34125230229仍在运行，未重启。本批不连接生产，不部署、不调用支付/provider，旧站历史导入N/A。

#### 4b170a2自身CI失败与精确证据修正

`4b170a2bfbecc79f4896fdcab0852831453bc5ec` / [Actions34125230229](https://github.com/cinagroup/cinashop/actions/runs/34125230229)最终9/11成功，分片二和汇总门禁失败，不能计为A2b完成。分片一101752240762通过140文件1,053项、617.32秒；分片二101752240686为139文件940通过/1失败、507.65秒。失败是ordinary-query-index-alignment的源码定位断言：此前删除旧时间辅助函数和重构DAO使三份引用行号过期，原查询片段本身未变。已逐条定位store_bargain状态查询165→158、store_combination状态查询132→125、store_integral状态查询756→736；只更新orm-ordinary-index-reconciliation.json的这三个sourceLine，保留22对象、列序、定义与历史判定，原严格startsWith门禁不放宽。失败提示补充对象名/文件/行号，定向5项297ms通过。A2b继续待新提交CI；本批A2c1不能借这次失败CI获得验收。最终两套Worker类型通过；当前清单212勾选/151开放/363项。

冻结补验：六份索引/名称/历史定义对齐文件34项20.33秒通过，保留完整定义与失败关闭检查；与上述业务14文件219项合计20文件253项，均零跳过。修改限于秒杀分支，未把其他活动或全局可变规则默认为已覆盖。SQL写条件的设计参考[PostgreSQL事务隔离说明](https://www.postgresql.org/docs/current/transaction-iso.html)中Read Committed更新等待后重判目标行条件的规则；这不能替代本仓库尚待完成的独立连接竞争测试。Workers/PostgreSQL技能影响了本批把检查放在现有写边界而不是新增无锁查询的选择。

### 8784938自身Linux验收及A2c2数量入口候选（2026-09-07）

`8784938498ec08051ef17806c16fa5d9c6dc8dcc`的[Actions34126883602](https://github.com/cinagroup/cinashop/actions/runs/34126883602)最终11/11成功。单元分片一101757529724为140文件1,053项、463.31秒，分片二101757529583为139文件972项、522.56秒，合计279文件2,025项零跳过。两片inventory摘要均为114fcf15a2366c893d329631d29c324aef4779c36db3455bb511517c27194b07；执行文件摘要分别1b151b7ea14ed8480776c48a702a4be8a1dfe31e547e664ed2dd0b02e9668a63和67b1ecb007b595384c5ffb49d97c4ef708279f5f45e97646e2dddfec61e5a03e，完整互斥/实际执行匹配均true。五端构建、workerd、真实PG16目录及secret门禁成功；A2b/A2c1按代码/Linux候选收口，前次4b170a2失败记录不删除。

本批修复StoreCartService.setNum的秒杀分支：修复前四项真实SQL诊断全部错误放行已认领、超单笔限购、子活动quota不足和父活动停用的购物车；定向诊断中的57项未选择测试不算通过。新SeckillCartQuantityService在事务内按owner/type/isPay/isDel/status/staff/tourist/store精确锁定购物车，重新读取排期、商品审核/上下架、基础及活动SKU身份和退役状态，检查单笔/累计限购与六层库存/额度。累计数沿用PHP已支付或未删除待支付订单、pid为0/-1口径；SUM转text后检查安全整数，避免int累计溢出。最后UPDATE再次约束scope及数据库clock_timestamp处于所读有效窗口，失败自动回滚。通用数量参数收紧为1～32767整数；其他类型的业务分支未调整。

Workers/PostgreSQL技能影响了锁边界选择：只锁购物车，不在持有购物车时增加活动/SKU/商品锁，避免引入与建单反向的新依赖。数量修改不是库存预留；配置及其他订单仍可在非锁定读取后变化，最终资格必须由建单事务复核。本批受控插入的“报价后改数量”与“建单后改数量”顺序测试不等于独立PG连接并发证据。A2c仍须完成独立连接等待、取消/退库/后台锁序、共享配置和新增SKU幻读验证；加购合并与列表数量资格也仍未统一。本批无生产连接、DDL、部署或外部支付/provider调用，旧站历史复制N/A。

本地最终回归：秒杀文件57→97项（新增40项，原14种排期场景额外验证setNum拒绝）；相关11文件230项、23.54秒、零跳过，双Worker类型通过。覆盖六种非法数量、20种状态/库存/规格拒绝、五种累计订单口径、两种SKU身份合法修改、建单前后数量顺序、数据库时钟越界及业务快照不变；前两次61项和4文件114项为中间结果。新候选仍待自身Linux，不将8784938的CI算作当前补丁证据。

### FE-003L-A2c3：独立连接竞争验证候选（2026-09-07，待真实PG16执行）

前一数量修复`0286307cae6429aec4d63e57b2cddcee8cebea2a`的[Actions34128377900](https://github.com/cinagroup/cinashop/actions/runs/34128377900)终态11/11成功。分片一101762312575为140文件1,053项、610.89秒，分片二101762312151为139文件1,012项、423.62秒，合计279文件2,065项零跳过。inventory摘要114fcf15a2366c893d329631d29c324aef4779c36db3455bb511517c27194b07，两片执行文件摘要1b151b7ea14ed8480776c48a702a4be8a1dfe31e547e664ed2dd0b02e9668a63和67b1ecb007b595384c5ffb49d97c4ef708279f5f45e97646e2dddfec61e5a03e；完整互斥及实际执行匹配均true。A2c2按代码/Linux勾选，不借此关闭本批A2c3；当前清单215勾选/150开放/365项。workerd、五端、PG16目录和secret门禁均成功，仍不是生产发布或历史数据迁移。

新增financePeers辅助器只允许postgres/postgresql协议、127.0.0.1或localhost、专用cinashop_finance_test数据库及finance_test用户；拒绝query/hash注入。先从既有一次性夹具核对随机finance_test_<32hex> schema、数据库、角色和PG16版本，再创建三个max=1、关闭闲置/寿命换连接的独立客户端，验证连同观察连接的四个PID不同，并在结束时核对PID未重连。每个连接限制statement/lock/idle-in-transaction时限，异常最终有界关闭连接释放事务锁；没有生产URL回退，也不创建公共表。

12项新PG专项使用真实StoreOrderCreateService/StoreCartService及Drizzle事务，只替换外部Sequence编号器：父/时段未提交停用使建单等待后拒绝；建单已锁定排期并等待购物车时，父/时段编辑反向等待其提交；发现到锁父之间换父拒绝；真实建单认领后数量修改等待并拒绝；真实数量服务在测试外层事务持锁时，旧建单等待后以购物车CAS回滚；两个不同意图竞争库存或累计限购只允许一单；同键重试确实等待并返回同单；建单与数量修改各自等待购物车锁跨数据库截止后拒绝。失败核对业务快照不变，成功核对唯一订单/明细、认领购物车及四层库存精确扣减。夹具使用真实SQL引擎及业务服务，但只生成模型列/默认/主键，不包含生产完整二级索引/外键/触发器，不能据此宣称生产全部锁路径无死锁。

阻塞栅栏每25ms在4秒上限内查询[PG16 pg_blocking_pids](https://www.postgresql.org/docs/16/functions-info.html)，只有预期阻塞PID出现才继续；等待函数的短暂停顿不是证据。截止使用数据库clock_timestamp轮询而非假时钟。固定一连接客户端用于保证事务与PID一致；本地检查postgres-js实现发现reserve()返回对象的begin仍属于根池，因此没有预留唯一连接后再调用Drizzle事务的自锁设计。PostgreSQL技能促使本批直接验证锁依赖与提交结果，而非通过Promise.all或延时猜测竞争。

本机没有可调用的PostgreSQL Server，保持已有PG专项runIf口径；新增12项在本地明确跳过，不能算通过。既有CI两个单元分片均配置专用PG16.14，分片覆盖审计拒绝任何跳过；本批须自身CI实际执行才能勾选。边界辅助器两项验证覆盖允许目标、八类拒绝URL和五类非法/同连接PID，初始TypeScript暴露无FROM的Drizzle构造器错误已修正为单行probe来源，双Worker类型通过。未修改业务实现、依赖、CI步骤、扫描例外或时限，不重启前一运行0286307/34128377900。

本地冻结回归为7文件122项通过、1个文件12项PG专项跳过，19.43秒；不是134项通过。原秒杀97项全部执行，三个新辅助器边界项通过（新增数据库时钟查询真实执行与五类非法截止拒绝）；未连接生产或本机未知数据库。前一次121项是中间结果。待自身Linux真实执行，不因为测试已编写就关闭A2c3。

### FE-003L-A2c4：秒杀建单、取消与退款的库存锁环（2026-09-07，本地候选）

前一提交`07d8c1afe8ad97b6cd9d030fb1db888d7ea0e381`的[Actions34129974237](https://github.com/cinagroup/cinashop/actions/runs/34129974237)终态11/11成功。分片一101767486700为141文件1,065项、632.19秒，分片二101767486681为140文件1,015项、542.51秒，合计281文件2,080项零跳过。两片inventory为8d175f6cee96c97fa299bce22d2a1239b86a1904c1f12114d90072cbb9c1a96e，实际执行摘要分别cd0d26d2c5b8ac41a5b3be39d620463b43763cb8f3e63eb73f7b3a64daba423e和1a6f50c129955fd6203f71b7d3372ed0e1152113af80b708786461171b339103，完整互斥/实际匹配均true。seckill-concurrency-postgres明确12/12、12,017ms，父/时段锁等待、换父、数量竞争、库存/限购、幂等、两个截止均实际PG执行，A2c3按自身证据勾选；workerd、五端、PG16目录与secret同批成功。新A2c4不借旧SHA，当前216勾选/150开放/366项。

代码审查确认同一子活动可形成child→SKU→child环：建单先锁子活动再改SKU，而取消先改基础SKU/商品/活动SKU，最后才改子活动。修复将取消既有的子活动存在性读取改为FOR UPDATE，放在任何库存写入前，保持原订单锁、归属、未付款、状态、历史活动缺失策略和不可变取消日志。没有调用购买排期校验，因此父活动停用、时段停用、过期或主活动缺失不会凭空阻止既有订单取消。新增结构守卫修复前1失败/4通过，明确仅为源码顺序证据，不宣称已在本机复现数据库死锁。

继续追踪发现只改取消会新增与退款的反向依赖，因此同步审查完整StoreOrderRefundService：finalize在锁退款单/订单、确认渠道证据与累计金额数量之后，对type=1且status=0、有正活动ID的订单先锁子活动，再锁结算用户、变更账本与退SKU。不能仅在restoreRefundStock内部晚加锁，否则退款持用户锁等待活动、取消持活动等待用户仍可能构成环。已完成幂等重放在新锁前返回，已发货订单不回库存且不加该锁，其他活动和支付渠道处理不改。退款期望活动缺失时继续原有仅恢复现存资源策略；没有借修锁要求活动重新启用。

新增14项真实SQL顺序回归：9项取消覆盖五种失效排期/主活动、已付款、跨用户、活动SKU缺失回滚和退役基础SKU不增加可售库存；5项真实余额退款完成器覆盖停父/过期/缺主活动、重放、晚期SKU缺失完整回滚和已发货不退库。退款的paid/yue前态是隔离样本，不是实际支付；余额12.50、退款流水、库存、明细和退款状态均回读。新增2项结构守卫；4项PG独立连接竞争验证建单↔取消、重复取消、建单↔退款以及取消↔退款，最后一项在退款等待活动时以用户行FOR UPDATE NOWAIT证实其未先占结算用户。PG文件12→16项，业务SQL文件97→111项；新增4项尚待自身PG16 CI，不借上一SHA结果。

本地最终相关12文件168项通过、1文件16项PG专项跳过，23.59秒；不是184项通过。首轮新增退款用例因夹具未清理前一例user_bill而出现2条/3条累计账单，已在beforeEach恢复余额并清空该夹具账单后通过；未弱化一次退款一条流水的断言。两份索引定位回归通过，业务改动仅取消的一处行锁与退款的条件性行锁，没有新增依赖、DDL、CI重试或时限。Workers技能使用当日官方best practices及已核验latest的workers-types5.20260907.1；PostgreSQL技能影响了必须一起处理退款、且在结算用户锁之前获取活动锁的选择。

本项只用共同子活动锁串行化同活动的这三种库存事务，不宣称所有库存读写已采用统一全局顺序。跨活动/多SKU历史订单、后台商品/活动编辑、完整二级索引/外键/触发器、共享规则及新增SKU身份仍需A2c后续验证；活动不存在时也没有可锁行，不提供同ID重建的幻读保护。旧站历史迁移仍N/A，本批不连接生产、不部署，不执行真实支付或退款provider。

冻结检查：最终Worker unit/runtime两套类型均通过，git diff --check通过；当日npm官方latest类型查询仍为5.20260907.1，未升级任何依赖。本机无PG，因此上述16项跳过记录保留，推送后必须在既有零跳过CI执行。

### 8633ba8自身Linux验收与FE-003L-A3a PC秒杀入口（2026-09-08，本地候选）

前一锁序修复`8633ba89d502901666847ab1e0dbd7e9bf777b20`的[Actions34131920654](https://github.com/cinagroup/cinashop/actions/runs/34131920654)终态11/11成功。单元分片一101773798862为141文件1,070项、622.30秒；分片二101773798567为140文件1,030项、522.65秒；合计281文件2,100项零跳过。两片inventory摘要8d175f6cee96c97fa299bce22d2a1239b86a1904c1f12114d90072cbb9c1a96e，实际执行文件摘要分别cd0d26d2c5b8ac41a5b3be39d620463b43763cb8f3e63eb73f7b3a64daba423e、1a6f50c129955fd6203f71b7d3372ed0e1152113af80b708786461171b339103；完整互斥及实际执行匹配均true。日志逐项确认16项独立PG竞争、14,981ms，包括新增建单/取消、重复取消、建单/退款与取消/退款。A2c4按代码/自身PG16证据勾选，未发布、不关闭全局并发门禁。新增A3a后清单217勾选/150开放/367项。

继续审计PC实际调用方确认：apiSeckillIndex把PHP兼容对象错误声明为unknown[]；Seckill.vue读取不存在的is_active/start_time并用开始时间查询列表，商品展示store_name而非title，点击跳普通商品页丢失子活动身份。当前后端已返回seckillTime/seckillTimeIndex，且有严格view=skus目录。新共用seckillPurchase边界模块明确解包对象、时段/子活动/基础商品ID和活动unique，校验重复规格、价格、库存数量、排期与预期活动ID，旧raw detail或原始数组明确报错，不猜测或降级到普通商品SKU。图片限同源路径或无凭据HTTPS，标题按Vue文字渲染。

PC列表改为按钮选时段ID、独立选中态、20项服务端分页，错误与空结果区分，刷新/切时段/离页使旧请求失效。新增/seckill/:id专用详情，显式选择活动SKU并显示catalog_price参考价和max_quantity；北京时间排期状态、到期禁购、数量整数和上限检查都不冒充最终报价/库存预留。加购固定new=1、type=1、子活动ID和活动unique，后台实际转为匹配基础unique，跳转checkout仍携带mode=buy/cartId/type=1/seckillId。登录返回路径保存SKU/数量；仅首次恢复查询选择，超限保留原数量且禁买，不静默缩量，之后刷新不重新套用旧选择。重复点击阻止重复在途写入；失败要求刷新，不自动重试或换规格。身份变更与路由离开清理旧详情，迟到成功不能跳到旧结算。

新增可复用pcSeckillFixture，沿用一次性PGlite/专用CI PG16夹具，挂载真实秒杀index/list/detail和购物车add/list控制器；没有监听端口、没有生产认证替代入口、没有订单创建/支付路由。6项新测试回读真实HTTP/SQL：时段对象与两场次ID、空第二页、活动红6.25/蓝8.75与上限3/2、活动unique→当前用户基础SKU购物车、停父后旧选择拒绝且无额外写入、起点含/终点排除、错误形状/身份/价格/数量/图片等边界。相关8文件256项26.29秒通过、零跳过；Worker unit/runtime双类型通过。PC最终build包含27项真实Axios/Pinia会话测试、vue-tsc及Vite构建通过；原有VueUse注释标记告警仍在，非应用运行错误，未新增依赖或修改CI门禁。

前端测试技能要求渲染验收，不把构建当页面通过。当前没有列出的Browser/browser技能（Browser plugin not available），使用已安装Playwright1.62.1及本机Chrome headless，不安装新浏览器依赖。测试URL固定http://127.0.0.1:5189；Vite代理显式设为127.0.0.1:9避免回退生产，浏览器拦截/API桥接到同进程一次性SQL，非同源请求一律拒绝。截图与临时脚本仅放用户Temp，未提交。桌面1280×900及窄屏390×844检查身份、非空、无框架遮罩、无控制台error/warn、无水平溢出，截图人工复核通过；样本图为隔离占位图，不是生产素材。

真实页面交互验证：切4→8且等待旧响应实际完成后新时段不被覆盖；额外20条隔离活动形成20→1→20分页；列表点击20而非基础70；蓝规格8.75、数量3禁买/2可买；匿名进入登录且返回路径保留actblu20/2，模拟夹具身份后恢复；在真实加购响应暂缓时重复点击只生成一条cart（uid11、type1、activity20、base unique qablue01、数量2），库存不预留、订单为0，结算URL身份正确。随后数据库停父，旧已展示选择被真实加购接口拒绝且cart仍1；刷新显示停用，恢复后可重新选。浏览器注入短截止仅验证客户端到期刷新提示，不算数据库截止证据。旧详情离页后释放并等待响应完成不能污染活动21；注入列表业务失败不伪装空列表且可重试，旧数组合同明确报错后可恢复；返回数量超过新上限时保留3且禁买，刷新不偷偷恢复旧规格。

验收边界：认证采用固定隔离身份，并非真实登录；/pc/get_appid、/address/list、/store/list返回明确未纳入本夹具的业务拒绝，不算这些接口或完整checkout通过。未进入确认报价、提交订单、支付、provider或生产。当前已部署旧API仍返回旧时间数组，不能单独发布本前端后就宣称兼容；发布需核对同批后端合同与既有门禁。A3a仍待自身Linux，A3整体还包括PC拼团/砍价、UniApp活动规格及完整结算/真实角色/发布验收；旧站历史复制继续N/A。

### 365755c自身Linux验收与A3a过期返回、A3b UniApp入口（2026-09-08，本地候选）

`365755c4b40286fb60436ee54afb6ec09546cf75`的[Actions34174222066](https://github.com/cinagroup/cinashop/actions/runs/34174222066)终态11/11成功。分片一101900321269为141文件1,073项、435.05秒；分片二101900321409为141文件1,033项、510.39秒；合计282文件2,106项零跳过。两片inventory为025b2436170a7f726873df01e996dbab6abfa57e187c7dc5de5a249734fdb60f，执行文件摘要分别d99702b107c8f44ddd841a53e2c67c5e783e610ff5f18da4f24bff2240b75c8a、2037c4bb78c66cb92180053d71d3c5756c8d4873f3564d3d6a94f1ae0ce43596，完整互斥及实际匹配均true。pc-seckill-purchase明确6项、950ms实际执行；workerd、五端、PG16目录、secret与汇总门禁同批通过。此证据只属于365755c，不借给本轮修复；A3a/A3b继续开放，新清单217勾选/151开放/368项。

并行复核发现原PC测试漏掉已登录但令牌过期：原URL红规格1件，用户改蓝规格2件，加购返回410002后请求层按未更新URL返回红1。实际浏览器复现后，在购买前设置buying、校验输入、用router.replace保存当前显式选择（保留其他query/hash）；异步导航后再次核对revision、完整路径、会话和截止，再发送cart。导航取消、换身份、刷新、离页或截止均不得写入；迟到cart也不得跳旧结算。新增11项实际SFC setup、Vue生命周期/VueRouter及真实Axios请求层测试，PC总会话38/38、类型检查和1858模块构建通过；1280×900及390×844过期返回蓝2复验通过。没有修改全局认证/请求层；仍保留两条既有VueUse PURE注释构建告警。

UniApp秒杀之前使用原始any详情与仅基础SKU的ActivityPurchase，不能据普通商品选规宣称活动迁移完成。本轮复用共用严格目录解析器，新增api/seckill与useSeckillPurchase；列表改用时段ID、独立选中态、20项分页，错误不伪装为空，同页重试；切时段/切tab/hide/unload用revision隔离迟到响应。详情只接受view=skus、子活动/商品身份一致且唯一规格目录，不默认选择或回退基础SKU。数量输入显式转整数，排期按北京时间状态和截止提示，目录价只作参考，购买资格/价格/库存仍由后端裁决。

生命周期维护独立读写代次、会话版本与只用于登录返回的显式SKU/数量意图。普通hide清详情，onShow重读；匿名登录及当前会话410002返回才恢复意图，目录变化重新确认，限额缩小保留原数量并禁买。写入只发new=1/type=1/活动unique/子活动ID，无客户端金额；迟到旧身份或旧页面结果不导航。导航失败保留已创建cart并允许继续结算，不重复加购。复核又以延迟native navigateTo回调真实复现匿名/prepared窗口的重复跳转，新增navigating与单调navigationRevision：成功到hide前仍锁定、当前失败才解锁，旧回调不得影响新身份/新导航。按钮/规格/数量/刷新同步反映导航门禁。结算页面用inheritAttrs:false阻止已由onLoad验证的路由参数落入fragment DOM，不改变结算合同。

新增20项实际Vue/Pinia/请求/API解析/composable或列表SFC setup测试，包含13项详情基础边界、4项实际列表分页/迟到/失败/生命周期以及3项延迟导航隔离；仅native IO和生命周期触发被替换，不冒充真机。完整工具链72项、零跳过，UniApp类型、H5/MP-WEIXIN/APP三构建与3项产物验证通过；早期65/69项为中间结果。相关Worker7文件233项、18.01秒、零跳过，显式清空TEST_FINANCE_POSTGRES_URL使用本地一次性PGlite，未执行PG独立连接专项或连接任何生产库。本轮Worker业务/schema/依赖/CI均未更改。

前端测试技能要求渲染和实际交互，本会话未列出Browser/browser技能，延续已安装Playwright1.62.1+本机Chrome；无新依赖。H5固定127.0.0.1:5190，代理设127.0.0.1:9，浏览器所有API拦截桥接到365755c的真实Hono/SQL夹具，非同源请求拒绝；登录为合成响应和固定隔离身份，登录页自身输入、返回及会话处理真实执行。390×844/1280×900截图人工复核非空、布局无横溢、无框架遮罩。交互覆盖旧slot4延迟后slot8保留、业务错误同页重试、蓝规格数量3禁买/2可买、匿名蓝2和过期红3登录返回、在途重复点击单次add、结算URL保留type1/child20、返回重读清旧选规、数据库停父后旧选择被真实服务拒绝且cart不增加、旧detail20离开后不得覆盖21。SQL最终恰1条uid11/type1/activity20/base qablue01/数量2购物记录，订单0；没有库存预留或付款。

验收过程保留限制：早期脚本将UniApp自定义input的占位view当原生placeholder，及将含“重试”按钮的容器当独立精确文本，修正定位后继续；一次只等结算URL便立即history.back未取得详情重显，增加真实确认页可见栅栏后完整链路连续三次通过，不据此宣称所有快速系统返回/真机动画竞态已验收。结算fragment路由属性警告已修复并复验；每次仍有一条Vue Router深导入弃用警告，定位到已安装@dcloudio/vite-plugin-uni/dist/configResolved/plugins/resolveId.js的既有alias，单独记录，不把控制台宣称零警告。应用error为0，外部请求为0。/diy/get_suspended、/address/list、/store/list明确返回未纳入夹具的业务拒绝，不能算这些接口或完整checkout通过。截图/脚本只在用户Temp，未提交。
...
### 74648e7自身CI与A3d UniApp拼团入口（2026-09-08，本地候选）

精确SHA `74648e7708609e0494b33e2347781f7972477298` / [Actions34178371997](https://github.com/cinagroup/cinashop/actions/runs/34178371997)终态11/11成功，未取消或重跑。单元分片101912206937/101912206885各142文件，分别1,092/1,040项，合计284文件2,132项，零跳过。共同inventorySha256为47fc5898d735a046cd7372fa8b0fd8c26a8c5db822072012c91afd9f8569e18e，executedFileSha256分别0da983dbc6d620f12ab330a54f35ed9a946878a3cbcc10ae3deb8d011818e2ab、77ab31fa99e37ff542665c5f162b14fc66d115b91dc274f2ad510aa6d04ee37d；完整互斥/实际执行匹配均true。五端、workerd、PG16目录、secret和汇总门禁全部成功。A3c按代码/隔离候选勾选，不能借给本轮UniApp改动。

审计旧pink端点时，沿PHP StoreCombinationServices.getPinkInfo、StorePinkServices.getCurrentPink/getPinkMemberAndPinkK追踪确认：参数是团成员/团长记录ID，包含k_id解析、本人订单、退款和成败状态；PHP GET还会触发完成/退款等写副作用。当前Worker却把它解释为活动ID，返回形状也不兼容。进一步发现新UniApp activity/detail同样依赖这个错误形状，因此本批先解除该依赖，未直接改动旧端点，防止修复合同同时破坏现有调用者。API-006-ACTIVITY-PINK继续开放，未来需把安全只读投影与状态变更职责分离。

新增api/combination.ts及useCombinationPurchase，复用已验证的view=skus严格目录、活动SKU载荷和checkout参数生成器。活动30、基础商品70、团长400和活动unique/基础unique保持分离；明确选SKU/数量/开团或指定团，没有默认规格、客户端价格提交或将购买件数作为团员人数。登录前保存最新意图，匿名/过期返回先重新读取指定团资格；库存缩小时不静默改数量，满员/已加入/有待付/过期不自动开团。隐藏、卸载、身份版本变化和旧异步/原生导航回调均隔离；已生成购物记录但导航失败只重试导航。列表切换、隐藏和重入重新取20项分页，失败保留同页可重试，不显示虚假空态。

旧goods_combination_status/index?id的导航别名改为pinkRecordId，冷启动明确显示“旧拼团状态链接尚未迁移”，无活动查询且禁购，不再错误购买同ID活动。这是临时保护，不是旧状态页完成：旧151页/新60页、97别名（60候选/37部分替代）和51缺口原数不变。浏览器另外观察到同一路径只修改hash查询时H5复用旧实例，不触发onLoad；本次冷启动拦截需reload才验证，运行中深链接重入的完整覆盖仍保留在旧状态页任务，不夸称所有导航形态已覆盖。

新增27项实际Vue/Pinia/request/composable及活动列表SFC回归，覆盖组ID与活动ID不同、每单一席、两类登录恢复、无效团、显式放弃、同token续期、实例重用、点击瞬间截止、重复加购/导航、晚到结果及分页错误。完整UniApp toolchain最终99项全部通过、零跳过，vue-tsc通过；H5、MP-WEIXIN、APP三目标构建成功，产物检查3项通过。Worker纯合同/UniApp路由回归2文件10项通过，audit:uniapp通过。未改Worker逻辑、SQL、依赖版本或CI门禁；本轮仍待自己的Linux CI，不以本地成功关闭A3d。

前端测试技能要求实际页面闭环，因此使用可用CUA浏览器，未启用其他浏览器自动化后备。H5仅监听127.0.0.1:5190，API只代理到5192的一次性PGlite/Hono真实拼团/购物车夹具；TEST_FINANCE_POSTGRES_URL显式清空，认证是合成响应，未使用真实账号。默认桌面1265×712及390×844截图、页面标题/URL/非空内容、无框架错误层均确认；手机innerWidth390、document scrollWidth375，无横向越界。控制台无error，只有DCloud依赖内vue-router深路径导入弃用warning（重载后重复），未冒称零告警。

浏览器交互证据：蓝色数量3禁购，改2并选团400，真实登录页面返回仍蓝2/团400，后台重新GET带pink_id=400；参团后URL为confirm?mode=buy&cartId=1&type=3&combinationId=30&pinkId=400，确认页仅显示蓝色2件。配送/门店API明确不在夹具范围，页面待报价，未点击提交或支付，不据此声称完整结算通过。返回后清空SKU并重新读取资格；测试控制器模拟满员后保留指定团且禁参，明确选开新团/红色3件后注入410002，重新登录返回红3/开团并重读目录。随后直接将活动下架，真实cart服务拒绝，页面清除失效选择。列表模拟失败显示错误而非空态，重试仍page1/limit20并恢复正确标题；不存在的pinkId999须显式放弃才取正常目录，恢复后无选中SKU且禁购。截图已在任务中展示。
...
### faaa441实际PG失败修复与旧拼团状态页（2026-09-08，本地候选）

上一批`faaa44185c397c4794b71289379d410ad4466986`的[Actions34181942154](https://github.com/cinagroup/cinashop/actions/runs/34181942154)终态9/11成功：分片一101922639029为143文件、1,102通过/19失败、654.12秒，19项均在新legacy-pink-status-postgres文件；分片二101922639016为142文件、1,040通过、541.28秒、零跳过，已包含原38页DIY守卫，故819b447遗漏组件已取得实际回归证据。分片二inventory285文件、SHA df7de524d75e74a70fe52be4f7022c5ae73260d11665b843b0566193250e1039，executedFileSha256 77ab31fa99e37ff542665c5f162b14fc66d115b91dc274f2ad510aa6d04ee37d，实际匹配/完整互斥true；未用此单片证明整批通过。workerd、五前端、PG16目录和secret成功，单元一与汇总101924923884失败。没有重跑或取消来隐藏失败。

失败日志明确为postgres-js绑定timestamptz参数时收到Date，报ERR_INVALID_ARG_TYPE；不是只读事务或行级权限问题。LegacyPinkStatusService将now直接插入原始SQL模板，绕过Drizzle列mapToDriverValue；PGlite接受该输入，不能代表生产同类驱动。新回归监听真实start/stop列编码，旧代码调用0次，修复前1失败/30被-t筛掉；改用or(isNull(column),lte/gte(column,now))后，主活动和推荐两次查询均通过真实列编码得到ISO字符串。保留nullable和包含端点语义，不改事务/可见性/退款逻辑。新增前端解析器读取真实SQL投影的合同测试；最终该文件31项，相关6文件60项零跳过、43.77秒，Worker unit/runtime两套4GB类型检查通过。此处依然是本地PGlite加真实编码证据，修复后的专用PG16尚需新提交CI证明。

旧UniApp页面原位注册pages/activity/goods_combination_status/index，直接支持旧id及扫码scene中的id，不再仅靠跳错误活动详情的别名。新增共用pinkStatus边界解析器及usePinkStatus：严格正int32、重复/冲突ID、成员去重及团/活动/商品归属、当前用户匹配、人数/布尔/持久化状态一致性、推荐数量与图片协议；错误不降级成同ID活动，私有状态不落本地存储。H5监听当前状态路由的hashchange，冷启动、运行中同页query切换、重复ID都重新解析并使旧请求失效；hide/unload和同token会话更新清除旧用户投影，离页停止计时与事件监听。

页面展示实际成员、剩余人数和倒计时；倒计时归零只转等待结算确认，不制造拼团成功、退款完成或虚拟成员。参团显式跳活动规格页，传activity30及leader400而不是member401；下一页重新取指定团资格与待付预占，不把状态页count当可预留席位。重新开团明确舍弃旧团ID；本人订单只用经过归属验证的orderId并编码URL。活动已下架或状态失败时仍提供我的订单入口。提供最多20条推荐及截断说明、复制团长邀请链接和小程序分享回调，邀请中不附token/用户/订单；不声称微信H5/App provider及推广归因已完成。旧页的直接取消拼团/退款确认尚未重建，当前售后入口不作为其替代验收，也不关闭生命周期/超时维护及完整交易链路。

新增26项真实Vue/Pinia/request/composable及实际SFC setup回归，覆盖匿名/过期登录返回、同token续期、隐藏与卸载、迟到响应、原生导航失败与旧回调、当前订单安全编码、终态/待结算、点击瞬间截止、推荐/邀请、坏合同和H5重复/同页hash、下架订单查询路径。原124项是中间结果，最后toolchain125项零跳过、5,814.81ms通过。清单与审计同步重新计数：旧151条不变、新61页、直接注册4、兼容规则96（60候选/36部分替代）、缺口51；新路径由部分替代规则转为原位注册，不意味着该业务域完整。manifest LF哈希6AE096ACDB2788BFE3C24EE36CFA3155CA3E4A21E187E426C34DCD0862004446，audit:uniapp通过。首次回归因静态测试仍写60页而失败，按实际新页面更新到61、保留严格分母和逐路由互斥，不删断言。

前端测试技能要求浏览器实际闭环，使用CUA、未启用其他自动化后备。仅5190 H5→5192一次性Hono/PGlite，真实pink/detail控制器、规范排序的9表快照，合成登录响应且不挂cart/建单/支付/provider。首个已登录样本沿用本机之前夹具token，随后通过实际用户页退出，再从状态页登录按钮输入明确合成样本，真实登录页面返回401并重新GET；logout和operator/profile不在夹具，显示本机已退出/服务端撤销未确认，不作为生产登录或撤销验收。临时脚本仅Temp；初次esbuild因沙箱不能读取Temp上级元数据失败，经授权构建成功，无依赖安装。日期修复后主动停止旧夹具、重打包并启全新只读样本，最终重载验证，不因观察超时盲目重启。

CUA验证401→团长400/活动30→真实view=skus&pink_id=400目录，明确无SKU默认选中，未加购；返回状态页重读。运行中仅改hash到420显示等待结算且无参团/邀请；410显示成功和本人订单，点击只到orderId=isolated-own-success；430显示失败并说明退款到账以订单为准。999显示记录不存在，重复id=401&id=410显示无效且不查询同ID活动；匿名状态不显示成员，登录后回401。默认桌面918×780和390×844截图复核非空、无框架错误层；调整视口后DCloud页面曾缩放不正确，手机显式重载后innerWidth/clientWidth/scrollWidth均390，按钮可见无横溢，不能据此声称任意热调整视口均通过。复制按钮提示成功，单元测试确认链接组成为团长ID；浏览器clipboard.readText未取得文字，故不宣称跨应用粘贴已验证。

控制台保留DCloud vue-router深导入告警；访问未纳入的订单详情产生预期业务拒绝及“订单加载失败”console error，并观察到该旧详情页orderId fragment属性warning。它们不来自新状态投影，但明确不把整段控制台称为零错误，也不把只有订单URL当订单详情/售后通过。第一轮交互9表规范快照unchanged=true，日期修复后全新夹具重载再次true；没有任何购物记录/新订单/库存或资金写入。所有素材为灰色隔离占位，不是实际运营素材。Workers/PostgreSQL技能影响本轮优先处理真实驱动序列化，而非修改PG测试；官方best-practices及当日npm latest workers-types5.20260908.1已复核，配置/依赖/生产Hyperdrive不改。原历史复制仍N/A，清单220勾选/151开放/371项，待新候选CI和剩余业务合同完成。

本批冻结补验：125项完整前端工具链、vue-tsc、H5/MP-WEIXIN/APP三端最终构建及3项产物测试均通过。新增“我的订单”后显式重载手机页面，三个工具栏按钮与参团按钮均可见，390/390/390宽度无横溢；日期修复后的全新夹具最终8次GET、9表规范快照仍unchanged=true。自建浏览器标签已关闭、视口已重置、5190/5192两服务已停止。git diff --check通过，清单仍220/151/371，无生产操作。

### c066371全绿验收与取消拼团授权边界（2026-09-08，本轮候选）

`c0663716287af67e11903a2dfec452cd01545009` / [Actions34183548166](https://github.com/cinagroup/cinashop/actions/runs/34183548166)已终态11/11成功，包含workerd、五前端、PG16迁移目录、secret及单元汇总101929434550。分片一101927290465为143文件1,123项、651.72秒；分片二101927290434为142文件1,040项、528.39秒；共285文件2,163项零跳过。inventory SHA df7de524d75e74a70fe52be4f7022c5ae73260d11665b843b0566193250e1039；两片executed SHA分别1d51c09221179bf99fde37c9ea5c483b21d10fa7492f9a34820b5ec333962907和77ab31fa99e37ff542665c5f162b14fc66d115b91dc274f2ad510aa6d04ee37d，完整互斥及实际匹配均true。此前819b447/faaa441失败仍保留，最新全绿基线由74648e7前移至c066371；A3d仅按其明示代码/隔离候选口径勾选，清单221勾选/150开放/371总项，不代表业务域或生产完成。

继续比对PHP StorePinkServices::removePink（560起）：本人未截止团长、人数未满、整单退款和后续团长转移属于旧合同。Worker取消入口此前只以Number(orderIdKey)查到本人已付订单，缺少type3/活动/团长/订单号/删除及履约状态关联；并任取该订单一张活跃售后直接调用agreeRefund。新增一次性SQL反例插入已有1元部分售后，旧代码实际将其送入被拦截的批准入口并返回PROCESSING，回归应拒绝却resolve；这是明确红灯证据，不涉及生产退款。首次反例因JS与SQL时钟样本差异提前查不到团，改用数据库NOW()+1day生成截止后才取得有效反例，不将首次失败计为漏洞复现。

本轮ActivityJoinService使用只读REPEATABLE READ前置快照，查有效本人、严格正int32及规范十进制订单键，交叉验证订单号、本人、type3、活动、团长、paid1、status0、未删除和refundStatus0；截止缺失/已过、人数无效或满员拒绝。保留无后继成员时待付订单保护，排除系统已删除预占。不改PHP历史数据，非规范键拒绝而非猜测纠正。退款使用稳定pink_cancel_团ID_订单ID的专用申请，复用核心订单锁下的幂等创建及权威全额报价；已有其他活跃售后报错，不自动同意。调用agreeRefund时绑定本人/订单/退款单号/全额与可见已付条件，由已有短事务执行边界复核；未把provider请求放入前置事务。控制器不再将null/数组/坏ID断言为有效JSON，响应包含private,no-store。

新增39项实际Hono/一次性SQL/真实退款申请测试：十类订单不匹配、七类坏键、已有部分售后、六类团不可取消、满员延迟状态、停用账号、待付保护/系统删除、同一专用申请重放、processing响应、八类坏JSON和匿名边界。SystemConfig仅固定隔离售后期限0，agreeRefund明确拦截并返回PROCESSING；没有伪装余额到账或渠道调用。本轮5文件83项零跳过、68.07秒通过，另3文件18项通过；unit/runtime双类型通过。曾因测试MockInstance泛型声明失败，修正后两套类型通过；另一次误用不存在的测试文件过滤返回No test files，按rg发现实际文件后重跑。源码长度变化使既有索引定位702失效，按同一原SQL实际位置改为726，原严格断言及全部目录合同保留，无DDL/索引变更。

范围仍有限：前置一致快照不等于对参团/支付/超时的原子取消预留，退款申请后到执行间的资格变化尚未完整消除；处理中越过截止、已退款/已换团长的重复请求仍不能当作完整终态回读。取消掉专用退款后不新建第二张隐式申请，需后续明确恢复合同。尚需把这些并发/失败恢复连同页面确认、本人订单状态与维护任务一起验收，当前未开放前端取消按钮、不关闭API-006-ACTIVITY-PINK父项。Workers/PostgreSQL技能要求查当前官方规则与5.20260908.1类型、使用一致读和短事务；未升级依赖、未变更Hyperdrive、未启动浏览器或连接生产。旧历史复制仍N/A。本轮候选必须取得自身CI，不能借用c066371的全绿。

### 74e22c3全绿与取消退款结果恢复（2026-09-08，本轮候选）

`74e22c32eb9f44119498eff9b0e889deb56ee451` / [Actions34184620546](https://github.com/cinagroup/cinashop/actions/runs/34184620546)终态11/11成功，最新全绿基线前移。分片一101930345918为143文件1,123项、494.40秒，分片二101930345974为143文件1,079项、546.85秒，共286文件2,202项零跳过；汇总101932008221成功。两片inventory SHA均636173398ceef2c22c913e1d8274bec02bbd0c58802560a4caa1af4f0c5371c3；executed SHA分别1d51c09221179bf99fde37c9ea5c483b21d10fa7492f9a34820b5ec333962907、e3cca0280f975903ce2a0aa45c06c5d59d4265a6107e6de52b30ed00d4b9c397，完整互斥及实际文件匹配均true。此前失败与上一轮受限证据保留，不重写。

新反例通过实际Hono、SQL申请及余额执行完成第一次取消，然后向同一POST重放；临时用apply_patch恢复HEAD中的旧removePink方法，第一笔真实退款成功，但重放返回业务400而非确认成功，1项失败/5项因-t筛掉。finally恢复本轮方法后再跑完整测试。旧方法不是重复扣款，而是先按“尚可取消的团”过滤，导致已完成、过期或已更换团长时读不到原申请。新方法先在只读REPEATABLE READ快照内确认有效账号、原团长、规范订单键、订单号/本人/type3/活动/团/已付/可见身份，再定位唯一pink_cancel_团ID_订单ID。复核全额、件数、原因、类型；已撤销/删除/拒绝不重开，完成态要求订单全退、原团长自引用退款标记和已退金额一致。首次无申请仍执行原严格资格/人数/待付保护；已有申请进入同一refundId的核心锁定执行，完成重放无provider及重复补偿。

新增测试使用本机一次性PGlite，CI可切换专用loopback PostgreSQL16。余额路径没有mock申请、agreeRefund、库存、流水或团生命周期：验证6.25元一次入账、订单全退、明细退款件数1、基础商品/基础SKU/活动SKU/活动主记录四层回补，重复HTTP请求的规范结果快照不变；团长退款后保留成员转为新团长，成员订单pinkId重连且未退款。微信只替换requestRefund/queryRefund传输结果，明确不是实际渠道验收；PROCESSING和UNKNOWN在截止后查询原outRefundNo，确认SUCCESS后才提交补偿，不重发第二笔请求、不错误加余额。坏规格让最终事务回滚用户/团/订单/库存/明细/流水，保留唯一已提交申请；修复隔离快照后仍用原申请恢复。撤销、篡改金额/件数/身份/原因/完成证据均拒绝。

新增独立PG16重叠恢复用例：第三连接持有用户行锁，观察第一取消执行被其阻塞；在申请已提交但余额事务尚未完成时，将隔离团截止更新为过去，第二恢复必须等待第一退款锁，最后两请求均成功而资金/库存/明细只补偿一次。等待依据pg_blocking_pids而非sleep。PGlite不支持独立后端锁证明，因此本地明确跳过这一项，CI必须实际运行且零跳过才可采信；未使用生产数据库证明并发。

审计进一步确认原子受理仍是独立未完成条件：当前前置读快照与applyRefund写事务分离，reservePinkJoin/activatePaidPink没有取消意图门禁。不得仅加一个“处理中”页面就称并发已修复。锁顺序也须联审：建单先活动/SKU再团长，支付先订单及资金再团长，退款先订单及资金、团长/成员及其他订单，最后恢复库存；这是待独立PG定位和统一锁序的风险，不在没有反例时宣称已证实死锁。已扣款回调当前存在旧团失效后新开团的恢复分支，不能用简单抛错替代而丢掉已扣款状态。下一步须将首次取消原子受理、支付/预占、超时维护、无人干预恢复一起完成，不将本轮结果回读当替代终点。

后续A项以交错顺序逐项证明，不以单请求测试或终态截图替代：

| 竞争/故障顺序 | 必须证明的结果 | 当前证据边界 |
|---|---|---|
| 取消先受理，新参团后到达 | 不得继续占用将退出的团长席位；预占/库存一起回滚或进入明确定义的后继团合同 | 缺少取消意图原子门禁 |
| 支付先使原团满员，取消后到达 | 取消拒绝且不创建退款申请 | 仅有顺序满员校验，独立PG交错待补 |
| 已扣款回调晚于取消受理 | 金额不丢失，不把已扣款订单长期保留为未付；团归属变化有明确可回读结果 | 已有失效后新开团分支，但与取消受理未整体验证 |
| 超时维护与取消争用同一团 | 不重复退款、不错过其他成员，不把失败团重新标为成功 | 当前核心可复用活跃售后，跨任务互斥待补 |
| 申请提交后进程中断/渠道结果未知 | 只恢复原退款单，查询先于必要重发；无人访问页面也能最终处理 | 本轮人工重试有SQL/模拟渠道证明，无人干预恢复待补 |
| 显式撤销售后/人工拒绝 | 不偷偷新建申请；参团资格与取消意图释放遵守同一状态机 | 本轮拒绝隐式重开，原子受理后的释放合同待补 |

将既有父项内剩余条件明确拆为API-006-ACTIVITY-PINK-A/B/C三个开放子项，清单221勾选/153开放/374项，原父项及A3整体仍开放。没有新增状态4、改写既有状态含义、修改provider/支付/退款核心、新增DDL或发布前端取消按钮。Workers/PostgreSQL技能要求当前官方规则及5.20260908.1类型核验、一致读及不在数据库锁内调用provider；生产Hyperdrive和依赖不变，历史复制仍N/A。现有索引查询未改，源码定位随方法增量由726更新为760，严格定位断言保留。

本批冻结验证：新增文件最终16项（15项本地通过、1项仅PG16独立连接运行），相关8文件共113通过/1项明确跳过、76.35秒；所有比较按主键/UID规范排序，不把物理行顺序当一致性。Worker unit/runtime两套类型通过，快照排序补充后再次unit类型通过。初次类型检查因新测试未使用eq导入失败，该导入现由真实PG测试的指定团UPDATE使用，未关闭noUnusedLocals。git diff --check通过；上一74e22c3已终态成功后才准备推送本轮，不取消在途CI。新候选自己的PG16/workerd/全分片仍待验证，清单221/153/374，未操作生产或启动前端/浏览器。

### 78abdec全绿与取消原子受理第一片（2026-09-08，当前候选）

`78abdec686201415e23ed39c927ce67e2c6c4c44` / [Actions34185757025](https://github.com/cinagroup/cinashop/actions/runs/34185757025)已终态11/11成功，成为最新全绿基线。分片一101933635298为144文件1,139项、632.12秒，分片二101933635152为143文件1,079项、545.84秒，合计287文件2,218项零跳过；汇总101935806512成功。inventory SHA两片均为6aca5d9dce242e7f7e3dceae300e67af8da2f74c6b18e04be71fdbd289904a30；executed SHA分别45615d0bb7b6aca5b06d9f270bccdb857ffab5520169f1d8b381c4fbe25b7949、e3cca0280f975903ce2a0aa45c06c5d59d4265a6107e6de52b30ed00d4b9c397，完整互斥及实际文件匹配均true。恢复文件16项在该SHA自身PG16全部通过，日志明确执行独立连接跨截止重叠请求；模拟微信仅替换渠道传输，不是正式provider验收。74e22c3/c066371和更早失败记录保留。

本轮实现的是A的原子受理及付款准入切片，不是完整取消业务：

- StoreOrderRefundService增加内部SQL授权回调，在新申请分支持有订单锁后执行，专用已提交申请重放不重复首次资格判断。ActivityJoinService的预检保留，但其结果不再是申请写入前的最后检查。
- PinkCancellationIntent在同一事务内锁定团长，重验订单/用户/团长身份、可见性、状态、满员和无继任者的未付款预留。截止时间在实际取得团锁之后的另一条SQL用clock_timestamp检查，不能使用事务开始的NOW或锁等待前的时间结果。团锁一直保留到退款申请和申请日志提交。
- 以既有专用退款行pink_cancel_<pinkId>_<orderId>作为持久取消意图；仅匹配本人订单、未撤销/未删除及活动退款类型0/1/2/4/5，不新增持久化状态4或DDL。参团预留及支付前检查拒绝活动意图；明确撤销、删除、拒绝及无关申请不冒充取消处理中，但原取消恢复仍不隐式创建替代申请。
- 余额付款在最后持团锁的激活阶段再次遇到失效团/取消意图时抛错，整个余额、流水和paid事务回滚，要求用户重新确认。外部已扣款回调则沿用既有失效团新开团策略保留付款和交易号，不将该订单加入取消中的旧团；重复回调只保留一次支付事件。没有在数据库锁内增加provider调用。

新增回归证据：授权文件新增6项预检后数据变化（过期、满员、退款、订单改链、账号停用、新待付预留），实际调用退款申请核心，确认没有申请/日志/执行。付款新文件10项本地SQL覆盖活动退款各状态、撤销/删除/拒绝/无关申请、余额入账和已经扣款的回调新团承接/重复幂等。新增4项仅PG16独立连接测试分别覆盖等待团锁跨截止、取消先取得团锁而预留等待、预留先提交而取消等待、余额预检后取消提交导致资金事务回滚；以pg_blocking_pids观察准确阻塞者，不把Promise并发或本地PGlite当锁证明。预留测试仅覆盖实际reservePinkJoin及同事务预留订单插入，不冒充完整建单库存链；退款执行在授权测试中仍明确拦截。

仍开放的审计结论：建单活动/SKU→团长与退款订单/用户→团长/成员/其他订单→库存存在锁顺序风险，支付持自身订单而退款改链其他订单也需真实双方向并发验证。本轮不把未经证明的批量订单锁加到退款核心，因为现有订单advisory锁及行锁顺序也必须一起处理。完整退款/支付回调/实际建单/超时维护互斥、取消意图在状态和推荐只读投影的显示、无人干预恢复、真实渠道及C页面确认入口仍未完成。A/B/C、旧状态父项与A3均保持开放，清单仍221勾选/153开放/374项。

冻结验证：最终相关10文件151项通过、5项明确仅PG16运行跳过，94.72秒（其中1项是上一恢复文件已有的PG16并发测试）。较早6文件回归出现1项源码定位失败，原因是PinkLifecycleService新增代码使既有索引消费者行号340→356；ActivityJoinService定位760→764也同步更新。没有更改索引SQL或弱化源码精确断言，单独5项定位回归及最终10文件重跑全部通过。当前新候选的独立PG16/workerd/全分片仍待自身CI，不借用78abdec的全绿。Workers/PostgreSQL技能要求的当前官方规则、5.20260908.1类型及本地Wrangler Hyperdrive schema已核验；不升级依赖、不改生产Hyperdrive、不新增DDL，不启动浏览器或前端，不操作生产数据，历史复制继续N/A。

本轮最终Worker unit/runtime两套类型检查及git diff --check均通过；上一78abdec已经终态成功后才提交本轮，不取消前一批在途CI。

### fc188d2全绿与拼团退款锁顺序（2026-09-08，当前候选）

`fc188d28087a50177656e0048fc268543031e0a5` / [Actions34187151345](https://github.com/cinagroup/cinashop/actions/runs/34187151345)终态11/11成功，最新全绿基线前移。分片一101937681791为144文件1,139项、628.18秒；分片二101937681686为144文件1,099项、337.97秒；汇总101939826048成功，共288文件2,238项零跳过。两片inventory SHA均5000e834a3b1ba93ab69588b35f5466e78afe1cdcfb1291adafa5f6f25e485bb；executed SHA分别45615d0bb7b6aca5b06d9f270bccdb857ffab5520169f1d8b381c4fbe25b7949、39a3b6b37f192e5027c018c9429f1e24e5dcfd775fef9ffe9c602766e64b4189，完整互斥及实际文件匹配均true。授权48项/付款准入11项/退款恢复16项在自己的PG16运行中通过，上一轮新增4项独立连接测试不再只是候选。旧SHA成功和失败记录不改写。

本轮读完整建单、取消、退款和生命周期路径后，没有仅给退款追加一个团锁：此前未付款取消也在后半段回补活动库存，建单先认领购物车再写活动，而成员退款先锁成员再锁团长，与超时任务相反。这些组合都需要一起调整，不能用普通顺序测试证明无死锁。

| 路径 | 当前候选的加锁次序与边界 |
|---|---|
| 拼团建单 | 保留幂等键复查，随后活动NO KEY UPDATE → 购物车认领 → 活动/SKU → 团长预留 → 新订单及基础库存；编号和外部配置准备仍在事务前 |
| 未付款拼团取消 | 先读并检查本人订单 → 活动NO KEY UPDATE → 重新锁订单并复查活动身份 → 既有库存/购物车补偿；非拼团保留原业务补偿路径 |
| 拼团退款完成 | 退款执行锁 → 活动NO KEY UPDATE → 重新读取可能已被转移的订单团ID → 同团订单按ID排序先取settlement advisory再取行锁 → 用户资金 → 团长/成员 → 改链及库存；拒绝活动/团身份在持锁前后变化 |
| 支付与超时 | 支付保留自身订单/用户→活动KEY SHARE→团长；退款活动锁允许它完成，退款必须在持团锁前等齐其订单。成员退款改为团长先于成员，与超时团长→成员相同；不将外部退款调用移入事务 |

选择NO KEY UPDATE而非FOR UPDATE是刻意的：前者与支付KEY SHARE兼容，避免退款持活动锁等待订单、付款持订单等待活动的新环。此兼容关系按[PostgreSQL 16行锁定义](https://www.postgresql.org/docs/16/explicit-locking.html#LOCKING-ROWS)核对；本轮新增独立连接测试用于验证实际代码路径，不以文档关系或静态匹配代替运行证明。已有独立订单advisory→行锁顺序不反转，未引入全局串行锁、生产DDL或provider内部调用。

新增`pink-refund-lock-order-postgres.test.ts`共12项，夹具通过真实建单核心创建团长、已付成员和待付成员，再用真实余额付款激活团记录，不手写paid状态冒充付款。4项本地顺序流程验证团长退款/幂等、未付订单在退款前后取消，以及成员退款后实际超时任务完成余下退款；8项仅独立PG16运行，覆盖余额/已扣款回调先行而退款等订单、退款先行而回调读到新团长、实际建单与退款双方向、未付取消与退款双方向，以及成员退款在团长锁等待时不持成员锁并与实际超时任务衔接。使用pg_blocking_pids确认具体等待者，NOWAIT确认成员未被先锁；不调用支付provider。所有PG专用项都须在新SHA自己的零跳过CI里运行。

本地最终相关9文件97项通过、13项明确跳过，83.40秒；13项包含本轮8项及既有5项PG专用测试。初次类型检查发现测试未使用sql导入及void/string异步结果联合推断错误，已删除未使用导入并统一该两条仅检查成功状态的测试返回void；未关闭类型规则。顺序SQL回归先23通过/9跳过，再97通过/13跳过，两次完整相关回归结果一致。新增代码不影响既有索引查询源码锚点，逐项精确检查无漂移。

本轮仍是锁顺序候选，不能凭fc188d2全绿关闭A。新8项PG证据、完整超时/取消故障矩阵、只读状态和推荐取消中投影、无人干预恢复、正式provider及旧状态页取消确认仍未齐全；A/B/C和父项继续开放。Workers/PostgreSQL技能影响了活动锁模式、短事务和外部I/O边界的处理，当前Workers官方规则及5.20260908.1类型版本已核验，依赖/Hyperdrive配置不变。未操作生产数据、未发布、未启动前端或浏览器，历史数据复制继续N/A；清单221勾选/153开放/374项。

冻结前补充：对非拼团→拼团的持锁期间类型变化增加明确重试拒绝，避免走过非拼团预读后绕过活动/全团订单锁；未付款取消在活动锁之前先检查本人归属，并在订单锁之后再验一次。补充后再次Worker unit/runtime类型全部通过，退款相关3文件24通过/9项仅PG16跳过、29.94秒；所有索引查询源码定位和git diff --check通过。上一fc188d2已终态成功后才准备本轮推送，不取消其CI，新SHA的8项并发证据仍未取得。

### 1f4d3a9 全绿与取消中只读投影（2026-09-08，当前候选）

`1f4d3a9a2f672b216694c4d432754d995f46f7a3` / [Actions34188529838](https://github.com/cinagroup/cinashop/actions/runs/34188529838)终态11/11成功。分片一101941641844：145文件1,146项、655.08秒；分片二101941641882：144文件1,104项、539.64秒；汇总101943889189成功。合计289文件2,250项零跳过。`pink-refund-lock-order-postgres.test.ts`在分片二12项全部通过（8,776ms），包含上一轮8项独立PG16连接测试，不再仅为本地候选。

两片 inventory SHA 均为 `7fd7be2168b0f69e520ac8167be109bd73d8676903cb10bc1e021bb43ef081c9`；executed SHA 分别为 `4a3c030908b623ac1dbada39ae3ef4f8fb5e785bec13f385dd0827544cbedb80`、`6f3ba0f7422ac890b8400d074df60e4ddf2af7abdcaafb9b5a92e9480b8dafb5`。完整互斥及实际文件匹配均 true。旧 SHA 的历史成功和失败不改写。

本轮发现并修复读写合同不一致：准入已拒绝团长取消中的团，状态页及推荐却仍可展示参团。`PinkCancellationIntent`提取共用条件：团长规范订单键、本人 UID、专用申请单号、未撤销/删除，以及退款类型0/1/2/4/5。状态服务内部读订单键但不向响应投影，增加 boolean `cancellation_pending`，仅持续中的团映射为 `settlement_pending`，不虚构退款到账或新状态码。目录通过相关 NOT EXISTS 在 LIMIT 5 之前排除取消中的团，指定该团直接拒绝，不静默转开新团；保持最多6团的批量人数聚合。目录改为与状态服务相同的 READ ONLY REPEATABLE READ 一致快照；已有退款订单键和申请号索引可用，未新增DDL或声明生产性能已验证。

页面严格解析取消中标记，拒绝非boolean及与终态/非待结算矛盾的响应；旧响应缺少标记仍按false兼容。标题显示“团长取消处理中”，提示只能查询本人订单确认到账，参团/复制邀请/原生分享均不能继续邀请到旧团，刷新仍只有GET。没有发布取消确认按钮、修改支付/退款执行核心或调用provider。

验证：本地PGlite状态/目录2文件65项通过（67.09秒），包括5类活跃申请、撤销/删除/拒绝/已完成/外人/非专用申请、畸形订单键、过滤后补足5条、指定团拒绝、隐私和全表快照不变。既有迁移合同2文件8项通过。前端实际composable/SFC运行时28项通过，Worker unit/runtime及UniApp类型检查通过；既有索引源码锚点均精确匹配。当前新增投影仍待自身PG16、workerd和全量CI，不能借用1f4d3a9验收。

浏览器：当前会话没有旧式browser技能，实际可用并使用的是已授权CUA内置浏览器（未安装依赖、未用外部Chrome）。真实本地H5 `http://127.0.0.1:5190/#/pages/activity/goods_combination_status/index?id=440` 代理至5192的隔离PGlite HTTP夹具，未连接默认生产代理。页面标题/记录身份、非空、无框架错误遮罩、取消文案与隐藏参团/邀请通过；点击“刷新状态”后再次检查AX树与截图一致。控制台无error，只有DCloud vue-router直接导入的既有弃用warning。服务端事件为两次pink GET、一次悬浮导航GET、一次样本图片GET，所有快照表（含退款） unchanged=true。仅检查当前内置视口，未重测独立移动视口或真机。首次临时夹具打包因沙箱父目录读取被拒绝，已停止旧bundle进程，改为从已读TS通过stdin/工作目录解析重建成功后才进行浏览器验收；旧bundle不作为本轮证据。临时夹具/产物均在TEMP，不提交仓库。

Workers/PostgreSQL技能影响了只读一致性、共用资格条件及避免逐团查询；前端测试技能要求真实刷新交互、截图、控制台和边界验证。生产数据库、Hyperdrive绑定、依赖及部署均未改动。历史复制仍N/A，清单221勾选/153开放/374项；A/B/C和父项保持开放，后续仍需取消确认、完整后台恢复/故障矩阵及真实角色/provider门禁。

冻结补充：取消受理/付款准入/退款恢复3文件70项通过，5项仅独立PG16运行的用例在本地明确跳过（72.41秒）。前端完整工具链127项零跳过、H5构建均通过。CUA支持的当前视口截图已取得；尝试全页截图时提示 `pinkTab.playwright.screenshot is not a function`，不声称已取得全页或移动端证据。上一1f4d3a9终态成功后才准备本轮推送，不取消其在途CI。

### 取消确认流程的只读结果合同（2026-09-08，当前候选）

继续C前端闭环前确认了接口缺口：原有 `POST /api/combination/remove` 会申请、查询渠道并推进退款执行，不能把重复POST标作“刷新结果”。本轮先新增强制用户授权的 `GET /api/combination/remove/:id?cid=...` 与独立 `PinkCancellationStatusService`；前端取消确认仍是后续工作，不以本端点代替整个C完成。

查询严格绑定原团长记录ID、本人UID、活动ID、规范订单键、订单单号、已付type3和订单可见性；拒绝团员、其他用户或活动ID误用，不跟随退款替换团长。查询无需活动/商品表，因此原团过期、取消后转移团长、活动下架不影响本人结果恢复。响应只返回本人UID、原团/活动ID、本人订单号和状态，不泄漏渠道单号、错误诊断、其他用户订单或退款金额。事务使用 READ ONLY REPEATABLE READ，至多两条记录用于发现重复；不接收Env、不写数据、不查询provider、不自动申请/恢复/完成退款。

| 只读状态 | 持久证据及含义 |
|---|---|
| not_applied | 该快照无专用取消申请；不证明在途或超时POST失败，客户端不得据此丢弃原操作意图 |
| accepted | 申请身份/金额/件数一致，尚无渠道执行记录或状态CREATED；并非已退到账 |
| processing | REQUESTING/PROCESSING，或渠道SUCCESS但本地业务退款尚未完成；GET不补做结算 |
| unknown | 渠道记录UNKNOWN，结果仍待核实，不隐式重新申请 |
| completed | 退款单6、订单全额退款状态/金额、原团长自退款标记一致；原路渠道还须匹配原支付归属、金额与SUCCESS记录 |
| needs_attention | 重复/撤销/拒绝/删除/身份金额矛盾、无有效完成证据或渠道CLOSED/ABNORMAL/FAILED等；不提供普通自动重试 |

`resumable`只描述同一申请可供显式恢复操作重新校验，不是付款/退款授权；GET始终不推进退款。分支付父单时按实际父单金额和本人归属核验渠道不可变字段，不错误要求父单总额等于当前子单退款金额。

新增27项只读SQL/HTTP测试，验证全表（含渠道表）快照不变、无外部fetch、真实事务只读/可重复读设置、各状态、未受理/矛盾记录、身份/数量/金额/父支付单校验及畸形参数。已有真实余额/模拟原路退款恢复测试接入该读模型，覆盖完成后、转移后、处理中、未知及回补失败后的已受理记录；新增真实签名JWT测试证明本人可读，外人、错误角色、无token、无效token及改密后的旧token均拒绝。独立PG16重叠恢复测试增加“申请已提交而余额完成等待锁时GET读accepted、最终提交后读completed”断言，仍待当前SHA自身CI。

本地初次测试发现HTTP信封遗漏msg的断言，以及PGlite execute结果和未完成Drizzle select构造的测试写法问题；补上严格信封、使用完整select/from后专项通过，没有修改业务断言或关闭类型约束。新增签名测试前的4文件49通过/1项PG专用跳过（44.97秒），签名专项另1通过；最终整批结果另记。路由审计当前1,904 PHP/1,650 Worker、881精确匹配/863可执行、18不可用/17有证据退役，比例保持46.3%/45.3%/45.7%；新增读端点是辅助合同，不冒充又迁完一条旧PHP路由。所有既有索引源码锚点无漂移。

Workers/PostgreSQL技能要求的当前官方规则、5.20260908.1类型和Wrangler Hyperdrive schema已核对，网络受限的npm版本查询在获批只读重试后成功。没有依赖升级、DDL、生产访问或部署；没有改动前端、启动浏览器或真实退款。当前目录尚待自身CI，A/B/C及父项仍开放，历史数据复制仍N/A。

冻结验证：最终4文件50项通过、1项仅独立PG16跳过（45.24秒），包含新增27项只读合同和真实签名JWT隔离。Worker unit/runtime两套类型检查、git diff --check及既有查询源码锚点全部通过。PG专用重叠恢复新增断言仍须等待本轮提交自己的CI，不把本地跳过计为通过。

上一提交验收补记：`9c38154b3743fa957282dded79a7b09a5d06ef40` / [Actions34189886057](https://github.com/cinagroup/cinashop/actions/runs/34189886057)已终态11/11成功。分片一101945601090：145文件1,158项、676.38秒；分片二101945601143：144文件1,104项、544.97秒；汇总101948086421成功，共289文件2,262项零跳过。状态37项和目录28项已取得该SHA自身PG16证据。两片inventory SHA均为`7fd7be2168b0f69e520ac8167be109bd73d8676903cb10bc1e021bb43ef081c9`，executed SHA分别为`4a3c030908b623ac1dbada39ae3ef4f8fb5e785bec13f385dd0827544cbedb80`、`6f3ba0f7422ac890b8400d074df60e4ddf2af7abdcaafb9b5a92e9480b8dafb5`；完整互斥和实际文件匹配均true。首次GitHub查询遇到TLS握手超时，重查同一run确认成功；没有重跑或取消任务，也不将上一提交全绿归给本轮新增GET。

### c781f45 全绿与原团取消前端闭环（2026-09-08，前端当前候选）

`c781f45bfd4741ca737ae89942c79a37e851b158` / [Actions34191550362](https://github.com/cinagroup/cinashop/actions/runs/34191550362)终态11/11成功。分片一101950480776：145文件1,159项、654.68秒；分片二101950480814：145文件1,131项、357.71秒；汇总101952941969成功。合计290文件2,290项零跳过。新只读结果27项（6,606ms）及真实退款恢复17项（8,409ms，包括独立PG16等待锁时先读accepted、最终completed）已在该SHA执行，不再是本地候选。两片inventory SHA为`9c2790aa5f8fbf51bf7411f75a1e0486e12f54f8f6a0b3c88661f4133a227bb1`；executed SHA分别为`4a3c030908b623ac1dbada39ae3ef4f8fb5e785bec13f385dd0827544cbedb80`及`3af76756aaa2fcc051037d9629a60cf990b3fe499528cc7f17ce67301b680fd3`，完整互斥和实际文件匹配均true。等待其终态后才准备本轮推送，没有取消在途CI。

源端对照：PHP `view/uniapp/pages/activity/goods_combination_status/index.vue`只向本人团长显示“取消开团”，直接调用`postCombinationRemove({id:pinkId,cid:storeCombination.id})`。本轮恢复独立团长取消入口，并增加明确确认；不继续将普通订单售后作为取消拼团的替代。前端身份严格区分页面原团ID、当前替换团长ID和活动ID，只有本人原团长、有效未成团资格及只读未受理结果才允许首次提交，最终资格仍由后端重验。

新增`view/common/pinkCancellation.ts`严格解析六类持久结果及完成/恢复标记，校验本人/原团/活动和本人订单号；存储仅保留版本/UID/原团ID/活动ID，按UID和原团隔离，不保存token、渠道结果或权限。弹窗确认前、取消弹窗、打开页面、刷新和登录恢复均不POST；确认后先同步保存并回读核验，存储损坏/失败拒绝发送。迟到POST成功不会直接当完成，必须GET读持久证据；未知传输或GET未查到申请不删除原意图。同申请恢复必须再次明确确认，不能把恢复POST称为刷新；撤销/拒绝/矛盾/完成状态不显示普通继续处理。

结果面板独立于旧成员视图：重入时先按存储原身份GET，再查当前团状态，下架/退款/团长转移导致成员视图不可用也不隐藏本人取消结果；跨账号、同token续期、离页、换路由和旧异步回调不能污染新会话或改为取消替换团长。原意图存在时不再参团/分享该团。页面时钟同时失效邀请和首次取消控件，点击及确认回调再检查截止时间。已完成记录保留为重入查询入口，不自动删除或重新申请。

验证：新增23项实际Vue/Pinia/request/composable及SFC运行时测试纳入CI工具链；最终完整工具链150项零跳过（5,691.76ms）、UniApp类型检查通过。覆盖取消弹窗、同步存储/readback失败/损坏、HTTP成功但渠道未完成、未知POST+GET无申请、另一设备已受理记录、下架/转移重入、重复原生回调、在途刷新/导航抑制、跨账号/同token/离页/路由/迟到GET和POST，以及过期/满团/成员/别名拒绝。旧28项状态页回归继续通过。三端构建及产物/i18n检查的最后终态另记；构建不是原生真机或provider验收。

浏览器按前端测试技能验证流程：本地真实H5 `http://127.0.0.1:5190/#/pages/activity/goods_combination_status/index?id=400` → 取消弹窗 → 再次明确确认 → 实际隔离PGlite余额退款 → 点击刷新 → 新标签重入。当前无旧式browser技能，使用已授权CUA内置浏览器和其Playwright定位；未安装依赖或改用外部Chrome。代理明确指向本机5192，夹具只允许原团400/活动30取消，使用合成登录身份和实际业务控制器/余额退款SQL，禁用外部fetch；不是生产角色验证。首次临时打包缺stdin TypeScript loader而失败，补loader后才启动夹具；无旧产物作为证据。临时脚本/产物在TEMP，不提交仓库。

浏览器证据：取消弹窗后POST为0、退款表rowCount为0；明确确认后最终只有1次POST、1条专用申请（refund_type=6）、1条余额退款流水，余额6.25、订单refund_status=2；9次GET（含HMR/刷新/重入）全表快照均unchanged=true，结果顺序not_applied→completed。后来代码HMR仅增加只读请求，不改变该不变量。成员视图显示“订单已退款”时，原团完成结果仍显示且没有继续处理入口。已核对页面标题/URL、非空、无框架遮罩、弹窗与完成截图；控制台无error，仅既有DCloud vue-router直接导入弃用warning。默认语言曾显示Cancel，本轮显式改为“暂不取消”并复核。取得当前CUA视口及新标签1280×720截图，不宣称手机视口/原生真机已经测试；本轮未测试真实微信/支付宝或正式账户。

前端测试技能推动了真实弹窗、确认/取消、刷新、重入、截图/控制台及异步边界验证。本轮未改后端、数据库结构、依赖、生产数据或部署；新前端不能借用c781f45全绿，待自身CI。清单221勾选/153开放/374项，A/B/C及父项仍开放；后台无人干预恢复、完整故障矩阵和真实渠道/发布门禁继续待办，历史复制仍N/A。

冻结补充：最终可见控件时钟修正后，完整工具链150项、UniApp类型、H5/微信小程序/App三端构建再次通过；产物检查3项（151.52ms）及运行时i18n检查9项（25,028.77ms）均终态成功。CUA已取得确认及完成/重入截图，临时5190开发服务与5192隔离数据库服务已停止。git diff --check通过，清单计数仍221/153/374；无生产发布或数据库改动。

### 8a98b50 全绿与已受理取消后台恢复（2026-09-08，后台当前候选）

`8a98b508d27df740473bafa0f632a36423ea099f` / [Actions34192820887](https://github.com/cinagroup/cinashop/actions/runs/34192820887)终态11/11成功。分片一101954195139：145文件1,159项、434.85秒；分片二101954195093：145文件1,131项、577.03秒；汇总101956304227成功。共290文件2,290项零跳过。两片inventory SHA均为`9c2790aa5f8fbf51bf7411f75a1e0486e12f54f8f6a0b3c88661f4133a227bb1`，executed SHA分别为`4a3c030908b623ac1dbada39ae3ef4f8fb5e785bec13f385dd0827544cbedb80`和`3af76756aaa2fcc051037d9629a60cf990b3fe499528cc7f17ce67301b680fd3`；完整互斥及实际文件匹配均true。UniApp job101954195322确认150项零跳过、三端构建及产物/i18n检查通过，不再将上一轮前端称作未验收候选。

本轮审计证实后台缺口：`StoreOrderRefundService.reconcilePendingRefunds`仅从渠道表读取REQUESTING/PROCESSING/UNKNOWN，不能发现专用取消申请已提交但无渠道行、余额回补失败、CREATED或SUCCESS未业务结算。只靠前端显式恢复不能保证用户离页后的处理。新增`PinkCancellationRecoveryService`，并纳入原有五分钟Cron所投递的独立`pink_cancellation_recovery`根任务；复用通用队列校验、重试及DLQ归档/重放，不新增平台绑定或部署。签到时段根任务15个，其他时段14个，测试同步验证序列化和完整任务集合。

恢复范围严格限定为专用前缀、仅退款、固定取消原因/说明、未撤销/删除、未完成且未拒绝的已有申请，且至少早于本轮计划时间60秒。扫描以READ ONLY REPEATABLE READ冻结最大退款ID，按主键游标每页最多执行5项，continuation携带同一最大ID，避免持续新增拖长一轮遍历。普通售后不自动审批。每条再使用本人原团只读合同验证身份、完整金额/件数、重复申请、原支付归属及渠道证据；终态/矛盾/失联记录报告待核对或错误，不自动重开。

执行直接调用原退款ID的`agreeRefund`，不调用`removePink`或`applyRefund`，因此扫描后删除原申请不会创建替代申请。UID/订单/申请号/金额/已支付及可见性在退款核心锁内再次校验；专用原因/说明/类型/件数通过已有授权钩子在同一refund advisory→refund row顺序内复查，没有提前持有活动、订单或用户锁。渠道调用不放进扫描/执行事务。REQUESTING有效租期继续等待；CREATED补首次发送，超租期/UNKNOWN先查原单，NOT_FOUND沿用同一CNSR号，SUCCESS只补本地结算；CLOSED/ABNORMAL/FAILED不走普通自动重试。

单条失败不阻塞后续ID，未完成申请保留在数据库，下一个五分钟根任务重新检查；分页发送失败向队列抛错，已处理结果靠原退款核心幂等重放。输出独立结构化refund事件，包含计数及低基数错误码、不含UID/订单/申请号或异常原文；一般队列成功事件仅表示本页执行结束，退款完成须看completed及持久状态。账号停用、数据不一致、终态渠道仍需运营核对，不声称系统自动修好所有业务错误。

新增32项后台SQL测试：30项本地通过，2项仅独立PG16运行；前者通过真实取消受理后注入执行前中断制造持久申请，再验证后台余额回补、原路发送/查询、未知传输、CREATED/超租期REQUESTING/SUCCESS、60秒宽限、撤销/删除/拒绝/普通售后排除、金额身份件数/重复矛盾、查询后改动的锁内复核、删除不重建、坏行分页、固定最大ID及Queue发送失败重放、日志无敏感标识。仅模拟渠道传输，不访问真实provider。2项PG专项新增后台+HTTP重叠锁等待，以及后台+后台重叠时真实NOWAIT验证渠道I/O期间未持退款/订单/团/用户行锁、活跃REQUESTING不重复发送；本地明确跳过，待新SHA自身CI。

本地验证历程：首批2文件33通过/1跳过；加入锁内变化保护后5文件85通过/2跳过；最终5文件87通过/3跳过（共90项），32项新测试中30通过/2跳过，另1项跳过为既有PG退款恢复并发。初次新增测试的this类型及tuple展开引发类型检查错误，已显式声明this并逐项传参，不关闭检查规则。22项既有查询源码锚点逐项精确匹配，未改其查询证据。Worker unit/runtime最终类型检查结果另记。

Workers/PostgreSQL技能影响了短事务、无外部I/O持锁、稳定游标/最大ID、固定批量及结构化日志。已复核[当前Workers规则](https://developers.cloudflare.com/workers/best-practices/workers-best-practices/)、最新5.20260908.1类型、Wrangler Queue/Hyperdrive/Cron schema与本地配置；依赖、绑定、追踪策略及Cron频率未变。本轮没有生产SQL/DDL、发布、前端更改、浏览器或真实退款。既有主键及订单关联索引提供访问基础，但LIMIT只限制返回/执行量，不能证明物理扫描成本有界；新候选的大规模查询计划/专用索引仍需验收，不能据此关闭DB/性能门禁。清单221勾选/153开放/374项，A/B/C及父项继续开放，历史复制仍N/A。

冻结补充：Worker unit/runtime两套类型检查均通过；发现并同步修正workerd Cron测试中旧的任务数量（普通时段总投递21、签到提醒时段22），并显式断言新增恢复根任务的完整消息，修改后runtime类型检查再次通过。最终相关回归为5文件87通过/3项PG专用跳过，22项既有查询源码锚点精确匹配，git diff --check通过。未在本机运行workerd；新增PG16并发和Linux workerd运行结果仍待本提交自身CI，不借用8a98b50全绿。清单仍221/153/374，无生产操作。

### 4a43042 后续扫描计划审计与CI下载韧性（2026-09-08，当前候选）

本轮起点为已推送且工作区干净的`4a4304299adfc44b1a5dc6ad500ed996d3b96321` / [Actions34194676902](https://github.com/cinagroup/cinashop/actions/runs/34194676902)。秘密扫描job101959716091在下载固定Gitleaks 8.29.0时因curl35连接重置失败，尚未执行扫描，不能写成发现泄漏或扫描通过。仅给该只读GET增加3次重试、120秒重试窗口、10秒连接/40秒单次上限；按[curl官方说明](https://curl.se/docs/manpage.html#--retry-all-errors)继续使用--output文件以清除失败部分内容，非管道追加。SHA-256验证仍先于解压，后续完整历史扫描仍失败关闭，没有增加allowlist、忽略失败或修改版本/校验值。新增静态回归固定这一执行次序；真实下载/扫描以新提交CI为准。旧CI其他任务终态另记，未取消在途任务。

扫描性能不是从LIMIT推测：将既有服务扫描提取为同一query builder，执行路径和EXPLAIN都调用它，金额/身份验证、锁序和渠道执行未变。新`pink-cancellation-query-plan.test.ts`在financePostgres隔离夹具建立两张实际模型表及全部现有模型二级索引，再注入100,000条已完成/撤销/删除/普通原因或说明记录和30条专用候选（其中5条未达到60秒宽限）。仅合成记录、无生产URL或provider。默认PGlite运行，CI强制专用PG16服务；夹具不声称复制全部生产约束/触发器。

首次真实计划：未添加候选索引时返回5条、主键扫描过滤100,005行、shared hit3516/read0。测试库候选`(id, add_time)`部分索引只含未撤销/删除、apply_type1、refund_type[0,1,2,4,5]、专用原因/说明/前缀的申请；实际计划hit11/read1、过滤0，空尾页hit1。原服务的固定条件被Drizzle绑定为参数，强制generic计划仍走主键、过滤100,005行、hit3520。依据[PostgreSQL部分索引规则](https://www.postgresql.org/docs/current/indexes-partial.html)，将固定业务不变量保留为SQL常量，动态cursor/highWater/age cutoff/limit仍是参数[0,100030,1000,5]，不内联用户输入。修改后通用计划选候选索引、hit7/read0、过滤0；实际查询返回值前后一致。生产客户端当前prepare=false，本轮未改变它；generic实验是明确的防御性证明，不声称生产当前正在使用通用计划。

最终实验还验证末页、空页、冻结最大ID，以及已完成/撤销/删除/原因变化/apply_type变化后退出索引、恢复资格后重新进入查询。输出PINK_RECOVERY_QUERY_AUDIT明确candidateMigrationApplied=false。测试仅证明所选100k不相关历史分布下候选设计可行，不能推广为任意积压/近期候选分布的恒定成本，也不是线上P95。正式模型及两类迁移路径尚未增加索引，现有扫描成本并未因此消失；新增B1待办要求先取得自身PG16计划，再完成受保护增量DDL、九路径目录/升级/幂等/错误定义拒绝、其他分布及发布验收。没有改写旧目录原始证据或既有迁移。

本地验证：最初2项设计/下载合同通过；固定谓词后4文件40通过/2项PG跳过（53.82秒）；最终相关7文件89通过/3项PG专用跳过。后加索引成员变化断言的专项1项再次通过（7.33秒）。Worker unit/runtime均终态成功，22项既有查询源码锚点全部匹配、git diff --check通过。Workers与PostgreSQL技能推动当前平台文档/5.20260908.1类型/本地Wrangler schema核验、短事务及真实EXPLAIN验证；本轮无平台配置/依赖升级、生产SQL/DDL、部署、真实退款或浏览器操作。清单221勾选/154开放/375项，父项不收口。

上一提交终态补记：4a43042 / Actions34194676902最终10/11，除扫描器下载失败外全部成功。分片一101959716396：146文件1,191项、682.02秒；分片二101959716411：145文件1,131项、583.98秒；汇总101962736996成功，共291文件2,322项零跳过。两片inventory SHA均为`68602ad3a52a286c241f74ba18a86dc48bf519dc7336af2e116f631a584eff2b`，executed SHA分别`ad762c00aacb50fb50ea2d29020c74d2af831a648785a21325715d3f07ea4a2e`和`3af76756aaa2fcc051037d9629a60cf990b3fe499528cc7f17ce67301b680fd3`，完整互斥和实际文件匹配均true。后台32项15,352ms全通过，包括两项独立PG并发，不再按本地跳过口径评价该SHA。workerd101959716325、PG目录101959716332及五个前端任务均通过。最近整轮全绿仍8a98b50；没有重跑/取消旧任务，等待此次终态后才准备当前提交推送。

### d579fbc 全绿与恢复扫描正式索引迁移（2026-09-08，当前候选）

起点d579fbcc48864c5d648f010730b848ed37b3e9d5 / [Actions34196097660](https://github.com/cinagroup/cinashop/actions/runs/34196097660)终态11/11成功。分片一101964032075：147文件1,194项、671.69秒；分片二101964031973：146文件1,130项、588.49秒；汇总101967209776成功，共293文件2,324项零跳过。两片inventory SHA均为`f1d058d318d8b0d2aaea4941e1f5b7ada2d60bd7aa1163ba87335dd07320c3f8`，executed SHA分别为`d9330295298668c0733e3963b3468802f00957d7839879a0ac11f00e8a89728a`和`799b27a812387043425c33dbe83d119c300cc59fdbd431a4cc13c7a0307fa9c6`，完整互斥及实际文件匹配均true。秘密扫描、workerd、PG目录和五个前端均成功；4a43042扫描器下载失败历史保留，不重跑或取消。

该SHA的PG16查询计划专项1项1,735ms通过：100,030行、原查询返回5条但过滤100,005行，hit3516/read0；测试候选普通计划hit2/read1、通用计划hit7/read0且均过滤0，空尾页hit1/read0。candidateMigrationApplied=false是该旧实验的真实口径，不能当作正式DDL已经存在。它支持本轮索引设计，但不代替本轮迁移验证或生产容量证明。

本轮新增外部0146_pink_recovery_index.sql、字节等价SQL镜像、内嵌0152注册、ORM声明及独立runPinkRecoveryIndex。索引sor_pink_recovery_scan以(id,add_time)为键，仅包含未撤销/删除、仅退款、状态0/1/2/4/5、固定专用原因/说明/前缀的申请。时间不是易变now()谓词；查询仍用绑定的游标、最大ID、宽限时间和LIMIT。没有更改退款执行、金额/归属校验或自动审批范围。

升级器只接受根DB连接，默认public或显式命名schema，固定search_path并在DO开始前设置最多30秒语句/5秒事务空闲期限，保留更严格调用方预算；DO将锁等待限制为最多2秒。新建采用普通CREATE INDEX，需要维护窗口，会阻塞写入但不阻塞普通读取，绝非并发在线建索引。根据[PostgreSQL CREATE INDEX规则](https://www.postgresql.org/docs/16/sql-createindex.html)，不使用IF NOT EXISTS替代验证。守卫校验普通永久表、九个操作数的内建类型/宽度/非空/排序规则、完整索引定义/有效就绪状态/所有者/非约束归属，拒绝错误同名对象、继承表及未经审查的DDL事件触发器，不删除、替换或修改业务行/序列。独立入口不调用包含历史补写的runAll；全量建库仍顺序注册0152，失败停止。

测试改为执行同一个正式升级器，再执行原服务query builder的EXPLAIN、分页、冻结最大ID、空尾页、通用计划和状态变更；仅基线阶段明确排除新索引，保留其他全部现有模型二级索引。新增迁移专项覆盖有行升级、空表建库、重复执行保留目录OID/文件/业务行/序列状态、13类同名定义漂移、列类型漂移、整事务回滚、临时同名对象、显式schema隔离、非法schema和嵌套事务拒绝。新增独立PG16真实写锁阻塞/超时/回滚/解除后成功及LOCAL设置恢复专项，本地明确跳过。真实drizzle-kit生成的完整SQL也执行同一守卫两遍并核对原索引OID，不能只靠手写模型片段证明ORM一致。

九路径审计增加store_order_refund.sor_pink_recovery_scan正向必需存在合同，并保留所有原有完整目录/历史升级证明；两边都遗漏新索引也不能通过。新DDL的九路径和真实锁竞争须由新提交自身PG16 CI证明。首次测试暴露查询构建器缺FROM，已修正；注册回归正确发现旧152步骤断言，更新为153并增加0152失败关闭测试，不削弱0151/历史skip/0118 supersession语义。最终本地结果另记。

Workers/PostgreSQL技能推动完整迁移入口审查、当前[Workers规则](https://developers.cloudflare.com/workers/best-practices/workers-best-practices/)、5.20260908.1类型和本地Wrangler Hyperdrive schema核验，以及部分索引、事务/锁预算和实际目录验证。未升级依赖或修改平台绑定。本轮无生产SQL/DDL、部署、真实provider或业务账户操作；B1近期候选洪峰、积压分布、预算和发布验收继续开放，清单仍221勾选/154开放/375项，历史复制N/A。
...
### 9cb862d 全绿与近期申请分段遍历（2026-09-08，当前候选）

`9cb862dff7d202620a10eff5c2a1217ab8081e7a` / [Actions34200881817](https://github.com/cinagroup/cinashop/actions/runs/34200881817)终态11/11成功。分片一101979142259：148文件1221项、675.29秒；分片二101979142237：147文件1139项、595.19秒；汇总101982762281成功，共295文件2360项零跳过。两片inventory SHA=`2a183ebff15b75b0eaef21dd15ebc4277c85cf540cb606ff259516d5953cf1a1`，executed SHA分别`8dcd70e68c646c0a91cfe54a527576055c2966fcda31a7fd34905d40375ffd86`与`4f968849e2db78f6e940944a020c59e472cf6d759f7291722f58751ac2d87e53`，完整互斥/实际文件匹配均true。旧序列断言修复后的专项11项28634ms通过，未删除其30类拒绝/目录保护。目录101979142274、秘密扫描101979142308及五个前端均成功；6fe767d失败记录保留，没有重跑/取消在途CI。

该SHA预算专项8项在PG16实际通过（4944ms），覆盖真实MAX/候选锁等待55P03和注入慢SQL57014及后端设置恢复。PG16容量样本serverVersionNum=160014，近期1万/10万hit46/292、空结果275、generic292、全积压首中尾19、交错前后569/571、VACUUM后569，与上一轮本地发现一致。Linux workerd job101979142096为4文件26项（19.85秒），新增两项55P03/57014通过真实Queue分派的重试/不ack/成功重放专项89ms全通过。因此“仅加超时不能保证游标推进”有自身PG16证据，仍需要本轮修复。

当前实现增加`pinkCancellationRecoveryWindow`，复用与到期查询完全相同的不可变业务谓词和0146部分索引，在年龄过滤前按ID读取最多1000个专用候选ID。全局最大退款ID仍冻结；在同一READ ONLY REPEATABLE READ事务内，把随后到期查询的上界限制为本段最后ID，返回/执行最多5笔。若返回5笔，游标只到第5笔，不越过剩余到期记录；若少于5笔，游标推进至本段最后ID，即使全部未满宽限期也能继续。窗口满1000或结果满5则可投递continuation；精确末端可能多一次空终止页，但不会原地无限循环。每个新根任务重新从0扫描、按新scheduledAt检查宽限；大ID间隙按实际索引成员跳过，不按数值区间生成数百万页。

原3秒语句/500ms锁/5秒事务空闲LOCAL预算保持，普通售后排除、本人/原团/完整金额件数/当前状态复核、原退款ID执行和provider锁边界不变。新增examined计数区分读取的窗口大小与实际处理数量，不记录用户/订单/申请标识。两阶段必须同一快照：否则窗口之后资格变化可能在其ID范围里加入未计数候选；新增独立PG16测试在第二阶段order表真实锁屏障期间由另一个后端提交资格变化，要求当前快照不见、下一根可见。同时新增第二阶段order关联锁超时测试，不能只验证第一阶段或MAX预算。

容量测试保留未分段查询作为历史故障对照，另对每个分布执行当前实际snapshot及两阶段EXPLAIN。当前本地PGlite 180003证据：近期1万和10万首段都读取1000候选、返回0、nextCursor1000/hasMoretrue，窗口hit29+到期查询hit4，合计33；全近期空结果及冻结上界同为33。generic窗口/到期两阶段也为29/4并选正式索引。10万近期前缀后的5条到期记录完整遍历102页（101空页，含最终终止页），共检查100005候选、准确返回100001～100005，无扫描重启、丢失或重复结果。选定分布回归门禁要求10万首段合计hit+read<150、较1万增长≤32；这是所选夹具的回归阈值，不是线上SLO。

成本权衡同样记录：全积压首段窗口hit36+到期19，中段37+19，尾段4+19；与旧单查询19相比新增了窗口读取。到期密集时仅处理5条、后续可能重复前瞻其余候选，不能声称所有分布吞吐更快。交错前段37+11，维护后12+11；旧未分段在当前实验中为566（上一SHA为569，执行/维护历史不同，不改写旧数值）。1000是活的候选窗口上限，不是所有MVCC死元组/不可见版本/缺索引/错误计划下的绝对物理页数上限，仍依靠数据库期限失败关闭，持续超时的运维处理与积压清空SLO保持开放。

新增本地窗口测试覆盖空段推进、超过5条的到期覆盖、混合年龄多页准确性、20亿ID间隙、冻结最大ID不接纳新插入、下一根重新检查近期申请、恰好1000条后的空终止页。另为后台实际SQL/金额链路增加空段发送失败重放→到期真实受理申请余额回补一次→再次重放不重复的回归。workerd新增空段即使checked=0也投递下一游标，发送失败不ack并重试原游标、成功发送才ack；不以mock代替SQL测试，两层分别验收。

本地首批2文件35通过/5跳过，窗口4项通过；通用计划补丁初次错放到仅5个候选的旧分布测试作用域，回归明确失败，已移动到10万近期分布，不放宽1000窗口断言。最终容量专项2项25.67秒通过，机器证据改用stdout输出以便独立采集；随后相关6文件65通过/8项独立PG跳过（132.47秒）。新增真实余额链路后的专项、最终unit/runtime类型及diff结果另记。新PG16分段窗口/快照并发和新增workerd场景必须由新SHA验证，不能借用9cb862d的全绿。

Workers/PostgreSQL技能推动短事务、索引顺序与实际计划验证。当前Workers官方规则、5.20260908.1 Hyperdrive类型和本地Wrangler schema已核对；首次npm元数据读取受网络沙箱EACCES阻断，确认该进程结束后以授权只读查询取得版本，没有安装依赖或调整平台配置。全程仅本地/专用CI合成数据库，无生产SQL/DDL、部署、真实provider或浏览器操作。清单221勾选/154开放/375项，B1及所有父项继续开放，历史复制N/A。

冻结补记：新增空段真实余额链路后的后台专项33项中31本地通过/2项PG并发跳过（55.20秒），原有退款归属/幂等保护回归保持；最终unit/runtime两套类型检查终态成功。9cb862d目录日志现已取得：九路径263/3700/570/1007/227、所有比较零差异、required恢复索引与cleanup均true，步骤及external/ORM SQL哈希与6fe767d记录完全相同。本轮没有新DDL；待当前分段代码自身CI，git diff及清单计数复核后提交。

### cbf1937 自身全绿、DB-009G 隔离证据与浏览器只读复核（2026-09-08）

`cbf19378ac6237c67fb56990e3fb471f111f7780` / [Actions34202776093](https://github.com/cinagroup/cinashop/actions/runs/34202776093)终态11/11成功。分片一101985176072为148文件1224项、568.31秒；分片二101985175533为148文件1143项、578.03秒，共296文件2367项零跳过，汇总101988446095成功。inventory SHA=`74f01de78038d10ba0c54474259dc0f73919e4fee2d16a4784947f55e326393a`，executed SHA依次为`e870952cbabfad4c4669939dd48b54ffbdad8a6f18f6b02275ea955ba6bbdb43`、`e16e47329f22e0922e2493b20db5a8b846f6b2112c7de159a68a4958c12bdda1`，两片完整互斥/实际文件匹配均true。目录101985175318、秘密扫描101985175113及五个前端均成功；本轮不把旧目录逐项计数冒充重新提取的新日志。Linux workerd101985175363为4文件27项通过，其中恢复Queue专项3项115ms，包含空段continuation发送失败后重放以及55P03/57014不ack/成功重试。

自身PG16容量证据serverVersionNum=160014：近期1万/10万两阶段均window1000行hit29、due0行hit4，nextCursor1000/hasMoretrue；generic也是29/4。完整近期前缀遍历102页（101空页，含终止页），检查100005候选并准确返回尾部100001～100005；全积压首段仍有36+19开销，不声称普遍吞吐提高。此批CI关闭“尚待自身CI”这一证据缺口，不关闭B1持续超时运维、积压SLO或部署门禁。

DB-009G新增两项真实模型关系测试，采用financePostgres既有防护：默认内存PGlite，CI仅允许loopback的finance_test角色、cinashop_finance_test库和PG16随机schema。显式安装模型所有二级索引、CHECK和FK及其NOT VALID/动作，不使用去约束的宽松夹具。每组4父行/100003子行，50001个NULL、一个热引用键、两个罕见引用键，分别表示活跃和删除/关闭态。不是高基数或生产分布普遍证明。

按[PostgreSQL16外键实现](https://raw.githubusercontent.com/postgres/postgres/REL_16_STABLE/src/backend/utils/adt/ri_triggers.c)的父端检查SQL构造SELECT 1 FROM ONLY子表、引用列比较、FOR KEY SHARE探针，不额外加LIMIT；SPI执行行数限制不是SQL LIMIT，额外添加会改变规划。所选键最多一个匹配，另实际执行父DELETE与键UPDATE，报告EXPLAIN中的对应约束触发次数。探针不是捕获的嵌套触发器计划，不声称覆盖其他角色、RLS、生产统计或连接行为。PG18的RESTRICT删除用23001、PG16用23503，NO ACTION键更新仍严格23503，避免将任意错误当成正确拒绝。

最终本地PGlite180003验证2项通过（21.62秒），unit类型检查终态成功。callback引用无匹配原hit2273，reply引用原hit1471，均过滤100003行；测试候选普通单列非唯一索引后均read2、过滤0，活跃/删除态hit4，forced generic无匹配hit2。父端引用态DELETE/键UPDATE在加索引前后均按预期拒绝；无引用操作实际执行后回滚，父子全行指纹和FK定义/验证状态不变。只报告本地缓冲块证据，不用毫秒数作为生产SLO。正式模型、外部/内嵌升级未改；新测试的自身PG16 CI及后续正式索引升级/锁预算/目录一致性仍待完成，DB-009G保持开放。

用户授权后通过CUA连接Codex内置浏览器，分别打开[生产PC首页](https://cinashop-pc.pages.dev/)与[秒杀页](https://cinashop-pc.pages.dev/seckill)，待加载后重新获取对应标签页面树。标题均为CinaShop - PC 商城：首页包含商品、导航和六类分类，秒杀页仍显示三个“－”时段与空商品提示，重现已有线上表现。本轮只获得URL/标题/非空页面树/无可见框架覆盖层的证据；当前返回的工具文档未给出可调用的点击、控制台、截图或viewport接口，未猜测接口，也未启动替代浏览器。不能判定控制台健康、视觉布局、交互或移动端通过，FE-003L-A3与发布验收保持开放。

专项日志补记：cbf1937的PG16窗口5项1267ms全通过，含两阶段之间资格变化的真实快照并发；预算9项5527ms全通过，含第二阶段order关联的锁期限及慢语句取消。此证据属于cbf1937，不是新外键测试的PG16验收。
...
### d1d3485推送后审计：客户引用索引0148/0154（2026-09-08）

`d1d348558ac3ebf8f4f1bc305a0c6eaaeb9e77f6` / [Actions34210192598](https://github.com/cinagroup/cinashop/actions/runs/34210192598)终态failure，11个任务均runner_id=0。check-run102009086500注释确认账户近期付款失败或消费额度限制导致未启动；不是代码测试结果。未重跑、取消、修改计费或绕过门禁。此前44b6ef4的PG16/目录证据和34abedc完整全绿基线保留，不能替代本候选验收。

为0148前唯一无引用首列候选的wcao_client_fk新增普通非唯一、非部分B-tree `wcao_client_ref(corp_id,client_id)`，ORM、外部0148、字节一致的内嵌SQL、编号0154和独立根事务入口同步接入。MigrationService共155个编号步骤；旧步骤正文及0151～0153失败关闭保持。九路径目录审计增加该索引正向存在门禁，保留全目录严格比较。本地离线清单仍45个FK，其中30个完整非部分首部、12个较窄候选、3个部分索引候选，无首列候选为0；这不是生产目录或全库性能完成声明。44b6ef4实测1009索引仍是旧基线，新候选尚无自身PG16目录计数。

升级在READ COMMITTED根事务中运行，DO前最多30秒语句/5秒事务空闲期限，锁等待最多2秒，保留更严格调用方设置。固定显式schema，绕开临时同名对象，拒绝系统/非法schema、嵌套入口、缺表、非永久/继承表、列类型/可空/排序规则漂移、同名错误对象和启用的DDL事件触发器。验证索引方法、顺序、非唯一、非部分、无表达式/INCLUDE/存储参数/约束归属及valid/ready/live等状态，拒绝覆盖漂移；重复执行保留对象身份。没有业务DML、序列推进、统计配置修改或ANALYZE；普通CREATE INDEX会阻塞写入，须批准的维护窗口，不能用包含历史补写的runAll替代单项升级。参见[PG16 CREATE INDEX](https://www.postgresql.org/docs/16/sql-createindex.html)。

新测试覆盖有行/空表、15类同名索引/对象漂移、6类列漂移、整批回滚、临时及第二schema隔离、继承/非永久/缺表拒绝、DDL钩子拒绝及三组期限设置恢复。形状夹具只安装目标表实际列/默认/主键，未冒充全FK语义；另以未改写的完整Drizzle模型独立可销毁数据库验证重复执行，实际父子保护由完整模型计划场景验证。DDL事件钩子测试只用独占可销毁数据库，避免共享CI数据库副作用。两个PG专属并发测试分别要求锁期限55P03与更严格500ms语句期限57014、准确阻塞关系、回滚保留、设置恢复及解锁后重试，本地明确跳过。

计划测试的故障基线只排除准确的新索引DDL，完整保留全部45个FK和其他模型DDL，再调用正式0148升级器。最终本地PGlite180003：100003子行无索引无匹配hit4001/过滤100003；正式升级后的custom无匹配read2、罕见状态hit4。五次默认ANALYZE试验中trial2/3/4因两列n_distinct漏估为1而generic回到hit4001/过滤100003；没有移除不利样本。统计目标1000诊断分支下custom/generic为2块、罕见态4块，只是诊断，不写入正式迁移。随机采样影响见[PG16 ANALYZE](https://www.postgresql.org/docs/16/sql-analyze.html)。报告formalMigrationApplied=0148、defaultStatisticsUniversalIndexUseProven=false、productionLatencyClaim=false、nestedTriggerPlanCaptured=false。真实父端DELETE/键更新准确拒绝，无引用操作回滚后父子指纹/FK状态保持。

最终6文件105项中100通过、5项PG专属跳过，59.76秒；跳过为本批2项、旧0147两项及旧序列1项。Worker unit/runtime类型检查均终态成功，最后补测后unit类型再次独立成功；git diff --check通过。既有12复合引用的245.43秒本地证据保留，未在本轮重跑或宣称自身PG16通过。源码仍未发现该客户父表物理删除的线上热路径，保留/清理策略及更多数据分布仍需验收。

PostgreSQL技能促使保留有效部分/窄索引并明确统计不确定性，Workers技能促使使用有界、等待完成、无导入时I/O的根事务；核对当前官方Workers建议及Hyperdrive类型，未改依赖、连接、绑定或认证。没有生产SQL、部署、浏览器或真实provider操作；历史复制N/A。清单仍221勾选/156开放/377项；G2的PG16、默认统计/维护策略及生产门禁继续开放。

### d520255推送后审计：PC商品图片失败态（2026-09-08）

工作区起点干净，上一轮提交`d5202553ad54ccc03aee361d15105e668d6f31e6`已推送。复核[Actions34211932031](https://github.com/cinagroup/cinashop/actions/runs/34211932031)终态failure；分片102014703566注释明确近期付款或消费额度限制导致未启动。没有重跑/取消任务或修改计费。0148的PG16锁竞争与目录验证继续等待，不把已有本地测试变为CI通过。本轮从开放清单选择不依赖此限制的FE-002C继续实施。

新增ProductImage实际Vue组件，接入首页、普通列表/搜索/详情、收藏、券范围、秒杀/拼团目录及详情、砍价目录、购物车、结算、订单列表/详情共15页。空/空白/null源不渲染img，明确“暂无商品图片”；真实加载错误移除损坏img并以role=img、商品名称加状态的可访问标签显示“图片加载失败”。不生成替代素材或外部URL、不修改API数据、没有定时重试。图片DOM按来源区分，新源重置失败状态，旧已脱离元素的延迟错误被身份检查拒绝；兄弟组件状态独立。保持lazy请求、几何尺寸、cover/contain和商品名称；交易缩略图按实际storeName合同而非误用store_name。普通详情空轮播使用已有真实主图，主图缺失时显示提示，不伪造轮播素材。

新Node测试编译实际SFC模板与setup并用Vue自定义宿主执行，7项覆盖缺图、真实属性、失败不循环重试、同源/仅标题更新、换源恢复、A→B→A旧事件、兄弟隔离/卸载和15页接入。它不模拟浏览器图片解码或冒充视觉验收。首次构建揭示旧活动测试的无模板加载器不能处理新增子组件，现向测试服务器加入已有Vue插件，原60项实际Axios/Pinia/活动生命周期测试全部保留通过；随后类型检查揭示订单图片名称应使用camelCase，已按原文本合同修正并加静态回归。没有引入依赖或修改锁文件。

最终本地PC 60项旧测试加7项图片测试零跳过，vue-tsc及Vite6.4.3构建通过；构建只报告既有VueUse两处PURE注释警告。Worker五文件33项零跳过（5.20秒）涵盖PC响应式结构、详情实际服务合同、旧PC兼容、规格选择和秒杀隔离SQL。没有把这些计为全仓库本轮测试数或全部页面浏览器通过。

前端测试技能要求实际渲染闭环。本会话没有browser技能；先检查已连接CUA，但其返回接口仅提供页面导航，没有交互/截图/控制台接口。按技能的Browser不可用分支使用已有Playwright1.62.1和本机Chrome，未安装依赖。临时脚本及截图位于系统临时目录，不进入仓库。Vite只绑定127.0.0.1:5239、关闭env文件和代理；夹具精确白名单只读GET，浏览器阻断跨源请求，无生产数据库/凭据。数据全部合成，有效SVG显式标为LOCAL TEST IMAGE；另以真实404、HTTP200损坏PNG和空URL验证原生加载行为。

1280×844和390×844均完成首页四类图片→点击坏图商品详情、空轮播提示和好坏混合轮播切换；图片框分别277.5×277.5、175×175，首页clientWidth/scrollWidth分别1280/1280和390/390，详情无横向溢出。价格99.90及无有效SKU的购买禁用不变。追加列表销量排序→搜索→坏图提示、秒杀红色坏图→蓝色有效图→红色坏图，活动价格6.25/8.75与选中状态同步且不创建购物车。首次浏览器脚本错点Element Plus隐藏radio，被可见label拦截；改为点击实际label并验证checked后完整重跑通过，没有force点击或改应用迎合脚本。最终只有11条预设坏图404，相关应用console error/warn与pageerror均0；外部请求和未知API为0，所有API方法GET。已检视桌面首页、窄屏详情及选规截图；浏览器和本地服务器均正常关闭。
...
### 59b8876推送后审计：砍价发起、列表与购物车状态（2026-09-08）

上一轮为实质进展：目录已提交推送`59b887657c184a2068c53188461299f26c85ad1c`。本轮起点工作区干净；只读复核[Actions34215711840](https://github.com/cinagroup/cinashop/actions/runs/34215711840)终态failure，check 102026840299仍因付款/额度限制未启动。没有重跑、取消或修改计费；该限制不阻止本地实现与验证，因此目标继续保持活动，不标记整体阻塞或完成。

修复startBargain仅复用状态1的缺口：同一用户/活动沿用事务级advisory串行入口，READ COMMITTED保证等待前任提交后读新状态；锁定最多2条非删除1/3参与，1条复用原ID，2条拒绝，0条才新建。活动SHARE锁防止准入途中规则被修改，参与FOR UPDATE在帮砍/消费更新后重查有效谓词；等待后再次用数据库墙上时钟检查日期，不沿用事务开始NOW。最多5秒语句、5秒事务空闲、2秒锁等待并保留更严格调用方设置，完成或失败后局部设置恢复。没有新的全局唯一索引、生产数据清理、库存修改或provider调用；仍允许只有已关闭/已用记录的用户开始下一次参与，不以禁止正常复购回避订单身份问题。

myBargains恢复当前Worker语义：1/3且严格分币计算已到参与底价才具备参与层面的ready，2/4不能购买；活动未开始、过期、停用、删除或不存在时pay_status=false。过期1/3仅在响应中映射为2，保持数据库记录及4状态不变。窗口计数在分页前检测同一活动的多个有效记录，不因另一条位于其他页而错误放行；只做同一查询内聚合，不逐记录追加SQL。保持现有snake_case字段与residue_price，不暴露内部活动/计数列，HTTP本人列表加private no-store。此标志不替代商品可见性、SKU库存和最终报价核验。

StoreCartService两处参与查询从LIMIT1改为LIMIT2并拒绝歧义；加购与购物车展示复用严格参与金额/状态函数。异常负数、超额已砍、未到底价及3但未完成均不能加购；直接购买读取拒绝失效整组，旧未限定列表保留isValid=false提示，不悄悄选另一条参与。新目录已有严格金额/状态拒绝，相关交叉测试保持一致。没有改实际起价变更公式，A3i继续开放。

首轮51项中44通过、7失败，6处发起失败来自真实时区问题而非伪造夹具：本机PGlite会话Etc/GMT-8，Node为10:38Z，start_time/stop_time按Drizzle写入09:38/11:38，而clock_timestamp为18:38+08；无时区列被隐式解释为本地时区，unexpired=false。直接读取已安装Drizzle PgTimestamp实现确认mapToDriverValue为toISOString、mapFromDriverValue在无时区文本后加+0000。发起改为clock_timestamp() AT TIME ZONE 'UTC'，UTC/Asia-Shanghai/America-New-York会话均验证同一活动可参与；并未全局修改会话时区或生产数据来掩盖错误。剩余1项是负金额列表沿用decimalToCents明确报错，测试修正为期待拒绝而非虚构成功列表；加购同样拒绝。

新增24项本地状态/实际HTTP测试：状态1/3重复发起行与序列不变，结束后新建一次并重试复用，歧义跨分页、真实owner、软删排除、金额与日期、过期展示只读、旧取消仅允许本人状态1、成功/失败设置恢复等。既有真实加购/完整报价目录回归保留。最终8文件运行7通过/1跳过，86通过/6项PG跳过、60.52秒；随后补活动规则锁竞争用例，独立重跑PG文件终态7项跳过、3.75秒。Worker unit/runtime类型检查均成功，最后补测后unit类型再次独立成功；未把跳过计为通过，也不是本轮全仓库回归。

7项PG16候选使用现有financePeers严格loopback数据库/角色/版本/随机schema防护，并确认独立PID和未重连；涵盖双发起同ID、真实最后一次help与start、等待期间合成消费状态变更、advisory及参与锁等待后过期、500ms更严格锁期限55P03/回滚/重试，以及活动规则编辑等待。通过pg_blocking_pids精确等待关系和数据库时钟作为屏障，不以sleep当证据；本机无该专用服务，当前未执行。合成status4变更不代表真实订单消费验收；夹具仅实际列/默认/主键，未冒充完整生产FK/CHECK/触发器或PG16并发结果。

剩余核心缺口已用源码定位：建单把bargainActivityId写入store_order.activity_id，购物快照未持久保存该单消费的bargainParticipantId；cancelStoreOrder按活动ID或参与ID别名查询status4并要求唯一，多次参与可使恢复无法定位。已用旧单存在时发起新参与、随后取消旧单，还可能恢复出多个有效记录；本轮发起防重复不构成这一链路的修复。需精确身份快照、消费/恢复统一事务锁与状态规则及实际建单/取消回归。砍价建单UPDATE仍有无时区列与NOW隐式比较，也未由本轮发起修复覆盖。A3f保持开放且明确下一实施项，A3g/A3h前端与A3i价格规则均不勾选。

Workers技能按[当前官方建议](https://developers.cloudflare.com/workers/best-practices/workers-best-practices/)推动请求内有界、等待完成的事务及原有绑定复用；核对5.20260908.1 Hyperdrive声明和本地Wrangler schema，无连接/依赖/配置变更。PostgreSQL技能推动短锁范围、分页前单次聚合和实际锁屏障验证。本轮无浏览器操作、生产SQL、部署、真实账号/provider副作用；旧历史复制N/A。清单仍221勾选/161开放/382项，未缩小整体迁移目标。

### 1c2a9c7推送后审计：砍价订单精确参与快照（2026-09-08）

起点main与已推送`1c2a9c704495014dc5b6ecf9a786bb8d41750a15`一致且工作区干净。只读复核[Actions34217359984](https://github.com/cinagroup/cinashop/actions/runs/34217359984)为completed/failure，check 102032165786注释确认账户付款/额度限制导致未启动；未重跑、修改计费或弱化门禁。该限制不阻止本地安全实现，整体目标继续活动。浏览器仅获取可用表面，确认Codex内置浏览器连接且无标签页，没有打开生产页面或取得新页面验收证据。

修复已定位的多次已用参与歧义：新type2订单在真实事务写入cart_info时增加bargainParticipation对象，version=1、participantId、activityId、uid均来自服务端已选且成功消费的记录，不使用客户端快照，不改变order.activity_id仍为活动ID的合同，也不引入DDL。取消解析所有商品快照并校验规范正整数、版本、本人/活动和跨行一致性。存在但为null、损坏、未知版本、错身份或混合有/无字段均拒绝，不悄悄回退活动ID别名；只有每个有效JSON对象完全不含该字段才保留此前唯一旧记录回退。旧歧义不猜测、不补写、不删除数据。

精确取消查询同时约束参与ID、活动、uid、status4、isDel0；末尾恢复UPDATE再次约束同样身份和状态，失配抛错让整个库存、购物车、订单、积分/券和流水事务回滚。实际建单预检复用严格参与ready规则，拒绝未到底价/超砍/异常金额；消费UPDATE新增活动ID与报价时bargainPrice、bargainPriceMin、price三项守卫，报价后变更不再消费旧价格的另一身份。订单仍沿用现有活动改价公式，A3i价格政策未擅自改变。主活动日期比较改为clock_timestamp() AT TIME ZONE 'UTC'，消除非UTC数据库会话的隐式NOW解释；不全局更改数据库时区。

新增36项测试：11项纯解析边界，25项真实SQL建单/取消，包括同一参与建单幂等、已有旧status4记录仍精确恢复、新建两张不同参与订单逐张取消且保留后发起的状态1、重复取消单次补偿、错owner/活动/软删/状态、混合快照、唯一与歧义旧回退、取消活动SKU缺失回滚、报价后参与身份/金额改变回滚、消费后SKU扣减失败回滚、未完成/超砍、报价后到期，以及UTC/上海/纽约同事务后端实际建单。合成KV与订单号运行时替身，无支付/provider调用；使用既有financePostgres隔离防护，夹具只有真实列/默认/主键，不冒充完整生产FK/CHECK/触发器。业务行按主键比较，非事务性的序列推进不冒充已回滚数据；未执行生产SQL。

另新增3项独立PG16连接候选：两个购物车竞争同一参与只消费一次、同订单重复取消等待并只补偿一次、取消等待期间参与owner改变使全部补偿回滚。沿用financePeers严格loopback数据库/角色/PG16/随机schema和独立稳定PID校验，pg_blocking_pids作为实际等待屏障；本机无专用URL，3项明确跳过，不能把定义好的测试计为执行通过。既有7项发起并发也跳过。最终相关10文件8通过/2跳过，119项通过/10项跳过，56.33秒；Worker unit/runtime类型检查终态成功，git diff --check通过，不是全仓库或本候选自身Linux/workerd/PG16验收。

明确保留未完成问题：本轮没有调整消费/取消的库存锁序，建单主活动→参与→活动SKU→基础SKU与取消基础SKU→主活动→参与仍需统一并考虑help/start兼容性；参与或SKU等待后活动到期也未由主活动UPDATE修复完全覆盖。取消旧单时存在新参与会恢复出多个有效记录，本轮实际双订单测试保留这一事实，未关闭或删除新参与；目录/购物车对多个有效记录仍拒绝，需后续完成显式参与选择/购物车绑定或确认业务规则。A3f、A3g/A3h前端及A3i价格政策保持开放，精确恢复身份不等于完整状态机完成。

Workers技能促使保持原有请求级绑定、等待所有事务I/O并避免provider副作用；PostgreSQL技能促使使用条件UPDATE与同事务失败回滚、真实SQL和独立后端证据分开记录。依赖、锁文件、Wrangler配置、生产连接和发布均未改；旧历史复制N/A。清单保持221勾选/161开放/382项，不以此局部修复虚增整体迁移完成度。

### 9197ad5推送后审计：砍价库存锁与等待后到期（2026-09-08）

上一轮为实质进展，`9197ad5320269e693bb807b41c77106f59651738`已推送；本轮起点main工作区干净。只读复核[Actions34219062925](https://github.com/cinagroup/cinashop/actions/runs/34219062925)终态failure，check 102037626236仍因账户付款/额度限制未启动。未重跑、修改计费或弱化门禁，目标保持活动，不将本地可继续工作误标为整体阻塞。本机只读检查未发现可用docker/podman/initdb/pg_ctl/psql命令或PostgreSQL服务，也没有专用finance测试URL；没有安装新服务或改连生产来伪造独立PG验收。

源码审计确认同活动库存反向依赖：建单持活动后等待基础SKU，取消/退款先占基础SKU再等待活动；只修取消会保留退款路径。新增BargainInventoryLocks，建单在购物车认领前取得活动NO KEY UPDATE，取消在任何SKU补偿前取得同一活动锁，退款在结算用户和SKU前取得该锁。[PostgreSQL 16锁兼容矩阵](https://www.postgresql.org/docs/16/explicit-locking.html)确认NO KEY UPDATE不阻塞KEY SHARE；因此保留help持参与行锁后查询活动KEY SHARE的路径，避免使用FOR UPDATE制造活动→参与→活动循环。start的活动SHARE会等待库存事务完成后再判断已消费记录，可继续发起下一参与，不限制正常复购。

退款新增一次性的锁定身份解析：有效version1参与快照按uid/活动校验后直接锁其活动；旧完全无字段的有效JSON保留直接活动优先、无主活动才取本人参与ID别名的原规则。解析结果连同主活动缺失标志传到restoreRefundStock，不在持有用户/SKU后重新定位或改选活动。有效新快照的主活动已删除时仍能恢复现存正确活动SKU，不把活动ID重新解释为参与ID；损坏/混合/未知版本快照拒绝。已支付退款保持参与status4，只有未支付取消恢复3；分次退款按实际数量单次补偿，已发货退款不退库存，活动已停用/删除/到期不作为取消或退款的购买准入条件。

原建单主活动UPDATE的UTC时间守卫发生在参与/SKU等待之前，单靠该守卫仍可在等锁期间到期。现在真实建单事务最后一条准入SQL重新检查status1、isDel0及UTC墙上时钟的起止窗口；此前订单、SKU、参与、购物车、快照/打印意图等仍在同一事务中，失败统一回滚。此处是最后SQL准入点，不承诺物理COMMIT完成时间早于截止，也未改变其他活动的时间政策或参与起价变更公式。

新增13项实际SQL补偿回归：三种失效活动取消、两次部分退款与重放、三类损坏新快照拒绝、两种旧活动/参与别名、主活动删除但精确快照仍可补偿、活动SKU丢失时退款/余额回滚、已发货不退库存，以及隔离schema中AFTER INSERT快照触发器使活动到期、真实建单全部业务行回滚。触发器仅安装于自有测试库，模拟截止发生在快照写入阶段，不冒充锁等待证明；序列推进不具有事务回滚性，比较业务行不错误要求序列回退。合成paid/yue状态不经过真实支付渠道，余额补偿仅发生于隔离数据库。

独立PG16文件由3项扩至11项，新增8项：建单与取消、建单与退款、取消与退款（NOWAIT额外确认等待者未先锁结算用户），参与/活动SKU/基础SKU三个等待跨过截止，实际help在建单持NO KEY UPDATE期间执行，以及start等待消费提交后允许新参与。全部沿用严格loopback数据库/角色/版本/随机schema、独立稳定PID与pg_blocking_pids屏障；到期用数据库墙上时钟验证。现有发起7项加本文件11项，共18项本机明确跳过，没有以PGlite并行Promise代替独立后端证据。

首轮3文件54项通过。扩展后发现新PG测试将Promise<void>与Promise<RefundFinalizationOutcome>的联合传给泛型outcome导致TS2345；改为各分支分别包装，不改服务逻辑、不移除断言。最终砍价相关9文件115项通过/18项PG跳过（60.17秒）；另外退款协议/权限、秒杀和拼团7文件144项通过/25项PG跳过（36.89秒），两组文件不重叠，共259通过/43跳过。最终Worker unit/runtime类型检查终态成功，git diff --check通过；没有全仓库测试、Linux workerd或生产并发证据。

Workers技能促使保持原有请求级Hyperdrive绑定与事务内SQL-only流程，核对当前官方建议、5.20260908.1类型及本地Wrangler schema；PostgreSQL技能促使先锁共享资源再补偿、最后准入检查和真实连接屏障分开验证。没有修改依赖/锁文件/Wrangler配置或生产连接。A3f仍开放：取消旧单后多个有效参与的显式选择/购物车身份、帮砍完整状态竞争、跨活动/后台编辑/商品退役的全局锁序、自身PG/CI和两端页面仍需完成。未作生产SQL、浏览器、部署或真实provider调用；历史复制N/A；清单221勾选/161开放/382项不变。

### 1fabfde推送后审计：砍价购物车报价原子认领（2026-09-08）

起点main干净且已推送`1fabfde760371ecf1170a473f18ced0d6d95d9f8`。只读复核[Actions34220596985](https://github.com/cinagroup/cinashop/actions/runs/34220596985)为completed/failure，check 102042539749仍说明账户付款/额度限制使任务未启动；没有重跑或修改计费/门禁。上一轮库存锁修复是实质进展，但不能借用其他SHA的PG/CI证据。本轮继续本地可执行工作，未将整体目标误标为阻塞或完成。

原计划接通持久参与身份，审计时先发现更靠前的原子认领缺口：type2报价读取购物车后，UPDATE is_pay只重新检查owner/代客/支付/删除/状态，未检查SKU、数量、商品、活动和模式。新测试在nextOrderId隔离替身中提交购物车修改，再进入真实建单事务：9个单字段变化及SKU+数量组合本应失败却成功，另6个原有owner/状态守卫确实拒绝、仅错误文案不匹配；元数据兼容正向用例通过。首轮17项新用例为16失败/1通过，不能将所有失败都说成绕过所有权。

新增ActivityCartQuoteGuard提取原秒杀购物车比较，不改变其8个字段和SQL语义；SeckillPurchaseSnapshot保留原导出别名，订单核心为type1/2两种单购物车订单共用。条件直接放进UPDATE is_pay：id、cartNum、productId、productAttrUnique、productType、activityId、type、isNew均必须与当前建单报价所读值一致，原有身份和状态守卫保留。失配抛出明确的砍价购物车刷新错误，同一事务失败回滚，不自动重算并继续消费。依据[PostgreSQL 16 Read Committed说明](https://www.postgresql.org/docs/16/transaction-iso.html)，等待并发UPDATE提交后WHERE会在新版本上重新判断；没有增加另一次不带锁SELECT来充当最终校验，也未新增锁或改变库存锁序。

17项新增真实SQL用例涵盖9类报价变化、6类原有身份/状态保护、拒绝旧报价后按蓝色SKU/数量2重新建单并精确取消、addTime非报价元数据兼容。后者同时验证历史活动ID和活动SKU别名仍可用于唯一有效记录，以及完成key重试不受后来购物车变更影响、取消按订单快照而非现购物车数量补偿。创建失败前的外部测试修改保留，订单、商品、SKU、活动、参与、购物快照、状态、打印意图、测试KV等业务状态与该时点完全一致；序列非事务推进明确排除。订单身份文件终态53项通过（60.08秒），没有支付/provider请求，夹具仍仅实际列/默认/主键，不冒充完整生产约束。

独立PG16订单文件由11项扩至22项：新增9类购物车修改在未提交时被买方读到旧版本、买方实际等待该编辑者PID、编辑提交后拒绝的候选；另2项验证编辑回滚或仅改addTime时等待后仍可建单。使用原financePeers的专用loopback数据库/角色/PG16版本、随机schema、独立稳定PID和pg_blocking_pids屏障，不以延时或PGlite并行模拟为证据。本机没有专用服务，这11项新增并发候选尚未执行；新增断言没有被算成本地通过。

Worker unit/runtime类型检查成功。扩展活动回归10文件7通过/3跳过，241项通过、45项独立PG测试跳过，113.01秒；包含砍价创建/取消/退款、状态与目录、实际秒杀购物车/报价/建单/退款和PC秒杀入口测试。订单创建/退款两文件另8项静态回归通过，两组文件互不重叠，共249通过/45跳过，不重复累加之前单独运行的53项。跳过为砍价订单22、砍价发起7、秒杀并发16，未冒充全仓库、Linux workerd、真实浏览器或独立PG16验收。遵循Workers技能核对当前官方最佳实践、5.20260908.1 Hyperdrive类型和本地Wrangler schema，保持请求级原绑定和等待事务I/O；PostgreSQL技能促使在已有写边界校验、缩短锁范围并将单后端与独立连接证据分开。无依赖/锁文件/Wrangler或生产连接变更，无DDL、生产SQL、部署、浏览器或真实provider调用。

本批只关闭一个已复现的报价认领缺口，不代表持久参与选择完成。store_cart仍无bargain_user_id，目录/加购/列表的多有效参与仍拒绝；专用字段与外部/内嵌/ORM一致升级、显式身份到真实订单的完整合同仍列为下一实施项。需要保留既有参与、隔离历史0值与活动别名、不跨参与合并，并扩展本次原子守卫校验绑定字段；不以禁止复购或删除/关闭新参与消除歧义。A3f、A3g/A3h前端和A3i价格政策均不勾选，清单保持221勾选/161开放/382项，历史复制N/A。

### 0c368ec推送后审计：砍价购物车持久参与字段与独立升级（2026-09-08）

起点main干净并与origin一致，上一报价原子认领修复为`0c368ec59cb03df8c1da84878b4d280f76752c02`。本轮只读复核[Actions34222304743](https://github.com/cinagroup/cinashop/actions/runs/34222304743)为completed/failure，check 102048057118注释仍说明付款失败或额度限制使任务没有启动。未重跑、调整计费或削弱门禁；继续可执行的本地迁移工作，没有将整体目标误标为完成/阻塞。

为A3f新增`store_cart.bargain_user_id INTEGER NOT NULL DEFAULT 0`，0明确表示旧/未绑定购物车。正式CHECK `sc_bargain_participation_ck`要求`bargain_user_id >= 0 AND (type = 2 OR bargain_user_id = 0)`；不建立指向0的伪外键，也不将正整数等同于本人有效参与，后续业务必须校验uid、活动、状态及金额。本次不查询参与表、不自动分配旧记录、不重写业务行或推进序列，也没有改变参与状态/复购规则。

外部`0149_bargain_cart_participation.sql`与`BARGAIN_CART_PARTICIPATION_SQL`内容一致，内嵌编号是0155（不能混用两个编号），实际MigrationService注册及验证getter已接入，真实Drizzle模型同步。独立`runBargainCartParticipation(db, schema)`要求根连接并自建READ COMMITTED事务；参数schema校验、schema pin及pg_catalog路径防临时对象覆盖，永久普通表/非继承/非分区/禁启用DDL事件钩子，显式ACCESS EXCLUSIVE锁后再次核对目标身份。锁等待最多2秒、语句30秒、事务空闲5秒，保留更严格调用方期限；不自动连接生产、重试或重放历史种子。

已存在同名列必须整数、NOT NULL、常量0默认、非identity/generated且类型/排序规则正确；已存在同名CHECK必须表达式及验证/继承/本地/延迟等元数据完全匹配。不用IF NOT EXISTS掩盖错误结构，不替换漂移约束，失败由整项事务回滚。新增常量默认不重写旧表堆，但新增CHECK需要验证已有行，且本升级包括重复执行都会取得ACCESS EXCLUSIVE，不能宣传为零阻塞在线变更；维护窗口依据[PostgreSQL 16 ALTER TABLE](https://www.postgresql.org/docs/16/sql-altertable.html)。现存库必须先审核并执行独立升级、核对后再发布引用新模型的Worker；严禁为了这一列重跑含历史补写/种子的runAll，本轮没有执行生产DDL。

验证过程保留失败事实：首次全量外部/内嵌建库通过PGlite预编译查询执行时拒绝多条SQL；测试改用真实事务simple protocol后，完整历史在外部0115/内嵌0121的既有`projection constraint count drift`拒绝，发生在新0149之前。PGlite18的NOT NULL目录行与目标PG16闭合集合不一致，未修改历史迁移或删减计数。完整外部/内嵌建库两项改为仅在专用PG16服务执行，生产URL没有回退通道；新升级本地仍执行未改动的原始外部0002和实际内嵌0002购物车DDL、完整真实ORM生成，以及三者的重复升级。测试配置曾误用each回调context.skip导致两项失败，已改为声明式skipIf，未把该次失败记为通过。

最终相关回归5文件通过，247项通过/4项跳过（85.25秒）：新增字段升级30项本地通过，另217项覆盖砍价订单身份/消费/取消、参与状态/目录与秒杀购买。最后将隔离schema测试的共享serial默认改为显式ID并加入源序列不变断言后，字段升级文件重新完整通过30项/4项跳过（43.20秒），不重复累计这30项。覆盖旧行默认0、堆文件未重写、序列不推进、重复执行保留OID、有效绑定/非砍价与负值拒绝、8种列漂移、5种约束漂移、表形状、坏数据回滚、临时遮蔽、独立schema、根事务/schema输入、三组更严格期限及DDL事件钩子。Worker unit/runtime类型检查均通过，git diff --check与221/161清单计数复核通过。

两个独立PG16候选通过pg_blocking_pids确认读锁等待，分别要求2秒lock_timeout与更严格500ms statement_timeout拒绝、回滚后释放读锁再成功。这两项和两条完整历史建库路径均未在本机执行，合计4项跳过；完整九路径目录/自身PG16 CI仍开放，不冒充PGlite或旧SHA已经覆盖。已有orm-ddl-audit动态读取全部外部迁移和当前模型，本次未改动其历史冻结清单或将旧3700列证据改写为新字段已验证。

Workers技能促使保持请求级原Hyperdrive绑定、不新增连接/部署配置；PostgreSQL技能推动独立有界维护入口、结构漂移拒绝及真实历史行不回填验证。没有修改依赖、锁文件、Wrangler、历史冻结清单或生产数据；无浏览器、部署、真实账号/provider调用。A3f业务接线尚未完成：目录精确选择、加购持久保存/不同参与不合并、旧0值选择规则、确认/计算/建单的参与ID与活动别名区分、认领绑定字段守卫仍为下一项。A3e/A3f/A3g/A3h/A3i均不新增勾选，清单221勾选/161开放/382项不变；PHP历史复制继续N/A。

### bd759bf推送后审计：砍价持久参与身份跨加购与订单接线（2026-09-08）

本轮承接结构候选`bd759bf1d7f0316ac9e28184c21416a5a3c4c53e`。只读核验其[Actions34224404289](https://github.com/cinagroup/cinashop/actions/runs/34224404289)终态failure，11/11任务runner_id为0；check 102054889731仍说明账户付款失败或额度限制导致任务未启动。未重跑、修改计费或弱化门禁；不能借用34abedc的旧全绿证明新结构或本批接线。继续本地工作，整体目标保持开放。

新增BargainParticipationSelection统一解析与查找：camel/snake参与参数必须一致、为规范非负32位整数，0保持通用结算客户端“未选择”兼容；bargainUserId只表示本人参与ID，bargainId只表示活动ID。原创建核心将参与ID同时尝试解释为活动ID的行为移除，历史活动调用应使用bargainId，不再允许跨命名空间碰撞后回退。显式查找同时约束uid、活动、is_del，可返回已用/关闭行用于目录展示，但购买调用仍复核可购买状态与金额。隐式只读取最多两条1/3记录，歧义明确拒绝，不静默取最新。

新type2加购保存精确参与ID。同一参与的普通购物车允许合并，不同参与、旧0记录、直接购买、代客/游客/门店范围保持分离；复用原用户购物车建议锁1128354388，在READ COMMITTED事务内选择/写入，语句最多5秒、锁等待2秒、事务空闲5秒，保留更严格期限。合并UPDATE比较原报价字段、绑定ID及身份/未支付/未删除/有效状态，若结算已认领或其他字段已变化则拒绝；不先占参与锁再等待购物车，避免反转已有购物车→参与锁序。加购只做准入，不预留参与或库存；参与在加购后失效仍由读取、报价及消费边界拒绝。

cartBargainParticipation贯穿列表、确认、计算和建单。正绑定严格校验本人活动身份，请求不可改绑，绑定记录失效不能选另一条。历史0不自动回填，即使请求携带ID也必须只有一条有效参与；有多个时明确要求重新选择并加购。type2原子认领在原8项字段守卫之外再比较bargain_user_id，省略请求选择器也必须实际扣活动库存并消费绑定记录。完整key幂等优先返回已建订单；未支付取消沿用订单精确快照恢复原参与，不删除、关闭或误恢复后来记录。未改动现有起价/底价计算公式、支付、provider或退款语义。

目录现在允许在多个有效参与间显式选择，本人列表按每条记录判断pay_status，不再因同活动另一参与而一律禁购；隐式目录/发起/加购仍拒绝歧义。早期测试曾要求显式目录也拒绝，首次相关3文件为106通过/1失败，更新为新的精确选择合同后通过；这是有意变更并记录，不以删断言掩盖错误。旧阶段“多有效记录一概拒绝”的描述仅作为历史，不再描述当前正绑定行为。

新增17项真实HTTP/SQL用例，真实OrderController、确认/计算、创建和取消运行在隔离数据库，安装本批实际购物车CHECK；认证、KV与序列分配为隔离替身，没有payType或支付渠道请求，不冒充完整生产外键集合。两条参与80/90分别以2元/4元完成目录→加购→确认→重算→建单→幂等重试→精确取消，后续参与91保留。覆盖同绑定合并、不同绑定分离、唯一隐式持久化、活动/参与ID碰撞、旧0歧义、六类参与状态/归属/金额失效、两类报价后绑定修改、别名冲突/非法入参、非砍价拒绝、代客范围和更严格期限恢复。业务行回滚快照不要求非事务序列回退。

新增3项独立PG16加购候选：同参与并发合并、不同参与不交叉、真实checkout占用购物车后加购等待并拒绝；原订单并发文件新增1项未提交绑定修改使买方读旧值、实际等待编辑者、提交后原子认领拒绝。均使用专用loopback PG16、独立稳定PID与pg_blocking_pids屏障，没有改连生产或用PGlite并行Promise代替。当前11文件活动回归7通过/4跳过，254项通过、49项PG跳过（97.67秒）；49为砍价加购3、订单23、发起7、秒杀16。本机未执行这49项，新增候选不算已证实并发安全。

另外8文件普通购买/兼容/迁移/索引回归首次39通过/1失败，原因是ActivityJoinService重构后sbu_uid的4行查询证据仍定位764，实际始于786。仅将orm-ordinary-index-reconciliation.json的sourceLine改为786；原SQL片段、冻结目录、原始SHA/run、索引定义与决策不变。最终8文件40项通过（6.18秒）。两组文件不重叠，合计19文件294项本地通过/49项PG跳过，不重复累计中间单独测试；Worker unit/runtime类型检查均终态成功。没有全仓库、Linux workerd、自身PG16或生产并发完成的结论。

用户再次授权浏览器后，通过可用的统一浏览器控制连接到Codex内置浏览器，只读打开并再次读取生产首页与/bargain。首页页面树包含推荐商品和六类分类；砍价页显示“砍价专区”“我的砍价”“砍价商品-测试商品A ¥99.90 可砍至 ¥59.90”和“发起砍价”。页面身份及非空内容已核对，页面树未显示框架错误提示；本次可用接口没有返回截图、控制台和点击操作文档，因此没有尝试猜测未提供API，也没有登录、发起砍价或购买。不把页面树无报错等同于控制台无错误，不把旧部署可加载等同于新候选端到端通过。前端测试技能促使明确区分这些证据层级，没有改用外部浏览器或新增依赖。

Workers技能促使保持请求级原Hyperdrive连接、等待全部事务I/O；PostgreSQL技能促使在已有写边界检查绑定、限制锁等待且区分本地SQL与独立连接证据。本批没有schema/依赖/锁文件/Wrangler变更、生产SQL、部署或真实provider调用。现存生产仍须先审核执行外部0149/内嵌0155对应独立升级并核验，再部署引用新字段的Worker；不可重跑历史runAll。A3e/A3f自身PG/CI、A3g/A3h两端页面、A3i改价政策及全局状态竞争仍开放，清单维持221勾选/161开放/382项，旧PHP历史复制N/A。

### d9abfba推送后审计：PC砍价参与目录、规格与结算入口（2026-09-08）

上一轮是实质进展，main与origin均为`d9abfbab90dc34e4319b4131f9a6184dd34c06e3`且起点干净。该提交[Actions34227223180](https://github.com/cinagroup/cinashop/actions/runs/34227223180)终态failure，11项runner_id均0，check 102064190238注释说明账户付款失败/额度限制导致未启动。没有重跑或更改计费；可继续实现PC/UniApp等本地缺口，不把整体目标误标为阻塞或完成。本批没有Worker业务代码、数据库升级、依赖/锁文件或生产配置变更。

审计确认PC原Bargain.vue读取storeName/minPrice，但列表真实合同是title/min_price；本人列表使用camelCase并重复拿起价作为当前价，进度不能反映已砍金额，缺少分页和实际购买入口。新增共享bargainPurchase合同将列表数值金额规范化为两位字符串；本人记录校验uid、精确参与ID、四类状态、金额和pay_status，异常金额明确不可购买，null活动标题显示不可用。新详情解析A3e的完整selection_only/type2、活动/商品/参与ID、四种state、金额恒等式、百分比、活动起价变化标记、时区日期及至多500项SKU映射；活动与基础unique冲突、重复规格、错误单价/身份/资格拒绝，不回退基础SKU。匿名结果不接受参与数据，指定参与必须匹配本人响应。

列表与本人弹窗各请求20条，独立页码、加载/失败/空页、相同页重试；旧请求、关闭弹窗、切身份与卸载不能回填私有内容。活动卡进入新增/bargain/:id，不再在列表盲发起；本人每条记录链接带bargainUserId，多个有效参与可明确选择。保留本人帮砍和取消，取消前确认并在对话框等待后核对身份/弹窗代次；在途禁重复操作，响应未知时要求刷新确认，不自动重试写入，不再把累计已砍当作下一次帮助金额。

专用详情按A3e显示本人参与状态、当前价、底价、已砍/剩余金额及结算参考价，活动改起价后参考价可能高于底价，明确提示但不改变A3i待决政策。用户显式选择活动unique与数量，不默认规格、不缩减超库存的恢复数量、不硬编码SKU或价格。加购发送productId/活动ID/本人参与ID/type2/new1，结算URL只携带真实cart与type2/bargainUserId。登录返回前保存最新参与/SKU/数量，匿名不请求私有参与，不自动发起；失效登录经实际请求层恢复意图。导航等待之后再次核验身份、路由和活动期限；旧加购响应不导航新页面或新身份。已成功加购而结算导航失败时保留cart并禁改选择，重试只导航；已成功发起而导航失败也保留参与ID，不重发start。

新增25項实际SFC setup/Vue/Axios/Vue Router回归，包含80/90两条不同价参与、匿名/过期登录恢复、5类导航等待失效、4类加购迟到、成功写入后导航重试、start结果恢复、切参与迟到、关闭/已用/未砍完/到期/库存变化、等待后截止、列表分页及私有弹窗隔离、严格响应与改价语义。沿用已被PC build调用的test:auth注册，不新增未接入CI的孤立测试命令。最终85项会话测试全部通过，7项图片测试通过并将新详情纳入16页图片集成断言。首次build的图片静态检查因引号格式失败；按既有导入格式修正，未删除断言。最终vue-tsc与vite build终态成功（1866模块，9.19秒），保留VueUse既有两处PURE注释构建警告。

新增pc-bargain-purchase.test.ts的4项实际Hono/SQL→共享合同回归，证实真实列表、每条本人当前价和进度、80/90选择下真实蓝色SKU加购持久化、2件确认报价分别4元/8元、匿名禁购与活动起价10→12时参与当前价2元/参考价4元。与目录、状态、绑定回归共4文件74项通过（59.97秒），Worker unit/runtime类型检查均终态成功；没有独立PG16服务或自身Linux/workerd证据，不将本地PGlite当作多连接测试。夹具沿用真实列/默认/主键，不代表所有生产约束。服务端创建、消费及取消保持上轮逻辑不变。

前端测试技能要求以实际渲染和交互作为证据。本会话没有专用browser技能/运行时（Browser plugin not available），按技能分支使用已有Playwright1.62.1与本机无头Chrome；没有安装依赖或打开外部生产浏览器。仅127.0.0.1:5199 Vite及5198内存SQL服务，浏览器阻断非loopback请求，配置明确指向隔离服务且拒绝TEST_FINANCE_POSTGRES_URL。目录、本人列表、加购走实际控制器；认证、KV、站点/登录配置为隔离替身，不声称通过真实登录/provider。测试脚本、服务与截图均在系统临时目录，不纳入仓库。

浏览器流程为匿名精确参与链接→选择蓝色2件→实际登录返回地址→隔离会话恢复→真实SQL加购→主动阻止checkout导航→继续结算只导航→本人列表选90→参考价4元。桌面1280×900和窄屏390×844均核验页面标题/URL、非空内容、无框架覆盖、实际按钮状态、控制台、截图及无横向溢出。最终每个视口各1条真实cart写入，重试导航目标仍相同cart/type2/participant80；最后一轮cart为5/6，基础unique qablue01、数量2，新增订单0，商品/SKU/活动/参与/帮砍数据与初始快照一致。多轮调试累计6条隔离cart，不能把总写入数说成2；没有生产写入、付款或订单副作用。浏览器主动拦截checkout导航用于验证恢复，完整结算页面和真实付款并未验收；真实SQL确认报价另由上段4项测试覆盖。

失败证据保留：首次浏览器每个视口有一个/api/pc/get_appid 404，为隔离登录配置未提供，补齐空appid替身后关闭外部微信入口，最终console/pageerror为0。最后语义标签改动触发HMR后，测试直接导入无时间戳router造成地址已变仍显示登录页，三次返回检查超时；读取实际Vite main源码确认应用导入router带?t时间戳，而测试导入未带参数。测试改用页面已挂载的$router实例，再跑两个视口均通过，不用重启或弱化业务断言冒充修复。截图滚动至顶部后重新采集，避免fullPage在滚动中把sticky导航画在长页中部；没有将截图采集差异当作业务布局问题。

本地候选使A3g的页面/身份入口更完整，但仍不勾选：真实角色发起/帮砍/取消、浏览器完整确认/提交/支付、失效恢复全矩阵、自身Linux/PG16及发布观察尚未完结。下一实质项为UniApp A3h专用入口及生命周期接线，后续还须统一A3i政策并完成全局状态竞争验证。生产仍须先审核执行独立字段升级，再发布新Worker及对应前端，不能单独将该PC候选与旧API混发。清单221勾选/161开放/382项不变；历史复制N/A。

### ff8a6b5推送后审计：UniApp砍价详情、本人记录与生命周期（2026-09-09）

本轮起点main/origin均为`ff8a6b526909d9d5d697fc566bc48b00b1b9247c`且工作区干净。只读复核[Actions34230301560](https://github.com/cinagroup/cinashop/actions/runs/34230301560)终态failure，11/11任务runner_id为0；check 102074457415明确为账户付款失败或额度限制导致未启动。没有重跑、修改计费或弱化门禁，34abedc仍是最近全绿基线。本轮仅改UniApp和审计文档，不修改Worker业务、数据库、依赖锁或生产配置。

源码审计确认旧bargainDetail只读取id，`mine=1`必为空；详情读取camelCase原价字段、错误匹配本人参与，并通过通用ActivityPurchase查询基础SKU，未在加购时持久绑定具体参与。新页面由单一useBargainPurchase同时承载详情和本人列表，避免同页多份生命周期相互覆盖。严格区分活动ID、商品ID和bargainUserId，拒绝混合/非法路由；复用PC已经接入真实后端的共享bargainPurchase解析，不发明新金额或回退基础SKU。匿名可看公共规格，显式本人记录与本人列表在登录前不发请求；服务端返回错误、歧义和异常金额均不能变成可购买记录。

详情展示四种参与状态、当前价、原参与底价、已砍/剩余金额、进度和活动参考价；活动起价改变时明确提示参考价不同，未改变A3i待决的服务端定价政策。必须显式选活动unique和数量，加购同时发送productId/activityId/bargainUserId/type2/new1，checkout仅携带真实cart和精确参与。保留原有发起、本人帮砍，使用真实start/help接口并重新读目录，不乐观编造参与或把已砍总额当本次帮助金额。start成功后先保存返回ID，即使后续读取失败也不能再次发起；未知写入结果清空可操作详情，要求刷新确认，不自动重试。

本人列表每页20条，按当前owner校验并显示真实当前价/进度/状态，支持失败同页重试、空页和逐条详情选择。活动列表同样补齐20条分页、正确minimum字段、错误与空页区分、onShow重读以及hide/tab/unload迟到隔离，并支持`type=bargain`直接入口。DiySuspendedNavigation保留；浏览器发现活动页原有fragment根导致type查询参数触发Vue warning，本批将悬浮导航移入已有根节点，未移除组件或测试断言。

详情使用页面代次、请求代次和原生导航代次隔离：hide/unload/身份更新清私有数据与prepared结果；旧请求/旧导航回调不能回填、跳转或解锁新身份。登录意图仅保存活动、参与、SKU、数量与owner，返回先读新资格；过期登录恢复精确参与，换账号不恢复原owner的选择。库存下降保留原数量并提示用户，不能静默缩量。写入边界重新读取当前时钟检查活动期限。加购成功但导航失败保留cart/参与并禁改选择，继续结算只重试导航；导航success也不会在onHide之前提前解锁。仍未验证真机生命周期、跨设备或所有原生导航异常，不把隔离替身当作实际设备。

新增26项实际SFC/composable/Vue/Pinia/request/API/shared-parser回归并加入既有test:toolchain，覆盖两条不同价参与、匿名/过期登录和换账号、库存变化、错误数据/基础SKU拒绝、四类不可购买状态、数量与时间边界、重复点击、prepared导航恢复、迟到读写/发起/帮砍、复用页面过期响应、start读取失败和不明结果、本人分页/归属、活动分页/错误/生命周期及改价展示。最终工具链176项零失败/零跳过；vue-tsc终态成功；H5、微信小程序、App三端构建成功，产物3项通过；三端实际构建图i18n审计9项通过。最终复跑设置CI=1关闭厂商更新检查；此前首轮构建没有显式设置，不能据此声明整个过程绝无厂商检查。Worker既有pc-bargain-purchase.test.ts的4项真实HTTP/SQL→共享模型回归重新通过（11.10秒），没有新增后端实现或将其冒充当前独立PG16/全仓库验收。

前端测试技能要求验证实际渲染。本会话未列出专用browser技能，分类为Browser plugin not available，使用已有Playwright1.62.1和本机无头Chrome，未新增依赖。流程为`/#/pages/activity/index?type=bargain`→匿名“我的砍价”不请求私有列表→实际登录表单填写隔离凭据→合成认证响应后实际navigateBack→本人#80→蓝色小号2件→真实SQL加购→人为模拟原生checkout导航失败→继续结算同cart→返回列表选择#90并显示4元参考价。认证、KV、悬浮导航配置为替身，目录/本人列表/加购使用真实控制器和PGlite；只访问127.0.0.1:5199和5198，浏览器阻断外部网络且最终阻断计数0，服务拒绝TEST_FINANCE_POSTGRES_URL。没有生产请求、真实认证/短信、订单或支付调用；本次故意不进入checkout页面，完整浏览器确认/提交/支付仍未验收。

390×844和1280×900均验证URL/标题、非空内容、无框架overlay、真实控件状态、截图与无横向溢出；console error/pageerror为0，每视口保留1条DCloud深导入Vue Router弃用warning。最终轮新增cart4/5，各为uid11/type2/activity40/participant80/base unique qablue01/数量2/new1，重试没有第二次加购；累计调试5条隔离cart，订单0，商品/SKU/活动/参与/帮砍快照不变。测试服务、脚本和截图均在系统临时目录，不提交仓库。

失败记录：最初工具补丁尝试同文件删除并新增被apply_patch拒绝，未部分写入，随后改为正常更新；初次准备临时服务使用不存在的btoa被拒绝，未启动进程。浏览器首次脚本用placeholder定位原生input超时；实查UniApp把提示渲染为独立节点，改为实际登录框内input。后续蓝色SKU文字漏了真实合同中的逗号导致超时；实读目录后修正定位，未改业务数据。第三次已实际加购但快照断言把ORM camelCase当snake_case而失败，修正夹具字段后两视口通过。截图首次保留了登录toast且fullPage固定栏遮挡数量，最终等待toast消失并滚到页面底部采集实际视口，再次完成整条流程；没有把采集问题当成业务失败或隐去那1条已产生的隔离cart。
...
### 本机完整 Worker unit 复审及帮砍数据库时钟修复（2026-09-09）

审计起点为已推送`cfb443f2668a1f7f0e5c9de1c516536e17ef0edd`。复用上一节独立Windows PG16.15，启动前确认已停止且控制库为空；只监听127.0.0.1:55432，测试子进程清除无关环境并显式设置专用URL，不继承生产数据库变量。不改CI、依赖、结构迁移、生产数据或部署；砍价起价调整政策仍待明确选择。

本次重新执行静态路由及PHP源结构审计：PHP1904、Worker1650、精确881、可执行863、明确不可用18、原始缺失1023、退役17、可执行缺口1006；可执行/退役后有效覆盖45.3%/45.7%。当前路由表合计1649是加总陈旧，已修正为1650，不改历史统计。201张PHP参考表全部完整映射到目标263表、缺源列0；该静态结构覆盖不是生产目录验证，也不代表HTTP行为完成。历史数据复制继续N/A。

完整unit原生分片确认314文件，157/157完整互斥，清单SHA256为`1a85648368eea8192c9f450eb1bc07b043be1bede81d3e5d44e1d6c09fc1bbb5`。首轮各4个worker、两片顺序运行，共2678通过/11失败/0跳过，原始报告分别`full-unit-shard1-initial.json`和`full-unit-shard2-initial.json`，保留失败，不包装为通过：

- 4项迁移调度断言仍停在0154/155步；测试现在正确注册0155/156步，验证根数据库独立runner调度，新增0155的already-exists、漂移及非Error拒绝三类失败闭合。实际迁移注册源码未改。
- 1项秒杀数量截止断言遗漏实际SQL条件更新的明确拒绝信息。现在按订单/数量分别限定准确业务消息和ValidateException/400，同时保留真实锁等待与状态不变证明；55P03等底层异常不能冒充截止验证。未改秒杀业务实现。
- 1项十万行外键计划测试在afterEach的默认10秒清理预算超时，主体索引/父表保护断言已通过。清理hook改45秒以容纳原有30秒SQL限额与两次5秒连接排空，并先捕获本夹具再清空引用，避免迟到清理覆盖下一夹具。SQL/业务超时和所有数据/索引断言不变；曾观察到checkpoint等待，但没有足够证据将其断言为该次超时唯一根因。
- 1项帮砍在数据库时钟已截止后仍成功写入。新增受控快/慢一分钟和真实事务触发器跨截止共4项，修复前全部错误成功、修复后全部正确拒绝。帮砍锁后与两项写入后均执行`clock_timestamp() AT TIME ZONE 'UTC'`条件检查，失败在同一事务中回滚help及参与金额。保留原helper→参与锁序、KEY SHARE兼容性、READ COMMITTED与2秒锁/5秒语句及空闲预算，不改价格公式。补两项应用时钟快/慢两小时而PG仍开放的正向用例，并在Asia/Shanghai会话下验证UTC列语义。记录用addTime仍采用原应用时钟，不将它视作准入证据。
- 另1项发起等待得到55P03、3项订单等待未在4秒内观察到PG截止；未修改相关业务或将超时纳入成功断言。定向双worker复跑四项通过，首轮具体时钟/调度根因未定，不能由复跑通过断言已经定位或修复。

定向6文件89项全通过（295.29秒），其中上述4项新增红→绿及12组100003行外键计划；后续新增的2项时钟正向用例不计入这89项。完整分片最终结果另记于本节后续和独立证据文件；禁止以定向通过替代全量。Worker unit/runtime类型检查通过。ActivityJoinService行数变化使sbu_uid查询引用797→805，只同步行号，不改原SQL片段或不可变目录。

本轮使用PostgreSQL技能保留短事务/连接预算防线，Workers技能要求复核当前[Cloudflare请求级连接规则](https://developers.cloudflare.com/workers/best-practices/workers-best-practices/)及[PG16实际时钟区别](https://www.postgresql.org/docs/16/functions-datetime.html)。最新unpkg类型获取失败，回退核查已安装Workers types5.20260828.1、Hyperdrive定义与本地Wrangler schema；不新增平台绑定、跨请求状态或事务内外部调用。本机结果不替代Linux PG16.14、workerd、前端、真实角色/provider或生产发布验收。

最终两片各2个worker、顺序执行：第一片157文件1414项通过（560.55秒），第二片157文件1284项通过（349.21秒），共314文件2698项零失败/跳过/todo。未修改的原生分片核验器确认完整互斥、实际文件匹配且全部断言通过；6项帮砍时钟新增用例均在最终12项帮助文件中通过，第一片原参与等待、第二片原3项订单截止也通过。最终unit/runtime类型检查通过。专用URL不改变显式离线PGlite等夹具的执行引擎，因此不是“2698项都在PG16上执行”，也不替代九路径独立audit:orm或workerd。

收尾发现首轮遗留数据库`cinashop_kefu_runner_ea5824d5c03147af8f2693a359440036`。核验准确数据目录、专用角色、数据库目录创建时间00:24:53 UTC，非空表仅work_callback_event100005、work_group_chat_current100003、work_group_chat_projection_fence100003，非fixture事件0、连接0；对应首轮wgcpf夹具，与旧afterEach迟到清空下一夹具引用的路径吻合。只对该准确目标执行一次DROP DATABASE，未用通配或FORCE；不是宣称首轮自动清理成功。最终仅4个基线库、public业务表0、fixture schema0、其他连接0。准确临时实例在01:03:33 UTC优雅停止，原PID14904与55432监听均消失；保留已停止集群、二进制及报告，删除的合成库可由夹具重建。
...
## 2026-09-09：砍价通用后台编辑防止丢失业务状态

本轮基线`fef93242afebf19215ea6504235bf89c49d6d279`已与origin/main同步。其[Actions34299347230](https://github.com/cinagroup/cinashop/actions/runs/34299347230)11项runner_id=0/steps=0，check102302749875的失败注释明确是GitHub账户付款/额度限制；未重跑或修改门禁。PHP StoreBargain控制器、Validate和StoreBargainServices::saveData共同确认创建与更新区别：只有创建写add_time；实际价格/库存/额度来自attrs，并校验底价、额度与人数。旧Worker通用分支却每次重置sales=0/people=10/addTime及省略字段默认值；管理页面还会把打开时的整份旧表单重新提交，故只修省略字段仍会回放旧库存。

新增BargainAdminService分别处理基本保存与状态。只对白名单中显式字段写入，禁止提交四个系统字段；ID/整数/金额/短文本严格验证，金额以整数分比较、底价必须低于起价、人数至少2且满足每人至少一分、额度不得大于库存。库存/额度编辑额外要求两项原值，READ COMMITTED中获得活动NO KEY UPDATE后对比最新行，冲突拒绝而不是覆盖；不回写整行。普通改名不携带库存，也不改变价格或创建时间；合法显式改价保留原业务公式。状态只接受0/1，已退役行和不存在行拒绝，允许非退役活动再次启用。局部锁/语句/事务空闲为2/5/5秒，保持更严格值，所有调用在事务内等待，不发外部请求。共用保存/状态入口采用64KiB有界JSON解析，其他三类活动有效创建和状态回归通过；未改路由和认证，查询证据行号1486→1490。

实际管理页面增加纯函数构造差异载荷；显式库存变化附带expected.stock/quota，固定本次表单type，支持人数/额度控件。失败保留用户输入，不自动重试，刷新并重开后获得新原值。截图发现固定560px弹窗在390px手机截断，且浮动消息不够醒目，修复为min(560px,100vw-24px)并显示内联错误。原价otPrice不是砍价字段，本页砍价隐藏该无效控件。

最初38项PGlite测试32失败/6通过；修复后的44项定向和9项独立PG16通过，扩展首轮20文件331项通过。最终新增测试共67项（52项实际控制器/SQL、6项实际前端payload函数、9项独立PG16竞争），完整相关集20文件336项零失败/跳过，134.23秒。准确后端PID屏障覆盖建单先锁→改名保留扣减、建单先锁→旧库存编辑拒绝、停用先写→建单回滚、真实删除后迟到保存/状态拒绝、help KEY SHARE期间停用→help回滚、取消回补后旧库存拒绝，以及两条写路径500ms锁超时后的设置恢复和重试。触发器验证READ COMMITTED及严格三类期限，SQL写后异常回滚。首次最终类型检查发现新测试未用import及union Promise推导错误，已修复；最终Worker unit/runtime双类型与Admin完整构建通过。最终回归没有替代整个Worker单元库存、九路径目录、Linux/workerd或CI。

页面环境为127.0.0.1:5209/activity，Vite仅代理本机5208；Browser skill未列出，按前端测试技能使用已有Playwright/Chrome。实际Vue/Element Plus/Axios调用真实控制器和内存SQL；认证、待办及站点配置是明确替身，阻止非loopback网络。初轮因站点配置夹具漏路由产生两条404；诊断复跑因沿用同一名称正确生成空patch而导致测试预期失败，已修为每次不同名称并重建夹具，未把错误算通过。最终1280×900与390×844均完成页面身份/非空/无框架遮罩/控制台0错误0警告/截图/交互检查：打开编辑→合成SQL扣减→改名仅发差异且保留扣减；再次编辑库存→合成SQL扣减→明确冲突且表单保留；刷新→编辑库存→成功。浏览器的“扣减”是专用合成SQL，不是真订单；真实订单并发只由PG测试证明。最终截图显示完整手机表单和红色内联冲突信息。最终候选连同上一截图前一轮累计12次保存（8成功/4预期拒绝）、8次合成扣减；末态stock8/quota0/sales11，orders0/help0，内存实例随后关闭。

使用Workers技能核查请求级资源/有界解析，PostgreSQL技能限定短事务及[PG16兼容行锁](https://www.postgresql.org/docs/16/explicit-locking.html)，前端技能驱动实际交互和双视口截图修复；[Cloudflare最佳实践](https://developers.cloudflare.com/workers/best-practices/workers-best-practices/)已检索，最新types下载失败后核查本地5.20260828.1、Hyperdrive与Wrangler schema。旧客户端全量库存表单不带expected将拒绝，发布时必须配套前后端并验证旧缓存升级策略。本轮无生产SQL、DDL、部署或provider调用。A3k1仅本地候选勾选；A3k继续明确保留完整PHP规格/图片/描述/日期/物流/复制/资格、商品重绑和商品退役全局锁序；当前基本创建没有创建SKU，商品存在检查没有行锁，不冒充这些合同已完成。A3i既有参与价格策略未擅自选择。当前223勾选/162开放/385项。
...
## 2026-09-09：砍价消费端配送选择（A3k8 本地候选）

起点`5beb2665c27be4b6b948ebb254b977f799ff291c`已推送；其Actions34311438285为failure，11项runner_id=0/steps=0，check102338743716记录账户付款/额度限制，未冒充Linux验收。沿用“不承接旧PHP真实历史数据”的范围，本轮不连接生产、部署、运行生产DDL或调用支付/provider。

PHP StoreOrderServices.php:2453的旧checkShipping采用配送方式合并，不能直接视为精确结算合同。新增仅砍价使用的严格响应：显式view=bargain，绑定本人有效cart及唯一活动/来源商品，活动deliveryType为权威，过滤store_func_status/store_self_mention和is_show=1/is_del=0/is_store=1门店；门店商品只能返回relationId所属门店。最多200购物车/200门店，超限拒绝而非静默截断，响应private,no-store。KV配置在事务前读取，SQL使用有界REPEATABLE READ/READ ONLY快照，不加业务行锁、不在SQL事务中做远端I/O；这不是KV与SQL共同原子快照，更不是履约授权/保留。

PC与UniApp结算接通活动方式及所属门店，首读选择可用方式；显式刷新后若原方式失效，不悄悄切换，立即使旧报价无效并等待用户选择。错误/旧响应格式不回退“两种都可用”，迟到结果受页面/请求与认证代次限制，原生上传返回重新读取规则且保留未保存表单。类型1/2/3无需地址：真实报价addressId=0不自动使用已有默认地址，共享报价校验允许null但仍核对商品类型，实物与地址不匹配仍拒绝。未修改砍价价格公式，截图中既有“会员优惠”归类不等于已完成A3i政策选择。

首轮PGlite34项中18失败：新SQL把UTC无时区字段与带时区墙钟直接比较，诊断证实本机Etc/GMT-8导致8小时偏移，另有it.each数组参数展开错误。修复为显式UTC比较、补UTC/新加坡/纽约与空历史窗口回归，并修正测试元组。PC92项中1失败、Uni182项中3失败，原因包含null地址校验和夹具砍价/商品类型不一致；修正合同和夹具后再用真实HTTP证明无物流报价不带默认地址。中间42项有1项静态源码断言仍期待无条件全局门店请求，已更新为砍价专用分支，保留所有首轮失败记录。

最终专项3文件54项（含2个独立PG后端提交/回滚场景）、离线2文件52项、相关45文件665项（310.46秒）均零失败/跳过。独立读连接在写事务仍持活动/门店行锁时返回原已提交规则，释放写者后读到相应新规则，业务表不被选择接口改写。Worker unit/runtime类型、PC92认证运行时/7图片/构建、Uni182工具链/类型/H5/微信/App构建及3项产物验证通过。不是全Worker目录重审、Linux/workerd或真实设备验收。

前端技能的browser技能未列出，按fallback用已安装Playwright/Chrome，不重复安装插件。127.0.0.1:5210/checkout与5211/#/pages/order/confirm接5208内存SQL真实控制器，认证/KV/站点和地址列表替身，所有外部浏览器请求阻止，未挂载建单/付款路由。1280×900及390×844两端4组页面身份、非空、无overlay/横向溢出、截图人工检查通过；真实自提规则隐藏快递、通过后台保存改为配送、刷新保留旧选择且不发报价、手动选配送后重新报价2元。订单0，购物车/库存/参与不变。控制台error=0，Uni仅已有vue-router导入路径弃用warning。前三轮临时脚本因加载等待及PC/Uni文字定位不匹配失败；调整等待条件/定位后同流程通过，日志保留。

证据与LF源码摘要见`workers-ts/audit/local-pg16-bargain-selector-acceptance.json`。Workers技能用于请求内绑定/Promise边界；PostgreSQL技能用于短只读快照与独立连接验证；前端技能驱动双视口真实交互。所有测试终态后核实4个基线库、public表0、fixture schema0、其他客户端0，无人工删库，临时集群/二进制/报告保留。

清单新增A3k8本地候选，230勾选/162开放/392项。A3k父项仍开放：选择接口之外，quote/create必须重验全局开关与门店归属并处理规则变化；同城范围/费用/派单、模板生命周期/内容并发、其他活动旧合并选择、完整PHP编辑/复制/重绑/多SKU、A3i价格策略，以及真实角色/设备/provider/生产发布尚未完成。

临时PG的fast停止等待20秒超时，未强杀或重新发送停止；原进程继续完成检查点，服务器日志确认05:12:59.095 UTC正常关闭，原执行句柄终态退出。55432/5208/5210/5211监听与PG PID12280均已消失。超时日志保留，不把停止请求本身当成停止完成。

## 2026-09-09：砍价自提权威报价与建单准入（A3k9 本地候选）

起点`8dc43e7d0197a6b6361eb349c8949854c1396bf4`已推送；其Actions34314149574因账户付款/额度限制未启动，11项runner_id=0/steps=0，check102346716938留有注释。本轮沿用新系统、不继承旧PHP真实历史数据的范围，不连接生产、不运行生产DDL或部署，不使用此前浏览器验收冒充当前后端验收。

PHP StoreProductServices.php:2574的自提合同要求全局功能与自提开关，以及可用所属门店。SQL SystemConfigDao以is_store=0、sort DESC/id DESC选同名配置；status表示配置字段的后台显示，不是业务开关。此前消费端选择走KV缓存，quote/create没有完整重验。初始14项真实SQL测试中12项失败，复现开关关闭、无效/不属商品的门店，以及真实报价后规则改变仍能创建订单。

新增BargainPickupPolicy，按PHP缺少store_func_status默认1、缺少store_self_mention默认0处理；显式空值关闭，规范化JSON字符串0/1，畸形值失败关闭。选择接口现在也在原只读一致快照内读取SQL开关，不再用可能陈旧的KV授权。quote用单条SQL同时读取来源商品、精确门店及两个按优先级排序的开关，具备语句级一致快照，不改变调用者隔离/只读模式；它仍是报价建议，不预留资源。初版另开REPEATABLE READ/READ ONLY在现有嵌套事务时区测试中触发25001，首轮相关696项中3项失败；修复实现而未放宽或删除这三项测试。首次类型检查另发现测试未使用导入，已移除。

实际type=2/shippingType=2建单在活动锁后、认领购物车前重验来源和开关，并持有system_config表SHARE NOWAIT与精确可用system_store行SHARE NOWAIT至提交。表锁是明确取舍：配置允许重复优先级覆盖与缺省行，单行锁无法防止更高优先级INSERT；因此此锁阻塞全部配置写入而非仅两项开关，不阻塞普通读取。沿用更严格值设置statement/lock/idle-in-transaction为5/2/5秒上限，这不是整个事务的总时限；高负载下配置写入延迟和全局其他写入者锁顺序仍须发布前验证。锁冲突明确要求重试，失败全部回滚。

不提前锁源商品，避免倒置其他交易的SKU→商品顺序；最终商品扣库存UPDATE同时比较报价中的type/relationId/productType以及is_show=1/is_del=0，获得行更新锁后保持到提交。期间若改变归属、履约类型、下架或退役，整单回滚；普通改名不误拒。新规则仅作用砍价自提，快递/免物流合同、已创建订单幂等和取消库存恢复不变，A3i价格公式未作选择或修改。

验收证据记录于`workers-ts/audit/local-pg16-bargain-pickup-acceptance.json`，包含初始红测、嵌套事务失败、最终回归、离线结果及LF源码摘要。真实PG独立连接测试通过pg_blocking_pids核验配置UPDATE/INSERT/DELETE和门店编辑的双向边界：编辑者未提交时建单快速失败，建单快照未提交时后来的编辑者等待；来源变更则由实际库存UPDATE决定回滚，未提前取得来源锁。使用Workers技能核对请求内资源/既有Hyperdrive类型，PostgreSQL技能指导短事务、NOWAIT与实际锁屏障。未修改前端、依赖、用户环境或生产；不将局部并发证据视为整个A3k或生产完成。

最终新增24项服务/HTTP与14项独立PG竞争、离线65项、Worker unit/runtime双类型通过。最终相关47文件703项为702通过/1失败/零跳过（403.64秒），唯一失败是未改动的历史external cart DDL测试30秒超时；服务器同期间279.599秒检查点，05:36:55.783 UTC明确记录DROP DATABASE语句超时。不能写成单轮全绿。所有测试终态后核对残留cinashop_kefu_runner_bdd95244ae504b918b6b58c01aaa8e8f属于finance_test、datconnlimit=-2、连接0，再精确删除；删除的是无效合成测试库，不可恢复且不含真实业务数据。未修改测试超时门槛，原文件随后单独34/34通过（27.85秒）。失败与成功原报告均保留。

最终四个基线数据库、public表0、fixture schema0、其他客户端0，PG16.15临时进程PID332于05:45:29.382 UTC正常停止，55432监听消失，原执行句柄终态退出；本次停止没有超时、强杀或重复请求，未安装系统数据库服务。临时集群/二进制/日志/报告保留。清单231勾选/162开放/393项，只关闭A3k9本地候选。A3k同城履约、模板生命周期/内部并发、完整编辑兼容/复制/重绑/多SKU、全局库存/写入锁序及A3i价格选择、真实角色/设备/provider/生产发布均仍开放。

## 2026-09-09：总后台运费模板原子保存与后续缺口（A3k10 本地候选）

起点`26aa5339069ace11f21d10816902fb431d94d8a5`已推送；其Actions34316300416仍为failure，上一轮check102353124413确认付款/额度导致任务未启动。本轮审计模板报价后编辑/停用时，发现必须先修复写端：PHP ShippingTemplatesServices将父模板、区域、包邮及禁配规则放在事务内；现有供应商服务也在同一事务内持父模板FOR UPDATE，但总后台AdminCrudController原来逐次提交父表、删区域和逐行插区域。即使为报价增加快照，也无法消除原写端已提交的半套数据。

旧代码真实PG红测6项中5项失败：创建/编辑遇到后续区域插入错误会遗留部分提交，单独改名会重置默认元数据并清空区域，不存在或已软删模板仍返回写入成功。修复为独立AdminShippingTemplateService，以同一个事务保存所有提交字段和区域，先持父模板NO KEY UPDATE直至提交；等待后重新确认未删除。未提供元数据或regions时保留原值，显式[]仍表示清空；归属、创建时间及特殊包邮/禁配规则不从该扁平表单覆盖。新区域批量插入，并写入当前计费分组。新建固定平台归属；没有改变现有路由权限或总后台可管理范围。

请求限制256KiB、区域最多1000项，校验安全整数、名称和最多两位小数的非负数值；继续允许既有零计量/空区域请求，不能将格式校验视为完整业务费率有效性验收。事务内语句/锁/空闲分别以5/2/5秒为上限并保留调用者更严格值，这不是事务总时限。锁超时返回明确重试错误，其他失败回滚；无外部网络I/O、全局表锁、DDL或价格公式变化。Workers技能用于请求内资源与有界输入审查，PostgreSQL技能用于短事务和父行锁协议；没有修改前端或依赖，也没有生产SQL、部署或真实角色/浏览器验收。

26项实际Hono控制器/SQL合同测试与7项独立PG连接竞争通过，另26项离线PGlite通过，Worker unit/runtime双类型通过。认证在夹具中省略，不把HTTP测试称为真实权限验证。独立连接通过pg_blocking_pids确认：保存未提交期间旧提交数据不变，父行SHARE读取、第二管理员、实际供应商保存及删除等待；等待后已删除则拒绝，另一模板可独立保存，更严格超时后原连接设置恢复。此证据只证明这些已知写入口和遵守父行锁的读取者，不保证任意多次无锁SELECT看到同一版本，也不保护绕过协议的直接子表SQL写入。原始红/绿、定向/相关回归、离线日志和LF源码摘要归档于`workers-ts/audit/local-pg16-admin-shipping-atomic-acceptance.json`。

后续拆成三项明确待办，均未勾选：A3k11，StoreOrderCreateService目前分开读模板/区域/条件包邮/禁配/城市，尚无完整一致快照、商品所属方过滤和报价后模板内容重验；全局包邮分支跳过此读取及计算器的禁配判定，须先以真实业务合同测试确认并修复。A3k12，总后台删除只有软删，缺商品/活动引用与新增绑定竞争保护；本次保存/删除串行不等于引用生命周期完成。A3k13，总后台列表仍是两次无界独立读取，当前界面仅扁平regions，缺PHP分组城市、完整三种计量及包邮/禁配编辑与显式旧值冲突保护，须接通后再做真实桌面/手机流程。父项A3k、同城履约、全局锁序、A3i价格选择以及所有发布门禁继续开放。

最终相关回归30文件415项零失败/跳过，200.49秒；范围为本轮模板控制器/服务、供应商保存、运费计算/既有迁移合同、请求体、砍价后台及下游配送/自提和三份索引证据，不冒充完整Worker测试。本轮未修改订单核心文件，因此没有借用上一轮703项结果或将其历史超时删除出记录。清单实际逐行计数232勾选/165开放/397项，增加一项本地候选和三项拆分待办。

测试终态后确认四个基线数据库、public表0、非系统附加schema0、其他客户端0。临时PG16.15 PID17012收到fast停止请求，pg_ctl等待20秒超时退出1；未重复停止或强杀，原进程继续完成23.866秒关闭检查点，日志确认06:09:42.647 UTC已停止，原执行句柄退出0，PID和55432监听均消失。测试schema已自行清理，本轮没有人工删除数据库；临时集群、二进制和原始日志保留。此本机隔离验证不替代Linux/workerd/远端CI、Cloudflare/Hyperdrive或生产发布。

## 2026-09-09：模板一致快照、禁配与同请求建单守卫（A3k11a 本地候选）

承接已推送`bee185aae34bce66b0f37df865dcdc11e104096f`。上一提交Actions34317983511终态failure，11项runner_id=0/steps=0，check102358157076确认账户付款/额度问题导致未启动。本轮未访问生产、未运行生产DDL/DML、未部署或修改计费。Workers技能用于请求内数据/有界结果和现有Hyperdrive接线，最新类型远端读取失败后使用本地5.20260828.1核验；PostgreSQL技能用于一致语句快照、短事务和无反向等待锁边界。

PHP StoreCartServices::handleCartList在824～844行先按城市关系与temp_id读取禁配、921行据此使商品无效，独立于费用优惠。旧Workers仅在有地址且非全局/套餐包邮时读模板，9项真实PG红测全部失败：全局包邮跳过禁配、跨所属方模板可报价，以及同一次真实创建的计价结束后费率、模板status/isDel/owner、指定包邮、禁配或城市路径改变仍成功建单。该红测没有使用旧真实历史数据，业务数据均为隔离合成夹具。

ShippingTemplateSnapshot以一个SQL语句返回父模板、区域费率、条件包邮、禁配和所选城市path，金额字段转text，按ID确定顺序；同语句MVCC避免不同SELECT夹杂提交，不另设隔离级别，因此嵌套只读事务可用。最多200模板，每类子集读取10001行作溢出哨兵，超过10000立即拒绝，不截断后计算。绑定模板必须存在、启用、未删除且owner_type/relation_id精确匹配商品；未知正城市ID拒绝，无绑定固定/包邮不强制引用模板1。计费型旧tempId=0仍使用模板1并检查其禁配，全国[0]规则也对有城市ID的地址生效。满额或套餐包邮先验证禁配、再保留原有totalPostage/payPostage为零的金额口径，不修改A3i砍价价格政策。

真实建单只在现有有地址实物快递配送段建立此快照。保留原活动/SKU/商品顺序，在原子购物车认领、SKU扣减和商品扣减处比较数量/身份、计量、所属方及有效配送绑定；无初始模板的固定/包邮请求也保护中途改绑。拼团/积分活动的freight/postage/tempId在原库存UPDATE比较，砍价/秒杀沿用既有活动守卫。事务末端按ID持模板SHARE NOWAIT和所选城市SHARE NOWAIT到提交后重读快照，变更拒绝，不静默改价；名称/排序等非计价元数据不导致失败。NOWAIT避免模板编辑者与业务资源之间形成新增反向等待。事务必须READ COMMITTED，拒绝嵌套旧快照创建而不改变其隔离设置；保留更严格语句/锁/空闲5/2/5秒上限，这不是整个事务的期限。任何失败会回滚订单、认领、库存及相关写入。

独立PG测试使用实际原子保存服务、实际创建服务及pg_blocking_pids屏障：模板父读取被隔离SQL函数阻塞期间另一事务提交整套变更，当前报价保持旧一致版本、下一次报价读新版本；后台区域替换未提交时只读报价读旧值，而实际建单NOWAIT拒绝并全退；实际订单已校验但外层事务未提交时，后台保存/退役/城市编辑等待。另验证城市锁冲突、嵌套REPEATABLE READ只读报价及创建拒绝、调用者更严格设置恢复。新增活动用例覆盖砍价/拼团/积分/套餐真实创建与回滚、套餐包邮禁配及实际砍价HTTP确认/重算。初轮活动专项12项中3项因夹具使用7字符SKU unique、PG CHAR(8)补空格与精确Map匹配不同而失败；改用真实生成合同的8字符合成标识后12项通过，原失败报告保留，没有修改套餐选择实现或放松断言。

严守未完成边界：A3k11b，实际OrderController允许缺失/无效addressId回落空值及客户端地址字段覆盖，除积分商品外未统一要求实物配送完整地址，hasDeliveryAddress=false仍跳过配送段。A3k11c，rememberCheckout仅保存cartIds；用户此前confirm/computed与后来create并无规则/金额版本绑定，本轮所谓“报价后变更”测试仅证明同一次创建内部计价到提交，不能宣称实现跨HTTP确认报价承诺。A3k11d，全局包邮/线下运费/会员权益配置仍经KV及独立读取，不在模板快照之内。三项均明确保留为待办；A3k11父项、A3k12引用保护、A3k13完整模板管理及真实角色/浏览器/Linux/workerd/生产发布仍开放。直接绕过应用父行锁协议的子表SQL不受本轮锁保护，不能将已知写入口的竞争证据推广为所有数据库写者。

最终验证：新增24项服务/SQL、12项活动/HTTP及8项独立PG测试。首轮相关72文件910项通过；加入无初始绑定的原子守卫和活动专项后，最终73文件924项零失败/跳过（399.08秒），离线3文件46项通过（67.60秒），最终Worker unit/runtime双类型检查退出0。相关集合覆盖所有直接引用订单创建/运费计算的unit测试及前轮模板/砍价后台队列，混合静态、纯函数、实际SQL和PG竞争测试，不冒充924项都是并发SQL或完整Worker套件。原始红测、活动夹具失败、各轮结果及源码LF摘要保存于`workers-ts/audit/local-pg16-shipping-template-quote-acceptance.json`。实际清单233勾选/168开放/401项，仅增加A3k11a本地候选，b～d、父项及发布门禁保持开放。
...
## 2026-09-09 依赖候选提交与自身Linux CI跟进

c78f01f（依赖与回归）和761b7875ad72bf9a96d1ca42318b65a0e327d773（后台API快照）已分开提交并推送main。六份项目锁文件只在Worker出现Hono4.13.5，Vitest/mocker只在Worker与Kefu出现且均4.1.11，没有遗漏其他锁文件中的副本。客服再次npm ci --ignore-scripts后17项应用、11项工具链及类型/构建全通过。

[Actions34323812650](https://github.com/cinagroup/cinashop/actions/runs/34323812650)对应完整761b787 SHA，已确认实际运行。最新快照八项成功：五端构建、密钥扫描、隔离PostgreSQL16目录、Linux workerd；两个Worker完整单元分片仍运行，最终汇总尚未完成，不能宣称全绿。runtime任务102376352030实际4文件27项全通过（9.81秒）；Kefu任务102376352051实际17项应用和11项工具链通过，全树审计0漏洞。Linux结果补足相应平台运行时证据，但不表示Windows启动崩溃已修复。旧cd5ac6c的失败不删除；独立候选复核仍无结论，TEST-004G/H/I暂不勾选，当前233勾选/171开放/404项。本段仅文档更新，不改变761b787被测代码，工作流路径过滤不会为根文档单独启动新CI；后续须继续核验原运行终态，不借给未来源码变更。

## 2026-09-09 761b787自身CI终态与地址入口后续审查

Actions34323812650已终态success，首轮11/11，无重跑或门禁调整。两单元分片各171文件，分别1620与1518项全通过，共342文件3138项、0跳过；UNIT_SHARD_AUDIT双方inventory摘要9e37d28a29b9efba736317d878f75af2c9890641da043ce643fa9ef5df372ffd一致，完整/不重叠及实际执行匹配均true。两片测试耗时424.27/398.66秒。Worker双类型、生产审计、SQL源201→263零源列缺口/窄范围漂移0、路由/观测、隔离PG16目录、五端构建、密钥扫描、workerd4文件27项均通过。关闭TEST-004I，234勾选/170开放/404项；依赖项G/H保留独立候选复核缺口，不把整个迁移标记完成。

A3k11b调用链审查：普通创建OrderController:351先查地址但仅对查到的跨用户/已删行拒绝，缺失/无效ID可回落空值；360后的参数允许客户端收货人/手机/省份/城市/地址覆盖。StoreOrderCreateService:881先查既有幂等订单，1482的hasDeliveryAddress则使无地址跳过运费段；公共控制器在核心幂等之前查已删地址，后续必须验证地址退役后仍能取得原订单。LegacyOrderCompatibilityService:299默认地址/空地址预览、OrderCouponController:43无地址优惠券报价、AdminAssistedOrderService:805允许代客手填地址是不同合同，不能统一禁用。PHP StoreOrder::create在222～244行普通实物快递要求addressId、按uid/id/is_del读取，自提仅填联系人；核心使用服务端addressInfo生成快照。当前UserCenterService:599～621已经按city_area完整层级解析省市区并拒绝ID冲突，可作为复用依据，不能另用system_city猜同一ID空间。
