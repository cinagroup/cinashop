-- Bounded scans for the five-minute second-card reminder workflow.
-- Keep advent and expiry flags separate because they represent distinct notices.
-- The existing order outbox is deliberately fail-closed, so expand its event
-- whitelist in the same transaction as the scan indexes.
