...
export async function auditRuntimeBusinessPrivileges(db: DbClient, kind:'app'|'admin', names:{app:string;admin:string;maintenance:string}) {
  if(!Object.hasOwn(db,'$client') || !db.$client)throw Error('Root connection required');
  Object.values(names).forEach(pricingIdentifier);
  if(new Set(Object.values(names)).size!==3)throw Error('Distinct identities required');
