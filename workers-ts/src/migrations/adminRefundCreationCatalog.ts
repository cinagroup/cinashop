...
export const ADMIN_REFUND_CREATION_CATALOG_SQL = `
...
export const ADMIN_REFUND_CREATION_EXPECTED_SHAPE = {
  columns: [
    ...
    [4,'order_id','integer',true,null,'','',null],
    [5,'refund_id','integer',false,null,'','',null],
    [6,'outcome','character varying(16)',true,null,'','','pg_catalog.default'],
    [7,'created_at','timestamp with time zone',true,'clock_timestamp()','','',null],
  ],
  constraints: [
    ['arc_hash_ck','c',"CHECK (((request_hash)::text ~ '^[0-9a-f]{64}$'::text))"],
    ['arc_identity_ck','c','CHECK (((admin_id > 0) AND (order_id > 0)))'],
    ['arc_key_ck','c',"CHECK (((request_key)::text ~ '^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'::text))"],
    ['arc_outcome_ck','c',"CHECK (((((outcome)::text = 'created'::text) AND (refund_id IS NOT NULL) AND (refund_id > 0)) OR (((outcome)::text = 'abandoned'::text) AND (refund_id IS NULL))))"],
    ['arc_pk','p','PRIMARY KEY (admin_id, request_key)'],
  ].map(([name,type,definition])=>[name,type,definition,true,false,false,type!=='c',true,0,'0']),
  indexes: [
    ['arc_order_history','CREATE INDEX arc_order_history ON public.admin_refund_creation USING btree (order_id, created_at, admin_id, request_key)',
      false,false,true,true,true,true,false,false,false,4,4,'4 7 1 2','0 0 0 0',null,null],
    ['arc_pk','CREATE UNIQUE INDEX arc_pk ON public.admin_refund_creation USING btree (admin_id, request_key)',
      true,true,true,true,true,true,false,false,false,2,2,'1 2','0 0',null,'arc_pk'],
  ],
};

export const ADMIN_REFUND_CREATION_COMPATIBILITY_SQL = `SELECT present,oid,
  present AND safe AND shape = $arc_shape$${JSON.stringify(ADMIN_REFUND_CREATION_EXPECTED_SHAPE)}$arc_shape$::jsonb AS compatible
FROM (${ADMIN_REFUND_CREATION_CATALOG_SQL}) catalog`;
