...
import { shippingTemplatesRegion, shippingTemplatesFree, shippingTemplatesNoDelivery, systemCity } from '@/models/schema';
import { ValidateException } from '@/utils/errors';

/** Shared rule contract: independent of admin/supplier writers and replay orchestration. */
...
export function shippingRuleInteger(
  value: unknown,
  label: string,
  minimum: number,
  maximum: number,
...
function flag(value: unknown, label: string): 0 | 1 {
  return shippingRuleInteger(value ?? 0, label, 0, 1) as 0 | 1;
}
...
function cityPaths(value: unknown, label: string, allowNationwide: boolean): number[][] {
  ...
  return value.map((rawPath) => {
    ...
    const path = rawPath.map((entry) => shippingRuleInteger(entry, `${label}地区ID`, 0, 2_147_483_647));
    if (path.includes(0)) {
...
export function normalizeSupplierShippingTemplateInput(
  ...
  const billingType = shippingRuleInteger(input.type, "计费方式", 1, 3) as 1 | 2 | 3;
  const requestedAppoint = flag(input.appoint, "指定包邮状态");
  const requestedNoDelivery = flag(input.no_delivery, "禁配状态");
  const sort = shippingRuleInteger(input.sort ?? 0, "排序", 0, 2_147_483_647);
