...
export class MigrationService {
  ...
  /** Exact short-video compatibility DDL used by production-engine verification. */
  shortVideoCompatibilityMigrationSqlForVerification(): string {
  ...
  /** Exact Enterprise WeChat callback inbox/outbox DDL used by production verification. */
  workCallbackPipelineMigrationSqlForVerification(): string {
    return this.migration_0115();
  }

  /** Exact Enterprise WeChat follow-removal projection guard DDL. */
  workCallbackFollowProjectionMigrationSqlForVerification(): string {
    return this.migration_0116();
  }

  /** Exact Enterprise WeChat callback projection-state separation DDL. */
  workCallbackProjectionStateMigrationSqlForVerification(): string {
    return this.migration_0117();
  }

  /** Exact canonical Enterprise WeChat member projection DDL. */
  workMemberCurrentProjectionMigrationSqlForVerification(): string {
    return this.migration_0118();
  }

  /** Exact immutable resolved-rename edge fence DDL. */
  workMemberResolvedRenameFenceMigrationSqlForVerification(): string {
    return this.migration_0119();
  }
  ...
  /** Exact canonical Enterprise WeChat external-tag catalog projection DDL. */
  workExternalTagCurrentProjectionMigrationSqlForVerification(): string {
          ...
          if (i === 118) {
            const markerResult = await tx.execute(sql`
              SELECT EXISTS (
                ...
                WHERE constraint_row.conrelid = to_regclass('work_member_identity_alias')
                  AND constraint_row.conname = 'wmia_resolved_link_required_ck'
              ) AS applied
            `);
            const markerRows = Array.from(
              markerResult as unknown as Iterable<{ applied: boolean }>,
            );
            if (markerRows[0]?.applied === true) {
              supersededByResolvedRenameFence = true;
              return;
            }
          }
          await tx.execute(sql.raw(migrations[i]));
        });
        executed.push(supersededByResolvedRenameFence
          ? "0118 (superseded by 0119)"
          : String(i).padStart(4, "0"));
      } catch (e) {
        // Historical migrations predate exact catalog verification and retain
        // their compatibility skip. Modern callback migrations (0115+) are
        // fail-closed: an "already exists" name collision may have rolled the
        // whole transaction back before the shape verifier ran.
        const msg = e instanceof Error ? e.message : String(e);
        if (i < 115 && msg.includes("already exists")) {
...
CREATE INDEX IF NOT EXISTS "so_activity_type_visible"
  ...
  private migration_0109(): string {
    return `-- Worker-owned short-video compatibility extension.
...
CREATE INDEX IF NOT EXISTS "system_config_lookup"
  ...
  private migration_0111(): string {
    return `-- Worker-owned anonymous customer-service sessions. Raw bearer tokens and IP
...
CREATE INDEX IF NOT EXISTS "sst_customer_scope_time"
  ...
  private migration_0115(): string {
    return `CREATE TABLE IF NOT EXISTS "work_callback_event" (
  "id" serial PRIMARY KEY,
...
`;
  ...
  private migration_0116(): string {
    return `-- Add an explicit terminal projection state and canonical active identities
...
`;
  ...
  private migration_0117(): string {
    return `-- Separate durable callback transport/order state from business projection outcome.
ALTER TABLE "work_callback_event"
...
`;
  ...
  private migration_0118(): string {
    return `-- Expand-only canonical Enterprise WeChat member projection. The three legacy
...
`;
  ...
  private migration_0119(): string {
    return `-- Preserve the immutable callback fence on every resolved rename edge.
...
`;
  ...
  private migration_0146(): string {
    return EXTERNAL_DUPLICATE_INDEX_RETIREMENT_SQL;
  }

  private migration_0147(): string {
    return COLUMN_DEFAULT_ALIGNMENT_SQL;
  }
