...
import { inspectPurchaseCancellationEvidence, runPurchaseCancellationEvidence } from '../src/migrations/runPurchaseCancellationEvidence';
...
try {
  ...
  const target = process.env.PURCHASE_CANCELLATION_MAINTENANCE_DATABASE_URL;
  ...
  const result = await (mode==='inspect' ? inspectPurchaseCancellationEvidence(db,role) : runPurchaseCancellationEvidence(db,role));
  ...
  process.stderr.write('Purchase cancellation maintenance failed; explicit target and confirmation required, no fallback, retry or repair attempted.\n');
  process.exitCode=1;
} finally {
  try { await client?.end({ timeout:5 }); }
  catch { process.stderr.write('Purchase cancellation connection cleanup failed.\n'); process.exitCode=1; }
}
