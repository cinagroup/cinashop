...
$taskAuditName = "cinashop-supplier-sku-audit-" + [Guid]::NewGuid().ToString("N").Substring(0, 12)
$taskConfigPath = "test/integration/supplier-product-sku-lifecycle-audit.wrangler.jsonc"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary Supplier SKU lifecycle audit Worker deployment failed"
    }
    $taskDeployed = $true
    ...
    if (-not $taskUrlMatch.Success) {
        throw "Temporary Supplier SKU lifecycle audit Worker URL was not reported"
    }
    ...
    if ($taskUnauthorizedStatus -ne 403) {
        throw "Temporary Supplier SKU lifecycle audit Worker did not enforce token authorization"
    }
    $taskWrongMethodStatus = Get-AuditStatus -Method Get -Uri "$taskWorkerUrl/run" `
        -Headers @{ "X-Audit-Token" = $taskToken }
    if ($taskWrongMethodStatus -ne 404) {
        throw "Temporary Supplier SKU lifecycle audit Worker accepted an unsupported method"
    }
...
if ($taskDeployed -and (-not $taskDeleted -or -not $taskUrlMissing)) {
    throw "Temporary Supplier SKU lifecycle audit Worker cleanup did not converge"
}
