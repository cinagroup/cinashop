...
$taskAuditName = "cinashop-virtual-card-refund-audit-" + [Guid]::NewGuid().ToString("N").Substring(0, 12)
$taskConfigPath = "test/integration/virtual-card-refund-audit.wrangler.jsonc"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Temporary virtual-card refund audit Worker deployment failed" }
    $taskDeployed = $true
    $taskDeploymentText = $taskDeployOutput -join "`n"
    $taskUrlMatch = [regex]::Match($taskDeploymentText, "https://[A-Za-z0-9.-]+\.workers\.dev")
    if (-not $taskUrlMatch.Success) { throw "Temporary virtual-card refund audit Worker URL was not reported" }
...
if ($taskDeployed -and (-not $taskDeleted -or -not $taskUrlMissing)) {
    throw "Temporary virtual-card refund audit Worker cleanup did not converge"
}
