...
const checkout = args.length === 1 && args[0] === '--checkout';
const validArgs = args.length === 0 || checkout;
let valid = false;
try {
...
if (!validArgs) {
  process.stderr.write('Only the optional --checkout permission scope is accepted. No database was contacted.\n');
  process.exitCode = 2;
} else if (!valid || !connectionString) {
  ...
  try {
    const result = await (checkout ? auditCheckoutRuntimePermissions : auditPaidOrderRuntimePermissions)(drizzle(client));
