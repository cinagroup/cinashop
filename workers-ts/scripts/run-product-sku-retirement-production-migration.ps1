...
$taskAuditName = "cinashop-sku-retirement-migrate-" + [Guid]::NewGuid().ToString("N").Substring(0, 12)
$taskConfigPath = "test/integration/product-sku-retirement-production-audit.wrangler.jsonc"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary product SKU retirement migration Worker deployment failed"
    }
    $taskDeployed = $true
    ...
    if (-not $taskUrlMatch.Success) {
        throw "Temporary product SKU retirement migration Worker URL was not reported"
    }
    ...
    if ($taskUnauthorizedStatus -ne 403) {
        throw "Temporary product SKU retirement migration Worker did not enforce token authorization"
    }
