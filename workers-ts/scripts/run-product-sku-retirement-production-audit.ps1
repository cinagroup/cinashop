...
$taskAuditName = "cinashop-sku-retirement-audit-" + [Guid]::NewGuid().ToString("N").Substring(0, 12)
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary product SKU retirement audit Worker deployment failed"
    }
    $taskDeployed = $true
    ...
    if (-not $taskUrlMatch.Success) {
        throw "Temporary product SKU retirement audit Worker URL was not reported"
    }
    ...
    if ($taskUnauthorizedStatus -ne 403) {
        throw "Temporary product SKU retirement audit Worker did not enforce token authorization"
    }
