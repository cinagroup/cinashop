...
$taskConfig = Join-Path $taskRoot 'test/integration/test-release-schema-migrate.wrangler.jsonc'
$taskCli = Join-Path $taskRoot 'node_modules/wrangler/bin/wrangler.js'
$taskName = 'cinashop-test-release-schema-migrate-' + [Guid]::NewGuid().ToString('N').Substring(0,12)
...
try {
    ...
    $taskWriteAuth = @{ 'X-Audit-Token' = $taskToken; 'X-Migration-Operation' = '0146-0151-test-release-schema' }
    ...
    do {
        ...
        $taskAccess.readinessStatus = [int](Invoke-WebRequest -Uri "$taskUrl/preflight/test-release" -SkipHttpErrorCheck -TimeoutSec 5).StatusCode
    ...
    $taskBefore = Get-MaintenanceJson "$taskUrl/preflight/test-release" Get $taskAuth
    ...
    $taskFirst = Get-MaintenanceJson "$taskUrl/migrate/test-release" Post $taskWriteAuth
    if ($taskFirst.migration -ne '0146-0151-test-release-schema' -or $taskFirst.ready -ne $true -or
    ...
    $taskAfter = Get-MaintenanceJson "$taskUrl/preflight/test-release" Get $taskAuth
    ...
    if ($VerifyIdempotence) {
        ...
        $taskRepeat = Get-MaintenanceJson "$taskUrl/migrate/test-release" Post $taskWriteAuth
