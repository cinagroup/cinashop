...
$taskAuditName = "cinashop-system-config-duplicate-audit-" + [Guid]::NewGuid().ToString("N").Substring(0, 12)
$taskConfigPath = "test/integration/system-config-duplicate-audit.wrangler.jsonc"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Temporary system config audit Worker deployment failed" }
    $taskDeployed = $true
    $taskDeploymentText = $taskDeployOutput -join "`n"
    $taskUrlMatch = [regex]::Match($taskDeploymentText, "https://[A-Za-z0-9.-]+\.workers\.dev")
    if (-not $taskUrlMatch.Success) { throw "Temporary system config audit Worker URL was not reported" }
