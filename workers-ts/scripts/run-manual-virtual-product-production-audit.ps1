...
$taskAuditName = "cinashop-manual-virtual-audit-" + [Guid]::NewGuid().ToString("N").Substring(0, 12)
$taskConfigPath = "test/integration/manual-virtual-product-audit.wrangler.jsonc"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Temporary manual virtual audit Worker deployment failed" }
    $taskDeployed = $true
    $taskDeploymentText = $taskDeployOutput -join "`n"
    $taskUrlMatch = [regex]::Match($taskDeploymentText, "https://[A-Za-z0-9.-]+\.workers\.dev")
    if (-not $taskUrlMatch.Success) { throw "Temporary manual virtual audit Worker URL was not reported" }
    ...
    if ($taskUnauthorizedStatus -ne 403) { throw "Temporary manual virtual audit Worker did not enforce token authorization" }
    $taskWrongMethodStatus = Get-AuditStatus -Method Get -Uri "$taskWorkerUrl/run" `
        -Headers @{ "X-Audit-Token" = $taskToken }
    if ($taskWrongMethodStatus -ne 404) { throw "Temporary manual virtual audit Worker accepted an unsupported method" }
...
if ($taskDeployed -and (-not $taskDeleted -or -not $taskUrlMissing)) {
    throw "Temporary manual virtual audit Worker cleanup did not converge"
}
