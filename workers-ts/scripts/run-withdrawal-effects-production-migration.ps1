# Explicit DB-007 only. No automatic retry after any uncertain mutation result.
[CmdletBinding()]
param([switch]$Apply, [switch]$VerifyIdempotence)
$ErrorActionPreference = 'Stop'
if (-not $Apply) { throw 'Explicit -Apply required for the fixed DB-007 maintenance operation.' }
if (-not $env:CLOUDFLARE_API_TOKEN) { throw 'CLOUDFLARE_API_TOKEN is required.' }
$taskRoot = Split-Path -Parent $PSScriptRoot
$taskConfig = Join-Path $taskRoot 'test/integration/withdrawal-effects-migrate.wrangler.jsonc'
$taskCli = Join-Path $taskRoot 'node_modules/wrangler/bin/wrangler.js'
$taskName = 'cinashop-withdrawal-effects-migrate-' + [Guid]::NewGuid().ToString('N').Substring(0,12)
...
try {
    ...
    $taskWriteAuth = @{ 'X-Audit-Token' = $taskToken; 'X-Migration-Operation' = '0131-0133-withdrawal-effects' }
    ...
    do {
        ...
        $taskAccess.readinessStatus = [int](Invoke-WebRequest -Uri "$taskUrl/preflight/0131" -SkipHttpErrorCheck -TimeoutSec 5).StatusCode
    ...
    $taskAccess.noToken = [int](Invoke-WebRequest -Uri "$taskUrl/migrate/0131" -Method Post -SkipHttpErrorCheck -TimeoutSec 20).StatusCode
    $taskAccess.wrongMethod = [int](Invoke-WebRequest -Uri "$taskUrl/migrate/0131" -Headers $taskAuth -SkipHttpErrorCheck -TimeoutSec 20).StatusCode
    $taskAccess.noExplicitOperation = [int](Invoke-WebRequest -Uri "$taskUrl/migrate/0131" -Method Post -Headers $taskAuth -SkipHttpErrorCheck -TimeoutSec 20).StatusCode
    ...
    $taskBefore = Get-MaintenanceJson "$taskUrl/preflight/0131" Get $taskAuth
    if ($taskBefore.supported -ne $true -or $taskBefore.withinBudget -ne $true) { throw 'Unsupported target or maintenance size.' }
    $taskStage = 'apply-0131'
    $taskFirst = Get-MaintenanceJson "$taskUrl/migrate/0131" Post $taskWriteAuth
    if ($taskFirst.migration -ne '0131-0133-withdrawal-effects' -or $taskFirst.ready -ne $true -or
    ...
    $taskAfter = Get-MaintenanceJson "$taskUrl/preflight/0131" Get $taskAuth
    ...
    if ($VerifyIdempotence) {
        ...
        $taskRepeat = Get-MaintenanceJson "$taskUrl/migrate/0131" Post $taskWriteAuth
