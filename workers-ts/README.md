...
- 保留旧 `supplier_ticket_print` 历史配置与现行 `print_document`；收据打印已恢复 Admin/Supplier 严格租户隔离的打印机/内容管理、易联云/飞鹅云异步任务、脱敏账本与 UNKNOWN 人工处置。电子面单另以 `0091` 独立任务账本恢复一号通 HTTPS 签发、整单/拆单发货、并发租约和人工重签/确认/应用已有面单/关闭；生产 Hyperdrive 随机 schema 已验证所有状态且 `public` 指纹不变（两条链路均尚未发布，生产电子面单表/配置/secrets 仍缺失，也未调用真实提供商）
...
- 客服独立 `/kefuapi` 已恢复 22 条核心合同：专用账号密码 token、来源 HMAC 脱敏的 Durable Object 登录限流、会话/聊天权限、用户分组/标签、公共/个人话术及个人分类；生产 PostgreSQL 已幂等应用 `0092` 四个查询索引，随机 schema 13 项授权/并发/回滚断言通过且公共业务行/序列指纹不变（主 Worker未发布；生产客服账号/会话均为 0，现有 3 条消息尚无可登录客服，前端/WebSocket/转接/上传/订单退款商品/扫码微信登录仍未完成）
...
- [ ] 从源 MySQL 复制并复核客服账号/bcrypt 密码/用户绑定、会话/消息、话术/分类；实现 Kefu 前端及专用 token WebSocket、消息/已读/在线、转接审计、私有 R2 上传、订单/退款/商品授权复用，再以真实账号验证登录限流与浏览器 E2E。二维码/微信/ticket 登录与 ERP 写入在挑战/回调验签和幂等协议完成前保持关闭
