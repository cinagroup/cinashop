...
| R2 / Images | operation status plus `r2_object_*` | slow GET >=500 ms is diagnostic | any native `internalError`; any application write/read/transform failure | Preserve object/metadata compensation ordering, inspect transform/cache failure rates, and check object existence before repair. Never include attachment IDs, object keys or signed URLs in events. |
