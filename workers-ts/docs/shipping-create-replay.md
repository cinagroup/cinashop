...
## 真实PG多标签与390px恢复增量（2026-09-14）

本轮直接验证a064815767daa7991706b104407bb311aef422e3，不改业务源码。对应候选CI
[34804509771](https://github.com/cinagroup/cinashop/actions/runs/34804509771)核验时仍
in_progress；五前端、workerd及secret scan已成功，两单元分片及PG目录作业仍运行。
没有重复dispatch，也没有把局部成功记为整个CI通过。线上与main仍9fb7d27。

用户指定的浏览器插件在 `http://127.0.0.1:5396/shipping-templates` 打开两个实际Supplier
标签。沿用前节临时服务及同一真实受限LOGIN连接；本次验证的是浏览器Web Lock/持久意图，
不是新增独立数据库连接竞争实验。登录签发/Redis仍为替身，Node而非Cloudflare。

| 实际操作 | 权威结果 |
| --- | --- |
| 标签A提交8.75后挂起响应正文，标签B点新增 | B显示“另一标签页正在处理”，保存禁用；没有发出B的创建请求，真实SQL仅10001和一条回执 |
| A达到30秒期限，B取消后重开 | B载入原名称“PG双标签原意图”和8.75并冻结，锁已释放；没有另造新键 |
| 合成账号改属40，B在390×844点击恢复查询、原请求重试 | 同一原键/原scope两次HTTP412，五表逐字相同、序列10001；错误说明换行可读，底部三个按钮完整可见 |
| 恢复归属20，两标签分别查询 | 都查回10001；B明确确认完成后才能创建下一意图 |
| B新建“PG新意图不被旧标签清除”/6.50后断响应；A点击旧10001的确认完成 | A明确提示意图已在另一标签改变，没有清除新意图，没有额外创建或查询请求 |
| B刷新→新增→恢复查询→确认完成 | 恢复新名称/6.50，原新键查回10002；五表与新提交后逐字相同，列表仅10001/10002/count2 |

原键 `2d70e803-78e9-4182-abbf-fc6378852411`，摘要
`b8f67a4034ccde852ef53fe7bf1dfe4947660b12ba4e5ed26ad19cbcae66df8d`；新键
`6bebd79c-8a4e-4442-a8ff-08135ad84f96`，摘要
`40f02d58035dc05d8c45197ab58f40ec607bde4fba1d523cfdc1a83aa5ffb0c8`。
共7个业务POST：两个成功创建、一个被拒绝重试、一个被拒绝查询、三个成功查询；
两条不同键分别对应两次明确的新建，不是重复提交。所有POST scope均v1:2:20:27。

随机数据库 `cinashop_kefu_runner_8bc8c0b0019f4749a895240f4f1a1f1c`，角色
`cinashop_runtime_39f335b97cc64a4986ced232b116b9d9`；current_user=session_user，
PG160015、五项高权限false。独立psql复核模板3（含默认1）/配送2/回执2，回执
SELECT/INSERT=true、UPDATE/DELETE=false。明确确认后服务退出0，独立查询本次精确库和
角色均0残留；只删除本次可重建夹具，原有库未动，浏览器尺寸override已reset，测试标签关闭。

两标签URL/标题正确、非空，无框架遮罩，error/warn日志均为空。1280×720记录跨标签
拦截及旧确认拒绝；390×844记录412、10001和10002成功恢复，截图已通过插件输出。
手机输入区域可内部滚动，底部恢复/确认按钮未遮挡。服务仍有favicon.ico缺失；人工
断响应提示包含“Failed to fetch”，不能说所有错误文案已统一中文。启动时旧夹具JWT被
拒绝，再通过可见合成登录重入；这不证明真实登录或登出。没有重验Admin手机、真机、
所有账号切换/权限路径、真实Redis和生产部署。前端测试技能驱动插件交互/截图/日志，
PG技能驱动受限角色与独立ACL/清理核验；A3k13及240/164/404保持开放。
