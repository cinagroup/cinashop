...
`runShippingLifecycle`，然后挂载实际 `adminapiRoutes`、`v1Routes`、`supplierapiRoutes`。
不注入 adminId/supplierId 或替换权限服务：JWT 验签、数据库账号状态/密码版本、角色读取、
控制器、生命周期服务与数据库触发器均实际执行。仅令牌桶为内存替身，外部 fetch 被禁止。

98 项覆盖两个 Admin 兼容入口与 Supplier 的保存/删除：缺令牌、签名/过期、撤销、桶身份、
桶类型与 JWT 类型分别拒绝、账号禁用/删除/改密、仅查看权限、角色缺失/禁用和跨请求权限撤销。
另验证查看角色仍能读取、合法编辑/删除保持历史订单与其他所属方不变、管理权限不能删除
保留引用、Supplier 主账号也受引用/租户约束、伪造请求身份或引用他方角色不能越权。
拒绝结果要求精确业务码及整组行快照不变，不把未知 SQL 错误计作保护成功。

首轮 86 项有 9 项夹具失败：六项错误期待 JWT 层错误码，但被桶类型层提前拒绝；
三个 Supplier 正向/跨属用例遗漏接口必需的两组规则数组。校正后并额外区分 JWT 类型门禁，
98 项全部通过。本轮没有据此修改业务代码，亦不将夹具失败写成漏洞修复。
证据为 `audit/shipping-lifecycle-route-auth-20260913.json`。

最终与生命周期服务、规则防护、权限目录、Supplier表单和CI合同联合回归：6 文件 217 项
零失败/跳过；最终双 TypeScript 配置通过，本机随机库/schema/角色均为零。

这里的真实授权指本机真实路由/JWT/账号角色 SQL，不代表线上身份、真实 Redis、登录流程、
浏览器、Worker 顶层 CORS/CSRF 或正式数据库最小权限验收。商品/活动全部写入口、请求内撤权
竞争、跨表规则与高权限维护、容量及线上安装仍待继续，A3k12 不勾选。
