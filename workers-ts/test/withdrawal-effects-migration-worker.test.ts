...
import worker from './integration/WithdrawalEffectsMigrationWorker';
...
vi.mock('@/migrations/runWithdrawalEffectsUpgrade',() => ({ runWithdrawalEffectsUpgrade:mocks.run,inspectWithdrawalEffectsUpgrade:mocks.inspect }));
const token = 'c'.repeat(64);
let env: PaidRuntimeAuditEnv;
let logs: unknown[][];
const req = (method='POST',path='/migrate/0131',credential=token,operation='0131-0133-withdrawal-effects') =>
...
beforeEach(async () => {
  ...
  mocks.run.mockResolvedValue({ migration:'0131-0133-withdrawal-effects',applied:true,ready:true });
  mocks.inspect.mockResolvedValue({ supported:true,ready:false,withinBudget:true });
});
afterEach(()=>vi.restoreAllMocks());
describe('explicit temporary DB-007 HTTP maintenance capability',()=>{
  it('uses the independent read-only inspector for GET preflight, not the upgrade',async()=>{
    const response=await worker.fetch(req('GET','/preflight/0131'),env);
  ...
  it.each(['','d'.repeat(64),'c'.repeat(65)])('rejects invalid credential %s before any SQL',async credential=>{
    expect((await worker.fetch(req('POST','/migrate/0131',credential),env)).status).toBe(403);
  ...
  it.each(['/migrate/0139','/migrate/0131?sql=ignored'])('rejects other operations %s',async path=>{
    expect((await worker.fetch(req('POST',path),env)).status).toBe(404);
    expect(mocks.create).not.toHaveBeenCalled();
  });
  it('requires an explicit matching operation header',async()=>{
    expect((await worker.fetch(req('POST','/migrate/0131',token,''),env)).status).toBe(400);
    expect(mocks.create).not.toHaveBeenCalled();
  });
  it('calls only the fixed upgrade, awaits connection close, and never caches success',async()=>{
    const response=await worker.fetch(req(),env);
    expect(response.status).toBe(200); expect(response.headers.get('Cache-Control')).toBe('no-store');
    expect(await response.json()).toEqual({ migration:'0131-0133-withdrawal-effects',applied:true,ready:true });
  ...
  it.each(['create','run','end'] as const)('reports an unconfirmed outcome and redacts %s errors',async stage=>{
    ...
    expect(logs).toContainEqual([JSON.stringify({ event:'withdrawal_effects_upgrade_unconfirmed' })]);
