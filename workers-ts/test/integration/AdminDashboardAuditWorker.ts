import { runAdminDashboardPostgresScenario } from "./AdminDashboardPostgresScenario";
...
export default {
  async fetch(request: Request, env: AuditEnv): Promise<Response> {
    ...
    try {
      return Response.json(await runAdminDashboardPostgresScenario(
        env.HYPERDRIVE.connectionString,
      ));
    } catch (error) {
      console.error(JSON.stringify({
        event: "admin_dashboard_audit_failed",
