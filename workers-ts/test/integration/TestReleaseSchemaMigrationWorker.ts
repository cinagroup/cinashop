...
import { inspectTestReleaseSchemaUpgrade, runTestReleaseSchemaUpgrade } from '@/migrations/runTestReleaseSchemaUpgrade';

const headers = { 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff' };
const operation = '0146-0151-test-release-schema';
export default {
  async fetch(request: Request, env: PaidRuntimeAuditEnv): Promise<Response> {
    ...
    if (!['/migrate/test-release','/preflight/test-release'].includes(url.pathname) || url.search) return Response.json({ error:'not found' }, { status:404,headers });
    const inspecting = url.pathname === '/preflight/test-release';
    ...
    try {
      db = createDbFromConnectionString(env.HYPERDRIVE.connectionString,1,{ applicationName:'cinashop_test_release_schema_upgrade' });
      const result = inspecting ? await inspectTestReleaseSchemaUpgrade(db) : await runTestReleaseSchemaUpgrade(db);
      ...
      console.error(JSON.stringify({ event:'test_release_schema_upgrade_unconfirmed' }));
      ...
      if (db) {
        ...
        catch { console.error(JSON.stringify({ event:'test_release_schema_upgrade_close_failed' })); }
      }
    }
  },
