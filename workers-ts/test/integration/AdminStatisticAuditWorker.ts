import { runAdminStatisticPostgresScenario } from "./AdminStatisticPostgresScenario";
...
export default {
  async fetch(request: Request, env: AuditEnv): Promise<Response> {
    ...
    try {
      return Response.json(await runAdminStatisticPostgresScenario(env.HYPERDRIVE.connectionString));
    } catch (error) {
      console.error(JSON.stringify({
        event: "admin_statistic_audit_failed",
