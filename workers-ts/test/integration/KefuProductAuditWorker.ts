import {
  applyKefuProductIndexes,
  runKefuProductPostgresScenario,
} from "./KefuProductPostgresScenario";
  ...
  async fetch(request: Request, env: AuditEnv): Promise<Response> {
    ...
    try {
      return Response.json(pathname === "/apply-indexes"
        ? await applyKefuProductIndexes(env.HYPERDRIVE.connectionString)
        : await runKefuProductPostgresScenario(env.HYPERDRIVE.connectionString));
    } catch (error) {
      console.error(JSON.stringify({
        event: "kefu_product_audit_failed",
