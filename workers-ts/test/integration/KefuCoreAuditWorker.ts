import type { KefuAuthEnv } from "@/services/kefu/KefuAuthService";
import {
  applyKefuCoreIndexes,
  runKefuCorePostgresScenario,
} from "./KefuCorePostgresScenario";
...
export default {
  async fetch(request: Request, env: AuditEnv): Promise<Response> {
    ...
    const runtimeEnv: KefuAuthEnv = {
      APP_KEY: crypto.randomUUID(),
      UPSTASH_REDIS_URL: "",
      UPSTASH_REDIS_TOKEN: "",
    };
    try {
      return Response.json(pathname === "/apply-indexes"
        ? await applyKefuCoreIndexes(env.HYPERDRIVE.connectionString)
        : await runKefuCorePostgresScenario(
            env.HYPERDRIVE.connectionString,
            runtimeEnv,
          ));
    } catch (error) {
      console.error(JSON.stringify({
        event: "kefu_core_audit_failed",
