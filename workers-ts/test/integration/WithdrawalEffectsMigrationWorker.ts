...
import { inspectWithdrawalEffectsUpgrade, runWithdrawalEffectsUpgrade } from '@/migrations/runWithdrawalEffectsUpgrade';

const headers = { 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff' };
const operation = '0131-0133-withdrawal-effects';
export default {
  async fetch(request: Request, env: PaidRuntimeAuditEnv): Promise<Response> {
    ...
    if (!['/migrate/0131','/preflight/0131'].includes(url.pathname) || url.search) return Response.json({ error:'not found' }, { status:404,headers });
    const inspecting = url.pathname === '/preflight/0131';
    ...
    try {
      db = createDbFromConnectionString(env.HYPERDRIVE.connectionString,1,{ applicationName:'cinashop_withdrawal_effects_upgrade' });
      const result = inspecting ? await inspectWithdrawalEffectsUpgrade(db) : await runWithdrawalEffectsUpgrade(db);
      ...
      console.error(JSON.stringify({ event:'withdrawal_effects_upgrade_unconfirmed' }));
      ...
      if (db) {
        ...
        catch { console.error(JSON.stringify({ event:'withdrawal_effects_upgrade_close_failed' })); }
      }
    }
  },
