import { runKefuMediaPostgresScenario } from "./KefuMediaPostgresScenario";
...
export default {
  async fetch(request: Request, env: AuditEnv): Promise<Response> {
    ...
    try {
      return Response.json(await runKefuMediaPostgresScenario(
        env.HYPERDRIVE.connectionString,
        env.ASSETS_BUCKET,
      ));
    } catch (error) {
      console.error(JSON.stringify({
        event: "kefu_media_audit_failed",
