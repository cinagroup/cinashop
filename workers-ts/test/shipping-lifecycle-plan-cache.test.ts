...
describe.runIf(Boolean(process.env.TEST_FINANCE_POSTGRES_URL))('actual shipping parent plans after hot-key warmup', () => {
  it('keeps absent-template UPDATE and DELETE selective with candidate indexes and default auto planning', async () => {
    ...
    try {
      ...
      for (const [i,table] of tables.entries()) {
        ...
        await f.exec(`CREATE INDEX qa_shipping_ref_${i} ON public.${table}((${packageTable ? SHIPPING_PACKAGE_REFERENCE_EXPRESSION_SQL : SHIPPING_REFERENCE_EXPRESSION_SQL})); ANALYZE public.${table}`);
      }
      ...
      for (const record of absent) {
        ...
        expect(scans.filter(s => s.loops > 0).map(s => s.index).sort()).toEqual(tables.map((_,i) => `qa_shipping_ref_${i}`));
      }
