...
describe("supplier frontend export and queue-history migration", () => {
  ...
  it("uses navigable screens rather than raw Vue file count for the parity denominator", () => {
    ...
    expect(parity.counting.target.pageVueFiles).toBe(17);
    expect(parity.counting.target.screenRouteRecords).toBe(18);
    expect(parity.screens).toHaveLength(19);
    expect(parity.screens.filter((screen) => screen.status === "candidate_covered")).toHaveLength(16);
    expect(parity.screens.filter((screen) => screen.status === "partial_replacement")).toHaveLength(3);
    ...
    expect(parity.checklist.filter((item) => item.done).map((item) => item.id)).toEqual([
      "FE-004A", "FE-004B", "FE-004C", "FE-004D", "FE-004E", "FE-004F",
    ]);
  });
