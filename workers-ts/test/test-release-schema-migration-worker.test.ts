...
import worker from './integration/TestReleaseSchemaMigrationWorker';
...
vi.mock('@/migrations/runTestReleaseSchemaUpgrade',() => ({ runTestReleaseSchemaUpgrade:mocks.run,inspectTestReleaseSchemaUpgrade:mocks.inspect }));
const token = 'c'.repeat(64);
let env: PaidRuntimeAuditEnv;
let logs: unknown[][];
const req = (method='POST',path='/migrate/test-release',credential=token,operation='0146-0151-test-release-schema') =>
...
beforeEach(async () => {
  ...
  mocks.run.mockResolvedValue({ migration:'0146-0151-test-release-schema',applied:true,ready:true });
...
describe('explicit temporary 0146-0151 test-release schema HTTP maintenance capability',()=>{
  it('uses the independent read-only inspector for GET preflight, not the upgrade',async()=>{
    const response=await worker.fetch(req('GET','/preflight/test-release'),env);
  ...
  it.each(['','d'.repeat(64),'c'.repeat(65)])('rejects invalid credential %s before any SQL',async credential=>{
    expect((await worker.fetch(req('POST','/migrate/test-release',credential),env)).status).toBe(403);
  ...
  it.each(['/migrate/0139','/migrate/test-release?sql=ignored'])('rejects other operations %s',async path=>{
    expect((await worker.fetch(req('POST',path),env)).status).toBe(404);
    expect(mocks.create).not.toHaveBeenCalled();
  });
  it('requires an explicit matching operation header',async()=>{
    expect((await worker.fetch(req('POST','/migrate/test-release',token,''),env)).status).toBe(400);
    expect(mocks.create).not.toHaveBeenCalled();
  });
  it('calls only the fixed upgrade, awaits connection close, and never caches success',async()=>{
    const response=await worker.fetch(req(),env);
    expect(response.status).toBe(200); expect(response.headers.get('Cache-Control')).toBe('no-store');
    expect(await response.json()).toEqual({ migration:'0146-0151-test-release-schema',applied:true,ready:true });
  ...
  it.each(['create','run','end'] as const)('reports an unconfirmed outcome and redacts %s errors',async stage=>{
    ...
    expect(logs).toContainEqual([JSON.stringify({ event:'test_release_schema_upgrade_unconfirmed' })]);
