...
describe("订单退款 PostgreSQL 迁移", () => {
  it("秒杀未发货退款在结算用户和库存之前锁活动，重放与已发货退款不额外占锁", () => {
    ...
    expect(locked).toBeLessThan(finalize.indexOf("await lockOrderSettlementUsers(tx, order, lineCompensation?.earnedIncome?.orderId)"));
  ...
  it("把余额退款与全部补偿副作用收敛到可验证的生产事务核心", () => {
    ...
    expect(source).toContain("await lockOrderSettlementUsers(tx, order, lineCompensation?.earnedIncome?.orderId)");
    expect(source).toContain("await restoreRefundStock(");
    expect(source).toContain("await reverseOrderRewards(tx, order, cumulativeCents, now, cumulativeNum, lineCompensation)");
    expect(source).toContain("await reverseOrderBrokerage(tx, order, cumulativeCents, now, lineCompensation)");
    const finalize = source.slice(source.indexOf("export async function finalizeStoreOrderRefund("), source.indexOf("async function createOrderRefundApplication("));
    const planned = finalize.indexOf("await loadRefundLineCompensation(tx, order.id,");
    expect(planned).toBeGreaterThan(0);
    expect(planned).toBeLessThan(finalize.indexOf("await lockOrderSettlementUsers(tx, order, lineCompensation?.earnedIncome?.orderId)"));
    expect(planned).toBeLessThan(finalize.indexOf(".set({ refundType: 6, refundedTime: now, refundedPrice: refundAmount })"));
    expect(source).toContain("await recordSupplierRefund(tx, order, refundId, refundAmount, cumulativeCents, now, lineCompensation)");
