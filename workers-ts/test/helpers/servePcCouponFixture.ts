/** Local coupon browser harness. Real isolated SQL quote/wallet, synthetic login; create only captures requests, never creates orders or pays. */
import { createServer } from "node:http";
import { createPcCouponFixture } from "./pcCouponFixture";
...
if (process.env.TEST_FINANCE_POSTGRES_URL) throw new Error("Browser coupon fixture requires in-memory PGlite only");
const fixture = await createPcCouponFixture();
let rejectedWalletOnce = false;
const submissions: Array<{ key: string; body: unknown }> = [];
...
app.post("/api/order/create/:key", async (c) => { submissions.push({ key: c.req.param("key"), body: await c.req.json() }); return c.json({ status: 400, msg: "本地模拟未知建单结果；不会建立订单或付款" }); });
app.get("/api/qa/state", async (c) => c.json({ events, submissions, snapshot: await fixture.snapshot() }));
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (path === "/api/coupons/user/2" && !rejectedWalletOnce) {
      rejectedWalletOnce = true; event.outcome = "wallet-load-failure"; response = Response.json({ status: 503, msg: "本地模拟优惠券加载失败，请重试" });
    } else if (session === "expired" && path === "/api/cart/list") {
    ...
    res.writeHead(response.status, { "Content-Type": response.headers.get("Content-Type") ?? "application/json", "Cache-Control": response.headers.get("Cache-Control") ?? "no-store", "X-Coupon-Next-Cursor": response.headers.get("X-Coupon-Next-Cursor") ?? "" }).end(await response.text());
  } catch (error) { console.error("LOCAL_COUPON_QA_ERROR", error); res.writeHead(500).end(); }
});
server.listen(5218, "127.0.0.1", () => console.log("PC_COUPON_FIXTURE 127.0.0.1:5218; PGlite only; synthetic expired-qa/fresh-qa/new-qa/offline-qa; password local-qa"));
