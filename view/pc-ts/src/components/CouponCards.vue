  ...
  <ul class="coupon-grid" aria-label="优惠券列表">
    <li v-for="coupon in coupons" :key="coupon.id" class="coupon-card" :class="{ selected: selectedId === coupon.id }">
      ...
      <div class="coupon-detail">
        ...
        <button v-if="inspectable" type="button" :disabled="disabled" @click="$emit('inspect', coupon.id)">查看详情与规则</button>
...
defineProps<{ coupons: OwnedCoupon[]; selectable?: boolean; browsable?: boolean; inspectable?: boolean; selectedId?: number; disabled?: boolean }>();
defineEmits<{ select: [id: number]; browse: [id: number]; inspect: [id: number] }>();
</script>
<style scoped>
