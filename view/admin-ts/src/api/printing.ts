...
const previewDocuments: PrintDocumentView[] = [{
  id: 1, type: 2, supplier_id: 0, print_name: "平台仓库打印机",
...
export function apiPrintDocuments(params: Record<string, string | number> = {}) {
  if (previewMode) return Promise.resolve({ list: previewDocuments.map((row) => ({ ...row })), count: previewDocuments.length });
  return getData<{ list: PrintDocumentView[]; count: number }>(request.get("/print/list", { params }));
}
...
export function apiSavePrintDocument(id: number, data: Record<string, string | number>) {
  if (previewMode) {
    const row = previewDocuments.find((item) => item.id === id) ?? previewDocuments[0];
...
export function apiPrintContent(id: number) {
  if (previewMode) return Promise.resolve(structuredClone(previewContent));
...
export function apiSavePrintContent(id: number, content: PrintContent) {
  if (previewMode) return Promise.resolve({ ...previewDocuments[0], id });
