...
import {
  consumeVirtualInventoryExportTicket,
  createVirtualInventoryExportTicket,
  getVirtualInventory,
  importVirtualInventory,
} from "@/api/supplier";
import type {
  VirtualInventoryCard,
...
async function load(reset = true) {
  loading.value = true;
  try {
    const result = await getVirtualInventory(productId, {
...
async function submitImport() {
  ...
  try {
    const result = await importVirtualInventory(productId, {
...
async function exportAvailableCards() {
  ...
  try {
    ...
    const authorization = await createVirtualInventoryExportTicket(productId, {
      attr_unique: selectedSku.value,
      reason: value.trim(),
      confirm: "EXPORT_AVAILABLE_VIRTUAL_CARDS",
    });
    const result = await consumeVirtualInventoryExportTicket(productId, authorization.ticket);
...
<template>
  <section class="page-section inventory-page" v-loading="loading">
    <header class="page-heading">
      <div class="heading-title">
        <el-button circle plain :icon="ArrowLeft" aria-label="返回商品列表" @click="router.push('/products')" />
        <div>
          <h1>卡密库存</h1>
          <p>{{ inventory?.product.store_name ?? `商品 #${productId}` }} · 仅能管理当前供应商自己的商品</p>
        </div>
      </div>
    </header>
    ...
    <el-alert
      ...
      :closable="false"
      show-icon
      description="卡密密码只用于安全导入和订单自动交付；库存列表只展示脱敏卡号、是否已配置密码和交付状态。"
    />

    <div class="summary-grid">
      <article class="surface summary-card"><span>卡密总数</span><strong>{{ inventory?.summary.total_cards ?? 0 }}</strong></article>
      <article class="surface summary-card"><span>未分配</span><strong>{{ inventory?.summary.available_cards ?? 0 }}</strong></article>
      <article class="surface summary-card"><span>已交付</span><strong>{{ inventory?.summary.assigned_cards ?? 0 }}</strong></article>
    </div>
    ...
    <article class="surface section-card">
      ...
      <el-table :data="cards" row-key="id" empty-text="暂无卡密记录">
        ...
        <el-table-column label="密码" width="100"><template #default="scope">{{ scope.row.password_configured ? "已配置" : "缺失" }}</template></el-table-column>
        <el-table-column label="状态" width="110"><template #default="scope"><el-tag :type="scope.row.status === 'available' ? 'success' : 'info'">{{ scope.row.status === "available" ? "未分配" : "已交付" }}</el-tag></template></el-table-column>
      </el-table>
    ...
    <article class="surface section-card">
      <header>
        <h2>受控导出未分配卡密</h2>
        <p>只导出未分配库存，不包含已交付给客户的卡密。操作需要填写原因和二次确认，并使用60秒、绑定当前账号/供应商/商品/SKU的一次性票据。</p>
      </header>
      <el-alert
        title="导出文件含明文秘密"
        type="warning"
        :closable="false"
        show-icon
        description="票据只保存摘要且成功后立即失效。请将下载文件保存在受控位置，并在对账或灾备操作完成后及时清理。"
      />
    ...
    <article class="surface section-card">
      <header><h2>批量导入一次性卡密</h2><p>每行使用“卡号 + Tab + 密码”；只有密码时可直接粘贴一列。单次最多 1000 行。</p></header>
      <el-input v-model="importText" type="textarea" :rows="10" :disabled="selectedSkuInfo?.disk_info_configured" placeholder="CARD-001&#9;PASSWORD-001&#10;PASSWORD-ONLY" />
      <div class="import-actions"><el-button type="primary" :icon="Upload" :loading="importing" :disabled="selectedSkuInfo?.disk_info_configured" @click="submitImport">导入卡密</el-button></div>
    </article>
  </section>
</template>
...
.heading-title, .toolbar, .import-actions { display: flex; align-items: center; }
.heading-title { min-width: 0; gap: 12px; }
.heading-title > div { min-width: 0; }
.heading-title h1, .section-card h2 { margin: 0 0 4px; }
.heading-title p, .section-card header p { margin: 0; color: var(--muted); line-height: 1.5; overflow-wrap: anywhere; }
...
.section-card { min-width: 0; padding: 20px; }
