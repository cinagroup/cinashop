import { ref } from 'vue';
import type { ProductShippingOption } from '@/api/productShipping';

/** Selected identity is independent of the currently browsed/search-result page. */
export function useShippingTemplateSelection(read: (id: number) => Promise<ProductShippingOption>, current: () => boolean) {
  ...
  async function hydrate(id: number) {
    ...
    try {
      ...
      if (!data || data.id !== id || typeof data.name !== 'string' || !data.name.trim() || data.name.length > 255 || !['按件数', '按重量', '按体积'].includes(data.type)) {
        throw new Error('当前模板响应无效');
      }
      selected.value = { id, name: data.name, type: data.type };
    } catch (failure) {
