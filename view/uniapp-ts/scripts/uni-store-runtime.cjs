...
function runtime({ storage = new Map(), send, navigationFails = false, modal, feature = 'useCheckout', component, props = {} } = {}) {
  ...
  const lifecycle = Object.fromEntries(['onLoad', 'onShow', 'onHide', 'onUnload', 'onShareAppMessage', 'onReachBottom'].map(name => [name, fn => { hooks[name] = fn; }]));
  function load(file) {
    ...
    new Function('require', 'exports', 'uni', output)(id => {
      ...
      if (id.startsWith('qrcode-terminal/')) return require(id);
