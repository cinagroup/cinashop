...
function runtime({ storage = new Map(), send, navigationFails = false, feature = 'useCheckout' } = {}) {
  ...
  const checkout = scope.run(() => load(path.join(root, 'src/composables', feature + '.ts'))[feature]());
