...
export async function apiCouponScopeDescription(couponId: number, before?: number) {
  ...
  if (!auth.isLoggedIn || auth.uid <= 0) throw new Error("请先登录后查看券范围说明");
  const owner = { uid: auth.uid, token: auth.token, version: auth.sessionVersion };
  const data = await http.get<unknown>(`/coupons/user/${couponId}/products`, { view: "scope", limit: 20, ...(before !== undefined ? { before } : {}) });
